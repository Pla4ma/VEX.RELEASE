  ...
  switch (field.type) {
    ...
    case "password":
      ...
      if (
        commonPasswords.some((common) =>
          passwordValue.toLowerCase().includes(common),
        )
      ) {
    ...
    case "number":
      ...
      if (typeof numValue === "number") {
        ...
        if (field.max !== undefined && numValue > field.max) {
          fieldErrors.push(
            `${field.label || field.name} must be no more than ${field.max}`,
          );
        }
      }
      ...
      if (dateValue instanceof Date) {
        ...
        if (field.min !== undefined) {
          ...
          if (dateValue < minDate) {
            fieldErrors.push(
              `${field.label || field.name} must be after ${minDate.toLocaleDateString()}`,
            );
          }
        }
        if (field.max !== undefined) {
          ...
          if (dateValue > maxDate) {
            fieldErrors.push(
              `${field.label || field.name} must be before ${maxDate.toLocaleDateString()}`,
            );
          }
        }
    ...
    case "select":
      if (field.options && !field.options.includes(field.value as string)) {
        fieldErrors.push(
          `${field.label || field.name} must be one of the available options`,
        );
      }
