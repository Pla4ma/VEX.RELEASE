...
import { applyCustomFieldValidation, buildFieldValidationResult } from "./formFieldFinalizers";
...
export const validateFormField = (
  ...
  if (!field.value && !field.required) {
    return { isValid: true, errors: {}, warnings: {}, sanitizedData: { [field.name]: field.value } };
  }
  switch (field.type) {
    case "text":
    case "textarea":
      ...
      if (field.maxLength && stringValue.length > field.maxLength) {
        fieldErrors.push(
          `${field.label || field.name} must be no more than ${field.maxLength} characters`,
        );
    ...
    case "password":
      ...
      if (["password", "123456", "qwerty", "admin", "letmein"].some((common) => passwordValue.toLowerCase().includes(common))) {
    ...
    case "date":
      ...
      if (dateValue instanceof Date) {
        ...
        if (dateValue > now && !field.max) fieldWarnings.push("Date is in the future");
        if (dateValue < now && !field.min) fieldWarnings.push("Date is in the past");
      }
      sanitizedValue = dateValue;
      break;
    case "select":
      if (field.options && !field.options.includes(field.value as string)) {
  ...
  applyCustomFieldValidation(field, sanitizedValue, formData, fieldErrors);
