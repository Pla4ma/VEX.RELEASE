...
import { useMyRivals } from '../../../features/rivals/hooks';
...
export function useSessionSetupStakes({
  ...
  const { data: rivals } = useMyRivals(userId);
  ...
  const rivalStake = React.useMemo<SessionSetupStakes['rivalStake']>(() => {
    ...
    const myMinutes = Math.floor((mainRival.weeklyScore?.mine || 0) / 60);
    const theirMinutes = Math.floor((mainRival.weeklyScore?.theirs || 0) / 60);

    return {
      gapMinutes: theirMinutes - myMinutes,
      myMinutes,
