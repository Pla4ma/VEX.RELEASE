...
import type { SessionHistoryEntry as BaseSessionHistoryEntry } from '../../session/types';

// Extended history entry with UI-specific fields
type SessionNavigationProp = NativeStackNavigationProp<SessionStackParams>;

interface SessionHistoryEntry extends BaseSessionHistoryEntry {
  id: string;
  name?: string;
  finalScore: number;
...
interface HistoryItemProps {
  ...
  onPress: (entry: SessionHistoryEntry) => void;
}

const HistoryItem: React.FC<HistoryItemProps> = ({ entry, onPress }) => {
  ...
  return (
    <Pressable onPress={() => onPress(entry)} style={{ width: '100%' }}
  ...
  accessibilityHint="Activates this control">
      ...
      >
        ...
        <Box
          ...
          mr="md"
        >
          <Text variant="h4" color={getStatusColor(entry.status)}>
            {getGrade(entry.finalScore)}
          </Text>
        </Box>
        ...
        <Box alignItems="flex-end">
          <Text variant="body" fontWeight="600" color="text.primary">
            {formatDuration(entry.effectiveDuration)}
          </Text>
          <Text variant="caption" color="text.secondary">
...
export const SessionHistoryScreen: React.FC = () => {
  ...
  // Use a local state for history since the hook might not exist yet
  const [history, setHistory] = useState<SessionHistoryEntry[]>([
    ...
    },
    {
      ...
      completedAt: Date.now() - 171600000,
      status: 'COMPLETED',
      finalScore: 720,
  ...
  const handleItemPress = useCallback((entry: SessionHistoryEntry) => {
    if (entry.summary) {
  ...
  const renderItem = useCallback(({ item, index }: { item: SessionHistoryEntry; index: number }) => (
    ...
    >
          ...
          {(['all', 'completed', 'abandoned'] as const).map((f) => (
            ...
            accessibilityHint="Activates this control">
              <Text
                variant="caption"
                fontWeight="600"
                color={filter === f ? 'text.inverse' : 'text.secondary'}
      ...
      <Box flex={1} p="lg" pt="sm">
        {filteredHistory.length > 0 ? (
          ...
          <Box flex={1} justifyContent="center" alignItems="center">
            ...
            <Text variant="caption" color="text.tertiary" textAlign="center" mt="sm">
      ...
      <Box p="lg" pb="xl">
        <Button
          ...
          size="lg"
          fullWidth
          onPress={() => navigation.navigate({ name: 'SessionSetup', params: {} })}

        accessibilityLabel="Start New Session button"
        accessibilityRole="button"
        accessibilityHint="Activates this control">
          Start New Session
        </Button>
