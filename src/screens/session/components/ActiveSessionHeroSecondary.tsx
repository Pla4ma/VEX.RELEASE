...
export function DailyProgress({ viewModel }: { viewModel: ActiveSessionHeroViewModel }): React.JSX.Element | null {
  if (viewModel.dailyProgress === null || viewModel.todayFocusSeconds === null) {return null;}
  return (
...
export function SessionStats({ viewModel, labelColor }: { viewModel: ActiveSessionHeroViewModel; labelColor: string }): React.JSX.Element | null {
  if (viewModel.heroDensity === 'minimal') {return null;}
  return (
    <Box flexDirection="row" justifyContent="center" gap="xl" mt="xl">
      <Stat label="Elapsed" value={formatTime(viewModel.elapsedSeconds)} />
      <Stat label="Complete" value={`${Math.round(viewModel.completionPercentage)}%`} color={labelColor} />
    </Box>
  );
}
