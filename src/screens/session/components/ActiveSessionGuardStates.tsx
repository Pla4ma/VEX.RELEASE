...
export function ActiveSessionGuardStates({
  ...
  if (!userId) {
    return (
      <Box flex={1} bg="background.primary" justifyContent="center" alignItems="center" p="lg">
        ...
        <Button variant="primary" onPress={onGoBack} accessibilityLabel="Go back to the previous screen">
          Go Back
        </Button>
      </Box>
    );
  ...
  if (error) {
    return <SessionErrorState error={error} onRetry={onRetry} onGoBack={onGoBack} />;
  }

  if (!session) {
    return <SessionEmptyState onCreateSession={onCreateSession} onBrowsePresets={onBrowsePresets} />;
  }
