...
export function usePowerUserHomeModel(input: PowerUserModelInput): HomeViewModel & {
  ...
  const returnReason = useMemo<HomeReturnReason>(() => {
    ...
    const reasonState = buildHomeReturnReasonState({
      activeStudyPlan: studyData
        ? { completedTasks: (studyData.completedTasks as number) ?? 0, remainingMinutes: (studyData.remainingMinutes as number) ?? 0, title: (studyData.title as string) ?? '', totalTasks: (studyData.totalTasks as number) ?? 0 }
        : null,
  ...
  const controller: HomeController = {
    ...
    openProgress, openSocial, openContentStudy, continueStudyPlan,
