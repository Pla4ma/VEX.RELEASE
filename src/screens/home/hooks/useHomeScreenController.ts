...
import { useCreateRecommendation, useUpdateRecommendationStatus } from '../../../features/ai-coach/hooks/useRecommendationMutations';
...
export function useHomeScreenController() {
  ...
  const progressionQuery = useProgressionSummary(
    canQueryProgressSignals ? userId : null,
  );
  ...
  const activeBossQuery = useActiveBoss(
    runtime.canQueryBoss ? userId || null : null,
  );
  ...
  const todayFocusMinutes = historyQuery.history.reduce(
    (sum, entry) => sum + getFocusedMinutesForToday(entry),
    0,
  );
  ...
  const { continueStudyPlan, openContentStudy, openNextAction, openProgress, openSetup, openSocial } = useHomeNavigationActions({
    ...
    canNavigateSocial: isFeatureAvailableForNavigation(
  ...
  const { returnReason } = useHomeReturnReason({
    ...
    comebackMessage: comebackQuery.data?.isComeback
      ? comebackQuery.data.message
      : null,
