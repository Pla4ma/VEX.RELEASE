...
export const STUDY_RECOMMENDATION_RULES: RecommendationRule[] = [
  ...
  },
  {
    ...
    priority: 70,
    condition: (ctx) => Boolean(ctx.streak && ctx.streak.currentDays >= 2 && ctx.hasCompletedSessionToday),
    generate: (ctx) => ({
  ...
  },
  {
    ...
    condition: (ctx) => {
      ...
      return (ctx.activeBoss.healthRemaining / ctx.activeBoss.maxHealth) * 100 >= 30;
    },
    generate: (ctx) => {
      const boss = ctx.activeBoss;
      const healthPercent = boss ? Math.round((boss.healthRemaining / boss.maxHealth) * 100) : 0;
      return {
