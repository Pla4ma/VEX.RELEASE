...
function getTimeBasedMessage(): string {
  const hour = new Date().getHours();
  if (hour < 6) {return 'The quiet hours are powerful';}
  if (hour < 12) {return 'Start the day with intention';}
  ...
  return (
    <View style={containerStyle}>
      <View style={greetingSectionStyle}>
        <Text variant="h3" color={theme.colors.text.primary} style={greetingStyle}>
          {greeting}
        </Text>
        <Text variant="body" color={theme.colors.text.secondary} style={messageStyle}>
          {message}
        </Text>
      </View>
      ...
      {currentStreak > 0 && (
        <View style={streakBadgeStyle}>
          <Icon name="flame" size={16} color={theme.colors.success[500]} style={streakIconStyle} />
          <Text variant="caption" color={theme.colors.success[500]} style={streakTextStyle}>
