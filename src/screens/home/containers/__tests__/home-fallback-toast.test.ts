...
jest.mock('../../../../session/analytics/SessionAnalytics', () => ({
  ...
  setOrchestratorHandlesCompletion: (...args: unknown[]) => mockSetOrchestratorHandle(...args),
}));
...
describe('Home session:completed fallback toast', () => {
  ...
  it('orchestrator handles completion returns true by default', () => {
    const { getOrchestratorHandlesCompletion } = require('../../../../session/analytics/SessionAnalytics');
