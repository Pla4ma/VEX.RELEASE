...
export function getHomeCompanionMood(
  ...
  if (!latestEndedAt || Date.now() - latestEndedAt > 48 * 60 * 60 * 1000) {return 'SLEEPY';}
  if (streakDays > 0) {return 'FOCUSED';}
  const score = latest.summary?.focusPurityScore ?? latest.summary?.focusQuality ?? 0;
  if (score >= 95) {return 'ECSTATIC';}
  if (score >= 70) {return 'CONTENT';}
  return 'SLEEPY';
}

export function readSuggestedDuration(intervention: ActiveIntervention): number {
...
): ReturnType<typeof resolveSessionMode> {
  return resolveSessionMode(intervention.metadata.suggestedMode);
}
