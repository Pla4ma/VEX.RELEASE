...
import { RealTimeBossCombat, calculatePurityMultiplier, createBossEncounter } from '../../features/boss-realtime';
...
const ATTACK_PRESETS = [
  { minutes: 15, label: '⚡ 15m Quick Strike' },
  { minutes: 25, label: '🎯 25m Pomodoro' },
  { minutes: 60, label: '🔥 60m Deep Strike' },
] as const;
const BossDamageSchema = z.object({ bossDamage: z.number().optional() }).passthrough();
...
const estimateDamage = (minutes: number, streakMultiplier: number) => Math.round(minutes * streakMultiplier * 1.5);
...
export function BossScreenSections({
  ...
  useEffect(() => {
    if (!isFocused) {return;}
    const intervalId = setInterval(() => setElapsedSeconds((value) => (value + 1) % 301), 1000);
    return () => clearInterval(intervalId);
  }, [isFocused]);
  const demoEncounter = useMemo(
    () => createBossEncounter(encounter.bossId, template?.name ?? 'Boss', '👹', userId, 'preview', 5, progressionLevel),
    [encounter.bossId, progressionLevel, template?.name, userId],
  );
  const purityScore = Math.round(75 + Math.sin(elapsedSeconds / 10) * 15);
  const recentAttacks = useMemo(() => {
    const since = Date.now() - 7 * 24 * 60 * 60 * 1000;
    return historyQuery.history
      .filter((entry) => entry.startedAt >= since && entry.summary)
      .map((entry) => {
        const parsed = BossDamageSchema.safeParse(entry.summary);
        const duration = entry.summary?.effectiveDuration ?? entry.summary?.actualDuration ?? entry.config.duration;
        const purity = entry.summary?.focusPurityScore ?? entry.summary?.focusQuality ?? 75;
        const damage = parsed.success && parsed.data.bossDamage !== undefined
          ? parsed.data.bossDamage
          : Math.round((duration / 60) * calculatePurityMultiplier(purity));
        return { damage, duration, endedAt: entry.endedAt ?? entry.startedAt, purity };
      })
      .filter((entry) => entry.damage > 0)
      .slice(0, 5);
  }, [historyQuery.history]);
  const initialMinutes = useMemo(() => {
    const lastDuration = historyQuery.history[0]?.config.duration;
    if (!lastDuration) {return 25;}
    const lastMinutes = Math.round(lastDuration / 60);
    let closest: number = ATTACK_PRESETS[1].minutes;
    ATTACK_PRESETS.forEach((preset) => {
      if (Math.abs(preset.minutes - lastMinutes) < Math.abs(closest - lastMinutes)) {closest = preset.minutes;}
    });
    return closest;
  }, [historyQuery.history]);
  const leaderboard = useMemo(() => {
    const parsed = ContributorsSchema.safeParse(encounter);
    const contributors = parsed.success ? parsed.data.contributors ?? [] : [];
    const rows = contributors.length > 0
      ? contributors.map((item) => ({
        damage: item.damageDealt ?? item.damage ?? 0,
        name: item.name ?? (item.userId === userId ? 'You' : `Hunter ${item.userId.slice(0, 4)}`),
        userId: item.userId,
      }))
      : [{ damage: userDamage, name: 'You', userId }];
    return rows.sort((left, right) => right.damage - left.damage).slice(0, 5);
  }, [encounter, userDamage, userId]);
  useEffect(() => {
    ...
    <>
      ...
      <View style={cardStyle(theme)}>
        ...
        {recentAttacks.map((entry) => (
          <View key={`${entry.endedAt}-${entry.damage}`} style={{ alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }}>
      ...
      <View style={cardStyle(theme)}>
        ...
        {ATTACK_PRESETS.map((preset) => {
          ...
          return (
            ...
            accessibilityLabel="est. damage`} button"
      ...
      {activeSquad ? (
        <View style={cardStyle(theme)}>
          ...
          {leaderboard.map((entry, index) => (
            <View key={`${entry.userId}-${index}`} style={{ alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }}>
          ...
          <Pressable onPress={onOpenSquad} style={{ alignItems: 'center', borderColor: theme.colors.border.light, borderRadius: theme.borderRadius.xl, borderWidth: 1, padding: theme.spacing[4] }}
  accessibilityLabel="Open squad button"
  accessibilityRole="button"
  accessibilityHint="Activates this control">
