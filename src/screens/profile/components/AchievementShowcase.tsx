...
import { launchColors } from '@theme/tokens/launch-colors';
 export interface FeaturedAchievement{id:string;name:string;description:string;icon:string;emoji:string;unlockedAt:number;rarity:'common'|'rare'|'epic'|'legendary';}interface AchievementShowcaseProps{achievements:FeaturedAchievement[];onReorder?:(newOrder:FeaturedAchievement[])=>void;onSelectSlot?:(slotIndex:number)=>void;style?:ViewStyle;isEditable?:boolean;}const RARITY_COLORS = {common:launchColors.hex_94a3b8,rare:launchColors.hex_3b82f6,epic:launchColors.hex_a855f7,legendary:launchColors.hex_f59e0b}; const LockedSlot:React.FC<{slotIndex:number;onPress?:()=>void;isEditable?:boolean;}> = ({slotIndex,onPress,isEditable})=>{const{theme} = useTheme(); return<Pressable onPress={onPress}disabled={!isEditable}accessibilityLabel={`Choose achievement showcase slot ${slotIndex + 1}`}accessibilityRole="button"accessibilityHint="Opens the achievement picker for this profile slot">
      ...
      <Pressable onPress={onPress}onPressIn={handlePressIn}onPressOut={handlePressOut}disabled={!isEditable}accessibilityLabel={`Open featured achievement ${achievement.name}`}accessibilityRole="button"accessibilityHint="Selects this featured profile achievement">
      ...
      <Box flexDirection="row"justifyContent="space-between"alignItems="center"mb={16}>
        ...
        {unlockedCount >= 2 && <Pressable onPress={handleEditToggle}accessibilityLabel={isEditMode ? 'Finish editing featured achievements' : 'Edit featured achievements'}accessibilityRole="button"accessibilityHint="Changes the order of the achievements shown on this profile">
