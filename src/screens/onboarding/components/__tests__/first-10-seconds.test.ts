...
describe('first 10 seconds clarity', () => {
  it('intro contains adaptive promise', () => {
    expect(ONBOARDING_PROMISE_COPY.primary).toBe('VEX changes based on how you work.');
    expect(ONBOARDING_PROMISE_COPY.secondary).toContain('start one focused session');
  });
  ...
  it('minimal progression uses Understand, Match, Start', () => {
    expect(ONBOARDING_PROGRESS_PHASES).toEqual(['Understand', 'Match', 'Start']);
  });
  ...
  it('Day 0 does not explain all four lanes at once', () => {
    ...
    expect(laneCopy).not.toMatch(/Study OS|Run Board|Project Thread|Clean Mode/);
  });
});
