...
import type { Phase9ExitGateResult, Phase9BlockingIssue } from "../ExitGate";
...
describe("Phase9ExitGate", () => {
  ...
  describe("Performance Verification", () => {
    it("should pass performance verification with good metrics", async () => {
      ...
      ).generatePerformanceReport.mockResolvedValue({
        score: 92,
        metrics: {
          appStartupTime: 2000,
          averageFPS: 58,
  ...
  describe("Paywall Verification", () => {
    it("should pass paywall verification with full compliance", async () => {
      ...
      ).performFullVerification.mockResolvedValue({
        passed: true,
        score: 92,
        results: {
      ...
      (
        ...
        results: {
          productCatalog: {
            valid: true,
            issues: [],
            warnings: ["Minor catalog issue"],
          },
          purchaseFlow: { valid: true, issues: [], warnings: [] },
          subscriptionManagement: {
            valid: true,
            issues: [],
            warnings: ["Minor subscription issue"],
          },
          receiptValidation: { valid: true, issues: [], warnings: [] },
          analyticsIntegration: { valid: true, issues: [], warnings: [] },
          compliance: { gdprCompliant: true, issues: [], warnings: [] },
    ...
    it("should fail paywall verification with critical issues", async () => {
      (
        ...
        results: {
          productCatalog: {
            valid: false,
            issues: ["Critical catalog issue"],
            warnings: [],
          },
          purchaseFlow: {
            valid: false,
            issues: ["Critical purchase issue"],
            warnings: [],
          },
          subscriptionManagement: { valid: true, issues: [], warnings: [] },
          receiptValidation: { valid: true, issues: [], warnings: [] },
      ...
      mockAppStoreSubmissionPack.prepareFullSubmissionPack.mockResolvedValue({
        ...
        results: {
          metadata: {
            complete: true,
            issues: [],
            warnings: ["Minor metadata issue"],
          },
          assets: {
            complete: true,
            issues: [],
            warnings: ["Minor asset issue"],
          },
          privacy: { complete: true, issues: [], warnings: [] },
          compliance: { complete: true, issues: [], warnings: [] },
          testing: { complete: true, issues: [], warnings: [] },
    ...
    it("should fail App Store verification with critical issues", async () => {
      mockAppStoreSubmissionPack.prepareFullSubmissionPack.mockResolvedValue({
        ...
        results: {
          metadata: {
            complete: false,
            issues: ["Critical metadata issue"],
            warnings: [],
          },
          assets: {
            complete: false,
            issues: ["Critical asset issue"],
            warnings: [],
          },
          privacy: { complete: true, issues: [], warnings: [] },
          compliance: { complete: true, issues: [], warnings: [] },
