 ...
 * API Client Tests
 *
 * Comprehensive tests for API client with:
  ...
  describe('Circuit Breaker', () => {
    it('should open circuit after threshold failures', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        try {
          await client.get('/test');
        } catch (error) { captureSilentFailure(error, { feature: 'api', operation: 'network-fallback', type: 'network' });
          // Expected
        }
      }
  ...
  describe('Request Deduplication', () => {
    it('should deduplicate identical requests', async () => {
      ...
      const [r1, r2] = await Promise.all([
        client.get('/test', { deduplicate: true }),
        client.get('/test', { deduplicate: true }),
      ]);
    ...
    it('should not deduplicate different requests', async () => {
      ...
      await Promise.all([
        client.get('/test1', { deduplicate: true }),
        client.get('/test2', { deduplicate: true }),
      ]);
