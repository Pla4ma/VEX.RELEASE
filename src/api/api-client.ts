  ...
  private async executeRequest<T>(endpoint: string, config: ApiRequestConfig): Promise<ApiResponse<T>> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw this.createError(errorData.code ?? 'HTTP_ERROR', errorData.message ?? response.statusText, response.status, errorData.details);
      }

      const data = await response.json();

      if (config.schema) {
        try { config.schema.parse(data); }
        catch (error) { throw this.createError('VALIDATION_ERROR', 'Response validation failed', response.status, { validationError: error }); }
      }

      this.circuitBreaker.recordSuccess();
      const result: ApiResponse<T> = { data, status: response.status, headers: this.parseHeaders(response.headers) };
