  ...
  private async executeRequest<T>(
    ...
    try {
        ...
        try {
          ...
          if (validationError instanceof z.ZodError) {
            throw this.createError(
              'VALIDATION_ERROR',
              `Response validation failed: ${validationError.errors.map(e => e.message).join(', ')}`,
              500,
      ...
      if (!processedResponse.ok) {
        throw this.createError(
          this.getErrorCode(processedResponse.status),
          (data as Record<string, string>)?.message || 'Request failed',
          processedResponse.status,
          data,
          this.isRetryableStatus(processedResponse.status)
        );
      }
