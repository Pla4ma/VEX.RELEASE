 ...
 */
import { experience, accessFor, getFeatureAvailability, assertCoreAvailable } from './debloat-test-helpers';
...
describe('Group 4 — Completion', () => {
  it('4a: max 4 major beats in completion sequence', () => {
    ...
    const majorBeats = sequence.filter((s: string) => !['quiet_xp', 'next_action'].includes(s));
  ...
  it('4c: study completion shows study next step', () => {
    const exp = experience('study_focused', { studyUsageRatio: 0.7, totalCompletedSessions: 6 });
  ...
  it('4h: XP/streak/progress still update', () => {
    const exp = experience('study_focused', { completionStreak: 5, totalCompletedSessions: 10 });
    expect(exp.completionSequence).toContain('core_saved');
...
describe('Group 5 — Progressive Unlock Runtime', () => {
  it('5a: locked features do not query', () => {
    ...
    HIDDEN_FEATURE_KEYS.forEach((key: string) => {
      const a = getFeatureAvailability(accessFor(0)[key as keyof ReturnType<typeof accessFor>]);
      expect(a.canQuery).toBe(false);
    });
    HIDDEN_FEATURE_KEYS.forEach((key: string) => {
      const a = getFeatureAvailability(accessFor(999)[key as keyof ReturnType<typeof accessFor>]);
      expect(a.canQuery).toBe(false);
    });
  });
  ...
  it('5d: hidden features do not route', () => {
    const { HIDDEN_FEATURE_KEYS } = require('./debloat-test-helpers');
    HIDDEN_FEATURE_KEYS.forEach((key: string) => {
      const a = getFeatureAvailability(accessFor(5)[key as keyof ReturnType<typeof accessFor>]);
...
describe('Group 6 — Coach', () => {
  ...
  it('6b: after session 1 coach references real session', () => {
    const exp = experience('coach_led', { completedSessionDurations: [60], totalCompletedSessions: 1 });
    expect(exp.behaviorAdaptations).toContain('needs_more_signal');
