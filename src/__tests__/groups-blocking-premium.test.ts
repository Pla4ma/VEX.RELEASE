...
describe('premium RevenueCat gating tests — ci-required', () => {
  ...
  it('premium-billing contains monetization tests', () => {
    expect(groupContains(config.groups['premium-billing'], 'features/monetization')).toBe(true);
  });

  it('premium-billing contains shared monetization (entitlements)', () => {
    expect(groupContains(config.groups['premium-billing'], 'shared/monetization')).toBe(true);
  });
