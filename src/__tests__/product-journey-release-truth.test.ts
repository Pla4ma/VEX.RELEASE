...
import {
  experience, firstWeek, accessFor,
  HIDDEN_FEATURE_KEYS, assertFullyHidden, assertCoreAvailable,
...
describe('Group 7 — Release Truth', () => {
  ...
  it('7b: no runtime imports from src_impl_archive', () => {
    ...
    for (const file of srcFiles) {
      ...
      const hasImport = content.includes("'../src_impl_archive") || content.includes('"../src_impl_archive') ||
        content.includes("'../../src_impl_archive") || content.includes("'src_impl_archive") || content.includes('"src_impl_archive');
  ...
  it('7c: final-release deactivated features never visible', () => {
    for (const sessions of [0, 1, 5, 10, 50, 100, 999]) {
      ...
      HIDDEN_FEATURE_KEYS.forEach((key) => { expect(features[key]!.isUnlocked).toBe(false); });
    }
  });

  it('7d: app store copy excludes hidden features', () => {
    const hidden = ['battle_pass', 'shop', 'inventory', 'wagers', 'rivals', 'squads', 'leaderboards'];
    const exp = experience('calm', { totalCompletedSessions: 20 });
    for (const h of hidden) { expect(exp.hiddenSystems).toContain(h); }
    expect(exp.release.hidden).toEqual(expect.arrayContaining(['shop', 'inventory', 'battle_pass', 'wagers']));
  });

  it('7e: core execution loop always free', () => {
    const exp = experience('study_focused', { premiumFeatureAttempts: ['weekly_intelligence'], totalCompletedSessions: 7 });
  ...
  it('7f: no feature teases premium before user has real value', () => {
    expect(experience('calm', { totalCompletedSessions: 1 }).premium.shouldTease).toBe(false);
    expect(experience('calm', { totalCompletedSessions: 4 }).premium.shouldTease).toBe(false);
  });
...
describe('Consolidation — No regression', () => {
  ...
  it('core features available from day zero', () => {
    const f = accessFor(0);
    const core: FeatureKey[] = ['focus_session', 'home_tab', 'focus_tab', 'profile_tab', 'progress_view', 'ai_coach_basic'];
