...
import {
  createRescuePlan, buildRescueCompletionMemory,
  buildRescueCompletionRecord, generateRescueReflection,
...
describe('Phase 17 — Completion: Reflection', () => {
  it('generates reflection without shame for abandoned', () => {
    const plan = createRescuePlan({ userId: 'u1', lane: 'student', reason: 'anxious' });
  ...
  it('generates positive reflection for completed', () => {
    const plan = createRescuePlan({ userId: 'u1', lane: 'game_like', reason: 'too_big' });
    expect(generateRescueReflection(plan, 'completed')).toMatch(/Completed|step/i);
  });
});

describe('Phase 17 — Completion: Progress saved', () => {
  it('completion record captures duration, outcome, and next recommendation', () => {
    const plan = createRescuePlan({ userId: 'u1', lane: 'deep_creative', reason: 'unclear' });
  ...
  it('partial completion records worked=true', () => {
    const plan = createRescuePlan({ userId: 'u1', lane: 'minimal_normal', reason: 'tired' });
...
describe('Phase 17 — Completion: Memory candidate', () => {
  it('creates memory on completion', () => {
    const plan = createRescuePlan({ userId: 'u1', lane: 'student', reason: 'unclear' });
  ...
  it('partial completion creates lower confidence memory', () => {
    const plan = createRescuePlan({ userId: 'u1', lane: 'game_like', reason: 'tired' });
  ...
  it('abandoned completion creates low-confidence memory', () => {
    const plan = createRescuePlan({ userId: 'u1', lane: 'deep_creative', reason: 'anxious' });
...
describe('Phase 17 — Completion: Next action', () => {
  it.each(ALL_LANES)('%s provides lane-specific next recommendation', (lane) => {
