...
function day0Map(lane: Lane): HomeSurfaceMap {
  const m: Record<Lane, { style: string; goal: string; intensity: 'minimal' | 'medium' | 'strong' }> = {
    student: { style: 'study_focused', goal: 'study', intensity: 'medium' },
    game_like: { style: 'game_like', goal: 'work', intensity: 'strong' },
    deep_creative: { style: 'coach_led', goal: 'creative', intensity: 'medium' },
  ...
  return decideHomeSurfaces({
    ...
    personalizationProfile: {
      motivationStyle: p.style as any, primaryGoal: p.goal as any,
      gamificationIntensity: p.intensity, studyLayerName: 'Growth Path', userStage: 'new',
    },
    behaviorStats: {
      ...
      premiumFeatureAttempts: [], completionStreak: 0,
    },
    hasActiveStudyPlan: false, hasActiveRecommendation: false, hasActiveBoss: false,
    isFirstSession: true, laneProfile: { primaryLane: lane },
  });
}
...
describe('Phase 17 — Study Home (student) Day 0', () => {
  ...
  it('tiny study preview visible as tiny_tease', () => { expect(map.study_layer).toBe('tiny_tease'); });
  it('no upload CTA', () => {
  ...
  it('start_session is primary', () => { expect(map.start_session).toBe('primary'); });
});

describe('Phase 17 — Run Home (game_like) Day 0', () => {
  const map = day0Map('game_like');
  it('run_board blocked', () => { expect(map.run_board).toMatch(/hidden|blocked/); });
  it('no shop in surface keys', () => { expect(Object.keys(map)).not.toContain('shop'); });
  it('boss_full_cta blocked', () => { expect(map.boss_full_cta).toMatch(/hidden|blocked/); });
});

describe('Phase 17 — Project Home (deep_creative) Day 0', () => {
  const map = day0Map('deep_creative');
  it('project_thread blocked', () => { expect(map.project_thread).toMatch(/hidden|blocked/); });
  it('focus_window blocked', () => { expect(map.focus_window).toMatch(/hidden|blocked/); });
  it('start_session is primary', () => { expect(map.start_session).toBe('primary'); });
});

describe('Phase 17 — Clean Home (minimal_normal) Day 0', () => {
  const map = day0Map('minimal_normal');
  it('no boss teaser', () => { expect(map.boss_teaser).toBe('hidden'); });
  it('no boss compact', () => { expect(map.boss_compact).toBe('hidden'); });
  it('no challenge teaser', () => { expect(map.challenge_teaser).toBe('hidden'); });
  it('no weekly quest', () => { expect(map.weekly_quest).toBe('hidden'); });
  it('has at least 3 visible surfaces', () => { expect(visibleCount(map)).toBeGreaterThanOrEqual(3); });
  it('start_session is primary', () => { expect(map.start_session).toBe('primary'); });
  it('today_strip present', () => { expect(map.today_strip).not.toBe('hidden'); });
});

describe('Phase 17 — Day 0 system blocks across lanes', () => {
  const blockedSurfaces = ['memory_insight', 'weekly_intelligence', 'premium_tease', 'focus_score', 'progress_proof', 'progress_detail'];
