...
describe('Phase 17 — Notifications: Day 0 no unsolicited', () => {
  it.each(ALL_LANES)('blocks unsolicited nudges on Day 0 for %s', (lane) => {
    const d = decideNudge({ lane, completedSessions: 0, daysSinceOnboarding: 0 });
...
describe('Phase 17 — Notifications: Dismissal lowers priority', () => {
  it('recentDismissals >= 2 suppresses low-trust nudges', () => {
    const d = decideNudge({ lane: 'student', completedSessions: 4, daysSinceOnboarding: 3, recentDismissals: 2 });
    expect(d.allowed).toBe(false);
  ...
  it('rescue still allowed with dismissals when context is avoidance', () => {
    const d = decideNudge({
      lane: 'game_like', completedSessions: 4, daysSinceOnboarding: 3,
...
describe('Phase 17 — Notifications: Lane copy', () => {
  it('produces lane-specific copy', () => {
    const d = decideNudge({ lane: 'student', completedSessions: 4, daysSinceOnboarding: 3, context: 'deadline' });
    expect(d.allowed).toBe(true);
  ...
  it('clean lane nudge is gentle', () => {
    const d = decideNudge({ lane: 'minimal_normal', completedSessions: 2, daysSinceOnboarding: 2 });
    expect(d.allowed).toBe(true);
...
describe('Phase 17 — Notifications: Budget', () => {
  it('minimal_normal max 1/day', () => {
    const d = decideNudge({ lane: 'minimal_normal', completedSessions: 3, daysSinceOnboarding: 2, sentToday: 1 });
    expect(d.allowed).toBe(false);
    expect(d.budgetRemaining).toBe(0);
  });

  it('other lanes max 2/day', () => {
    const d = decideNudge({ lane: 'student', completedSessions: 3, daysSinceOnboarding: 2, sentToday: 2 });
    expect(d.allowed).toBe(false);
    expect(d.budgetRemaining).toBe(0);
  });
});
...
describe('Phase 17 — Notifications: User mute + Quiet hours', () => {
  it('blocks when user muted', () => {
    const d = decideNudge({ lane: 'student', completedSessions: 5, daysSinceOnboarding: 3, userMuted: true });
    expect(d.allowed).toBe(false);
    expect(d.reason).toContain('mute');
  });

  it('blocks during quiet hours', () => {
    const d = decideNudge({ lane: 'deep_creative', completedSessions: 3, daysSinceOnboarding: 2, quietHoursActive: true });
    expect(d.allowed).toBe(false);
