...
describe('Phase 17 — Hidden systems: Feature map', () => {
  const hidden = [
    'shop', 'inventory', 'battle_pass', 'wagers', 'squads', 'rivals', 'rankings',
    'economy_advanced', 'economy_basic', 'gems_prominent',
    'streak_insurance', 'boss_bounties', 'seasonal_features', 'social_tab',
  ];

  hidden.forEach((f) => {
    it(`${f} is hidden in final release`, () => { expect(isFeatureHidden(f)).toBe(true); });
  });

  hidden.forEach((f) => {
    it(`${f} is not included`, () => { expect(isFeatureIncluded(f)).toBe(false); });
  });

  const included = ['focus_session', 'progress_view', 'home_tab', 'ai_coach_basic', 'content_study'];
  included.forEach((f) => {
    it(`${f} is included`, () => { expect(isFeatureIncluded(f)).toBe(true); });
  });
});
...
describe('Phase 17 — Hidden systems: No economy surfaces in Day 0', () => {
  it('Day 0 surface keys exclude economy/social across all lanes', () => {
    const forbidden = ['shop', 'economy', 'social', 'wallet', 'wager', 'ranking'];
    ALL_LANES.forEach((lane) => {
      ...
      expect(keys.some((k) => forbidden.some((fp) => k.includes(fp)))).toBe(false);
    });
  });
});
...
describe('Phase 17 — Lane schema integrity', () => {
  it('LaneSchema has exactly 4 lanes', () => {
    expect(LaneSchema.options).toEqual(['student', 'game_like', 'deep_creative', 'minimal_normal']);
  });

  it('all lanes produce valid rescue plans', () => {
    ALL_LANES.forEach((lane) => {
      const plan = (() => {
        const { createRescuePlan } = require('../../features/rescue-mode/service');
