...
import { resolvePremiumTiming, EARLY_HIDDEN_THRESHOLD } from '../../features/monetization/premium-timing';
...
describe('Phase 17 — Premium: No Day 0 hard sell', () => {
  it('hidden_early for 0 completed sessions', () => {
    const r = resolvePremiumTiming({ completedSessions: 0, revenueCatHealthy: true, billingConfigured: true });
  ...
  it('hidden_early for all sessions < EARLY_HIDDEN_THRESHOLD', () => {
    for (let i = 0; i < EARLY_HIDDEN_THRESHOLD; i++) {
      expect(resolvePremiumTiming({ completedSessions: i, revenueCatHealthy: true, billingConfigured: true }).tier).toBe('hidden_early');
    }
  });
});
...
describe('Phase 17 — Premium: Hidden if RevenueCat unhealthy', () => {
  it('blocked_unhealthy when RevenueCat is down', () => {
    const r = resolvePremiumTiming({ completedSessions: 50, revenueCatHealthy: false, billingConfigured: true });
  ...
  it('blocked_unhealthy even with high session count', () => {
    const r = resolvePremiumTiming({ completedSessions: 100, revenueCatHealthy: false, billingConfigured: true });
    expect(r.tier).toBe('blocked_unhealthy');
    expect(r.reason).toMatch(/RevenueCat|billing|unhealthy|unavailable/i);
  });

  it('blocked_unhealthy when billing not configured', () => {
    const r = resolvePremiumTiming({ completedSessions: 50, revenueCatHealthy: true, billingConfigured: false });
    expect(r.tier).toBe('blocked_unhealthy');
  });
});
...
describe('Phase 17 — Premium: Durable value copy only', () => {
  it('premium reason does not contain economy terms', () => {
    const r = resolvePremiumTiming({ completedSessions: 40, revenueCatHealthy: true, billingConfigured: true });
    expect(r.reason).not.toMatch(/coin|gem|shop|inventory|chest|loot|battle|streak save|pay to win|xp boost/i);
  });
});
