...
describe('Phase 2F — Import blocking', () => {
  ...
  describe('3. Notification routing blocks dead systems', () => {
    ...
    it('open_chest routes to OPEN_HOME', () => {
      testBlocked('open_chest', 'OPEN_HOME');
    });
    it('open_shop routes to OPEN_HOME', () => {
      testBlocked('open_shop', 'OPEN_HOME');
    });
    it('view_squad routes to OPEN_HOME', () => {
      testBlocked('view_squad', 'OPEN_HOME');
    });
    it('join_duel routes to OPEN_HOME', () => {
      testBlocked('join_duel', 'OPEN_HOME');
    });
    it('custom routes fall back to OPEN_HOME', () => {
      testBlocked('custom', 'OPEN_HOME');
    });
    it('start_session routes to START_SESSION', () => {
      testBlocked('start_session', 'START_SESSION');
    });
    it('view_boss routes to OPEN_HOME when boss hidden', () => {
      testBlocked('view_boss', 'OPEN_HOME');
    });
    it('view_progress routes to OPEN_PROGRESS', () => {
      testBlocked('view_progress', 'OPEN_PROGRESS');
    });
  });
    ...
    it('notification action types include no live-ops/purchase routes', () => {
      ...
      for (const archived of archivedTypes) {
        const result = resolveNotificationAction({
          type: archived as NotificationActionType,
        });
