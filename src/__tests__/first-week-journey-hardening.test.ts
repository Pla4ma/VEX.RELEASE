...
function profile(lane: Lane) {
  if (lane === 'student') {
    return { gamificationIntensity: 'medium' as const, motivationStyle: 'student' as const, primaryGoal: 'study' as const, studyLayerName: 'Study OS', userStage: 'engaged' as const };
  }
  if (lane === 'game_like') {
    return { gamificationIntensity: 'strong' as const, motivationStyle: 'game_like' as const, primaryGoal: 'work' as const, studyLayerName: 'Deep Work Plan', userStage: 'engaged' as const };
  }
  if (lane === 'deep_creative') {
    return { gamificationIntensity: 'medium' as const, motivationStyle: 'coach_led' as const, primaryGoal: 'creative' as const, studyLayerName: 'Project Focus Path', userStage: 'engaged' as const };
  }
  return { gamificationIntensity: 'minimal' as const, motivationStyle: 'calm' as const, primaryGoal: 'focus' as const, studyLayerName: 'Deep Work Plan', userStage: 'engaged' as const };
}

function firstWeek(lane: Lane, completedSessions: number) {
  const p = profile(lane);
  return resolveFirstWeekExperience({
    behaviorStats: { bossEngagement: lane === 'game_like' ? 'high' : 'none', studyUsageRatio: lane === 'student' ? 0.7 : 0 },
    completedSessions,
    daysSinceLastSession: 0,
    daysSinceOnboarding: completedSessions,
    featureAvailability: { boss: true, premium: true, social: false, study: true },
    motivationStyle: p.motivationStyle === 'student' ? 'study_focused' : p.motivationStyle,
...
describe('first-week journey hardening', () => {
  it.each([
    ...
    ['deep_creative', 'project_thread', 'Resume project block', 'Project Focus Path'],
    ['minimal_normal', 'today_strip', 'Start clean session', 'Today Strip'],
  ] as const)('keeps %s journey wired through week one', (lane, surface, ctaLabel, pathName) => {
    const home = decideHomeSurfaces({
      behaviorStats: { ...stats(5), bossChallengeEngagement: lane === 'game_like' ? 'high' : 'none', projectFocusUsageRatio: lane === 'deep_creative' ? 0.7 : 0, studyUsageRatio: lane === 'student' ? 0.7 : 0 },
      featureAvailability,
    ...
    const completion = buildCompletionPersonalization({ lane, summary: createSessionSummary() });

    expect(firstWeek(lane, 5).hiddenSurfaces).toEqual(expect.arrayContaining(['shop', 'inventory', 'wagers']));
  ...
  it('covers comeback, offline setup, and minimal Today Strip recovery', () => {
    ...
    const setup = buildLaneSessionBrief({ isOffline: true, isRescue: true, lane: 'minimal_normal' });
    const today = buildTodaySystem({ dayFeelsMessy: true, lane: 'minimal_normal', reducedMotion: true });

    expect(comeback.allowedHomeSurfaces).toEqual(['coach_presence_line', 'recovery_cta', 'start_session']);
    expect(setup.offlineMessage).toContain('offline');
    expect(setup.suggestedDurationSeconds).toBeLessThanOrEqual(12 * 60);
    expect(today.sections.find((section) => section.key === 'recovery')?.visible).toBe(true);
  });
