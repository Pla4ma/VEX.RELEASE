import { loadGroups, groupContains, type GroupConfig } from './groups-validation-helpers';
...
describe('archived-economy group exists', () => {
  ...
  it('archived-economy contains economy tests', () => {
    expect(groupContains(config.groups['archived-economy'], 'economy')).toBe(true);
  });
  ...
  it('archived-economy contains wallet tests', () => {
    expect(groupContains(config.groups['archived-economy'], 'wallet')).toBe(true);
  });

  it('archived-economy contains inventory tests', () => {
    expect(groupContains(config.groups['archived-economy'], 'inventory')).toBe(true);
  });

  it('archived-economy contains battle-pass tests', () => {
    expect(groupContains(config.groups['archived-economy'], 'battle-pass')).toBe(true);
  });
});
