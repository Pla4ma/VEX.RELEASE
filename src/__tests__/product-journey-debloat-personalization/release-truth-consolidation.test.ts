...
describe('Group 7 — Release Truth', () => {
  ...
  it('7c: deactivated features never visible at any session count', () => {
  ...
  it('7d: app store copy excludes hidden features', () => {
    const hidden = [
      'battle_pass', 'shop', 'inventory', 'wagers',
      'rivals', 'squads', 'leaderboards',
    ];
  ...
  it('7g: all profiles get correct primary home CTA', () => {
...
describe('Consolidation — No regression', () => {
  it('hidden systems match between resolveVexExperience and firstWeek', () => {
    const exp = experience('calm', { totalCompletedSessions: 20 });
    ...
    for (const s of fw.hiddenSurfaces) {
      if (s === 'premium_hard_sell' || s === 'premium_currency' || s === 'advanced_economy') {
        expect(exp.hiddenSystems).toContain(
          s === 'premium_currency' ? 'premium_currency'
            : s === 'advanced_economy' ? 'advanced_economy'
            : 'premium_currency',
        );
      }
    }
  ...
  it('deactivated features inaccessible at every tested session count', () => {
  ...
  it('core features available from day zero', () => {
    ...
    const core: FeatureKey[] = [
      'focus_session', 'home_tab', 'focus_tab',
      'profile_tab', 'progress_view', 'ai_coach_basic',
    ];
