...
describe('cold-start runtime policy', () => {
  ...
  it('first Home fallback does not query lane-heavy systems', () => {
    const fallback = read('src/screens/home/containers/HomeColdStartFallback.tsx');
  ...
  it('premium offerings do not block first screen', () => {
    expectNoStaticImport([...rootNavigationFiles, 'src/screens/home/containers/HomeStageResolver.tsx'], [
      'shared/monetization',
      'screens/paywall',
      'premium-revenuecat-health-checks',
    ]);
    expect(read('src/navigation/RootNavigator.tsx')).not.toContain('useFeatureHealth');
  });
  ...
  it('hidden systems do not appear in the root static dependency graph', () => {
    expectNoStaticImport([...rootFiles, ...rootNavigationFiles], [
      'features/ai-coach',
      'features/boss',
      'features/content-study',
      'features/spectacle',
