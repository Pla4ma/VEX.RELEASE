import { makeFeatureAccess, resolveNotificationAction } from "./helpers";
...
describe("resolveNotificationAction", () => {
  ...
  it("view_squad with disabled squads → fallback to OPEN_HOME", () => {
    ...
    const result = resolveNotificationAction({ type: "view_squad" }, features);
    expect(result.intent).toBe("OPEN_HOME");
  ...
  it("open_coach with disabled AI coach → fallback to OPEN_HOME", () => {
    ...
    const result = resolveNotificationAction({ type: "open_coach" }, features);
    expect(result.intent).toBe("OPEN_HOME");
  });
