...
describe('pauseSession', () => {
  it('should pause active session', async () => {
    ...
    const pausedSession = await ctx.service.pauseSession(session.id, { reason: 'USER_INITIATED' });
  ...
  it('should track pause reason', async () => {
    const session = await ctx.service.createSession(mockSessionConfig);
    await ctx.service.startSession(session.id);
    await ctx.service.pauseSession(session.id, { reason: 'INTERRUPTION', interruptionId: 'int-123' });
