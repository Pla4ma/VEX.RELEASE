...
export const TIME_CONSTANTS = {
  ...
  DEFAULT_WARNING_THRESHOLDS: [300, 60, 10], // 5min, 1min, 10sec
} as const;
...
export function calculateElapsedTime(startTime: number, currentTime: number, pausedDuration: number = 0): number {
  if (startTime <= 0) {return 0;}
  if (currentTime < startTime) {return 0;}

  const rawElapsed = Math.floor((currentTime - startTime) / TIME_CONSTANTS.MILLISECONDS_PER_SECOND);
  return Math.max(0, rawElapsed - pausedDuration);
}

export function calculateRemainingTime(duration: number, elapsedTime: number): number {
  const remaining = duration - elapsedTime;
  return Math.max(0, remaining);
}

export function calculateCompletionPercentage(elapsed: number, duration: number): number {
...
export function breakdownDuration(totalSeconds: number): TimeBreakdown {
  const hours = Math.floor(totalSeconds / (TIME_CONSTANTS.SECONDS_PER_MINUTE * TIME_CONSTANTS.MINUTES_PER_HOUR));
  const remainingAfterHours = totalSeconds % (TIME_CONSTANTS.SECONDS_PER_MINUTE * TIME_CONSTANTS.MINUTES_PER_HOUR);

  const minutes = Math.floor(remainingAfterHours / TIME_CONSTANTS.SECONDS_PER_MINUTE);
...
export function formatDurationLong(totalSeconds: number): string {
  ...
  if (breakdown.seconds > 0 || parts.length === 0) {parts.push(`${breakdown.seconds}s`);}
...
export function calculateProgressMetrics(
  ...
  return {
    ...
    phase,
    interval,
    estimatedCompletionTime: Date.now() + (remaining * TIME_CONSTANTS.MILLISECONDS_PER_SECOND),
  };
}
...
export function getTimeStatus(
  ...
  if (elapsed <= 0 && !isPaused) {return 'PENDING';}
  if (isPaused) {return 'PAUSED';}
  if (elapsed >= duration) {return 'COMPLETE';}
  if (elapsed > duration * 1.5) {return 'OVERDUE';}
  return 'ACTIVE';
}
...
export function validateDuration(duration: number): { valid: boolean; error?: string } {
  ...
  if (duration < TIME_CONSTANTS.MIN_SESSION_DURATION_SECONDS) {
    return { valid: false, error: `Duration must be at least ${TIME_CONSTANTS.MIN_SESSION_DURATION_SECONDS} seconds` };
  }

  if (duration > TIME_CONSTANTS.MAX_SESSION_DURATION_SECONDS) {
    return { valid: false, error: `Duration cannot exceed ${TIME_CONSTANTS.MAX_SESSION_DURATION_SECONDS} seconds (8 hours)` };
  }
...
export function calculateCurrentInterval(
  ...
  while (timeRemaining > 0) {
    const isLongBreakInterval = interval % intervalsBeforeLongBreak === 0 && interval > 0;
    ...
    if (timeRemaining >= focusDuration) {
      ...
      // Break phase
      const currentBreakDuration = isLongBreakInterval ? longBreakDuration : breakDuration;
