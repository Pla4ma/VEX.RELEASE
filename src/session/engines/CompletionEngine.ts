...
export class CompletionEngine {
  ...
  completePartial(
    ...
    const adjustedScore =
      this.scoringEngine.calculateFinalScore(scoreCalc) * penaltyMultiplier;
    ...
    const rewardsGranted = this.scoringEngine.isEligibleForRewards(
  ...
  private createSessionSummary(
    ...
    const focusPurityScore =
      this.scoringEngine.calculateFocusPurityScore(session);
    ...
    const modeBonus = Math.max(
      0,
      scoreCalc.intervalBonus - this.getBaseIntervalBonus(session),
    );
    return {
      sessionId: session.id,
