 ...
 * ScoringEngine Tests
 *
 * Comprehensive test suite for the ScoringEngine.
 */
...
describe('ScoringEngine', () => {
  ...
  const createMockFocusMetrics = (overrides: Partial<FocusQualityMetrics> = {}): FocusQualityMetrics => ({
  ...
  describe('Streak Multipliers', () => {
    it('should apply streak multiplier', () => {
      engine.setUserStats(7, 1); // 7-day streak
  ...
  describe('Mode Differentiation', () => {
    ...
    it('halves light focus pause penalties', () => {
      ...
      expect(engine.calculateScore(lightSession, metrics).pausePenalty)
    ...
    it('adds creative mood bonus points', () => {
      ...
      expect(engine.calculateScore(session, createMockFocusMetrics()).intervalBonus)
