...
describe('BonusCalculator', () => {
  describe('calculateQualityBonus', () => {
    const createMetrics = (overallScore: number): FocusQualityMetrics => ({
      sessionId: 'test', timeInDeepFocus: 1000, timeInShallowFocus: 200,
      timeDistracted: 100, focusSegments: [], consistencyScore: overallScore,
      depthScore: overallScore, recoveryScore: overallScore,
      overallScore, calculatedAt: Date.now(),
    });

    it('should award excellent bonus for high quality', () => {
      const bonus = calculateQualityBonus({
        focusMetrics: createMetrics(95), interruptions: 0, pauses: 0,
      });
    ...
    it('should award good bonus for moderate quality', () => {
      const bonus = calculateQualityBonus({
        focusMetrics: createMetrics(80), interruptions: 0, pauses: 0,
      });
    ...
    it('should award average bonus for acceptable quality', () => {
      const bonus = calculateQualityBonus({
        focusMetrics: createMetrics(60), interruptions: 1, pauses: 2,
      });
    ...
    it('should not award bonus for poor quality', () => {
      const bonus = calculateQualityBonus({
        focusMetrics: createMetrics(30), interruptions: 5, pauses: 10,
      });
      expect(bonus).toBe(0);
    });
    ...
    it('should reduce bonus for disruptions', () => {
      ...
      const disruptedBonus = calculateQualityBonus({
        focusMetrics: createMetrics(95), interruptions: 3, pauses: 5,
      });
  ...
  describe('calculateIntervalBonus', () => {
    it('should award bonus for multiple intervals', () => {
      const bonus = calculateIntervalBonus({
        completedIntervals: 4, totalIntervals: 4, allIntervalsCompleted: false,
      });
    ...
    it('should award larger bonus for completing all intervals', () => {
      const partial = calculateIntervalBonus({
        completedIntervals: 3, totalIntervals: 4, allIntervalsCompleted: false,
      });
      ...
      expect(complete).toBeGreaterThanOrEqual(BONUS_CONSTANTS.POMODORO_COMPLETE_BONUS);
    });

    it('should scale with interval count', () => {
      const small = calculateIntervalBonus({
        completedIntervals: 2, totalIntervals: 4, allIntervalsCompleted: false,
      });
      const large = calculateIntervalBonus({
        completedIntervals: 8, totalIntervals: 8, allIntervalsCompleted: false,
      });
