...
export function calculateBaseScore(input: BaseScoreInput): BaseScoreResult {
  ...
  const durationPoints = Math.floor(effectiveMinutes * SCORING_CONSTANTS.BASE_SCORE_PER_MINUTE);
...
export function calculateBaseScoreFromSession(session: SessionState): BaseScoreResult {
...
export function validateScoringInput(input: BaseScoreInput): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (input.durationSeconds > 28800) { // 8 hours
