import type { SessionState, InterruptionRecord, RecoveryRecord, SessionSummary, FocusQualityMetrics } from "./types";
...
export function getActiveSession(orch: SessionAccessorHost): SessionState | null {
...
export function getPurityLabel(orch: SessionAccessorHost): "Elite" | "Good" | "Okay" | "Distracted" {
  return orch.antiCheatEngine.getPurityLabel();
}

export function getInterruptions(orch: SessionAccessorHost): InterruptionRecord[] {
...
export function applyStudyQuizBonus(orch: SessionAccessorHost, correctAnswers: number): void {
...
export function updateFocusQuality(orch: SessionAccessorHost, quality: number): void {
  if (!orch.session) return;
...
export function addDocument(orch: SessionAccessorHost, documentId: string): void {
  if (!orch.session) return;
...
export function removeDocument(orch: SessionAccessorHost, documentId: string): void {
  if (!orch.session) return;
  const docs: string[] = (orch.session.metadata?.documents as string[]) ?? [];
