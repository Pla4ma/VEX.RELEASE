...
export class PresetService {
  ...
  private initializeSystemPresets(): void {
    ...
    const now = Date.now();
    PresetService.systemPresets = DEFAULT_PRESETS.map((preset) => ({
      ...preset,
  ...
  private async loadUserPresets(): Promise<void> {
    ...
    try {
      const key = `session:presets:${this.userId}`;
      const storage = getMMKVStorageAdapter();
      const data = await storage.getItem(key);
      if (data) {
        const parsed = JSON.parse(data);
        this.userPresets = new Map(Object.entries(parsed));
        debug.info("Loaded %d user presets", this.userPresets.size);
      }
    } catch (error) {
  ...
  private async saveUserPresets(): Promise<void> {
    ...
    try {
      const key = `session:presets:${this.userId}`;
      const data = Object.fromEntries(this.userPresets);
      const storage = getMMKVStorageAdapter();
    ...
    try {
      ...
      for (const preset of presets) {
        try {
          ...
          const existing = Array.from(this.userPresets.values()).find(
            (p) => p.name === preset.name,
          );
          if (existing) {
            await this.updatePreset(existing.id, preset);
          } else {
