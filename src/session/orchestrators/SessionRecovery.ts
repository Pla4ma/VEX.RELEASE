...
import type { InterruptionRecord, RecoveryRecord, InterruptionType, InterruptionSeverity, SessionState } from "../types";
...
export async function attemptRecovery(
  ...
  if ((orch.session.recoveryAttempts || 0) >= (orch.session.maxRecoveryAttempts || 3)) return false;
  ...
  const result = orch.completionEngine.attemptRecovery(orch.session, recoveryType, orch.focusMetrics, 0);
  if (result.success) {
    orch.isActive = true;
    ...
    if (recoveryType === "USER_RESUME" && orch.timerEngine) orch.timerEngine.resume();
    orch.eventEmitter.emitRecoverySuccessful(Date.now(), result.summary.effectiveDuration);
...
export function completeLastInterruption(orch: SessionOrchestrator, duration: number): void {
...
export function recordInterruption(
  ...
  const interruption: InterruptionRecord = {
    id: uuidv4(), sessionId: orch.session.id, type, severity, timestamp: Date.now(),
    reason: "Session interruption", duration: 0, resolvedAt: Date.now(), autoRecovered: false,
  ...
  const damage = orch.scoringEngine.calculateDamage(orch.session, "INTERRUPTION");
  if (damage.totalDamage > 0) orch.eventEmitter.emitDamageTaken(damage.totalDamage, `Interruption: ${type}`);
  return interruption;
}
...
export function handleAntiCheatViolation(
  ...
  if (flags.length > 0) {
    ...
    orch.session.antiCheatStatus = orch.antiCheatEngine.getStatus() as SessionState["antiCheatStatus"];
...
export function logInterruption(
  ...
  if (!orch.session) return;
  const severity: InterruptionSeverity = type === "USER_PAUSE" ? "MINOR" : "MODERATE";
...
export function logRecovery(
  ...
  if (!orch.session) return;
  const recovery: RecoveryRecord = {
    id: uuidv4(), sessionId: orch.session.id, type: type as RecoveryRecord["type"],
    timestamp: Date.now(), recoveredTime: 0, success: true, attemptedAt: Date.now(),
  };
  orch.recoveries.push(recovery);
