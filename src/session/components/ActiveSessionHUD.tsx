 ...
 * Active Session HUD
 *
 * Real-time session display with timer, progress, and status indicators.
 * Shows current phase, time remaining, completion percentage.
 */
...
export const ActiveSessionHUD: React.FC<ActiveSessionHUDProps> = ({
  ...
  const [companionState, setCompanionState] = React.useState<CompanionState | null>(null);
  ...
  React.useEffect(() => {
    ...
    const loadCompanionState = async () => {
      try {
        const state = await companionService.getCompanionState(userId);
  ...
  const getStatusColor = (): string => {
    if (isPaused) {return '#FFA500';} // Orange
  ...
  return (
    <Animated.View style={[styles.container, pulseStyle]}>
      ...
      {/* Living Companion - Primary Visual */}
      {companionState && (
        <View style={styles.companionContainer}>
          <LivingCompanion
            ...
            totalSeconds={session.duration || 1800}
            isPaused={isPaused}
          />
        </View>
      )}
      ...
      <View style={styles.progressContainer}>
        <View style={styles.progressBar}>
          <View
            style={[
              styles.progressFill,
              { width: `${completionPercentage}%`, backgroundColor: getStatusColor() },
            ]}
          />
        </View>
      ...
      <View style={styles.controls}>
        ...
        <Pressable
          style={({ pressed }) => [styles.button, styles.dangerButton, pressed && { opacity: 0.8 }]}
          onPress={() => {
            abandonSession();
            onAbandon?.();
          }}
