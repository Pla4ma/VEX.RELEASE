  ...
  return (
    <Animated.View entering={FadeInUp.duration(400)} exiting={FadeOut.duration(300)}>
      <Box flexDirection="row" alignItems="center" gap="sm" px="md" py="sm" borderRadius="xl"
        bg={`${theme.colors.success[500]}20`} borderWidth={1} borderColor={theme.colors.success.DEFAULT}
      >
        <Text fontSize={16}>✓</Text>
        <Text variant="bodySmall" color={theme.colors.success.DEFAULT} fontWeight="600">{message}</Text>
      </Box>
    </Animated.View>
  );
}

export function SquadEncouragementToastView({ message }: { message: string }): JSX.Element {
  const { theme } = useTheme();

  return (
    <Animated.View entering={FadeInUp.duration(400)} exiting={FadeOut.duration(300)}>
      <Box flexDirection="row" alignItems="center" gap="sm" px="md" py="sm" borderRadius="xl"
        bg={`${theme.colors.primary[500]}20`} borderWidth={1} borderColor={theme.colors.primary[500]}
      >
        <Text fontSize={16}>💪</Text>
        <Text variant="bodySmall" color={theme.colors.primary[500]} fontWeight="600">{message}</Text>
      </Box>
    </Animated.View>
  );
}
