...
describe('SessionService', () => {
  ...
  beforeEach(() => {
    ...
    service = createSessionService(mockUserId);
  });

  describe('createSession', () => {
    it('should create a new session with valid config', async () => {
      const session = await service.createSession(mockSessionConfig);
    ...
    it('should reject session with duration below minimum', async () => {
      const invalidConfig = { ...mockSessionConfig, duration: 30 }; // Below 60s minimum

      await expect(service.createSession(invalidConfig)).rejects.toThrow('duration');
    });

    it('should reject session with duration above maximum', async () => {
      const invalidConfig = { ...mockSessionConfig, duration: 90000 }; // Above 24h maximum

      await expect(service.createSession(invalidConfig)).rejects.toThrow('duration');
    });

    it('should emit session:created event', async () => {
      await service.createSession(mockSessionConfig);

      expect(eventBus.publish).toHaveBeenCalledWith('session:created', expect.any(Object));
    });

    it('should persist session to repository', async () => {
      const session = await service.createSession(mockSessionConfig);

      expect(mockRepository.saveSession).toHaveBeenCalledWith(session);
    });
  });
  ...
  describe('startSession', () => {
    it('should transition session from CREATED to ACTIVE', async () => {
      const session = await service.createSession(mockSessionConfig);
      const startedSession = await service.startSession(session.id);
    ...
    it('should emit session:started event', async () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('session:started', expect.any(Object));
    });

    it('should reject starting non-existent session', async () => {
      await expect(service.startSession('non-existent-id')).rejects.toThrow('not found');
    });
    ...
    it('should reject starting already active session', async () => {
      ...
      await expect(service.startSession(session.id)).rejects.toThrow('already active');
    });
  });
  ...
  describe('pauseSession', () => {
    it('should pause active session', async () => {
      ...
      const pausedSession = await service.pauseSession(session.id, { reason: 'USER_INITIATED' });
    ...
    it('should emit session:paused event', async () => {
      const session = await service.createSession(mockSessionConfig);
      await service.startSession(session.id);
      await service.pauseSession(session.id, { reason: 'USER_INITIATED' });

      expect(eventBus.publish).toHaveBeenCalledWith('session:paused', expect.any(Object));
    });
    ...
    it('should track pause reason', async () => {
      ...
      await service.pauseSession(session.id, { reason: 'INTERRUPTION', interruptionId: 'int-123' });

      expect(eventBus.publish).toHaveBeenCalledWith(
        'session:paused',
        expect.objectContaining({ reason: 'INTERRUPTION', interruptionId: 'int-123' })
      );
    });
  });
  ...
  describe('resumeSession', () => {
    it('should resume paused session', async () => {
      ...
      const resumedSession = await service.resumeSession(session.id);
    ...
    it('should emit session:resumed event', async () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('session:resumed', expect.any(Object));
    });
  });
  ...
  describe('completeSession', () => {
    it('should complete session and calculate rewards', async () => {
      ...
      const completedSession = await service.completeSession(session.id, {
    ...
    it('should emit session:completed event', async () => {
      const session = await service.createSession(mockSessionConfig);
      await service.startSession(session.id);
      await service.completeSession(session.id, { completionPercentage: 100 });
  ...
  describe('abandonSession', () => {
    it('should mark session as abandoned', async () => {
      ...
      const abandonedSession = await service.abandonSession(session.id, 'USER_CANCELLED');
    ...
    it('should emit session:abandoned event', async () => {
      const session = await service.createSession(mockSessionConfig);
      await service.startSession(session.id);
      await service.abandonSession(session.id, 'USER_CANCELLED');
  ...
  describe('error handling', () => {
    it('should handle repository save failure', async () => {
      mockRepository.saveSession.mockRejectedValue(new Error('DB error'));

      await expect(service.createSession(mockSessionConfig)).rejects.toThrow('DB error');
    ...
    it('should handle concurrent session operations', async () => {
      ...
      // Try to pause twice simultaneously
      const pause1 = service.pauseSession(session.id, { reason: 'TEST' });
      const pause2 = service.pauseSession(session.id, { reason: 'TEST' });
  ...
  describe('session history', () => {
    it('should retrieve session history', async () => {
      ...
      mockRepository.getSessionHistory.mockResolvedValue(mockHistory);

      const history = await service.getSessionHistory(mockUserId, { limit: 10 });

      expect(history).toHaveLength(2);
      expect(mockRepository.getSessionHistory).toHaveBeenCalledWith(mockUserId, { limit: 10 });
    });
  });
});
