...
describe("SessionService", () => {
  ...
  beforeEach(() => {
    ...
    } as unknown as jest.Mocked<ReturnType<typeof getSessionRepository>>;
  ...
  describe("createSession", () => {
    ...
    it("should reject session with duration below minimum", async () => {
      ...
      await expect(service.createSession(invalidConfig)).rejects.toThrow(
        "duration",
      );
    });
  ...
  describe("startSession", () => {
    ...
    it("should reject starting already active session", async () => {
      const session = await service.createSession(mockSessionConfig);
      await service.startSession(session.id);
      await expect(service.startSession(session.id)).rejects.toThrow(
        "already active",
      );
    });
  });
  describe("pauseSession", () => {
    it("should pause active session", async () => {
      const session = await service.createSession(mockSessionConfig);
      await service.startSession(session.id);
      const pausedSession = await service.pauseSession(session.id, {
  ...
  describe("abandonSession", () => {
    it("should mark session as abandoned", async () => {
      const session = await service.createSession(mockSessionConfig);
      await service.startSession(session.id);
      const abandonedSession = await service.abandonSession(
        session.id,
        "USER_CANCELLED",
      );
  ...
  describe("getSessionState", () => {
    ...
    it("should calculate remaining time correctly", async () => {
      ...
      await service.startSession(session.id);
      const state = service.getSessionState(session.id);
      expect(state.remainingSeconds).toBeLessThanOrEqual(
  ...
  describe("error handling", () => {
    it("should handle repository save failure", async () => {
      ...
      await expect(service.createSession(mockSessionConfig)).rejects.toThrow(
        "DB error",
      );
      expect(eventBus.publish).toHaveBeenCalledWith(
        "session:failed",
  ...
  describe("session history", () => {
    it("should retrieve session history", async () => {
      ...
      const history = await service.getSessionHistory(mockUserId, {
        limit: 10,
      });
