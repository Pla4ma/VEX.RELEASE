...
export type CompanionModeBehaviorV2 = "focused" | "intense" | "gentle";
export interface SessionModeV2Config {
  ...
  scoringWeights: { consistency: number; depth: number; recovery: number };
  companionBehavior: CompanionModeBehaviorV2;
  comebackBonusActive: boolean;
}
export const SESSION_MODE_V2_CONFIG: Record<
  SessionModeV2,
  SessionModeV2Config
> = {
  [SessionModeV2.FLOW]: {
  ...
  [SessionModeV2.CHALLENGE]: {
    ...
    bossAttackFrequency: 4,
    pausePenaltyMultiplier: 2.0,
    ...
    scoringWeights: { consistency: 0.45, depth: 0.45, recovery: 0.1 },
  ...
  [SessionModeV2.RECOVERY]: {
    ...
    scoringWeights: { consistency: 0.2, depth: 0.3, recovery: 0.5 },
...
export function isModeEligibleV2(
  ...
  if (mode === SessionModeV2.CHALLENGE && userLevel < 5) {
    return {
      eligible: false,
      reason: "Reach level 5 to unlock Challenge mode",
    };
  }
