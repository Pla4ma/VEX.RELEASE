    ...
    it('should unlock first completion achievement', () => {
      ...
      if (completionPercentage >= 100) {
        eventBus.publish('achievement:unlock', {
          achievementId: 'first_complete_session', userId,
        });
      }
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        'achievement:unlock',
        expect.objectContaining({ achievementId: 'first_complete_session', userId }),
      );
    });
    ...
    it('should unlock week warrior badge', () => {
      ...
      if (streakDays >= 7) {
        eventBus.publish('achievements:unlock_badge', {
          userId, badgeId: 'week_warrior', rarity: 'silver',
        });
      }
      expect(mockEventBus.publish).toHaveBeenCalledWith(
    ...
    it('should track 100 hour milestone', () => {
      ...
      if (totalFocusHours >= 100) {
        eventBus.publish('session:analytics:milestone', {
          sessionId: 'test', userId, milestone: '100_hours_focused',
          value: totalFocusHours, timestamp: Date.now(),
        });
      }
      expect(mockEventBus.publish).toHaveBeenCalledWith(
    ...
    it('should handle missing summary gracefully', () => {
      ...
      expect(() => {
        eventBus.publish('session:completed', {
          sessionId, userId, summary: null, timestamp: Date.now(), duration: 1500,
        });
      }).not.toThrow();
    });

    it('should handle zero duration sessions', () => {
      const summary = createMockSummary({ actualDuration: 0, effectiveDuration: 0 });
      const baseXP = Math.floor(summary.effectiveDuration / 60) * 10;
      expect(baseXP).toBe(0);
    });

    it('should handle very long sessions', () => {
      const summary = createMockSummary({ actualDuration: 28800, effectiveDuration: 28800 });
      const baseXP = Math.floor(summary.effectiveDuration / 60) * 10;
