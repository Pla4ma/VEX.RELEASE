...
describe('SessionCoachIntegration CoachPresence delegation', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    availabilityMock.mockReturnValue(available as ReturnType<typeof getAvailabilityFor>);
  ...
  it('does not interrupt active focus for non-critical risk', () => {
    new SessionCoachIntegration();
    const riskHandler = eventBusMock.subscribe.mock.calls.find(([event]) => event === 'session:interruption:risk')?.[1];

    riskHandler?.({ sessionId: 's1', riskLevel: 'HIGH' });

    expect(eventBusMock.publish).not.toHaveBeenCalledWith('coach:intent', expect.any(Object));
  });
  ...
  it('runtime file does not contain old generic coach spam', () => {
    const source = require('fs').readFileSync('src/session/integration/SessionCoachIntegration.ts', 'utf8') as string;

    expect(source).not.toMatch(/Stay focused|doing great|focus is at risk|You're crushing it|You've got this/);
  });
});
