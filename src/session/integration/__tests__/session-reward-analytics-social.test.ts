...
describe('SessionRewardIntegration', () => {
  ...
  describe('analytics system integration', () => {
    it('should emit session completion analytics', () => {
      ...
      eventBus.publish('analytics:track', {
        ...
        properties: {
          userId, duration: summary.actualDuration,
          completion: summary.completionPercentage,
          xpEarned: summary.xpEarned, streakDays: 5,
          interruptions: summary.interruptions, pauses: summary.pauses,
        },
      });
    ...
    it('should emit engagement metrics', () => {
      ...
      eventBus.publish('session:analytics:engagement', {
        sessionId: 'test', userId, metric: 'focus_time',
        value: focusTime, timestamp: Date.now(),
      });
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        'session:analytics:engagement',
        expect.objectContaining({ userId, metric: 'focus_time', value: focusTime }),
      );
    });
  });
  ...
  describe('social system integration', () => {
    it('should create social activity on completion', () => {
      const userId = 'user-123';
      const summary = createMockSummary({ actualDuration: 1500, xpEarned: 250 });
  ...
  describe('challenge system integration', () => {
    it('should progress daily focus time challenge', () => {
      ...
      eventBus.publish('challenge:progress', {
        userId, challengeId: 'daily_focus_time',
        progress: focusTime, target: 3600,
    ...
    it('should progress streak challenge', () => {
      ...
      const streakDays = 5;
      eventBus.publish('challenge:progress', {
        userId, challengeId: 'maintain_streak',
        progress: streakDays, target: 7,
    ...
    it('should progress perfect session challenge', () => {
      ...
      if (interruptions === 0 && pauses === 0) {
        eventBus.publish('challenge:progress', {
          userId, challengeId: 'perfect_sessions',
          progress: 1, target: 10, percent: 10,
        });
      }
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        'challenge:progress',
