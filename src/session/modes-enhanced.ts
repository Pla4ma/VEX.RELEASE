...
export const ENHANCED_SESSION_MODE_CONFIG: Partial<
  Record<SessionMode, EnhancedSessionModeConfig>
> = {
  [SessionMode.FLOW]: {
    ...
    description:
      "Gentle focus sessions with forgiving mechanics. Perfect for building habits.",
  ...
  [SessionMode.CHALLENGE]: {
    ...
    tacticalNote:
      "Boss enters shield phase at 50% and 25%. Maintain 85%+ purity or take damage!",
  ...
  [SessionMode.CREATIVE]: {
    ...
    description:
      "Flow-state creativity. Orbs of inspiration appear mid-session.",
    tacticalNote:
      "Idea Orbs appear at 40% boss health. Tap them for bonus damage!",
    idealFor: ["Writing", "Design", "Brainstorming", "Art"],
  },
};
export function getEnhancedModeConfig(
  mode: SessionMode,
): EnhancedSessionModeConfig {
  return (
    ENHANCED_SESSION_MODE_CONFIG[mode] ??
    ENHANCED_SESSION_MODE_CONFIG[SessionMode.FLOW]!
  );
}
...
export const MODE_COMPATIBILITY_MATRIX: Partial<
  Record<
    SessionMode,
    {
...
export function getModeEffectivenessAgainst(
  ...
  const compat =
    MODE_COMPATIBILITY_MATRIX[attackerMode] ??
    MODE_COMPATIBILITY_MATRIX[SessionMode.FLOW]!;
