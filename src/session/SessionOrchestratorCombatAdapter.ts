import {
  RealTimeBossService,
  createBossEncounter,
} from "../features/boss-realtime/service";
import type {
  RealTimeBossEncounter,
  CombatEvent,
} from "../features/boss-realtime/types";
...
export class SessionCombatAdapter {
  ...
  tick(): CombatTickResult {
    if (!this.bossService || !this.encounter || this.isPaused) {
      return { damageDealt: 0, bossDefeated: false, combatStateChanged: false };
    }
    const now = Date.now();
...
export function createCombatAdapter(
  config: CombatAdapterConfig,
): SessionCombatAdapter {
