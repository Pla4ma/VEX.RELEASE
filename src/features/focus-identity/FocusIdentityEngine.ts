...
const IDENTITY_STATEMENTS: Record<
...
export class FocusIdentityEngine {
  ...
  private calculateDiversityFactor(
    ...
    const uniqueContexts = new Set(sessions.map((s) => s.context || "default"))
      .size;
    let score = 0;
  ...
  public async calculateFocusScore(data: {
    ...
    currentStreak: number;
    longestStreak: number;
    streakHistory: Array<{ start: number; end: number | null; length: number }>;
  ...
  public calculateScoreChange(
    ...
    if (modifiers.sessionGrade && change > 0) {
      ...
      change +=
        gradeBonus[modifiers.sessionGrade as keyof typeof gradeBonus] || 0;
    }
