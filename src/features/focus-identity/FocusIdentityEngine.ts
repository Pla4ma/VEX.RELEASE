...
export const FOCUS_SCORE_CONFIG = {
  ...
  FACTOR_WEIGHTS: {
    ...
    STREAK_STABILITY: 0.30, // Length of streak history
    SESSION_QUALITY: 0.15,  // Focus purity, grades
    DIVERSITY: 0.10,        // Different modes, times, contexts
    RECENCY: 0.10,          // Recent activity weighting
  },
...
export class FocusIdentityEngine {
  ...
  private calculateStreakStabilityFactor(
    currentStreak: number,
    longestStreak: number,
    streakHistory: { start: number; end: number | null; length: number }[]
  ): FocusScoreFactors['streakStability'] {
    // Calculate average streak length
    const completedStreaks = streakHistory.filter(s => s.end !== null);
    const avgLength = completedStreaks.length > 0
      ? completedStreaks.reduce((sum, s) => sum + s.length, 0) / completedStreaks.length
      : 0;

    // Calculate break frequency (breaks per month)
    const now = Date.now();
    const breaksLast90Days = completedStreaks.filter(
      s => now - s.end! < 90 * 24 * 60 * 60 * 1000
    ).length;
  ...
  private calculateSessionQualityFactor(
    ...
    // Grade distribution
    const gradeCounts = completed.reduce((acc, s) => {
  ...
  private calculateDiversityFactor(
    ...
    const timeSlots = new Set(sessions.map(s => {
  ...
  private calculateRecencyFactor(
    ...
    if (daysSinceLastSession === 0) {score -= 0;}
    else if (daysSinceLastSession === 1) {score -= 10;}
    else if (daysSinceLastSession === 2) {score -= 25;}
    else if (daysSinceLastSession >= 3) {score -= 50;}
  ...
  public async calculateFocusScore(data: {
    sessionsLast30Days: number;
    targetSessionsPerWeek: number;
    missedDaysLast30Days: number;
    currentStreak: number;
    ...
    const factors: FocusScoreFactors = {
      ...
      streakStability: this.calculateStreakStabilityFactor(
        data.currentStreak,
        data.longestStreak,
        data.streakHistory
      ),
      sessionQuality: this.calculateSessionQualityFactor(data.sessionDetails),
      diversity: this.calculateDiversityFactor(data.sessionDetails.map(s => ({
        mode: s.mode,
        hour: new Date(s.startTime).getHours(),
        dayOfWeek: new Date(s.startTime).getDay(),
        context: s.context,
      }))),
      recency: this.calculateRecencyFactor(
    ...
    const composite =
      factors.consistency.score * FOCUS_SCORE_CONFIG.FACTOR_WEIGHTS.CONSISTENCY +
      factors.streakStability.score * FOCUS_SCORE_CONFIG.FACTOR_WEIGHTS.STREAK_STABILITY +
      factors.sessionQuality.score * FOCUS_SCORE_CONFIG.FACTOR_WEIGHTS.SESSION_QUALITY +
  ...
  public getScoreBand(score: number): ScoreBand {
    ...
    ) || FOCUS_SCORE_CONFIG.BANDS[FOCUS_SCORE_CONFIG.BANDS.length - 1];
  }
...
export class FocusIdentityService {
  ...
  public async getMonthlyReport(): Promise<FocusIdentityProfile['monthlyReport']> {
    ...
    const sessionsCompleted = profile.scoreHistory.filter(
      h => h.reason.includes('SESSION_COMPLETE') && new Date(h.date) >= monthStart
    ).length;
    ...
    const highlights = [
      ...
      sessionsCompleted >= 20 ? `${sessionsCompleted} sessions completed this month` : null,
