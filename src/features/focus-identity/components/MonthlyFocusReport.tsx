import React,{useMemo}from'react'; import{View,Pressable,Share}from'react-native'; import Animated,{useSharedValue,useAnimatedStyle,withSpring,withTiming,FadeIn,SlideInDown}from'react-native-reanimated'; import{Text}from'../../../components/primitives/Text'; import{Box}from'../../../components/primitives/Box'; import{Button}from'../../../components/primitives/Button'; import{useTheme}from'../../../theme'; import{useMonthlyReport}from'../hooks'; import{useFocusScoreColor}from'../hooks'; import{publishMonthlyReportViewed,publishMonthlyReportShared,publishMonthlyReportDismissed}from'../events';
import { launchColors } from '@theme/tokens/launch-colors';
 interface MonthlyFocusReportProps{userId:string;onClose:()=>void;visible:boolean;}function MonthlyReportSkeleton():JSX.Element{const{theme} = useTheme(); return<View style={{flex:1,backgroundColor:theme.colors.background.primary,paddingHorizontal:theme.spacing[6],paddingTop:theme.spacing[8]}}>
      <View style={{flexDirection:'row',justifyContent:'space-between',marginBottom:theme.spacing[4]}}>
        <View style={{width:200,height:32,backgroundColor:theme.colors.border?.DEFAULT || launchColors.hex_e5e5e5,borderRadius:4}}/>
        <View style={{width:32,height:32,backgroundColor:theme.colors.border?.DEFAULT || launchColors.hex_e5e5e5,borderRadius:16}}/>
      </View>
      <View style={{gap:theme.spacing[4]}}>
        <View style={{height:200,backgroundColor:theme.colors.surface?.card || launchColors.hex_f5f5f5,borderRadius:16}}/>
        <View style={{height:150,backgroundColor:theme.colors.surface?.card || launchColors.hex_f5f5f5,borderRadius:16}}/>
        <View style={{height:100,backgroundColor:theme.colors.surface?.card || launchColors.hex_f5f5f5,borderRadius:16}}/>
      </View>
