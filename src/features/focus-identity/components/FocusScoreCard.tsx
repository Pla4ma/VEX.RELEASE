...
export function FocusScoreCard({
  ...
  if (loadingState === 'error' && !profile) {
    return (
      ...
      >
        <Text variant="heading3" color="error" style={{ fontSize: 32 }}>⚠️</Text>
        ...
        <Pressable
          onPress={retry}
          disabled={isRetrying}
          style={{ paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8, marginTop: 8, backgroundColor: theme.colors.primary[500] }}
        >
      ...
      <Animated.View style={[animatedStyles]}>
        ...
        >
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <View>
              <Text
                variant={size === 'large' ? 'display' : size === 'small' ? 'heading3' : 'heading2'}
                color="text"
          ...
          {showTrend && scoreChange !== 0 && (
            <View style={{ marginTop: 8 }}>
              ...
              >
                {isPositiveChange ? '↑' : '↓'} {Math.abs(scoreChange)}
                {' '}from last check
              </Text>
            </View>
          )}
