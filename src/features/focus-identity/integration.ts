...
function readSummaryMetric(summary: unknown, key: string): unknown {
...
export function getXpMultiplierForScore(score: number): number {
  if (score >= 800) {return 2.0;}     // Legendary: 2x XP
  if (score >= 740) {return 1.75;}    // Elite: 1.75x
  if (score >= 670) {return 1.5;}     // Exceptional: 1.5x
  if (score >= 580) {return 1.25;}    // Strong: 1.25x
  if (score >= 500) {return 1.0;}      // Good: 1x
  if (score >= 420) {return 0.9;}      // Fair: 0.9x
 ...
 */
async function maybeTriggerMonthlyReport(
  userId: string,
  profile: any
): Promise<void> {
  try {
