...
jest.mock("../events", () => ({
  publishMonthlyReportViewed: (...args: unknown[]) =>
    mockPublishViewed(...args),
  publishMonthlyReportShared: (...args: unknown[]) =>
    mockPublishShared(...args),
  publishMonthlyReportDismissed: (...args: unknown[]) =>
    mockPublishDismissed(...args),
}));
...
describe("MonthlyFocusReport", () => {
  ...
  it("renders loading skeleton while the report is pending", () => {
    ...
    const { toJSON } = render(
      <MonthlyFocusReport userId="user-123" onClose={mockOnClose} visible />,
    );
  ...
  it("renders report data", () => {
    ...
    expect(
      screen.getByText("Outstanding consistency this month!"),
    ).toBeTruthy();
  });
  ...
  it("publishes view event when report loads", async () => {
    ...
    await waitFor(() => {
      expect(mockPublishViewed).toHaveBeenCalledWith(
        "user-123",
        "2025-01",
        "A",
        70,
