...
export async function analyzePeakFocusWindow(
  ...
  try {
    ...
    debug.error(
      "Error analyzing peak focus window",
...
async function generateStreakNotification(
  userId: string,
...
async function generateBossNotification(
  userId: string,
): Promise<NotificationContent | null> {
  try {
...
async function generateSocialNotification(
  userId: string,
): Promise<NotificationContent | null> {
  try {
...
async function generatePositiveNotification(
  userId: string,
): Promise<NotificationContent | null> {
  try {
...
async function generateRankReportNotification(
  userId: string,
): Promise<NotificationContent | null> {
  try {
    const now = new Date();
    ...
    if (percentile >= 99) {
      ...
      tier = "DIAMOND";
      tierIcon = "💎";
      sessionsToNext = Math.ceil(
        (0.99 * totalUsers - (totalUsers - rankPosition)) * 25,
      );
    } else if (percentile >= 90) {
      tier = "PLATINUM";
      tierIcon = "⭐";
      sessionsToNext = Math.ceil(
        (0.95 * totalUsers - (totalUsers - rankPosition)) * 25,
      );
    } else if (percentile >= 75) {
      tier = "GOLD";
      tierIcon = "🥇";
      sessionsToNext = Math.ceil(
        (0.9 * totalUsers - (totalUsers - rankPosition)) * 25,
      );
    } else if (percentile >= 50) {
      tier = "SILVER";
      tierIcon = "🥈";
      sessionsToNext = Math.ceil(
        (0.75 * totalUsers - (totalUsers - rankPosition)) * 25,
      );
    } else {
      tier = "BRONZE";
      tierIcon = "🥉";
      sessionsToNext = Math.ceil(
        (0.5 * totalUsers - (totalUsers - rankPosition)) * 25,
      );
    }
...
async function generateComebackNotification(
  userId: string,
): Promise<NotificationContent | null> {
  try {
...
export async function processSmartNotifications(): Promise<void> {
  try {
    ...
    debug.error(
      "Error processing smart notifications",
      error instanceof Error ? error : undefined,
    );
  }
}
export async function processUserSmartNotification(
  ...
  try {
    ...
    if (!isInPeakWindow(peakWindow.peakHour)) {
      debug.info("Not in peak window", {
        userId,
        peakHour: peakWindow.peakHour,
      });
      return;
    }
    ...
    debug.info("Would send smart notification", {
      userId,
      title: content.title,
    });
