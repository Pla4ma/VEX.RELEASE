...
export const StreakInputSchema = z.object({
  userId: UserIdSchema,
...
export type ReminderDraft = { type: RetentionReminderType; scheduledFor: number; message: string; metadata: Record<string, unknown> };
...
export function localTimestamp(timezone: string, year: number, month: number, day: number, hour: number): number {
...
export function daysLaterAt(days: number, hour: number, timezone: string): number {
