...
export function resolveUserBehaviorSignals(input: BehaviorResolverInput): BehaviorSignalSummary {
  ...
  const premiumGateClickedCount = countByType(recentSignals, 'premium_gate_clicked');
  const premiumGateSeenCount = countByType(recentSignals, 'premium_gate_seen');
  const premiumAttemptSurfaces = countDistinctSurfaces(recentSignals, 'premium_gate_clicked');
  const premiumFeatureAttempts = premiumGateClickedCount >= SIGNIFICANT_CLICK_WINDOW
  ...
  const coachSurfaceClicked = countByType(recentSignals, 'coach_surface_clicked');
  const coachInteractions = Math.max(
    recentSessions.completedSessions > 0 ? 1 : 0,
    coachSurfaceClicked,
  );

  const studySurfaceClicked = countByType(recentSignals, 'study_surface_clicked');
  const studyUsageRatio = recentSessions.totalSessions > 0
    ? Math.min(1, (recentSessions.studySessions + studySurfaceClicked * 0.5) / recentSessions.totalSessions)
    : 0;

  const deepWorkSessions = recentSessions.deepWorkSessions ?? 0;
  const deepWorkUsageRatio = recentSessions.totalSessions > 0
    ? Math.min(1, deepWorkSessions / recentSessions.totalSessions)
    : 0;

  const learningSessions = recentSessions.learningSessions ?? 0;
  const learningUsageRatio = recentSessions.totalSessions > 0
    ? Math.min(1, learningSessions / recentSessions.totalSessions)
    : 0;

  const creativeSessions = recentSessions.creativeSessions ?? 0;
  const projectFocusUsageRatio = recentSessions.totalSessions > 0
    ? Math.min(1, creativeSessions / recentSessions.totalSessions)
    : 0;

  const structuredExecutionTotal = deepWorkSessions + learningSessions + creativeSessions;
  const structuredExecutionUsageRatio = recentSessions.totalSessions > 0
    ? Math.min(1, (structuredExecutionTotal + studySurfaceClicked * 0.5) / recentSessions.totalSessions)
    : 0;

  const premiumAttemptCount = premiumGateClickedCount + premiumGateSeenCount;
  const hasSufficientSessions = recentSessions.completedSessions >= PREMIUM_SESSION_MINIMUM;
