...
describe('behavior-resolver-helpers', () => {
  const makeSignal = (type: string, surfaceKey = 'test_surface') => ({
    ...
    surfaceKey,
    signalType: type as unknown,
    source: 'home_hero' as unknown,
    timestamp: Date.now(),
  });
  ...
  it('countByType counts matching signals', () => {
    ...
    expect(countByType(signals as unknown, 'surface_clicked')).toBe(2);
  });
  it('countDistinctSurfaces returns unique surfaces', () => {
    ...
    const result = countDistinctSurfaces(signals as unknown, 'surface_clicked');
  ...
  it('hasMinimumSignals returns true when threshold met', () => {
    ...
    expect(hasMinimumSignals(signals as unknown, 'boss_cta_clicked', 2)).toBe(true);
  });
  it('hasSurfacesDismissedMultipleTimes surfaces meeting threshold', () => {
    ...
    expect(hasSurfacesDismissedMultipleTimes(signals as unknown)).toContain('annoying_surface');
  });
  it('hasSurfacesClickedMultipleTimes surfaces meeting threshold', () => {
    ...
    expect(hasSurfacesClickedMultipleTimes(signals as unknown)).toContain('liked_surface');
  });
});
