...
function getSignalValue(profile: BehaviorProfile | null | undefined, signalType: string): number | null {
  const signal = profile?.signals.find((item) => item.signalType === signalType);
...
export function buildActiveIntervention(input: {
  ...
  if (state.currentState === 'STREAK_AT_RISK') {
    ...
    const hoursRemaining = rawHours === null ? 6 : Math.max(0, Math.round(48 - rawHours));
    return {
  ...
  if (state.currentState === 'OVERLOAD_PROTECTION' || (sessionsLast24h >= 5 && avgQuality < 70)) {
    return {
  ...
  const bossMessage = messages?.find((message) =>
    message.category === 'CHALLENGE_PROMPT' && message.content.toLowerCase().includes('boss')
  );
  if (bossMessage) {
    return {
  ...
  if (state.currentState === 'LOW_CONFIDENCE' || xpGrowthDelta < -30) {
    return {
      ...
      type: 'PLATEAU',
      message: 'Your XP growth slowed this week. A longer session today breaks the plateau.',
