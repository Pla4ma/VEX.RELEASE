 ...
 * Coach Screen
 *
 * Phase 9.3 — Dedicated coach UI with chat-like layout
...
export function CoachScreen(): JSX.Element {
      ...
      return (
        <View
          style={[
            ...
            isCoach ? styles.coachMessageContainer : isUser ? styles.userMessageContainer : styles.systemMessageContainer,
          ]}
        >
          {isCoach && (
          ...
          <View
            style={[
              ...
              isCoach ? styles.coachBubble : isUser ? styles.userBubble : styles.systemBubble,
            ]}
          >
            <Text
              style={[
                styles.messageText,
                isCoach ? styles.coachText : isUser ? styles.userText : styles.systemText,
              ]}
            >
              {item.content}
            </Text>
            ...
            {/* Action Button */}
            {item.metadata?.hasAction && item.metadata.actionLabel && (
              <Pressable
  ...
  // Recommendation computed from state - fix type mismatch
  const recommendation = coachState ? getCurrentRecommendation(coachState) : null;
      ...
      >
        ...
        {error && (
          <View style={styles.errorBanner}>
            ...
            <Pressable onPress={() => setError(null)} style={({ pressed }) => [pressed && { opacity: 0.8 }]}
...
function getWelcomeMessage(coachState?: CoachState | null): string {
  if (!coachState) {
