...
function aggregateSignals(signals: BehaviorSignal[]): BehaviorSignal[] {
  ...
  for (const [signalType, typeSignals] of grouped) {
    ...
    const weightedSum = recentWindow.reduce((sum, s, idx) =>
      sum + s.value * (recentWindow.length - idx), 0
    );
    const weightSum = recentWindow.reduce((sum, _, idx) =>
      sum + (recentWindow.length - idx), 0
    );
    const averageValue = weightedSum / weightSum;

    // Calculate average confidence
    const averageConfidence = recentWindow.reduce((sum, s) => sum + s.confidence, 0) /
      recentWindow.length;
...
function calculateSignalConfidence(
  ...
  if (metadata.sampleSize && typeof metadata.sampleSize === 'number') {
    if (metadata.sampleSize < 3) {confidence *= 0.7;}
    else if (metadata.sampleSize < 10) {confidence *= 0.9;}
  }

  // Adjust based on data quality
  if (metadata.quality === 'high') {confidence *= 1.1;}
  if (metadata.quality === 'low') {confidence *= 0.8;}
...
function calculateConfidenceLevel(
  ...
  if (avgConfidence >= high && dataPoints >= 15) {return 'HIGH';}
  if (avgConfidence >= medium && dataPoints >= 10) {return 'MEDIUM';}
...
export function detectPatterns(profile: BehaviorProfile): DetectedPattern[] {
  ...
  if (!profile || profile.coldStart) {return patterns;}
...
export function generateBehaviorAnalytics(profile: BehaviorProfile): BehaviorAnalytics {
  ...
  if (morningSignal && nightSignal) {
    if (morningSignal.value > nightSignal.value + 0.2) {dominantChronotype = 'morning';}
    else if (nightSignal.value > morningSignal.value + 0.2) {dominantChronotype = 'evening';}
    else {dominantChronotype = 'variable';}
  }
  ...
  const engagementSignals = profile.signals.filter(s =>
    ['CHALLENGE_COMPLETION_RATE', 'BOSS_PARTICIPATION', 'SOCIAL_ENGAGEMENT'].includes(s.signalType)
  );
