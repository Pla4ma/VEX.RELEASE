...
async function readPromptState(): Promise<PromptState> {
  const raw = await getMMKVStorageAdapter().getItem(STORAGE_KEYS.NOTIFICATION_ONBOARDING_PROMPT);
  if (!raw) {
    return {};
  }
...
export async function hasRequestedOnboardingNotificationPrompt(userId: string): Promise<boolean> {
  const state = await readPromptState();
  return Boolean(state[userId]);
}

export async function markOnboardingNotificationPromptRequested(userId: string): Promise<void> {
  try {
    const state = await readPromptState();
...
export async function ensureNotificationChannel(): Promise<void> {
  ...
  try {
    await Notifications.setNotificationChannelAsync(NOTIFICATION_CONFIG.androidChannelId, {
