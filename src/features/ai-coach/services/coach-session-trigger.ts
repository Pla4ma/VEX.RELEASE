...
function toAnalyticsDifficulty(
  ...
  if (difficulty === 'PUSH' || difficulty === 'CHALLENGING') {return 'hard';}
  return 'medium';
}
...
function toAnalyticsSessionType(
  ...
  if (sessionType === 'CHALLENGE') {return 'challenge';}
  if (sessionType === 'BOSS_BATTLE') {return 'boss';}
  return 'focus';
}
...
export async function startStreakAtRiskSession(
  ...
  try {
    ...
    const config: CoachSessionConfig = {
      ...
      source: 'STREAK_PROTECTION',
      context: {
        coachReasoning: hoursRemaining < 4
...
export function getComebackBuilderConfig(userId: string): CoachSessionConfig {
  ...
  return {
    ...
    context: {
      coachReasoning: comebackCount > 0
