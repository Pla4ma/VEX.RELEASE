...
const RECOMMENDATION_RULES: RecommendationRule[] = [
  ...
  // LOW: Comeback opportunity (streak broken recently)
  {
    ...
    condition: (ctx) => {
      ...
      if (ctx.daysSinceLastSession > 7) {return false;} // Too late for comeback
      return true;
    },
    generate: (ctx, persona) => {
...
export class CoachRecommendationService {
  ...
  shouldRefresh(lastRefreshTime: number, currentRec: CoachRecommendation): boolean {
    ...
    if (currentRec.urgency === 'critical' && currentRec.type === 'protect_streak') {
      ...
      if (hoursLeft <= 1 && timeSinceRefresh > 60000) {return true;} // 1 minute
      if (hoursLeft <= 2 && timeSinceRefresh > 300000) {return true;} // 5 minutes
    }
 ...
 */
export function convertToHomeRecommendation(
  coachRec: CoachRecommendation
): {
  id: string;
  type: string;
  priority: number;
