...
function generateProtectStreakMessage(
  ctx: RecommendationContext,
  persona: CoachPersona,
): { headline: string; subtext: string; coachMessage: string } {
  ...
  switch (persona.id) {
    ...
    case "professor":
      return {
        ...
        coachMessage:
          "Consistency in practice yields compound results. A brief session maintains your established pattern.",
      };
  }
}
function generateStudyBehindMessage(
  ctx: RecommendationContext,
  persona: CoachPersona,
): { headline: string; subtext: string; coachMessage: string } {
  ...
  switch (persona.id) {
    case "mentor":
      return {
        ...
        coachMessage:
          "Falling behind happens. What matters is how you respond. One session today closes the gap.",
      };
    case "trainer":
      return {
        ...
        coachMessage:
          "No excuses. You committed to this plan. One focused session starts the recovery.",
      };
    case "peer":
      return {
    ...
    case "professor":
      return {
        ...
        coachMessage:
          "Academic progress requires consistent engagement. A single session today restores forward momentum.",
      };
  }
}
function generateBossOpportunityMessage(
  ctx: RecommendationContext,
  persona: CoachPersona,
): { headline: string; subtext: string; coachMessage: string } {
  const boss = ctx.activeBoss!;
  const healthPercent = Math.round(
    (boss.healthRemaining / boss.maxHealth) * 100,
  );
  switch (persona.id) {
    case "mentor":
      return {
    ...
    case "trainer":
      return {
        ...
        coachMessage:
          "Boss is bleeding. One focused session = victory. ATTACK.",
      };
    case "peer":
      return {
...
function generateMomentumBuildingMessage(
  ctx: RecommendationContext,
  persona: CoachPersona,
): { headline: string; subtext: string; coachMessage: string } {
  switch (persona.id) {
  ...
  switch (persona.id) {
    case "mentor":
      return {
        ...
        coachMessage:
          "Streaks break. What matters is returning. Your next session starts a new chapter.",
      };
    case "trainer":
      return {
        ...
        coachMessage:
          "The break is over. Time to rebuild. One session proves you're still in this.",
      };
    case "peer":
      return {
        ...
        coachMessage:
          "Hey, breaks happen! One session and you're right back in the game. Ready?",
      };
    case "professor":
      return {
        ...
        coachMessage:
          "Interruptions are inherent to learning. A single session re-establishes continuity.",
      };
  }
}
function generateStudyPlanCompleteMessage(
  ctx: RecommendationContext,
  persona: CoachPersona,
): { headline: string; subtext: string; coachMessage: string } {
  const plan = ctx.activeStudyPlan!;
...
function generateFocusSessionMessage(
  ctx: RecommendationContext,
  persona: CoachPersona,
): { headline: string; subtext: string; coachMessage: string } {
  switch (persona.id) {
    ...
    case "professor":
      return {
        ...
        coachMessage:
          "Consistent focus sessions yield compound cognitive benefits. Begin when ready.",
      };
  }
}
function generateStudyPlanMessage(
  ctx: RecommendationContext,
  persona: CoachPersona,
): { headline: string; subtext: string; coachMessage: string } {
  const plan = ctx.activeStudyPlan!;
...
function generateBossBattleMessage(
  ctx: RecommendationContext,
  persona: CoachPersona,
): { headline: string; subtext: string; coachMessage: string } {
  const boss = ctx.activeBoss!;
...
const RECOMMENDATION_RULES: RecommendationRule[] = [
  {
    ...
    generate: (ctx, persona) => {
      ...
      return {
        ...
        ...messages,
        ctaText:
          hoursLeft <= 2 ? "Quick 15-min session" : "Start focus session",
  ...
  },
  {
    ...
    condition: (ctx) => {
      ...
      const healthPercent =
        (ctx.activeBoss.healthRemaining / ctx.activeBoss.maxHealth) * 100;
