...
export function validateCoachMessage(
  ...
  if (userContext.lastMessageAt && data.type !== 'CHECK_IN') {
    const hoursSince = (Date.now() - userContext.lastMessageAt) / (60 * 60 * 1000);
    if (hoursSince < 1) {
  ...
  const hasBlocked = userContext.blockedKeywords.some(
    keyword => data.content.toLowerCase().includes(keyword.toLowerCase())
  );
  if (hasBlocked) {
...
export function shouldSendCheckIn(
...
): { shouldSend: boolean; reason: string; urgency: 'LOW' | 'MEDIUM' | 'HIGH' } {
  if (userState.optOut) {
  ...
  if (userState.lastSessionAt) {
    const hoursSince = (Date.now() - userState.lastSessionAt) / (60 * 60 * 1000);
