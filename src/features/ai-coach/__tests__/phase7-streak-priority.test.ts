...
describe('Phase 7 streak and priority integration', () => {
  ...
  it('does not intervene for low risk', async () => {
    await expect(handleStreakRiskIntegration('user-123', {
      currentStreak: 10,
  ...
  it.each([
    ...
    [{ streakCritical: false, pendingSync: false, dailyMissionReminder: true }, 'critical', true],
    [{ streakCritical: false, pendingSync: false }, 'high', true],
    [{ streakCritical: false, pendingSync: false, dailyMissionReminder: true }, 'low', false],
    ...
    expect(shouldCoachShowSuggestion(state, priority as 'critical' | 'high' | 'medium' | 'low'))
      .toBe(expected);
  });
  ...
  it('handles empty input contract gracefully', async () => {
    ...
    await expect(generateMissionSuggestion('user-123', emptyInput)).resolves.toBeDefined();
  });
