...
describe('Notification budget quiet hours and suppression', () => {
  ...
  it('blocks during quiet hours and allows outside quiet hours', async () => {
    ...
    mockCurrentHour(23);
    await expect(canSendNotification(createMockNotificationRequest('user-123'), budget))
      .resolves.toMatchObject({ allowed: false, reason: 'Quiet hours in effect' });
  ...
  it('handles overnight quiet hours and next active time', async () => {
    mockCurrentHour(2);
    const result = await canSendNotification(createMockNotificationRequest('user-123'), createMockNotificationBudget('user-123', {
      quietHoursStart: 23,
      quietHoursEnd: 6,
    }));
  ...
  it('respects opt-out for every priority', async () => {
    ...
    await expect(canSendNotification(createMockNotificationRequest('user-123'), budget))
      .resolves.toMatchObject({ allowed: false, reason: 'User has opted out of notifications' });
    await expect(canSendNotification(createMockNotificationRequest('user-123', {
      priority: 'STREAK_CRITICAL',
    }), budget)).resolves.toMatchObject({ allowed: false });
  });
  ...
  it('suppresses duplicates inside four hours and allows them later', async () => {
    ...
    await expect(canSendNotification(createMockNotificationRequest('user-123', {
      type: 'coach_session_suggestion',
      priority: 'COACH_NEXT_ACTION',
    }), recent)).resolves.toMatchObject({ allowed: false, reason: 'Duplicate notification suppressed' });
    ...
    await expect(canSendNotification(createMockNotificationRequest('user-123'), old))
      .resolves.toMatchObject({ allowed: true });
  });
  ...
  it('resets stale daily budgets only', () => {
    ...
    expect(resetDailyBudget(createMockNotificationBudget('user-123', {
      date: yesterday.toISOString().split('T')[0],
      sentCount: 2,
      notificationsSent: [{ id: 'notif-1', priority: 'COACH_NEXT_ACTION', sentAt: Date.now(), type: 'x', content: 'x' }],
