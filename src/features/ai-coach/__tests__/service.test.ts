...
describe('CoachService', () => {
  ...
  describe('activateComeback', () => {
    ...
    it('throws if comeback was previously declined', async () => {
      ...
      await expect(service.activateComeback(input)).rejects.toThrow(
        'Comeback was previously declined'
      );
    });
  });
  ...
  describe('trackComebackSession', () => {
    it('tracks completed session in comeback', async () => {
      ...
      expect(result).not.toBeNull();
      expect(repository.updateComebackPlanStatus).toHaveBeenCalledWith(
        'comeback-1',
        'ACTIVE',
        2
      );
  ...
  describe('createRecommendation', () => {
    ...
    it('expires existing recommendations of same type', async () => {
      ...
      expect(repository.updateRecommendationStatus).toHaveBeenCalledWith(
        'rec-1',
        'EXPIRED'
      );
    });
