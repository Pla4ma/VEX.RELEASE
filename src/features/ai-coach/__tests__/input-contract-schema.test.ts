...
describe('Coach input contract schema', () => {
  it('accepts valid coach input', () => {
    expect(() => CoachInputContractSchema.parse(createMockCoachInput())).not.toThrow();
  });
  ...
  it('rejects invalid grade, streak, and hour values', () => {
    expect(() => CoachInputContractSchema.parse(createMockCoachInput({
      recentSessionGrades: [{
        sessionId: 'session-1',
        grade: 150,
        duration: 1500,
  ...
  it('validates and sanitizes proper input', () => {
    const result = validateCoachInput(createMockCoachInput({
      timeContext: {
        currentHour: 14,
        dayOfWeek: 3,
        isWeekend: false,
