...
import {
  ...
  hasMemoryOfType as repoHasMemoryOfType,
  getMostRecentMemoryByType as repoGetMostRecentMemoryByType,
...
export async function storeStudyPattern(
  ...
  return storeMemory(
    ...
    `Pattern: ${pattern}`,
    evidence,
    { pattern, confidence, detectedAt: Date.now() }
  );
}
...
export async function storePreferredTechnique(
  ...
  return storeMemory(
    ...
    `Technique: ${technique}`,
    context,
    { technique, effectivenessScore, recordedAt: Date.now() }
  );
}
...
export async function storeFailureMode(
  ...
  return storeMemory(
    ...
    context,
    { failureType, suggestedIntervention, occurredAt: Date.now() }
  );
}
...
export async function checkLongestSession(
  ...
  if (sessionDuration <= previousBest) {return null;}
...
export async function checkBestStreak(
  ...
  if (currentStreak <= previousBest) {return null;}
...
export async function getMilestoneSummary(userId: string): Promise<{
  ...
  const favoriteType = Object.entries(typeCounts)
    .sort((a, b) => b[1] - a[1])[0][0] as MemoryType;
