...
export class PredictiveInterventionEngine {
   ...
   */
  verifyOutcomes(userId: string, actualEvents: {
    type: string;
    timestamp: number;
    prevented: boolean;
  }[]): void {
    ...
    for (const prediction of predictions) {
      ...
      if (match) {
        ...
        const intervention = this.interventions.get(userId)?.find(
          i => i.predictionId === prediction.id
        );
        if (intervention) {
  ...
  private calculateOptimalTime(
    userId: string,
    pattern: BehavioralPattern
  ): { confidence: number; nextWindow: number; evidence: string[]; action: string } {
