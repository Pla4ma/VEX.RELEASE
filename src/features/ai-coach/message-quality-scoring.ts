...
export function detectGenericPatterns(content: string): { isGeneric: boolean; reasons: string[] } {
  ...
  if (!containsSpecificData(content) || lowerContent.includes('try focusing more')) {
    isGeneric = true;
...
export function detectQualityElements(content: string): MessageQualityElement[] {
...
function isAllEncouragement(content: string): boolean {
  ...
  return words.length > 0 && encouragementCount / words.length > 0.7 && !containsSpecificData(content);
}
