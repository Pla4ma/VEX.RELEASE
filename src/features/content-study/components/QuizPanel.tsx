...
export const QuizPanel: React.FC<QuizPanelProps> = ({
  ...
  const handleOptionSelect = useCallback(
    (quizId: string, option: string) => {
      if (readOnly || answers[quizId]) {return;}
  ...
  const calculateScore = useMemo(() => {
    if (!showResults || !score) {return null;}
    return {
              ...
              styles.scoreBadge,
              {
                backgroundColor:
                  calculateScore.percentage >= 70
                    ? (theme.colors.success as any)[500]
                    : calculateScore.percentage >= 40
                    ? (theme.colors.warning as any)[500]
                    : (theme.colors.error as any)[500],
              },
            ]}
          >
            <Text style={[styles.scoreText, { color: (theme.colors.background as any).primary }]}>
              ...
              style={[
                styles.quizCard,
                {
                  backgroundColor: isAnswered
                    ? isCorrect
                      ? (theme.colors.success as any)[50]
                      : (theme.colors.error as any)[50]
                    : isActive
                    ? (theme.colors.primary as any)[50]
                    : (theme.colors.background as any).secondary,
                  borderColor: isActive
                    ? (theme.colors.primary as any)[500]
                    : isAnswered
                    ? isCorrect
                      ? (theme.colors.success as any)[500]
                      : (theme.colors.error as any)[500]
                    : (theme.colors.border as any)?.DEFAULT || theme.colors.border.DEFAULT,
                },
              ]}
            >
              {/* Question Header */}
              <View style={styles.questionHeader}>
                <Text style={[styles.questionNumber, { color: (theme.colors.text as any).muted }]}>
                  Q{index + 1}
                </Text>
                <View
                  style={[
                ...
                >
                  <Text
                    style={[styles.difficultyText, { color: difficultyConfig.textColor }]}
                  >
                ...
                {quiz.conceptTag && (
                  <View style={styles.conceptTag}>
                    <Text style={[styles.conceptText, { color: (theme.colors.text as any).muted }]}>
                      {quiz.conceptTag}
                    </Text>
                  </View>
                )}
              ...
              {/* Question */}
              <Text style={[styles.question, { color: (theme.colors.text as any).primary }]}>
                {quiz.question}
              </Text>
                      ...
                      >
                            ...
                            styles.optionText,
                            {
                              color: showCorrectness
                                ? isCorrectOption || isSelected
                                  ...
                                  : (theme.colors.text as any).primary
                                : isSelected
                                ? (theme.colors.background as any).primary
                                : (theme.colors.text as any).primary,
                            },
                        ...
                        {showCorrectness && isCorrectOption && (
                          <Icon
                            name="check"
                            size="sm"
                            color={(theme.colors.background as any).primary}
                          />
                        )}
              ...
              {!quiz.options && !isAnswered && !readOnly && (
                <View style={styles.shortAnswerContainer}>
                  ...
                  <Button
                    size="sm"
                    onPress={() => handleShortAnswerSubmit(quiz.id)}
                    disabled={!shortAnswers[quiz.id]?.trim()}
              ...
              {!isAnswered && !isRevealed && !readOnly && (
                <Pressable
                  style={({ pressed }) => [styles.revealButton, pressed && { opacity: 0.8 }]}
                  onPress={() => handleReveal(quiz.id)}
                  accessibilityLabel="Show Answer button"
                  accessibilityRole="button"
                  accessibilityHint="Activates this control"
                >
                  <Text
                    style={[styles.revealText, { color: (theme.colors.primary as any)[500] }]}
                  >
                    Show Answer
                  </Text>
                </Pressable>
              ...
              {(isRevealed || (isAnswered && !isCorrect)) && (
                ...
                >
                  <Text style={[styles.answerLabel, { color: (theme.colors.text as any).muted }]}>
                    Correct Answer:
                  </Text>
                  <Text style={[styles.answerText, { color: (theme.colors.success as any)[500] }]}>
                    {quiz.answer}
                  </Text>
                </View>
              )}
              ...
              {(isAnswered || isRevealed) && quiz.explanation && (
                ...
                >
                  <Text
                    style={[styles.explanationLabel, { color: (theme.colors.text as any).muted }]}
                  >
                    Explanation:
                  </Text>
                  <Text style={[styles.explanationText, { color: (theme.colors.text as any).secondary }]}>
                    {quiz.explanation}
                  </Text>
                </View>
              )}
