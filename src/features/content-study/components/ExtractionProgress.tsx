...
export const ExtractionProgress: React.FC<ExtractionProgressProps> = ({
  ...
  useEffect(() => {
    if (stage === 'failed' || stage === 'complete') {return;}
    ...
    >
      ...
      <View style={styles.header}>
        <View style={styles.statusBadge}>
          ...
          >
            <Icon
              name={STAGES[currentStageIndex]?.icon || 'loader'}
              size="lg"
              color={
                isFailed
                  ? theme.colors.error[500]
        ...
        <View style={styles.statusText}>
          ...
          {!isFailed && !isComplete && estimatedTimeRemaining !== undefined && (
            <Text style={[styles.timeLabel, { color: theme.colors.text.muted }]}>
        ...
        {STAGES.slice(0, -2).map((s, index) => {
          ...
          return (
            <View key={s.key} style={styles.stageIndicator}>
              ...
              >
                {isPast && (
                  <Icon name="check" size="xs" color={theme.colors.background.primary} />
                )}
              </View>
              <Text
                style={[
      ...
      {error && (
        <View style={styles.errorContainer}>
          ...
          {error.details && (
            <Text style={[styles.errorDetails, { color: theme.colors.text.muted }]}>
              {JSON.stringify(error.details)}
            </Text>
          )}
          {onRetry && (
            <Button variant="primary" size="sm" onPress={onRetry} style={styles.retryButton}
  accessibilityLabel="Retry Extraction button"
  ...
  accessibilityHint="Activates this control">
      ...
      {!isFailed && !isComplete && onCancel && (
        <Pressable onPress={onCancel} style={({ pressed }) => [styles.cancelButton, pressed && { opacity: 0.8 }]}
          accessibilityLabel="Cancel button"
          accessibilityRole="button"
          accessibilityHint="Activates this control">
          <Text style={[styles.cancelText, { color: theme.colors.text.muted }]}>
            Cancel
          </Text>
        </Pressable>
      )}
