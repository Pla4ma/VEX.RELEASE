...
export class DraftManager {
  ...
  async updateDraft(
    ...
    const updatedDraft: PersistedDraft = {
      ...
      ...updates,
      updatedAt: Date.now(),
      autoSaveVersion: drafts[draftIndex].autoSaveVersion + 1,
    };
  ...
  async getAllDrafts(): Promise<PersistedDraft[]> {
    ...
    } catch (error) { captureSilentFailure(error, { feature: 'content-study', operation: 'safe-fallback', type: 'data' });
      return [];
    }
  }
  ...
  async autoSave(
    ...
    const draftData = type === 'paste' ? { type, activeTab: type, pastedText: content, youtubeUrl: '', selectedFile: null } :
                      type === 'youtube' ? { type, activeTab: type, pastedText: '', youtubeUrl: content, selectedFile: null } :
...
export class StudySessionManager {
  ...
  async getActiveSession(generationId: string): Promise<PersistedStudySession | null> {
    const sessions = await this.getAllSessions();
    return (
      sessions.find((s) => s.generationId === generationId && !s.endTime) || null
    );
  }

  async getSessionsForGeneration(generationId: string): Promise<PersistedStudySession[]> {
    const sessions = await this.getAllSessions();
...
export async function getStorageUsage(): Promise<{
  ...
  const items = await Promise.all(
    relevantKeys.map(async (k) => [k, await storage.getItem(k)])
  );
  ...
  items.forEach(([itemKey, value]) => {
    ...
    if (itemKey === STORAGE_KEYS.DRAFTS) {sizes.drafts = size;}
    else if (itemKey === STORAGE_KEYS.SESSIONS) {sizes.sessions = size;}
    else if (itemKey === STORAGE_KEYS.SYNC_QUEUE) {sizes.syncQueue = size;}
    else if (itemKey.startsWith(STORAGE_KEYS.CACHE)) {sizes.cache += size;}
