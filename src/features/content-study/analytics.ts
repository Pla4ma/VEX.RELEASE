...
export class ContentStudyAnalyticsService {
  ...
  private sendToProvider(event: ContentStudyAnalyticsEvent): void {
    if (!this.analyticsProvider) {return;}
  ...
  private async flushQueue(): Promise<void> {
    if (!this.analyticsProvider || this.queue.length === 0) {return;}
...
export const analytics = {
  ...
  fileSelected(
    ...
    contentStudyAnalytics.track('content_study_file_selected', {
      ...
      file_name_hash: hashString(fileName), // Privacy preserving
    });
  },
  ...
  submitted(
    ...
    validationResult: { isValid: boolean; errorCount: number; warningCount: number }
  ): void {
