...
export function enforceDay0SurfacePolicy(
  ...
  const corrected: Record<HomeSurfaceKey, HomeSurfaceDecision> = {} as Record<
    HomeSurfaceKey,
    HomeSurfaceDecision
  >;
  ...
  if (primaryCount > limits.maxPrimaryCta) {
    ...
    for (const [key, val] of Object.entries(corrected) as [
      HomeSurfaceKey,
      HomeSurfaceDecision,
    ][]) {
      if (val === 'primary') {
  ...
  if (spotlightCount > limits.maxSpotlights) {
    violations.push(
      `Day 0 has ${spotlightCount} spotlights, max ${limits.maxSpotlights}`,
    );
    let keptSpot = false;
    for (const [key, val] of Object.entries(corrected) as [
      HomeSurfaceKey,
