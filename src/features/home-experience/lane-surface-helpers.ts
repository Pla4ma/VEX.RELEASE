...
export function applyLaneSurfaces(
  ...
  const isGameLike = canonicalLane
    ...
    : p.motivationStyle === 'game_like' || p.motivationStyle === 'intense' || p.gamificationIntensity === 'strong';
  const isDeepCreative = canonicalLane
    ? canonicalLane === 'deep_creative'
    : p.primaryGoal === 'creative' || b.deepWorkUsageRatio >= 0.35 || b.projectFocusUsageRatio >= 0.35;
  const isMinimal = canonicalLane
    ? canonicalLane === 'minimal_normal'
    : p.motivationStyle === 'calm' || p.gamificationIntensity === 'minimal';

  if (isStudent && parsed.featureAvailability.study && !ignored.includes('study_os')) {
    map.study_os = parsed.hasActiveStudyPlan ? 'secondary' : 'tiny_tease';
  }
  if (isGameLike && parsed.featureAvailability.boss && !ignored.includes('run_board')) {
