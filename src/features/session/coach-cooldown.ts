...
export const DEFAULT_ACTIVE_SESSION_RULES: CoachInterventionRule[] = [
  ...
  },
  {
    ...
    cooldownSeconds: 600,
    maxPerSession: 3,
    messageTemplate: 'You have completed {{percent}}% of your session. Keep going!',
    priority: 4,
  },
  {
    ...
    cooldownSeconds: 0,
    maxPerSession: 1,
    messageTemplate: 'Your streak is at risk. Complete this session to maintain it.',
    priority: 10,
  },
  {
    id: 'low-purity-nudge',
    ...
    cooldownSeconds: 180,
    maxPerSession: 2,
    messageTemplate: 'Your focus has drifted. Take a moment to center yourself.',
    priority: 5,
  },
  {
    id: 'idle-reminder',
...
export function getGlobalCooldownRemaining(sessionId: string): number {
  ...
  const remaining = Math.max(0, globalCooldownSeconds - secondsSinceLastIntervention);
