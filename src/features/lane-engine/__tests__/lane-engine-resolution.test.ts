...
describe('Lane Engine', () => {
  it('resolves STUDY goal plus study style to student with high confidence', () => {
    const result = resolveInitialLane({
      primaryGoal: 'study',
    ...
    expect(getLaneMechanicPolicy(result).preferredMechanics).toContain(
      'study_os',
    );
  });
  ...
  it('keeps conflicting study and game-like signals stable with secondary lane evidence', () => {
    ...
    expect(
      result.secondaryLane === 'student' ||
        result.secondaryLane === 'game_like',
    ).toBe(true);
