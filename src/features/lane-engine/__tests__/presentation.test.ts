...
describe('lane presentation policy', () => {
  it.each([
    ...
    ['game_like', 'medium_high', 'medium_high', 'strategic_energetic', 'shield'],
    ['deep_creative', 'medium', 'low_medium', 'reflective_continuity', 'pen-tool'],
    ['minimal_normal', 'low', 'minimal', 'concise_factual', 'check-circle'],
  ] as const)('maps %s to designed lane presentation', (lane, density, animation, copyTone, icon) => {
  ...
  it('disables animation when reduced motion is enabled', () => {
    const policy = getLanePresentationPolicy({ lane: 'game_like', reducedMotion: true });
