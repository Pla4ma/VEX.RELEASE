...
describe('Lane Engine ownership proof', () => {
  ...
  it('behavior can suggest reconsideration without silent override', () => {
    ...
    expect(shouldReconsiderLane({ completedSessions: 10, currentProfile: current, latestProfile: latest })).toBe(false);
    expect(shouldSuggestLaneReconsideration({ completedSessions: 10, currentProfile: current, latestProfile: latest })).toBe(true);
  });
  ...
  it('Home and completion do not own direct lane resolver files', () => {
    const home = fs.readFileSync(path.join(process.cwd(), 'src/screens/home/hooks/useHomeResolvedExperience.ts'), 'utf8');
    const completion = fs.readFileSync(path.join(process.cwd(), 'src/features/session-completion/completion-orchestrator.ts'), 'utf8');
  ...
  it('core consumers prefer LaneProfile over conflicting legacy signals', () => {
    ...
    const home = decideHomeSurfaces({
      behaviorStats: { bossChallengeEngagement: 'high', coachInteractions: 0, comebackSessions: 0, completionStreak: 0, ignoredFeatures: [], premiumFeatureAttempts: [], studyUsageRatio: 0, totalCompletedSessions: 3 },
      ...
      laneProfile: student,
      personalizationProfile: { gamificationIntensity: 'strong', motivationStyle: 'game_like', primaryGoal: 'work', studyLayerName: 'Study', userStage: 'engaged' },
    });
    expect(home.study_os).not.toBe('hidden');

    expect(buildLaneSessionBrief({ lane: 'minimal_normal', laneProfile: run }).sessionMode).toBe('SPRINT');
    expect(decideNudge({ completedSessions: 4, context: 'run_open', daysSinceOnboarding: 3, lane: 'minimal_normal', laneProfile: run }).type).toBe('run_continue');
    expect(createRescuePlan({ lane: 'minimal_normal', laneProfile: project, reason: 'unclear', userId: 'u1' }).sessionMode).toBe('CREATIVE');
  });
  ...
  it('completion, coach, premium, and active session consume LaneProfile', () => {
    ...
    const coach = buildCoachPresence({
      companion: null,
      featureAvailability: { focus: available(), progress: available(), study: available() },
      laneProfile: student,
      memorySummary: { coachMemoryCount: 0, companionMemoryCount: 0, latestMemory: null, syncAvailable: true },
