...
export const ORDERED_LANES: Lane[] = ['student', 'game_like', 'deep_creative', 'minimal_normal'];

export function confidenceBand(confidence: number): LaneProfile['confidenceBand'] {
...
export function ranked(scores: LaneScores): Array<{ lane: Lane; score: number }> {
  return ORDERED_LANES
    .map((lane) => ({ lane, score: Math.max(0, scores[lane]) }))
    .sort((a, b) => b.score - a.score);
}

export function buildProfile(scores: LaneScores, evidence: LaneEvidence[], observedAt: number): LaneProfile {
...
export function applyBehaviorScores(scores: LaneScores, evidence: LaneEvidence[], input: ResolveBehaviorLaneInput, observedAt: number): void {
