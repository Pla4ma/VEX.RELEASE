...
import { MotivationStyleSchema, PrimaryGoalSchema, SessionModeSchema } from '../personalization/core-schemas';
...
export const LaneEvidenceSchema = z.object({
...
export const LaneTraitsSchema = z.object({
...
export const LaneMechanicPolicySchema = z.object({
  lane: LaneSchema,
...
export const ResolveInitialLaneInputSchema = z.object({
...
export const ResolveBehaviorLaneInputSchema = ResolveInitialLaneInputSchema.extend({
  ...
  bossEngagement: z.enum(['none', 'low', 'medium', 'high']).optional().default('none'),
...
export const MergeLaneProfilesInputSchema = z.object({
  onboardingProfile: LaneProfileSchema,
  behaviorProfile: LaneProfileSchema,
  completedSessions: z.number().int().min(0),
}).strict();

export const LaneReconsiderationInputSchema = z.object({
