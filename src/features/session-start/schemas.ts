...
export const SessionStakeSchema = z.object({
  ...
  selectedMode: z.enum(['LIGHT_FOCUS', 'DEEP_WORK', 'SPRINT', 'CREATIVE', 'STUDY', 'RECOVERY', 'STARTER']),
...
export const LaneSessionBriefSchema = z.object({
  ...
  suggestedDurationSeconds: z.number().int().min(5 * 60).max(180 * 60),
  risk: z.object({
    type: z.enum(['deadline', 'avoidance', 'streak', 'project_stale', 'none']),
