...
function calculateBossDamageEstimate(
...
function calculateStreakRisk(streak: {
  ...
  if (streak.currentDays === 0) {return 'SAFE';}

  const lastSession = streak.lastQualifyingSessionAt;
  if (!lastSession) {return 'SAFE';}
...
function estimateChallengeProgress(
  challenge: { targetValue: number; currentValue: number },
  durationSeconds: number
): number {
...
export async function buildSessionStake(
  ...
  const bossDamage = bossEncounter
    ? calculateBossDamageEstimate(durationSeconds, mode, streak?.currentDays ?? 0)
    : null;
  ...
  const stake = {
    ...
    challenges: challenges.map(c => ({
      ...
      progressAfter: Math.min(
        ...
        (c.userChallenge.currentValue ?? 0) + estimateChallengeProgress({
          targetValue: c.challenge.targetValue,
          currentValue: c.userChallenge.currentValue ?? 0,
        }, durationSeconds)
      ),
      willComplete: (c.userChallenge.currentValue ?? 0) + estimateChallengeProgress({
        targetValue: c.challenge.targetValue,
        currentValue: c.userChallenge.currentValue ?? 0,
      }, durationSeconds) >= c.challenge.targetValue,
