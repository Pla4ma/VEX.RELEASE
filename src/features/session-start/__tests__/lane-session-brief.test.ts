...
describe("lane session brief", () => {
  describe("userFacingModeName", () => {
    it.each([
      ...
      'maps %s lane to userFacingModeName "%s"',
      (lane, expectedName) => {
  ...
  it.each([
    ...
    expect(JSON.stringify(brief)).not.toMatch(
      /wager|insuranceCost|bountyCost|wallet|gem|shop|inventory|coin/i,
    );
  ...
  it("rescue gives lane-specific success conditions", () => {
    const studentBrief = buildLaneSessionBrief({
      isRescue: true,
      lane: "student",
    });
    const gameBrief = buildLaneSessionBrief({
      isRescue: true,
      lane: "game_like",
    });
    const creativeBrief = buildLaneSessionBrief({
      isRescue: true,
      lane: "deep_creative",
    });
    const cleanBrief = buildLaneSessionBrief({
      isRescue: true,
      lane: "minimal_normal",
    });
