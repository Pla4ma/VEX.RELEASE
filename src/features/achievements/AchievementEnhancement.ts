...
/**
 * Get progression guide for user
 */
export function getProgressionGuide(
  ...
  const currentAchievement = unlockedIds.length > 0
    ? allAchievements.find((a) => a.id === unlockedIds[unlockedIds.length - 1]) || null
    : null;
  ...
  const nextUnlock = ACHIEVEMENT_FEATURE_UNLOCKS.find(
...
/**
 * Handle achievement unlock with side effects
 */
...
/**
 * Get next milestone days for display
 */
...
/**
 * Get achievement preview (respecting hidden status)
 */
