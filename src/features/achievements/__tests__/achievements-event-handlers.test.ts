...
describe('event-handlers', () => {
  ...
  describe('handleSessionCompleted', () => {
    it('calls checkCumulativeAchievements for SESSION_COMPLETE', async () => {
      await eventHandlers.handleSessionCompleted({ userId: 'user-1', duration: 1800, quality: 80, timestamp: Date.now() } as unknown);
    ...
    it('checks 60-min achievement for long sessions', async () => {
      await eventHandlers.handleSessionCompleted({ userId: 'user-1', duration: 3700, timestamp: Date.now() } as unknown);
    ...
    it('does not check 60-min for short sessions', async () => {
      await eventHandlers.handleSessionCompleted({ userId: 'user-1', duration: 1800, timestamp: Date.now() } as unknown);
    ...
    it('checks perfect session for quality >= 95', async () => {
      await eventHandlers.handleSessionCompleted({ userId: 'user-1', duration: 1800, quality: 98, timestamp: Date.now() } as unknown);
  ...
  describe('handleStreakUpdated', () => {
    it('checks streak achievements based on streak days', async () => {
      await eventHandlers.handleStreakUpdated({ userId: 'user-1', state: { currentStreak: 10 } } as unknown);
      expect(mockedAchievementUnlock.checkAchievement).toHaveBeenCalled();
    });

    it('checks all applicable tiers for high streaks', async () => {
      await eventHandlers.handleStreakUpdated({ userId: 'user-1', state: { currentStreak: 100 } } as unknown);
    ...
    it('does not check streak-365 for streak < 365', async () => {
      await eventHandlers.handleStreakUpdated({ userId: 'user-1', state: { currentStreak: 50 } } as unknown);
  ...
  describe('handleStreakMilestone', () => {
    it('adds Sentry breadcrumb for milestone', async () => {
      await eventHandlers.handleStreakMilestone({ userId: 'user-1', milestone: 7 } as unknown);
  ...
  describe('handleBossDefeated', () => {
    it('checks cumulative boss achievements', async () => {
      await eventHandlers.handleBossDefeated({ userId: 'user-1', bossId: 'boss-1', damageDealt: 50, participants: ['user-1'] } as unknown);
    ...
    it('checks solo boss when no participants', async () => {
      await eventHandlers.handleBossDefeated({ userId: 'user-1', bossId: 'boss-1', damageDealt: 50, participants: [] } as unknown);
    ...
    it('checks squad boss when participants exist', async () => {
      await eventHandlers.handleBossDefeated({ userId: 'user-1', bossId: 'boss-1', damageDealt: 50, participants: ['user-1', 'user-2'] } as unknown);
    ...
    it('checks critical hit when damage > 100', async () => {
      await eventHandlers.handleBossDefeated({ userId: 'user-1', bossId: 'boss-1', damageDealt: 150, participants: ['user-1'] } as unknown);
  ...
  describe('handleLevelUp', () => {
    it('checks level achievements for high levels', async () => {
      await eventHandlers.handleLevelUp({ userId: 'user-1', newLevel: 25 } as unknown);
    ...
    it('does not check for low levels', async () => {
      await eventHandlers.handleLevelUp({ userId: 'user-1', newLevel: 3 } as unknown);
  ...
  describe('handlePrestige', () => {
    it('checks first prestige achievement', async () => {
      await eventHandlers.handlePrestige({ userId: 'user-1' } as unknown);
  ...
  describe('handleStreakComeback', () => {
    it('checks phoenix achievement', async () => {
      await eventHandlers.handleStreakComeback({ userId: 'user-1' } as unknown);
  ...
  describe('handleStreakBroken', () => {
    it('does nothing (no-op handler)', async () => {
      await eventHandlers.handleStreakBroken({} as unknown);
      expect(mockedAchievementUnlock.checkAchievement).not.toHaveBeenCalled();
    });
  });
  ...
  describe('handleAchievementCheck', () => {
    it('adds breadcrumb for manual check', async () => {
      await eventHandlers.handleAchievementCheck({ userId: 'user-1', type: 'manual' } as unknown);
