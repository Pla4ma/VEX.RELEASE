...
describe('updateAchievementProgress', () => {
  ...
  it('skips already-unlocked achievements', async () => {
    mockedRepository.getUserAchievement.mockResolvedValue(
      mockUserAchievement({ isUnlocked: true }) as any
    );
  ...
  it('unlocks a GREATER_THAN achievement when value meets target', async () => {
    ...
    expect(mockedEventBus.publish).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ userId: 'user-1', achievementId: 'session-first' })
    );
  });
  ...
  it('emits economy event when achievement has coin reward', async () => {
    ...
    const economyCall = (mockedEventBus.publish as jest.Mock).mock.calls.find(
      ([event]) => event === 'economy:add-currency'
    );
...
describe('getRecentlyUnlockedAchievements', () => {
  ...
  it('returns unlocked achievements sorted by most recent', async () => {
    const now = Date.now();
    mockedRepository.getAllUserAchievements.mockResolvedValue([
      mockUserAchievement({ achievementId: 'session-first', isUnlocked: true, unlockedAt: now - 1000 }) as any,
      mockUserAchievement({ achievementId: 'session-10', isUnlocked: true, unlockedAt: now }) as any,
    ]);
  ...
  it('respects the limit parameter', async () => {
    const now = Date.now();
    mockedRepository.getAllUserAchievements.mockResolvedValue([
      mockUserAchievement({ achievementId: 'session-first', isUnlocked: true, unlockedAt: now }) as any,
      mockUserAchievement({ achievementId: 'session-10', isUnlocked: true, unlockedAt: now - 1000 }) as any,
      mockUserAchievement({ achievementId: 'session-50', isUnlocked: true, unlockedAt: now - 2000 }) as any,
    ]);
