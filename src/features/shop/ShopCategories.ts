...
export function getAvailableItems(
  ...
  const available = allItems.filter((item) => {
    ...
    if (item.requirements?.premiumOnly && !isPremium) {return false;}

    // Check limited time
    if (item.limitedTime) {
      const now = Date.now();
      if (now < item.limitedTime.startAt || now > item.limitedTime.endAt) {return false;}
    }

    // Check stock limit
    if (item.stockLimit && (item.purchasedCount || 0) >= item.stockLimit) {return false;}
...
export function purchaseItem(
  userId: string,
  itemId: string,
  deductCurrency: (currency: CurrencyType, amount: number) => boolean
): { success: boolean; inventory?: UserInventory; error?: string } {
...
export function equipCosmetic(
  userId: string,
  itemId: string,
  slot: keyof UserInventory['equippedCosmetics']
): UserInventory {
  const inventory = getUserInventory(userId);
...
export function getEquippedCosmetics(
  userId: string
): UserInventory['equippedCosmetics'] {
...
export function getFeaturedItems(limit: number = 4): ShopItem[] {
  return allItems
    .filter((i) => i.featured && i.category !== 'PREMIUM')
    .slice(0, limit);
}
