...
export function LimitedTimeOffer({
  ...
  gemBalance,
  coinBalance,
}: LimitedTimeOfferProps): JSX.Element | null {
  ...
  const limitedItems = useMemo(() => {
    return items.filter(
      (item) =>
        item.isFlashSale ||
        item.discount ||
        (item.totalAppearances && item.totalAppearances < 3)
    );
  }, [items]);
...
function OfferCard({
  ...
  const getCountdown = () => {
    if (!item.flashSaleEndsAt) {return null;}
  ...
  const gradientColors: readonly [string, string, ...string[]] = isFlashSale
    ? FLASH_SALE_GRADIENT
    : isRare
      ? RARE_GRADIENT
    : [rarityColors[item.rarity], rarityColors[item.rarity]];
          ...
          >
            <Text style={styles.bannerText}>
              {isFlashSale
                ...
                : isRare
                  ? `🔥 RARE - Last seen ${item.lastSeenDays}d ago`
