...
export function RotatingShopSection({
  items,
  gemBalance,
  coinBalance,
  nextRotationAt,
  onPurchase,
  onViewDetails,
}: RotatingShopSectionProps): JSX.Element | null {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={[
          ...
          isLowTime && styles.urgentCountdown,
        ]}>
          <Text style={[
            styles.countdownText,
            isLowTime && styles.urgentText,
          ]}>
            🕐 {rotationCountdown}
          </Text>
        </View>
      ...
      {isLowTime && (
        <View style={styles.urgencyBanner}>
          <Text style={styles.urgencyText}>
            ⚠️ These items rotate out soon! Grab them before they're gone.
          </Text>
        </View>
      )}
      ...
      <View style={styles.scheduleNote}>
        <Text variant="caption" color="text.tertiary" style={styles.scheduleText}>
          Items rotate every {items[0]?.rotationDuration || 24}h.
          Rare items appear {items[0]?.monthlyAppearances || 2}x per month.
        </Text>
      </View>
    </View>
  );
...
function RotatingItemCard({
  ...
  const canAfford = useMemo(() => {
    if (item.price.gems) {return gemBalance >= item.price.gems;}
    if (item.price.coins) {return coinBalance >= item.price.coins;}
    return false;
      ...
      accessibilityHint="Activates this control">
        ...
        <View style={[styles.rarityBadge, { backgroundColor: rarityColors[item.rarity] }]}>
          <Text style={styles.rarityText}>
            {isRare ? '🔥 RARE' : item.rarity.toUpperCase()}
          </Text>
        </View>
        ...
        <View style={styles.timeContainer}>
          <Text style={[styles.timeText, isLowTime && styles.urgentTime]}>
            ...
            {item.hoursRemaining < 1
              ? `${Math.floor(item.hoursRemaining * 60)}m left`
              : `${Math.floor(item.hoursRemaining)}h left`}
          </Text>
        </View>
          ...
          <View style={styles.priceRow}>
            {item.price.gems && (
              <Text
                variant="body"
                fontWeight="800"
                color={canAfford ? '#8B5CF6' : 'error.DEFAULT'}
              >
                💎{item.price.gems}
              </Text>
            )}
            {item.price.coins && (
              <Text
                variant="body"
                fontWeight="800"
                color={canAfford ? '#F59E0B' : 'error.DEFAULT'}
              >
                🪙{item.price.coins}
              </Text>
            )}
