...
export function getPriorityRecommendation(
...
): Omit<SessionRecommendation, "inputs"> {
  ...
  if (input.recoveryStatus === "urgent") {
    return {
      ...
      mode: "RECOVERY",
      reason:
        "Your focus needs recovery. Start with a gentle 20-minute session",
      fallback: false,
      confidence: 0.9,
      isBlocked: false,
    };
  ...
  if (input.dailyMissionType) {
    const missionRecommendation = getMissionBasedRecommendation(
      input.dailyMissionType,
    );
    return {
