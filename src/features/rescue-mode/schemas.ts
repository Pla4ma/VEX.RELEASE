...
// ── Eligibility ────────────────────────────────────────────────────────
export const RescueEligibilityInputSchema = z
  .object({
...
export const RescueEligibilityResultSchema = z
  .object({
    eligible: z.boolean(),
    trigger: RescueTriggerSourceSchema.nullable(),
    reason: z.string().min(1),
    lane: LaneSchema,
    recommendedDurationSeconds: z
      .number()
      .int()
      .min(0)
      .max(12 * 60),
...
export const RescuePlanInputSchema = z
  .object({
    userId: z.string().min(1),
    lane: LaneSchema,
    laneProfile: LaneProfileSchema.optional(),
