...
export const REWARD_ALIGNMENTS: RewardAlignment[] = [
  {
    ...
    earnedBecause: 'You showed up and focused. That is the only thing that matters.',
    pointsTo: 'consistency',
  },
  {
    rewardId: 'streak_milestone',
    rewardName: 'Streak Milestone',
    earnedBecause: 'Days of showing up compound into real proof of consistency.',
    pointsTo: 'consistency',
  },
  {
    rewardId: 'purity_elite',
    rewardName: 'Exceptional Focus',
    earnedBecause: 'You stayed deeply focused. Your brain is building stronger pathways.',
    pointsTo: 'quality',
  },
  {
    rewardId: 'xp_level_up',
    rewardName: 'Level Up',
    earnedBecause: 'Your accumulated focus has built a new tier of consistency.',
    pointsTo: 'growth',
  },
  {
...
export function getRewardAlignment(rewardId: string): RewardAlignment | undefined {
  return REWARD_ALIGNMENTS.find((r) => r.rewardId === rewardId);
}

export function getRewardWhy(rewardId: string): string {
  return getRewardAlignment(rewardId)?.earnedBecause ?? 'You earned this through focused work.';
}
