...
export const DurationValidators = {
  validate: (duration: unknown): ValidationResult<FocusDuration> => {
    const result: ValidationResult<FocusDuration> = { success: false, errors: [], warnings: [] };
    if (typeof duration !== 'number' || isNaN(duration)) {
      result.errors.push({ field: 'duration', message: 'Duration must be a valid number', code: 'INVALID_DURATION_TYPE' });
      return result;
    }
    if (!ValidDurations.includes(duration as FocusDuration)) {
      ...
      result.errors.push({
        field: 'duration', message: `Invalid duration. Did you mean ${closest} minutes?`, code: 'INVALID_DURATION_VALUE',
      });
    ...
    if (duration === 15) {
      result.warnings.push({
        field: 'duration', message: '15-minute sessions are great for starting out, but consider longer sessions for deep work',
        code: 'SHORT_DURATION_WARNING',
      });
    } else if (duration === 60) {
      result.warnings.push({
        field: 'duration', message: '60-minute sessions require strong focus stamina. Consider starting with 25 or 45 minutes.',
  ...
  recommendForGoal: (goal: FocusGoal): FocusDuration[] => {
    const recommendations: Record<FocusGoal, FocusDuration[]> = {
      WORK: [25, 45, 60], STUDY: [25, 45, 15], CREATIVE: [45, 60, 25], PERSONAL: [25, 15, 45],
    };
