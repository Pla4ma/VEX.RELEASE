...
const toCompletionResult = async (
  ...
  if (detail.xpReward > 0) {rewards.push(ChallengeRewardSchema.parse({ type: 'XP', amount: detail.xpReward, itemId: null, delivered: false, deliveredAt: null }));}
  if (detail.coinReward > 0) {rewards.push(ChallengeRewardSchema.parse({ type: 'COINS', amount: detail.coinReward, itemId: null, delivered: false, deliveredAt: null }));}
...
export async function updateChallengeProgress(input: UpdateChallengeProgressInput): Promise<ChallengeCompletionResult | null> {
  ...
  if (!userChallenge || !challenge) {throw new ChallengeNotFoundError(validated.challengeId);}
  if (userChallenge.status !== 'ACTIVE') {throw new ChallengeNotActiveError(validated.challengeId, userChallenge.status);}
  ...
  if (updated.currentValue < challenge.targetValue) {return null;}
...
export async function checkChallengeProgress(
  ...
  try {
    ...
    for (const detail of details) {
      if (detail.userChallenge.status !== 'ACTIVE') {continue;}
...
export async function claimChallengeReward(input: { userId: string; challengeId: string }): Promise<{ success: boolean; rewards: ChallengeReward[]; error: string | null }> {
  ...
  try {
    ...
    if (!detail) {return { success: false, rewards: [], error: 'Challenge not completed' };}
    if (detail.userChallenge.status === 'CLAIMED') {return { success: false, rewards: [], error: 'Reward already claimed' };}
...
export async function checkRerollEligibility(userId: string, challengeId: string): Promise<RerollEligibility> {
  const [userChallenge, rerollCountToday, freeRerollCountToday] = await Promise.all([
