  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (attempt === maxAttempts) {
        ...
        } catch (error) { captureSilentFailure(error, { feature: 'analytics', operation: 'safe-fallback', type: 'data' });
...
function isRetryableStorageError(error: unknown): boolean {
  ...
  const hasRetryableCode = retryableCodes.some(code =>
    errorObj.name?.includes(code) || errorObj.message?.toLowerCase().includes(code)
  );

  const hasRetryableStatus = errorObj.statusCode !== undefined &&
...
export async function getStorageMetrics(userId: string): Promise<{
  ...
  const timestamps = data
    .map(item => item.created_at ? new Date(item.created_at).getTime() : null)
