...
export async function fetchSessionHeatmapData(
  ...
  const timestamps = z.array(z.object({ timestamp: z.number().finite() })).parse(data ?? []);
...
export async function fetchInsights(
  userId: string,
  options?: { unreadOnly?: boolean; types?: string[]; severities?: string[]; limit?: number }
) {
  let query = supabase
    .from('insights')
    .select('*')
...
export async function markInsightAsActioned(
  ...
  const { data, error } = await supabase
    .from('insights')
    .update({ is_actioned: true, action_type: actionType, action_payload: actionPayload })
    .eq('id', insightId)
    .eq('user_id', userId)
    .select()
    .single();
...
export async function createDashboardLayout(layout: z.infer<typeof DashboardLayoutSchema>) {
  ...
  if (widgets?.length) {
    ...
    if (widgetError) {throw handleSupabaseError(widgetError);}
  }
...
export async function fetchAnalyticsPreferences(userId: string) {
  ...
  if (error && error.code !== 'PGRST116') {throw handleSupabaseError(error);}
...
export async function fetchDetectedPatterns(
  ...
  if (options?.minConfidence !== undefined) {query = query.gte('confidence', options.minConfidence);}
  const { data, error } = await query;
  if (error) {throw handleSupabaseError(error);}
