...
import {
  EmptyAnalytics,
  NetworkError,
} from "../../../shared/ui/primitives/EmptyState";
import {
  Skeleton,
  SkeletonChart,
  SkeletonList,
...
export function AnalyticsDashboard({
  ...
  const { data: insights, isLoading: insightsLoading } = useInsights(userId, {
    limit: 5,
  });
  React.useEffect(() => {
    if (dataError && dataErrorObj) {
      const analyticsError =
        dataErrorObj instanceof Error
          ? dataErrorObj
      ...
      return (
        <View style={styles.infoCard}>
          ...
          <Text style={styles.infoSubtitle}>
            We couldn&apos;t load your session pattern right now.
          </Text>
        </View>
      );
    }
              ...
              {analyticsData?.map((data) => (
                <View key={data.metric} style={styles.summaryCard}>
                  <Text style={styles.summaryValue}>
                    {formatValue(data.summary.total, data.metric)}
                  </Text>
