import React from'react'; import{View,Text,Pressable}from'react-native'; import{InsightSchema}from'../schemas'; import type{z}from'zod'; import{createSheet}from'@/shared/ui/create-sheet';
import { launchColors } from '@theme/tokens/launch-colors';
 type Insight=z.infer<typeof InsightSchema>;interface InsightCardProps{insight:Insight;onPress?:()=>void;onAction?:()=>void;onDismiss?:()=>void;}const SEVERITY_CONFIG:Record<'info'|'positive'|'warning'|'critical'|'celebration',{color:string;bgColor:string;icon:string;}> = {info:{color:launchColors.hex_3b82f6,bgColor:launchColors.hex_dbeafe,icon:'ℹ️'},positive:{color:launchColors.hex_10b981,bgColor:launchColors.hex_d1fae5,icon:'✅'},warning:{color:launchColors.hex_f59e0b,bgColor:launchColors.hex_fef3c7,icon:'⚠️'},critical:{color:launchColors.hex_ef4444,bgColor:launchColors.hex_fee2e2,icon:'🚨'},celebration:{color:launchColors.hex_8b5cf6,bgColor:launchColors.hex_ede9fe,icon:'🎉'}}; export function InsightCard({insight,onPress,onAction,onDismiss}:InsightCardProps){const severity = SEVERITY_CONFIG[insight.severity]; return<Pressable style={({pressed})=>[styles.container,{backgroundColor:severity.bgColor,borderColor:severity.color},insight.isRead && styles.readContainer,pressed && {opacity:0.8}]}onPress={onPress}accessibilityRole="button"accessibilityLabel={`Insight: ${insight.title}`}accessibilityHint="Activates this control">
