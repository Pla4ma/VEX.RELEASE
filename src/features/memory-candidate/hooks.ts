...
import { getMemoryCandidates, addMemoryCandidate, removeMemoryCandidate } from './service';
...
export function useAddMemoryCandidate() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, input) => {
      void queryClient.invalidateQueries({ queryKey: ['memory-candidate', input.userId] });
    },
  });
}
...
export function useRemoveMemoryCandidate() {
  ...
  return useMutation({
    mutationFn: ({ userId, candidateId }: { userId: string; candidateId: string }) =>
