...
import {
  ...
  EffectResultSchema,
  GetItemDefinitionInputSchema,
  ApplyItemEffectInputSchema,
  GetItemsByTypeInputSchema,
  GetItemsByRarityInputSchema,
  DropTableSchema,
  type ItemDefinition,
  type EffectResult,
  type ItemEffect,
  ...
  type GetItemsByTypeInput,
  type GetItemsByRarityInput,
  type DropTable,
} from './schemas';
...
export async function getItemDefinitionByName(
  name: string
): Promise<ItemDefinition | null> {
  const { data, error } = await supabase
    .from('item_definitions')
...
export async function getItemsByType(
  ...
  if (error) {throw new RepositoryError('getItemsByType', error);}
  return ItemDefinitionSchema.array().parse(data ?? []);
}

export async function getItemsByRarity(
  input: GetItemsByRarityInput
): Promise<ItemDefinition[]> {
  ...
  if (error) {throw new RepositoryError('getItemsByRarity', error);}
  return ItemDefinitionSchema.array().parse(data ?? []);
}
...
export async function getAllAvailableItems(): Promise<ItemDefinition[]> {
  ...
  if (error) {throw new RepositoryError('getAllAvailableItems', error);}
  return ItemDefinitionSchema.array().parse(data ?? []);
}
...
export async function getShopItems(): Promise<ItemDefinition[]> {
  const { data, error } = await supabase
    ...
    .order('shop_price->>amount', { ascending: true });

  if (error) {throw new RepositoryError('getShopItems', error);}
  return ItemDefinitionSchema.array().parse(data ?? []);
}
...
export async function getCraftableItems(): Promise<ItemDefinition[]> {
  const { data, error } = await supabase
    ...
    .select('*')
    .eq('is_available', true)
    .not('crafting_recipe', 'is', null)
    .order('required_level', { ascending: true });

  if (error) {throw new RepositoryError('getCraftableItems', error);}
  return ItemDefinitionSchema.array().parse(data ?? []);
}

export async function getItemsByLevel(
  maxLevel: number,
  minLevel: number = 1
): Promise<ItemDefinition[]> {
  const { data, error } = await supabase
  ...
  if (error) {throw new RepositoryError('getItemsByLevel', error);}
  return ItemDefinitionSchema.array().parse(data ?? []);
}
...
export async function applyItemEffect(
  ...
  for (const effect of definition.effects) {
    try {
      ...
      if (effect.durationUnit === 'MINUTES') {durationSeconds = effect.duration * 60;}
      else if (effect.durationUnit === 'HOURS') {durationSeconds = effect.duration * 3600;}
      else if (effect.durationUnit === 'DAYS') {durationSeconds = effect.duration * 86400;}
  ...
  analytics.trackItemEffectApplied(userId, definition.id, appliedEffects.map(e => e.type));
  ...
  return {
    ...
    message: appliedEffects.length > 0
      ? `Applied ${appliedEffects.length} effect${appliedEffects.length > 1 ? 's' : ''}`
      : failedEffects.length > 0
        ? `Failed: ${failedEffects.map(f => f.type).join(', ')}`
        : 'No effects applied',
    appliedEffects,
...
export function canPurchase(
  definition: ItemDefinition,
  userLevel: number
): { canPurchase: boolean; reason: string | null } {
