...
const BuffCard: React.FC<{
  ...
  const progress = Math.max(
    0,
    Math.min(1, (buff.expiresAt - Date.now()) / (buff.expiresAt - buff.startedAt))
  );
...
export const ActiveBuffsTray: React.FC<ActiveBuffsTrayProps> = ({
  buffs,
  onBuffPress,
  compact = false,
}) => {
  ...
  const handleBuffPress = useCallback((buff: ActiveBuff) => {
        ...
        accessibilityHint="Activates this control">
          {selectedBuff && (
            ...
            >
              ...
              >
                <Icon
                  name={BUFF_CONFIG[selectedBuff.type].icon}
                  size={32}
                  color={BUFF_CONFIG[selectedBuff.type].color}
                />
              </Box>
