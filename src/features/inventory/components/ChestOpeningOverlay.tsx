    ...
    transform: [
      {
        translateY: withDelay(
          delay,
          withTiming(-SCREEN_HEIGHT, { duration: 2000 })
        ),
      },
      {
    ...
    transform: [
      {
        scale: withSequence(
          withTiming(0, { duration: 0 }),
          withSpring(2, { damping: 5, stiffness: 100 }),
    ...
    transform: [
      {
        translateY: withDelay(
          delay,
          withSpring(-150, { damping: 12, stiffness: 100 })
        ),
      },
      {
        scale: withDelay(
  ...
  return (
    ...
    >
      <Box
        ...
        height={80}
        borderRadius="xl"
        bg={isItem ? 'accent.purple' : 'background.secondary'}
        justifyContent="center"
        alignItems="center"
        borderWidth={2}
        borderColor={isItem ? 'accent.purple' : 'border.light'}
      >
...
function ChestLid({
  ...
  const lidStyle = useAnimatedStyle(() => ({
    transform: [
      {
        rotateX: phase === 'OPENING' || phase === 'REVEALING' || phase === 'COMPLETE'
          ? withSpring('-110deg', { damping: 10, stiffness: 100 })
          : '0deg',
      },
    ],
    opacity: phase === 'REVEALING' || phase === 'COMPLETE'
...
export function ChestOpeningOverlay({
  ...
  onClose,
  onChestOpened,
}: ChestOpeningOverlayProps): JSX.Element {
  const { theme } = useTheme();
