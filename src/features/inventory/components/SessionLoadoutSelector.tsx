  ...
  return (
    ...
    >
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        <Text style={{ fontWeight: '600', color: rarityColors[item.rarity] }}>
          {item.name}
        </Text>
      ...
      {!item.compatible && item.incompatibilityReason && (
      ...
      {item.compatible && (
        <View style={{ flexDirection: 'row', marginTop: 8, gap: 8 }}>
          {item.projectedImpact.xpMultiplier > 1 && (
            <Text style={{ fontSize: 11, color: '#10B981' }}>
              +{Math.round((item.projectedImpact.xpMultiplier - 1) * 100)}% XP
            </Text>
          )}
          {item.projectedImpact.coinMultiplier > 1 && (
            <Text style={{ fontSize: 11, color: '#F59E0B' }}>
              +{Math.round((item.projectedImpact.coinMultiplier - 1) * 100)}% Coins
            </Text>
          )}
          {item.projectedImpact.streakProtection && (
            <Text style={{ fontSize: 11, color: '#3B82F6' }}>Streak Shield</Text>
          )}
          {item.projectedImpact.bossDamageBonus > 0 && (
            <Text style={{ fontSize: 11, color: '#EF4444' }}>
              +{item.projectedImpact.bossDamageBonus} Boss Dmg
            </Text>
          )}
        </View>
      )}
...
function ActiveBuffIndicator({ buff }: { buff: ActiveBuff }): JSX.Element {
  return (
    ...
    >
      ...
      {buff.effects.xpMultiplier > 1 && (
        <Text style={{ fontSize: 10, color: '#92400E', marginLeft: 4 }}>
          +{Math.round((buff.effects.xpMultiplier - 1) * 100)}% XP
        </Text>
      )}
    </View>
  );
 ...
 */
export function SessionLoadoutSelector({
  userId,
  mode,
  duration,
}: SessionLoadoutSelectorProps): JSX.Element {
  const { data: loadout, isLoading, error } = useLoadoutOptions(
    userId,
    mode,
    duration
  );
    ...
    return (
      <View style={{ padding: 16 }}>
        <Text style={{ color: '#EF4444' }}>
          Failed to load inventory. Some options may be unavailable.
        </Text>
      </View>
    );
  }
    ...
    <View style={{ padding: 16 }}>
      ...
      {loadout.activeBuffs.length > 0 && (
        <View style={{ marginBottom: 16 }}>
          <Text style={{ fontWeight: '600', marginBottom: 8 }}>
            Active Buffs ({loadout.summary.activeBuffs})
          </Text>
      ...
      {incompatibleItems.length > 0 && (
        <View style={{ marginBottom: 16, opacity: 0.6 }}>
          <Text style={{ fontWeight: '600', marginBottom: 8, color: '#9CA3AF' }}>
            Incompatible ({incompatibleItems.length})
          </Text>
          ...
          {incompatibleItems.length > 3 && (
            <Text style={{ fontSize: 12, color: '#9CA3AF', textAlign: 'center' }}>
              +{incompatibleItems.length - 3} more items
            </Text>
          )}
        </View>
      )}
      ...
      {loadout.offlineRestrictions.length > 0 && (
        ...
        >
          <Text style={{ fontWeight: '600', fontSize: 12, color: '#991B1B', marginBottom: 4 }}>
            Offline Restrictions
          </Text>
      ...
      {loadout.available.length === 0 && (
        <View style={{ alignItems: 'center', padding: 24 }}>
          <Text style={{ color: '#6B7280', textAlign: 'center' }}>
            No items in your inventory. Visit the shop to get boosts and consumables.
          </Text>
        </View>
      )}
    </View>
