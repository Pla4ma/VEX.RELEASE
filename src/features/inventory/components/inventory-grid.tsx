...
function InventoryItemCard({
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`${itemName}, quantity ${item.quantity}`}
  ...
  return (
    <View style={styles.filterBar}>
      <Pressable
        ...
        style={({ pressed }) => [styles.filterToggle, pressed && { opacity: 0.8 }]}
        accessibilityLabel="Toggle filters button"
        accessibilityRole="button"
        accessibilityHint="Activates this control">
          ...
          <View style={styles.filterChips}>
            {typeOptions.map(type => (
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.filterChip,
                  filter.types?.includes(type) && styles.filterChipActive,
                  pressed && { opacity: 0.8 },
                ]}
                accessibilityLabel="Interactive control"
                accessibilityRole="button"
                accessibilityHint="Activates this control">
                <Text style={styles.filterChipText}>{TYPE_ICONS[type]} {type}</Text>
              </Pressable>
            ))}
          </View>
          ...
          <View style={styles.filterChips}>
            {rarityOptions.map(rarity => (
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.filterChip,
                  filter.rarities?.includes(rarity) && styles.filterChipActive,
                ...
                accessibilityRole="button"
                accessibilityHint="Activates this control">
                <Text style={[styles.filterChipText, { color: RARITY_COLORS[rarity] }]}>
                  {rarity}
                </Text>
              </Pressable>
            ))}
...
export function InventoryGrid({
  ...
  const [activeFilter, setActiveFilter] = useState<InventoryFilter>(filter ?? {
    sortBy: 'acquiredAt',
    sortOrder: 'desc',
  });

  const { data: inventory, isLoading, isError, refetch } = useFilteredInventory(
    userId,
    activeFilter
  );
  ...
  const renderItem = useCallback(({ item }: { item: InventoryItem }) => (
    ...
    />
  ), [handleItemPress, handleItemLongPress, selectedItemIds, showQuickActions]);
  ...
  if (isError) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Pressable onPress={() => refetch()} style={({ pressed }) => [styles.retryButton, pressed && { opacity: 0.8 }]}
      ...
      <View style={styles.centerContainer}>
        {emptyState ?? (
          <EmptyState
            ...
            body="Visit the shop or complete sessions to start collecting items, boosts, and rewards."
          />
        )}
      </View>
    );
