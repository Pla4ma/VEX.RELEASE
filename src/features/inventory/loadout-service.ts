...
function getOfflineRestrictions(): string[] {
  return [
    'Wagers require online connection',
    'Boss bounty claims require sync',
    'Streak insurance requires verification',
  ];
}
 ...
 */
export async function buildLoadoutOptions(
  userId: string,
  mode: string,
  durationSeconds: number
): Promise<LoadoutOptions> {
  // Fetch real inventory data in parallel
  const [inventory, buffs] = await Promise.all([
    getUsableConsumables(userId).catch(() => [] as InventoryItem[]),
    getActiveBuffs(userId).catch(() => [] as InventoryItem[]),
  ]);
  ...
  const activeBuffs = buffs.map((buff): ActiveBuff => {
    ...
    return ActiveBuffSchema.parse({
      ...
      expiresAt: buff.equippedAt
        ? (buff.equippedAt as number) + 24 * 60 * 60 * 1000
        : null,
      effects: {
 ...
 */
export function useLoadoutOptions(
