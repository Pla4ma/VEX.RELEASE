...
export async function fetchUserInventory(
  ...
  if (error) {throw new RepositoryError('fetchUserInventory', error);}
...
export async function fetchInventoryItemsByDefinition(
  userId: string,
  ...
  if (error) {throw new RepositoryError('fetchInventoryItemsByDefinition', error);}
  return InventoryItemSchema.array().parse(data ?? []);
}
...
export async function fetchEquippedItems(userId: string): Promise<InventoryItem[]> {
  ...
  if (error) {throw new RepositoryError('fetchEquippedItems', error);}
  return InventoryItemSchema.array().parse(data ?? []);
}

export async function fetchItemInSlot(
  userId: string,
  slot: EquipmentSlot
): Promise<InventoryItem | null> {
  const { data, error } = await supabase
...
export async function createInventoryItem(
  ...
  if (error) {throw new RepositoryError('createInventoryItem', error);}
  return InventoryItemSchema.parse(data);
}

export async function updateInventoryItem(
  itemId: string,
  updates: Partial<Omit<InventoryItem, 'id' | 'createdAt' | 'userId'>>
): Promise<InventoryItem> {
  const { data, error } = await supabase
    .from('inventory_items')
    .update({
  ...
  if (error) {throw new RepositoryError('updateInventoryItem', error);}
  return InventoryItemSchema.parse(data);
}
...
export async function updateItemQuantity(
  ...
  if (error) {throw new RepositoryError('updateItemQuantity', error);}
  return InventoryItemSchema.parse(data);
}
...
export async function markItemDeleted(
  ...
  if (error) {throw new RepositoryError('markItemDeleted', error);}
}
...
export async function mergeItems(
  ...
  if (error) {throw new RepositoryError('mergeItems', error);}
}

export async function splitItemStack(
  itemId: string,
  ...
  if (error) {throw new RepositoryError('splitItemStack', error);}
  return InventoryItemSchema.parse(data);
}
...
export async function getInventoryCapacity(userId: string): Promise<{
  ...
  if (error) {throw new RepositoryError('getInventoryCapacity', error);}
...
export async function getUsableConsumables(userId: string): Promise<InventoryItem[]> {
  ...
  if (error) {throw new RepositoryError('getUsableConsumables', error);}
  return InventoryItemSchema.array().parse(data ?? []);
}
...
export async function getActiveBuffs(userId: string): Promise<InventoryItem[]> {
  ...
  if (error) {throw new RepositoryError('getActiveBuffs', error);}
  return InventoryItemSchema.array().parse(data ?? []);
}
...
export async function fetchQuickUseSlots(userId: string): Promise<QuickUseSlotData[]> {
  const { data, error } = await supabase
    .from('quick_use_slots')
    .select('*')
    .eq('user_id', userId)
    .order('slot_index', { ascending: true });

  if (error) {throw new RepositoryError('fetchQuickUseSlots', error);}
...
export async function setQuickUseSlot(
  ...
  if (error) {throw new RepositoryError('setQuickUseSlot', error);}
}
...
export async function checkDuplicatePurchaseItem(
  userId: string,
  purchaseId: string
): Promise<boolean> {
  const { data, error } = await supabase
  ...
  if (error) {throw new RepositoryError('checkDuplicatePurchaseItem', error);}
  return (data ?? []).length > 0;
}

export async function checkDuplicateItemDefinition(
  userId: string,
  itemDefinitionId: string,
  ...
  if (error) {throw new RepositoryError('checkDuplicateItemDefinition', error);}
  return (data ?? []).length > 0;
}
