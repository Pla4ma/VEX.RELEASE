...
/**
 * Get wallet summary (simplified view)
 */
export async function getWalletSummary(userId: string): Promise<{
  ...
  return {
    coins: wallet.coins,
    gems: wallet.gems,
    hasSeasonal: Object.keys(wallet.seasonal).length > 0,
  };
}
...
/**
 * Calculate earnings multiplier based on user level, streak, and other factors
 */
export function calculateEarningsMultiplier(input: CalculateEarningsInput): number {
  ...
  for (const [level, mult] of Object.entries(LEVEL_MULTIPLIERS)) {
    if (validated.userLevel >= parseInt(level)) {
      multiplier = mult;
    }
  }

  if (validated.streakDays) {
    if (validated.streakDays >= 30) {multiplier += 0.5;}
    else if (validated.streakDays >= 14) {multiplier += 0.35;}
    else if (validated.streakDays >= 7) {multiplier += 0.25;}
    else if (validated.streakDays >= 3) {multiplier += 0.1;}
  }
