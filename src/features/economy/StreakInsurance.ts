...
export const PurchaseInsuranceInputSchema = z.object({
  userId: z.string().uuid(),
  currentStreakDays: z.number().int().min(0),
}).strict();

export const CheckInsuranceStatusInputSchema = z.object({
  userId: z.string().uuid(),
}).strict();

export const ConsumeInsuranceInputSchema = z.object({
  userId: z.string().uuid(),
...
export async function checkInsuranceStatus(
  input: CheckInsuranceStatusInput
...
export async function consumeInsurance(
  input: ConsumeInsuranceInput
): Promise<{ success: boolean; restoredDays: number | null; error: string | null }> {
