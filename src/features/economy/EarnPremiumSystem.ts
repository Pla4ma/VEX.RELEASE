...
const REWARD_DEFINITIONS: Record<
  EarnPremiumAchievementType,
  Omit<EarnPremiumReward, 'claimed' | 'claimedAt' | 'unlockedAt'>
> = {
...
export class EarnPremiumSystem {
  ...
  checkAndUnlock(input: CheckEligibilityInput): EarnPremiumStatus {
    ...
    if (validated.streakDays >= 30) {
      const existingReward = status.rewards.find(
        (r) => r.type === EarnPremiumAchievementType.STREAK_30_DAYS
      );
      if (!existingReward) {
    ...
    const hasDefeatedAllBosses = ALL_BOSS_IDS.every((id) =>
      validated.defeatedBossIds.includes(id)
    );
    if (hasDefeatedAllBosses) {
      const existingReward = status.rewards.find(
        (r) => r.type === EarnPremiumAchievementType.ALL_BOSSES_DEFEATED
      );
      if (!existingReward) {
    ...
    if (validated.currentLevel >= 20) {
      const existingReward = status.rewards.find(
        (r) => r.type === EarnPremiumAchievementType.LEVEL_20
      );
      if (!existingReward) {
   ...
   */
  claimReward(
    status: EarnPremiumStatus,
    rewardType: EarnPremiumAchievementType
  ): { success: boolean; status: EarnPremiumStatus; error?: string } {
  ...
  private async syncTrialWithRevenueCat(
    userId: string,
    trialDays: number
  ): Promise<void> {
    try {
...
export function checkEarnPremiumEligibility(
  userId: string,
  streakDays: number,
  defeatedBossIds: string[],
  currentLevel: number,
...
export function claimEarnPremiumReward(
  status: EarnPremiumStatus,
  rewardType: EarnPremiumAchievementType
): { success: boolean; status: EarnPremiumStatus; error?: string } {
