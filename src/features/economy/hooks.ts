...
export const economyKeys = {
  ...
  balance: (userId: string, currency?: CurrencyType) =>
    [...economyKeys.all, 'balance', userId, currency] as const,
  transactions: (userId: string, filters?: Record<string, unknown>) =>
    [...economyKeys.all, 'transactions', userId, filters] as const,
  purchase: (purchaseId: string) => [...economyKeys.all, 'purchase', purchaseId] as const,
  offers: (userId: string, userLevel: number) =>
    [...economyKeys.all, 'offers', userId, userLevel] as const,
  analytics: (userId: string, period: string) =>
    [...economyKeys.all, 'analytics', userId, period] as const,
};
...
export function useProcessPurchasePayment() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      currency: CurrencyType;
      amount: number;
    }) => service.processPurchasePayment(purchaseId, currency, amount),
...
export function useCompletePurchaseDelivery() {
  ...
  return useMutation({
    ...
    }) => service.completePurchaseDelivery(purchaseId, inventoryItemIds),
    onSuccess: (result, variables) => {
      queryClient.invalidateQueries({
        queryKey: economyKeys.purchase(variables.purchaseId),
      });
...
export function useRequestRefund() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      userId: string;
      reason: string;
    }) => service.requestRefund(purchaseId, userId, reason),
    onSuccess: (result) => {
      queryClient.invalidateQueries({
        queryKey: economyKeys.purchase(result.purchaseId),
      });
...
export function useClaimOffer() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      purchaseId: string;
      userLevel: number;
    }) => service.claimOffer(offerId, userId, purchaseId),
    onSuccess: (result, variables) => {
      queryClient.invalidateQueries({
