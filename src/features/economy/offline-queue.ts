...
class OfflineQueue {
  ...
  enqueue(entry: Omit<QueueEntry, 'id' | 'status' | 'createdAt' | 'updatedAt' | 'retryCount'>): QueueEntry {
    ...
    if (entry.dedupeKey) {
      ...
      if (existing && existing.status === 'PENDING') {
        ...
        if (entry.payload.timestamp && existing.payload.timestamp &&
  ...
  resolveConflicts(): QueueEntry[] {
    ...
    for (const entries of userOps.values()) {
      const currencyOps = entries.filter(e =>
        e.type === 'EARN_CURRENCY' || e.type === 'SPEND_CURRENCY'
      );
...
): Omit<QueueEntry, 'id' | 'status' | 'createdAt' | 'updatedAt' | 'retryCount'> {
  return {
    userId,
    type: 'EARN_CURRENCY',
