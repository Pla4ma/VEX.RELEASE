...
export class VariableRewardEngine {
  ...
  calculateReward(input: CalculateVariableRewardInput): VariableRewardResult {
    ...
    try {
      ...
      const { modifiedChances, triggeredModifiers } = this.applyModifiers(
        BASE_CHANCES,
        validated.modifiers
      );
  ...
  private applyModifiers(
    ...
    const totalRewardWeight = rewardTiers.reduce(
      (sum, tier) => sum + baseChances[tier],
      0
    );
  ...
  private determineTier(
    roll: number,
    chances: Record<VariableRewardTier, number>
  ): VariableRewardTier {
    let cumulative = 0;
  ...
  private generateItems(
    ...
    const selectedTemplates = tier === VariableRewardTier.LEGENDARY
  ...
  getExpectedValueStats(baseCoins: number): {
    ...
    return Object.entries(BASE_CHANCES).map(([tier, chance]) => {
      ...
      if (tier === VariableRewardTier.COMMON) {
        ...
        // Sum of item values
        expectedValue = items.reduce((sum, item) => {
  ...
  simulateDistribution(
    rollCount: number,
    modifiers: VariableRewardModifiers
  ): Record<VariableRewardTier, number> {
 ...
 */
export function validateVariableRewardServerSide(
  clientResult: VariableRewardResult,
  serverModifiers: VariableRewardModifiers,
  serverSeed: string
