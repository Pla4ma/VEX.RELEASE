...
export async function saveDailyRewardClaim(
  claim: DailyRewardClaim
): Promise<{ data: DailyRewardClaim | null; error: RepositoryError | null }> {
  try {
    enqueue({
      operation: 'CREATE',
      feature: 'rewards',
...
export async function fetchDailyRewardsState(
  userId: string
): Promise<{ data: UserDailyRewardsState | null; error: RepositoryError | null }> {
  try {
...
export async function updateDailyRewardsState(
  userId: string,
  updates: Partial<UserDailyRewardsState>
): Promise<{ data: UserDailyRewardsState | null; error: RepositoryError | null }> {
  try {
    enqueue({
      operation: 'UPDATE',
...
export async function fetchUsersWithExpiredStreaks(): Promise<{
  ...
  try {
    const { data, error } = await withRetry('fetchUsersWithExpiredStreaks', async () => {
      return await supabase
        .from('user_daily_rewards')
        .select('user_id, current_streak, last_claimed_at')
...
export async function fetchClaimHistory(
  userId: string,
  limit: number = 30
): Promise<{ data: DailyRewardClaim[]; error: RepositoryError | null }> {
  try {
