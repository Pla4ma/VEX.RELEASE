...
export function processDailyLogin(
  ...
  const newState: DailyLoginState = {
    ...
    weeksCompleted: weekCompleted
      ? currentState.weeksCompleted + 1
      : currentState.weeksCompleted,
    totalLogins: currentState.totalLogins + 1,
    consistentBadgeUnlocked:
      currentState.consistentBadgeUnlocked ||
      (weekCompleted && currentState.weeksCompleted + 1 >= 1),
  };
 ...
 */
export function shouldShowDailyLogin(
  state: DailyLoginState,
  today: number = getTodayTimestamp()
): boolean {
...
export function loadDailyLoginState(): DailyLoginState {
  ...
  if (!raw) {return createInitialLoginState();}
