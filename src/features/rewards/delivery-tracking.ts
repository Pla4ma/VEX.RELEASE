...
export const RewardDeliveryStatusSchema = z.enum([
  'PENDING',
  'IN_PROGRESS',
  'DELIVERED',
  'FAILED',
...
export const RewardDeliverySchema = z.object({
  id: z.string(),
  userId: z.string(),
...
export async function createRewardDelivery(
  delivery: Omit<RewardDelivery, 'id' | 'attemptCount' | 'lastAttemptAt' | 'deliveredAt' | 'failedAt' | 'errorMessage' | 'retryAfter'>
...
export async function getDeliveriesNeedingRetry(): Promise<RewardDelivery[]> {
  ...
  const { data, error } = await supabase
    ...
    .or(`retry_after.lt.${Date.now()},retry_after.is.null`)
    .lt('attempt_count', supabase.rpc('get_max_attempts'))
    .limit(100);
