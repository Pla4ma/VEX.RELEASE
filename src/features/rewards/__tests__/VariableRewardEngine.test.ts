/**
 * VariableRewardEngine Tests
 *
 * Tests probability distributions and modifier calculations.
...
import {
  ...
  VariableRewardTier,
  calculateSessionVariableReward,
  validateVariableRewardServerSide,
} from '../VariableRewardEngine';
...
describe('VariableRewardEngine', () => {
  ...
  describe('Base Probability Distribution', () => {
    ...
    it('should have correct base chances', () => {
      ...
      expect(chances[VariableRewardTier.NONE]).toBeCloseTo(0.728, 3);
      expect(chances[VariableRewardTier.COMMON]).toBeCloseTo(0.15, 3);
      expect(chances[VariableRewardTier.UNCOMMON]).toBeCloseTo(0.08, 3);
      expect(chances[VariableRewardTier.RARE]).toBeCloseTo(0.03, 3);
      expect(chances[VariableRewardTier.EPIC]).toBeCloseTo(0.01, 3);
      expect(chances[VariableRewardTier.LEGENDARY]).toBeCloseTo(0.002, 3);
    });
  });
  ...
  describe('Reward Calculation', () => {
    it('should return NONE tier 72.8% of the time (within statistical bounds)', () => {
      ...
      const distribution = engine.simulateDistribution(iterations, modifiers);
      const noneRate = distribution[VariableRewardTier.NONE] / iterations;

      // Within 2% margin due to randomness
      expect(noneRate).toBeGreaterThan(0.70);
    ...
    it('should return LEGENDARY tier ~0.2% of the time', () => {
      ...
      const distribution = engine.simulateDistribution(iterations, modifiers);
      const legendaryRate = distribution[VariableRewardTier.LEGENDARY] / iterations;

      // 0.2% ± 0.1% margin
      expect(legendaryRate).toBeGreaterThan(0.001);
    ...
    it('should return COMMON tier with ~15% chance', () => {
      ...
      const distribution = engine.simulateDistribution(iterations, modifiers);
      const commonRate = distribution[VariableRewardTier.COMMON] / iterations;

      expect(commonRate).toBeGreaterThan(0.13);
      expect(commonRate).toBeLessThan(0.17);
    });
  });
  ...
  describe('Modifiers', () => {
    it('should increase drop chances with streak > 7 days', () => {
      ...
      const withStreak = engine.simulateDistribution(iterations, {
        streakDays: 10,
        isSGrade: false,
        bossActive: false,
        squadSession: false,
      ...
      const noRewardRate = noStreak[VariableRewardTier.NONE] / iterations;
      const withRewardRate = withStreak[VariableRewardTier.NONE] / iterations;

      // Streak should reduce NONE chance (increase reward chance)
    ...
    it('should increase drop chances with S grade', () => {
      ...
      const sGrade = engine.simulateDistribution(iterations, {
        streakDays: 0,
        isSGrade: true,
        bossActive: false,
        squadSession: false,
      ...
      const normalNoneRate = normalGrade[VariableRewardTier.NONE] / iterations;
      const sGradeNoneRate = sGrade[VariableRewardTier.NONE] / iterations;

      // S grade should reduce NONE chance
    ...
    it('should cap total modifiers at 15%', () => {
      ...
      // NONE chance should be at minimum (0.728 - 0.15 = 0.578)
      expect(result.modifiedChances[VariableRewardTier.NONE]).toBeGreaterThan(0.57);
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        if (result.tier === VariableRewardTier.COMMON) {
          ...
          if (coinItem) {
            ...
            expect(coinItem.amount).toBeGreaterThanOrEqual(150);
            expect(coinItem.amount).toBeLessThanOrEqual(250);
          }
        }
      }
    });
  ...
  describe('Seeded Random (Deterministic)', () => {
    ...
    it('should produce different results with different seeds', () => {
      ...
      const isDifferent = result1.rollValue !== result2.rollValue;
      expect(isDifferent || result1.tier !== result2.tier).toBe(true);
    });
  });
  ...
  describe('Server-side Validation', () => {
    it('should validate matching client and server results', () => {
      ...
      const clientResult = engine.calculateReward({
        userId: VALID_USER_ID,
        baseCoins: 100,
        modifiers,
        seed,
      ...
      const validation = validateVariableRewardServerSide(
        clientResult,
        modifiers,
        seed
      );
    ...
    it('should reject invalid roll values', () => {
      const fakeResult = {
        ...
        items: [],
        rollValue: 999, // Invalid: > 1
        modifiedChances: {} as Record<VariableRewardTier, number>,
        triggeredModifiers: [],
      };
  ...
  describe('Convenience Functions', () => {
    it('calculateSessionVariableReward should work with session data', () => {
      const result = calculateSessionVariableReward(
        VALID_USER_ID,
        100,
        {
          streakDays: 14,
  ...
  describe('Expected Value Statistics', () => {
    it('should calculate expected values for each tier', () => {
      ...
      for (const stat of stats) {
        expect(stat.chance).toBeGreaterThanOrEqual(0);
        expect(stat.chance).toBeLessThanOrEqual(1);
        expect(stat.expectedValue).toBeGreaterThanOrEqual(0);
      }

      // NONE tier should have 0 expected value
      const noneStat = stats.find(s => s.tier === VariableRewardTier.NONE);
      expect(noneStat?.expectedValue).toBe(0);
    });
  });
});
