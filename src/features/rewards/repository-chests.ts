...
export async function createChest(
  chest: Omit<Chest, 'opened' | 'rewards' | 'opened_at'>
): Promise<{ data: Chest | null; error: RepositoryError | null }> {
  try {
    enqueue({
      operation: 'CREATE',
      feature: 'rewards',
...
export async function openChest(
  chestId: string,
  rewards: NonNullable<Chest['rewards']>
): Promise<{ data: Chest | null; error: RepositoryError | null }> {
  try {
    enqueue({
      operation: 'UPDATE',
...
export async function fetchUserChests(
  userId: string,
  opened: boolean | null = null
): Promise<{ data: Chest[]; error: RepositoryError | null }> {
  try {
    let query = supabase
      .from('chests')
      .select('*')
...
export async function fetchChestById(
  chestId: string
): Promise<{ data: Chest | null; error: RepositoryError | null }> {
  try {
...
export async function createMysteryMultiplier(
  multiplier: Omit<MysteryMultiplier, 'used'>
): Promise<{ data: MysteryMultiplier | null; error: RepositoryError | null }> {
  try {
...
export async function markMultiplierUsed(
  multiplierId: string
): Promise<{ success: boolean; error: RepositoryError | null }> {
  try {
...
export async function fetchActiveMultiplier(
  userId: string
): Promise<{ data: MysteryMultiplier | null; error: RepositoryError | null }> {
  try {
...
export async function fetchChestAnalytics(
  userId: string
): Promise<{
  data: {
    totalOpened: number;
