...
export async function createReward(input: CreateRewardInput): Promise<Reward> {
  ...
  if (validated.triggerId) {
    const isDuplicate = await repository.checkDuplicateReward(
      validated.userId,
...
export async function getRewardStats(userId: string): Promise<{
  ...
  return {
    ...
    totalXp: rewards
      .filter(r => r.status === 'CLAIMED' && r.type === 'XP')
      .reduce((sum, r) => sum + (r.amount || 0), 0),
    totalCoins: rewards
      .filter(r => r.status === 'CLAIMED' && r.type === 'COINS')
      .reduce((sum, r) => sum + (r.amount || 0), 0),
    totalGems: rewards
      .filter(r => r.status === 'CLAIMED' && r.type === 'GEMS')
      .reduce((sum, r) => sum + (r.amount || 0), 0),
  };
}
...
const NO_DROP_CHANCE = 0.10;

/**
 * Roll for mystery chest drop after session completion
 * Returns chest if dropped, null if no drop
 */
...
/**
 * Get chest appearance based on rarity
 */
...
export function generateChestContents(rarity: ChestRarity): {
  ...
  return {
    ...
    ...(rarity !== 'COMMON' && rarity !== 'UNCOMMON'
      ? { gems: Math.floor(Math.random() * 5 * multiplier) }
      : {}),
    ...(rarity === 'EPIC' || rarity === 'LEGENDARY'
      ? { item: `${rarity} Mystery Item` }
      : {}),
  };
}

/**
 * Maximum chests held in inventory
 */
export const MAX_CHESTS_IN_INVENTORY = 3;

/**
 * Check if user can receive more chests
 */
