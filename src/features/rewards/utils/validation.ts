...
export function validateChestReward(
  ...
  const recentDuplicate = userHistory.recentChests.find(
    c => c.chestId === reward.chestId && c.openedAt > Date.now() - 60000
  );
  if (recentDuplicate) {
    result.violations.push({
      type: 'DUPLICATE',
...
function validateRarityDistribution(
  reward: ChestReward,
  result: ValidationResult<ChestReward>
): void {
...
export function validateLedgerBalance(
  ...
  const earned = ledger.transactions
