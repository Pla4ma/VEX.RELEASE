...
async function executeWithFallback<T>(
  ...
  try {
    ...
    const repoError = error instanceof RepositoryError
      ? new RewardsRepositoryError(operation, error.originalError, error.code)
      : new RewardsRepositoryError(operation, error);
    ...
    if (offlineFn) {
      ...
      } catch (error) { captureSilentFailure(error, { feature: 'rewards', operation: 'network-fallback', type: 'network' });
        // Cache miss
      }
    }
...
export async function createRewardEnhanced(
  userId: string,
  type: string,
  amount: number | null,
  triggerType: string,
...
): Promise<RepositoryResult<Reward>> {
  const newReward = {
...
export async function fetchRewardsEnhanced(
  userId: string,
  status?: string
): Promise<RepositoryResult<Reward[]>> {
  return executeWithFallback(
...
export async function checkDuplicateRewardEnhanced(
  userId: string,
  triggerType: string,
  triggerId: string
...
export async function recordLedgerEntryEnhanced(
  rewardId: string,
  action: string,
  details: Record<string, unknown>
): Promise<RepositoryResult<RewardLedger>> {
  enqueue({
    operation: 'CREATE',
    feature: 'rewards',
...
export async function batchClaimRewards(
  rewardIds: string[]
): Promise<{ successful: Reward[]; failed: Array<{ rewardId: string; error: RewardsRepositoryError }> }> {
