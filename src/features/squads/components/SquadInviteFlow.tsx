...
import { launchColors } from '@theme/tokens/launch-colors';
 type InviteStep='SEARCH'|'CONFIGURE'|'REVIEW'|'SUCCESS';interface UserSearchResult{id:string;displayName:string;avatarUrl:string|null;level:number}interface SquadInviteFlowProps{squadId:string;userId:string;searchUsers:(query:string)=>Promise<UserSearchResult[]>;onComplete?:()=>void;onCancel?:()=>void}const ROLE_OPTIONS:{value:SquadRole;label:string;description:string}[] = [{value:'MEMBER',label:'Member',description:'Can participate in sessions and chat'},{value:'MODERATOR',label:'Moderator',description:'Can kick members and moderate chat'},{value:'ADMIN',label:'Admin',description:'Can manage squad settings and invites'}]; export const SquadInviteFlow:React.FC<SquadInviteFlowProps> = ({squadId,userId,searchUsers,onComplete,onCancel})=>{const{data:squad} = useSquad(squadId); const{hasPermission} = useSquadPermissions(squadId,userId); const inviteMutation = useInviteToSquad(userId); const[step,setStep] = useState<InviteStep>('SEARCH'); const[selectedUsers,setSelectedUsers] = useState<UserSearchResult[]>([]); const[searchQuery,setSearchQuery] = useState(''); const[selectedRole,setSelectedRole] = useState<SquadRole>('MEMBER'); const[message,setMessage] = useState(''); const[isSearching,setIsSearching] = useState(false); const[searchResults,setSearchResults] = useState<UserSearchResult[]>([]); const[sentCount,setSentCount] = useState(0); const[error,setError] = useState<string|null>(null); const fadeAnim = useSharedValue(1); const animatedStyle = useAnimatedStyle(()=>({opacity:fadeAnim.value})); const animateTransition = useCallback((nextStep:InviteStep)=>{fadeAnim.value = withSequence(withTiming(0,{duration:150},()=>{runOnJS(setStep)(nextStep);}),withTiming(1,{duration:150}));},[fadeAnim]); const handleSearch = useCallback(async()=>{if(!searchQuery.trim()){return;}setIsSearching(true); setError(null); try{const results = await searchUsers(searchQuery); const selectedIds = selectedUsers.map(u=>u.id); const filtered = results.filter(u=>!selectedIds.includes(u.id)); setSearchResults(filtered);}catch(err){setError(err instanceof Error ? err.message : 'Failed to search users'); setSearchResults([]);}finally{setIsSearching(false);}},[searchQuery,searchUsers,selectedUsers]); const toggleUserSelection = useCallback((user:UserSearchResult)=>{setSelectedUsers(prev=>prev.find(u=>u.id === user.id) ? prev.filter(u=>u.id !== user.id) : [...prev,user]); setSearchResults(prev=>prev.filter(u=>u.id !== user.id));},[]); const removeSelectedUser = useCallback((userId:string)=>{setSelectedUsers(prev=>prev.filter(u=>u.id !== userId));},[]); const handleSendInvites = useCallback(async()=>{setError(null); try{let sent = 0; for(const targetUser of selectedUsers){await inviteMutation.mutateAsync({squadId,invitedUserId:targetUser.id,roleOffered:selectedRole,message:message || null,expiresInHours:48}); sent++;}setSentCount(sent); animateTransition('SUCCESS');}catch(err){setError(err instanceof Error ? err.message : 'Failed to send invites');}},[selectedUsers,squadId,selectedRole,message,inviteMutation,animateTransition]); const handleNext = useCallback(()=>{switch(step){case'SEARCH':if(selectedUsers.length > 0){animateTransition('CONFIGURE');}break; case'CONFIGURE':animateTransition('REVIEW'); break; case'REVIEW':void handleSendInvites(); break; case'SUCCESS':onComplete?.(); break;}},[step,selectedUsers.length,animateTransition,onComplete,handleSendInvites]); const handleBack = useCallback(()=>{switch(step){case'CONFIGURE':animateTransition('SEARCH'); break; case'REVIEW':animateTransition('CONFIGURE'); break; default:onCancel?.();}},[step,animateTransition,onCancel]); if(!hasPermission('INVITE_MEMBERS')){return<Card style={styles.container}>
  ...
  accessibilityHint="Activates this control">
    ...
    </Card>;}; const styles = createSheet({container:{padding:20,maxHeight:'80%'},content:{flex:1},stepIndicator:{flexDirection:'row',justifyContent:'space-between',marginBottom:24,paddingHorizontal:8},stepItem:{alignItems:'center',flex:1,position:'relative'},stepDot:{width:28,height:28,borderRadius:14,backgroundColor:launchColors.hex_e0e0e0,justifyContent:'center',alignItems:'center'},stepDotActive:{backgroundColor:launchColors.hex_007aff},stepDotCompleted:{backgroundColor:launchColors.hex_4caf50},stepNumber:{fontSize:12,fontWeight:'600',color:launchColors.hex_666},stepNumberActive:{color:launchColors.hex_fff},stepCheck:{fontSize:14,color:launchColors.hex_fff,fontWeight:'bold'},stepLabel:{fontSize:10,marginTop:4,color:launchColors.hex_999},stepLabelActive:{color:launchColors.hex_007aff,fontWeight:'600'},stepLine:{position:'absolute',top:14,right:'50%',left:'50%',height:2,backgroundColor:launchColors.hex_e0e0e0,zIndex:-1},stepLineCompleted:{backgroundColor:launchColors.hex_4caf50},stepContent:{flex:1},stepTitle:{fontSize:20,fontWeight:'600',marginBottom:16},searchContainer:{flexDirection:'row',gap:8,marginBottom:16},searchInput:{flex:1,borderWidth:1,borderColor:launchColors.hex_e0e0e0,borderRadius:8,padding:12,fontSize:14},searchLoading:{marginTop:32},userList:{maxHeight:200},userItem:{flexDirection:'row',alignItems:'center',padding:12,backgroundColor:launchColors.hex_f5f5f5,borderRadius:8,marginBottom:8},userItemSelected:{backgroundColor:launchColors.hex_e3f2fd,borderWidth:2,borderColor:launchColors.hex_2196f3},userInfo:{flex:1,marginLeft:12},userName:{fontSize:14,fontWeight:'500'},userMeta:{fontSize:12,color:launchColors.hex_666,marginTop:2},checkbox:{width:24,height:24,borderRadius:12,borderWidth:2,borderColor:launchColors.hex_ccc,justifyContent:'center',alignItems:'center'},checkboxChecked:{backgroundColor:launchColors.hex_2196f3,borderColor:launchColors.hex_2196f3},checkmark:{color:launchColors.hex_fff,fontSize:14,fontWeight:'bold'},emptySearch:{alignItems:'center',padding:32},emptySearchText:{fontSize:14,color:launchColors.hex_666},selectionPreview:{backgroundColor:launchColors.hex_e3f2fd,padding:12,borderRadius:8,marginTop:8},selectionText:{fontSize:14,fontWeight:'500',color:launchColors.hex_1976d2},errorContainer:{backgroundColor:launchColors.hex_ffebee,padding:12,borderRadius:8,marginTop:8},errorText:{color:launchColors.hex_c62828,fontSize:14},selectedUsersPreview:{marginBottom:16},sectionLabel:{fontSize:12,fontWeight:'600',color:launchColors.hex_666,marginBottom:8,textTransform:'uppercase'},selectedUsersList:{flexDirection:'row'},selectedUserChip:{flexDirection:'row',alignItems:'center',backgroundColor:launchColors.hex_f5f5f5,padding:8,borderRadius:20,marginRight:8,gap:6},selectedUserName:{fontSize:12,maxWidth:100},removeUser:{fontSize:18,color:launchColors.hex_666,marginLeft:4},roleOption:{flexDirection:'row',alignItems:'center',padding:16,backgroundColor:launchColors.hex_f5f5f5,borderRadius:8,marginBottom:8},roleOptionSelected:{backgroundColor:launchColors.hex_e3f2fd,borderWidth:2,borderColor:launchColors.hex_2196f3},roleRadio:{width:20,height:20,borderRadius:10,borderWidth:2,borderColor:launchColors.hex_999,justifyContent:'center',alignItems:'center',marginRight:12},roleRadioSelected:{width:10,height:10,borderRadius:5,backgroundColor:launchColors.hex_2196f3},roleInfo:{flex:1},roleLabel:{fontSize:14,fontWeight:'500'},roleDescription:{fontSize:12,color:launchColors.hex_666,marginTop:2},messageInput:{borderWidth:1,borderColor:launchColors.hex_e0e0e0,borderRadius:8,padding:12,fontSize:14,minHeight:80,textAlignVertical:'top'},messageCharCount:{fontSize:12,color:launchColors.hex_999,textAlign:'right',marginTop:4},reviewCard:{padding:16,marginBottom:16},reviewLabel:{fontSize:12,color:launchColors.hex_666,marginBottom:4},reviewValue:{fontSize:16,fontWeight:'500',marginBottom:12},reviewMessage:{fontSize:14,fontStyle:'italic',color:launchColors.hex_555,backgroundColor:launchColors.hex_f5f5f5,padding:12,borderRadius:8},reviewNote:{fontSize:12,color:launchColors.hex_666,fontStyle:'italic'},successContainer:{alignItems:'center',justifyContent:'center',padding:32},successIconContainer:{width:80,height:80,borderRadius:40,backgroundColor:launchColors.hex_e8f5e9,justifyContent:'center',alignItems:'center',marginBottom:16},successIcon:{fontSize:40},successTitle:{fontSize:24,fontWeight:'600',marginBottom:8},successText:{fontSize:16,color:launchColors.hex_666,marginBottom:8},successSubtext:{fontSize:14,color:launchColors.hex_999},permissionDenied:{alignItems:'center',padding:32},permissionIcon:{fontSize:48,marginBottom:16},permissionTitle:{fontSize:18,fontWeight:'600',marginBottom:8},permissionText:{fontSize:14,color:launchColors.hex_666,textAlign:'center',marginBottom:16},permissionButton:{minWidth:120},footer:{flexDirection:'row',justifyContent:'space-between',marginTop:16,paddingTop:16,borderTopWidth:1,borderTopColor:launchColors.hex_e0e0e0}});
