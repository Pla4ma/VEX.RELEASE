...
interface SquadHubProps {
  ...
  onMemberPress?: (member: SquadMemberDetail) => void;
}
...
export const SquadHub: React.FC<SquadHubProps> = ({
  ...
  const { data: squad, isLoading: squadLoading, error: squadError } = useSquad(squadId);
  const { data: members, isLoading: membersLoading } = useSquadMembers(squadId);
  const { hasPermission } = useSquadPermissions(squadId, userId);
  const startSession = useStartSquadSession(userId, squadId);
  ...
  const onlineMembers = activeMembers.filter(m => m.isOnline);
    ...
    >
      ...
      <Card style={styles.headerCard}>
        <View style={styles.headerRow}>
          {squad.avatarUrl ? (
            <Image source={{ uri: squad.avatarUrl }} resizeMode="cover" style={styles.avatar} />
          ) : (
      ...
      <Card style={styles.card}>
        ...
        <Text style={[styles.progressText, { color: theme.colors.text.secondary }]}>
          {squad.synergyLevel * 100} points to next level
        </Text>
        {squad.focusMultiplier > 1 && (
          <Text style={[styles.bonusText, { color: theme.colors.success.DEFAULT }]}>
            +{Math.round((squad.focusMultiplier - 1) * 100)}% bonus multiplier active
          </Text>
        )}
      </Card>
      ...
      <Card style={styles.card}>
        ...
        <View style={styles.actionRow}>
          <Button onPress={handleStartSession} isDisabled={isStartingSession} style={styles.actionButton} variant="primary"
  ...
  accessibilityHint="Activates this control">
          ...
          {hasPermission('INVITE_MEMBERS') && (
            <Button variant="secondary" onPress={onInvitePress} style={styles.actionButton}
  accessibilityLabel="Invite Member button"
  accessibilityRole="button"
  accessibilityHint="Activates this control">
              Invite Member
            </Button>
          ...
          {hasPermission('EDIT_SQUAD') && (
            <Button variant="ghost" onPress={onSettingsPress} style={styles.actionButton}
      ...
      {onlineMembers.length > 0 && (
        <Card style={styles.card}>
          ...
          <FlashList
            data={onlineMembers}
            horizontal
            keyExtractor={(item: SquadMemberDetail) => item.userId}
            renderItem={({ item }: { item: SquadMemberDetail }) => (
              <Pressable
                style={({ pressed }) => [
                  styles.onlineMemberItem,
                  pressed && { opacity: 0.7 },
                ]}
            ...
            contentContainerStyle={styles.onlineMembersList}
          />
        </Card>
      )}
        ...
        <View style={styles.statsGrid}>
          <View style={styles.statItem}>
            <Text style={[styles.statValue, { color: theme.colors.text.primary }]}>
              {Math.round(squad.totalFocusTime / 3600)}h
            </Text>
          ...
          <View style={styles.statItem}>
            <Text style={[styles.statValue, { color: theme.colors.text.primary }]}>
              {squad.focusMultiplier.toFixed(2)}x
            </Text>
            <Text style={[styles.statLabel, { color: theme.colors.text.secondary }]}>
              Multiplier
            </Text>
