/**
 * SquadLiveDashboard Component
 *
 * Shows all squad members with their session status today.
 * ✓ done / ⏳ not yet / 🔥 focusing now
 * Live timer for active sessions.
 * Squad streak counter at top.
...
export interface SquadLiveDashboardProps {
  squadName: string;
  streakDays: number;
  members: SquadMember[];
  onFocusTogether: () => void;
  onNudgeMember?: (userId: string) => void;
...
function SquadMemberCard({
  ...
  return (
    <Pressable onPress={() => onView?.(member.userId)}
  ...
  accessibilityHint="Activates this control">
      <Box
        ...
        p="md"
        borderRadius="xl"
        bg={member.isMe ? `${theme.colors.primary[500]}15` : 'background.secondary'}
        borderWidth={1}
        borderColor={member.isMe ? 'primary.500' : 'border.light'}
        mb="sm"
      >
        <Box flexDirection="row" alignItems="center" gap="md">
          ...
          <Box
            ...
            height={48}
            borderRadius="full"
            bg={member.isMe ? 'primary.500' : 'background.tertiary'}
          ...
          >
            {member.avatarUrl ? (
              ...
              <Text
                fontSize={18}
                color={member.isMe ? 'text.inverse' : 'text.tertiary'}
                fontWeight="600"
              >
                {initial}
              </Text>
        ...
        {/* Nudge button */}
        {canNudge && (
          <Pressable onPress={() => onNudge?.(member.userId)}
  accessibilityLabel="Nudge ⚡ button"
  accessibilityRole="button"
  accessibilityHint="Activates this control">
            <Box
              px="md"
...
/**
 * Squad live dashboard
 */
export function SquadLiveDashboard({
  squadName,
  streakDays,
  members,
  onFocusTogether,
  onNudgeMember,
  onViewMember,
}: SquadLiveDashboardProps): JSX.Element {
  const { theme } = useTheme();
  ...
  return (
    <Box flex={1} bg="background.primary">
      ...
      >
        ...
        <Box flexDirection="row" justifyContent="center" gap="lg" mb="lg">
          <Box alignItems="center">
            ...
            <Text variant="caption" color="text.tertiary">Done today</Text>
          </Box>
          <Box alignItems="center">
            <Text variant="h3" color="accent.orange" fontWeight="700">
              {focusingCount}
            </Text>
            <Text variant="caption" color="text.tertiary">Focusing now</Text>
          </Box>
          <Box alignItems="center">
            <Text variant="h3" color="text.tertiary" fontWeight="700">
              {members.length - doneCount - focusingCount}
            </Text>
            <Text variant="caption" color="text.tertiary">Not yet</Text>
          </Box>
        </Box>
      ...
      >
        <Button variant="primary" size="lg" fullWidth onPress={onFocusTogether}
  accessibilityLabel="🔥 Focus Together button"
  accessibilityRole="button"
  accessibilityHint="Activates this control">
          🔥 Focus Together
        </Button>
...
export default SquadLiveDashboard;
