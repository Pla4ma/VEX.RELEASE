...
function getHealthColor(healthRatio: number): string {
  if (healthRatio > 0.6) {return '#22C55E';}
  if (healthRatio >= 0.3) {return '#F59E0B';}
  return '#EF4444';
}

function SquadWarMemberRow({
  member,
  currentUserId,
  now,
  textColor,
  ...
  const statusLabel = member.isCurrentlyFocusing && member.sessionStartedAt
    ? formatFocusingDuration(member.sessionStartedAt, now)
    ...
    <Animated.View style={[styles.memberRow, rowStyle]}>
      <View style={styles.memberIdentity}>
        <View
          style={[
            styles.memberDot,
            { backgroundColor: member.isCurrentlyFocusing ? '#22C55E' : '#6B7280' },
          ]}
        />
...
export function SquadWarHUD({ squadId }: SquadWarHUDProps) {
  ...
  const healthRatio = useMemo(() => {
    if (!war || war.bossMaxHealth <= 0) {return 0;}
  ...
  useEffect(() => {
    ...
    const unsubscribe = watchActiveSquadWar(
      squadId,
      (nextWar) => {
        if (!isMounted) {return;}
        setWar(nextWar);
  ...
  useEffect(() => {
    ...
    if (
      ...
      war.bossCurrentHealth < previousHealthRef.current
    ) {
  ...
  if (!war) {
    return (
      <Card variant="elevated" size="lg" style={styles.card}>
        <EmptyState
          icon="⚔️"
          title="No active Squad War this week"
          subtitle="Check back after the weekly reset to jump into the next boss race."
        />
      </Card>
    );
  }
    ...
    <Card variant="elevated" size="lg" style={styles.card}>
      ...
      <View style={styles.healthSection}>
        ...
        >
          ...
          <Animated.View
            pointerEvents="none"
            style={[styles.healthFlash, healthFlashStyle]}
          />
        </View>
        <Text variant="caption" style={{ color: theme.colors.text.secondary }}>
      ...
      <View style={styles.memberList}>
        {war.members.map((member) => (
          <SquadWarMemberRow
            ...
            mutedColor={theme.colors.text.secondary}
          />
        ))}
      </View>
