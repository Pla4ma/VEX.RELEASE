...
import {
  ...
  ELEMENTS,
  SQUAD_TYPES,
  toCreatedSquadSummary,
  type ElementId,
  type SquadType,
} from './createSquadFlowData';
...
export function CreateSquadFlow({
  bottomSheetRef,
  userId,
}: CreateSquadFlowProps): JSX.Element {
      ...
      <View style={{ flex: 1, padding: theme.spacing[5], gap: theme.spacing[4] }}>
        ...
        {step === 1 ? (
          <View style={{ gap: theme.spacing[4] }}>
            <Input
              ...
              placeholder="Night Shift Legends"
            ...
            <View style={{ gap: theme.spacing[2] }}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing[2] }}>
                {ELEMENTS.map((element) => (
                  <Button
                    key={element.id}
                    variant={elementId === element.id ? 'primary' : 'secondary'}
            ...
            <View style={{ alignItems: 'center' }}>
              <SquadShareCard
                squad={previewSquad}
                weeklyStats={{ totalSessions: 0, totalFocusMinutes: 0, activeMemberCount: 0 }}
              />
            </View>
          </View>
        ) : null}
          ...
          <View style={{ gap: theme.spacing[4] }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing[2] }}>
              {SQUAD_TYPES.map((type) => (
                <Button
                  key={type}
                  variant={squadType === type ? 'primary' : 'secondary'}
                  onPress={() => setSquadType(type)}
        ...
        {step === 3 && createdSquad ? (
          <View style={{ gap: theme.spacing[4] }}>
            ...
            <Button onPress={() => void handleShare()}
  accessibilityLabel="Share to recruit members button"
  accessibilityRole="button"
  accessibilityHint="Activates this control">Share to recruit members</Button>
          </View>
        ) : null}
        ...
        <View style={{ flexDirection: 'row', gap: theme.spacing[3] }}>
          {step < 3 ? (
            <Button
              variant="ghost"
              onPress={step === 1 ? () => bottomSheetRef.current?.close() : () => setStep(1)}
          ...
          <Button
            onPress={step < 3 ? () => void handleContinue() : () => bottomSheetRef.current?.close()}
