...
function RankBadge({ rank }: { rank: number }): JSX.Element {
  ...
  const getRankStyle = () => {
    if (rank === 1) {return { bg: '#F59E0B', text: '#FFFFFF' };} // Gold
    if (rank === 2) {return { bg: '#6B7280', text: '#FFFFFF' };} // Silver
  ...
  return (
    <Animated.View entering={FadeInUp.duration(400).delay(index * 50)}>
      <Box
        ...
        p="md"
        borderRadius="xl"
        bg={entry.isMe ? `${theme.colors.primary[500]}15` : 'background.secondary'}
        borderWidth={2}
        borderColor={entry.isMe ? 'primary.500' : isTop3 ? 'accent.orange' : 'transparent'}
        mb="sm"
      >
        {/* Rank */}
 ...
 */
export function SquadWeeklyBoard({
  entries,
  squadName,
  weekNumber,
}: SquadWeeklyBoardProps): JSX.Element {
  const { theme } = useTheme();
