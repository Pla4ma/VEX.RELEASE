...
export function joinSquad(
  ...
  userName: string,
  joinCode?: string
): { success: boolean; squad?: SimplifiedSquad; error?: string } {
...
export function recordMemberActivity(
  ...
  squad.weeklyGoal.achieved =
    squad.weeklyGoal.currentTotalHours >= squad.weeklyGoal.targetFocusHours;
...
export function resetWeeklyProgress(squadId: string): SimplifiedSquad {
  ...
  for (const member of squad.members) {
    ...
    member.isActive = member.lastSessionAt
      ? now - member.lastSessionAt < 7 * 24 * 60 * 60 * 1000
      : false;
  }
 ...
 */
export function recordBossDamageActivity(
  squadId: string,
  userId: string,
  damage: number
): void {
 ...
 */
export function startSquadBoss(
  squadId: string,
  bossId: string,
  bossName: string,
  baseHealth: number
): { success: boolean; encounter?: SquadBossEncounter; error?: string } {
  const squad = squads.get(squadId);
  if (!squad) {
    return { success: false, error: 'Squad not found' };
  }
 ...
 */
export function damageSquadBoss(
  squadId: string,
  userId: string,
  damage: number
): { success: boolean; isDefeated: boolean; encounter?: SquadBossEncounter } {
  const squad = squads.get(squadId);
 ...
 */
export function discoverPublicSquads(
  limit: number = 20,
...
export function getUserSquads(userId: string): SimplifiedSquad[] {
  return Array.from(squads.values()).filter((s) =>
...
export function getSquadSummary(squadId: string, userId: string): SquadSummary | null {
  ...
  const member = squad.members.find((m) => m.userId === userId);
  const progress =
    squad.weeklyGoal.targetFocusHours > 0
      ? Math.min(
          100,
          (squad.weeklyGoal.currentTotalHours / squad.weeklyGoal.targetFocusHours) * 100
        )
      : 0;
