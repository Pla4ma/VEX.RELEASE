...
export const SquadAnalytics = {
  ...
  sessionCompleted: (
    ...
    participantResults: Array<{ userId: string; focusTime: number; completed: boolean }>
  ) => {
    eventBus.publish('analytics:track', {
...
function calculateEngagementScore(metrics: {
  ...
  return Math.round(
    (dailyActivity * 0.3 + weeklyActivity * 0.2 + sessionQuality * 0.2 + contributionRate * 0.15 + synergyBonus * 0.15) * 100
  );
}
