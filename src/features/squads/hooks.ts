...
export function useUserSquads(
  userId: string | undefined,
  options?: Omit<
    UseQueryOptions<SquadSummary[], Error, SquadSummary[], ReturnType<typeof SQUAD_QUERY_KEYS.list>>,
    'queryKey' | 'queryFn'
  >
) {
  return useQuery({
...
export function useInviteToSquad(invitedBy: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: Omit<InviteToSquadInput, 'squadId'> & { squadId: string }) =>
      service.inviteToSquad({ ...input, invitedBy }),
...
export function useKickMember(adminUserId: string, squadId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: Omit<KickMemberInput, 'squadId'>) =>
      service.kickMember(adminUserId, { ...input, squadId }),
    onSuccess: () => {
...
export function useUpdateMemberRole(adminUserId: string, squadId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: Omit<UpdateMemberRoleInput, 'squadId'>) =>
      service.updateMemberRole(adminUserId, { ...input, squadId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: SQUAD_QUERY_KEYS.members(squadId) });
    },
  });
...
export function useRequestToJoinSquad(userId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: { squadId: string; message?: string }) =>
      service.requestToJoinSquad(userId, input.squadId, input.message),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: SQUAD_QUERY_KEYS.detail(variables.squadId) });
    },
  });
...
export function useStartSquadSession(userId: string, squadId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: Omit<StartSquadSessionInput, 'squadId'>) =>
      service.startSquadSession(userId, { ...input, squadId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: SQUAD_QUERY_KEYS.detail(squadId) });
    },
  });
...
export function useSquadPermissions(squadId: string | undefined, userId: string | undefined) {
  ...
  return {
    ...
    hasPermission: (permission: import('./schemas').SquadPermission) =>
      member ? service.hasPermission(member, permission) : false,
  ...
  useEffect(() => {
    ...
    const fetchActiveCount = async () => {
      try {
        ...
        debug.error(
          'Failed to fetch squad active count',
    ...
    const channel = repository.subscribeToSquadSessions(squadId, (activeCount) => {
      if (!isCancelled) {
