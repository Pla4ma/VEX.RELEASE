/**
 * Help Request System
 *
 * Phase 3B: Social Systems - "Send Help" during difficult sessions
 *
 * Allows users to request help from squad members during difficult sessions.
 * Creates social obligation and support network for knowledge workers.
 *
 * Features:
 * - Send help requests during sessions (struggling, stuck, need motivation)
 * - Squad members can respond with encouragement, tips, or join session
 * - Help credits system to prevent abuse
 * - Help history and reputation tracking
 *
 * Dependencies:
 * - features/squads/types (Squad, SquadMember)
 * - feature-flags (gradual rollout)
 * - events (eventBus for help notifications)
 * - notifications (send help alerts to squad members)
 */

import { z } from 'zod';
import { featureFlags } from '../../feature-flags/FeatureFlagEngine';
import { eventBus } from '../../events';
import type { Squad, SquadMember } from './types';

// ============================================================================
// Help Request Constants
// ============================================================================

export const HELP_CONFIG = {
  // Help request types
  HELP_TYPES: {
    STRUGGLING: 'struggling',        // Having trouble focusing
    STUCK: 'stuck',                // Stuck on problem/task
    MOTIVATION: 'motivation',      // Need motivation boost
    BREAKTHROUGH: 'breakthrough',  // Want to share breakthrough
    ACCOUNTABILITY: 'accountability', // Need someone to keep them accountable
  },

  // Help credits (anti-abuse system)
  DAILY_HELP_CREDITS: 3,
  CREDIT_RESTORE_HOURS: 24,
  HELP_COST_CREDITS: 1,

  // Response types
  RESPONSE_TYPES: {
    ENCOURAGEMENT: 'encouragement',   // "You can do it!" messages
    TIP: 'tip',                      // Practical advice
    JOIN_SESSION: 'join_session',     // Offer to join parallel session
    CHECK_IN: 'check_in',             // "How's it going?" follow-up
  },

  // Time limits
  HELP_REQUEST_DURATION_MINUTES: 120, // 2 hours
  RESPONSE_TIMEOUT_MINUTES: 30,       // 30 minutes to respond

  // Reputation
  HELP_GIVEN_REP: 5,     // Points for giving help
  HELP_RECEIVED_REP: 1,  // Points for receiving help
  REP_DECAY_HOURS: 168, // 7 days decay
} as const;

// ============================================================================
// Types & Schemas
// ============================================================================

export const HelpRequestSchema = z.object({
  id: z.string(),
  squadId: z.string(),
  requesterId: z.string(),
  sessionId: z.string().optional(), // Optional: link to active session
  
  // Request details
  helpType: z.enum(['struggling', 'stuck', 'motivation', 'breakthrough', 'accountability']),
  message: z.string().min(1).max(500),
  urgency: z.enum(['low', 'medium', 'high']).default('medium'),
  
  // Status
  status: z.enum(['pending', 'active', 'resolved', 'expired']).default('pending'),
  
  // Timing
  createdAt: z.number(),
  expiresAt: z.number(),
  resolvedAt: z.number().nullable().default(null),
  
  // Response tracking
  responseCount: z.number().default(0),
  helpfulResponseCount: z.number().default(0),
  
  // Metadata
  tags: z.array(z.string()).default([]),
  context: z.record(z.unknown()).optional(),
});

export const HelpResponseSchema = z.object({
  id: z.string(),
  helpRequestId: z.string(),
  responderId: z.string(),
  
  // Response details
  responseType: z.enum(['encouragement', 'tip', 'join_session', 'check_in']),
  message: z.string().min(1).max(1000),
  
  // Status
  status: z.enum(['sent', 'viewed', 'helpful', 'not_helpful']).default('sent'),
  
  // Timing
  createdAt: z.number(),
  viewedAt: z.number().nullable().default(null),
  
  // Metadata
  isAnonymous: z.boolean().default(false),
  metadata: z.record(z.unknown()).optional(),
});

export const HelpCreditsSchema = z.object({
  userId: z.string(),
  squadId: z.string(),
  creditsRemaining: z.number().min(0).max(10).default(3),
  lastRestoreAt: z.number().default(Date.now()),
  totalHelpGiven: z.number().default(0),
  totalHelpReceived: z.number().default(0),
});

export const HelpReputationSchema = z.object({
  userId: z.string(),
  squadId: z.string(),
  reputationScore: z.number().default(0),
  helpGivenCount: z.number().default(0),
  helpReceivedCount: z.number().default(0),
  averageResponseTime: z.number().default(0), // minutes
  helpfulnessRating: z.number().min(0).max(5).default(0),
  lastActiveAt: z.number().default(Date.now()),
});

export type HelpRequest = z.infer<typeof HelpRequestSchema>;
export type HelpResponse = z.infer<typeof HelpResponseSchema>;
export type HelpCredits = z.infer<typeof HelpCreditsSchema>;
export type HelpReputation = z.infer<typeof HelpReputationSchema>;

export interface HelpStats {
  pendingRequests: number;
  activeRequests: number;
  resolvedToday: number;
  averageResponseTime: number;
  topHelpers: Array<{
    userId: string;
    name: string;
    helpGiven: number;
    reputation: number;
  }>;
}

// ============================================================================
// Help Request Service
// ============================================================================

export class HelpRequestService {
  private helpRequests: Map<string, HelpRequest> = new Map();
  private helpResponses: Map<string, HelpResponse[]> = new Map();
  private helpCredits: Map<string, HelpCredits> = new Map(); // userId_squadId -> credits
  private helpReputation: Map<string, HelpReputation> = new Map(); // userId_squadId -> reputation

  /**
   * Check if help request system is enabled
   */
  static isEnabled(): boolean {
    return featureFlags.isEnabled('help_request_system');
  }

  /**
   * Create a new help request
   */
  async createHelpRequest(input: {
    squadId: string;
    requesterId: string;
    sessionId?: string;
    helpType: typeof HELP_CONFIG.HELP_TYPES[keyof typeof HELP_CONFIG.HELP_TYPES];
    message: string;
    urgency?: 'low' | 'medium' | 'high';
    tags?: string[];
  }): Promise<{ success: boolean; request?: HelpRequest; error?: string }> {
    // Check feature flag
    if (!HelpRequestService.isEnabled()) {
      return { success: false, error: 'Help request system not enabled' };
    }

    // Check user credits
    const creditsKey = `${input.requesterId}_${input.squadId}`;
    const credits = this.helpCredits.get(creditsKey) || {
      userId: input.requesterId,
      squadId: input.squadId,
      creditsRemaining: HELP_CONFIG.DAILY_HELP_CREDITS,
      lastRestoreAt: Date.now(),
      totalHelpGiven: 0,
      totalHelpReceived: 0,
    };

    // Restore credits if needed
    this.restoreCreditsIfNeeded(credits);
    this.helpCredits.set(creditsKey, credits);

    if (credits.creditsRemaining < HELP_CONFIG.HELP_COST_CREDITS) {
      return { success: false, error: 'Insufficient help credits' };
    }

    // Create help request
    const now = Date.now();
    const helpRequest: HelpRequest = {
      id: `help_${now}_${Math.random().toString(36).substr(2, 9)}`,
      squadId: input.squadId,
      requesterId: input.requesterId,
      sessionId: input.sessionId,
      helpType: input.helpType,
      message: input.message,
      urgency: input.urgency || 'medium',
      status: 'pending',
      createdAt: now,
      expiresAt: now + (HELP_CONFIG.HELP_REQUEST_DURATION_MINUTES * 60 * 1000),
      resolvedAt: null,
      responseCount: 0,
      helpfulResponseCount: 0,
      tags: input.tags || [],
    };

    // Deduct credits
    credits.creditsRemaining -= HELP_CONFIG.HELP_COST_CREDITS;
    credits.totalHelpReceived += 1;
    this.helpCredits.set(creditsKey, credits);

    // Store request
    this.helpRequests.set(helpRequest.id, helpRequest);
    this.helpResponses.set(helpRequest.id, []);

    // Update reputation
    this.updateReputation(input.requesterId, input.squadId, 'received');

    // Emit events
    eventBus.publish('help:request_created', {
      helpRequestId: helpRequest.id,
      squadId: input.squadId,
      requesterId: input.requesterId,
      helpType: input.helpType,
      urgency: input.urgency || 'medium',
    });

    // Send notifications to squad members
    await this.notifySquadMembers(helpRequest);

    return { success: true, request: helpRequest };
  }

  /**
   * Respond to a help request
   */
  async respondToHelpRequest(input: {
    helpRequestId: string;
    responderId: string;
    responseType: typeof HELP_CONFIG.RESPONSE_TYPES[keyof typeof HELP_CONFIG.RESPONSE_TYPES];
    message: string;
    isAnonymous?: boolean;
  }): Promise<{ success: boolean; response?: HelpResponse; error?: string }> {
    // Check feature flag
    if (!HelpRequestService.isEnabled()) {
      return { success: false, error: 'Help request system not enabled' };
    }

    const helpRequest = this.helpRequests.get(input.helpRequestId);
    if (!helpRequest) {
      return { success: false, error: 'Help request not found' };
    }

    // Check if request is still active
    if (helpRequest.status !== 'pending' && helpRequest.status !== 'active') {
      return { success: false, error: 'Help request no longer active' };
    }

    // Check if expired
    if (Date.now() > helpRequest.expiresAt) {
      helpRequest.status = 'expired';
      return { success: false, error: 'Help request expired' };
    }

    // Check if responder is in the same squad
    // (This would typically involve checking squad membership)
    // For now, we'll assume the check passes

    // Create response
    const now = Date.now();
    const helpResponse: HelpResponse = {
      id: `resp_${now}_${Math.random().toString(36).substr(2, 9)}`,
      helpRequestId: input.helpRequestId,
      responderId: input.responderId,
      responseType: input.responseType,
      message: input.message,
      status: 'sent',
      createdAt: now,
      viewedAt: null,
      isAnonymous: input.isAnonymous || false,
    };

    // Store response
    const responses = this.helpResponses.get(input.helpRequestId) || [];
    responses.push(helpResponse);
    this.helpResponses.set(input.helpRequestId, responses);

    // Update request
    helpRequest.responseCount += 1;
    helpRequest.status = 'active';
    this.helpRequests.set(input.helpRequestId, helpRequest);

    // Update responder credits and reputation
    const creditsKey = `${input.responderId}_${helpRequest.squadId}`;
    const credits = this.helpCredits.get(creditsKey) || {
      userId: input.responderId,
      squadId: helpRequest.squadId,
      creditsRemaining: HELP_CONFIG.DAILY_HELP_CREDITS,
      lastRestoreAt: Date.now(),
      totalHelpGiven: 0,
      totalHelpReceived: 0,
    };
    credits.totalHelpGiven += 1;
    this.helpCredits.set(creditsKey, credits);

    this.updateReputation(input.responderId, helpRequest.squadId, 'given');

    // Emit events
    eventBus.publish('help:response_created', {
      helpRequestId: input.helpRequestId,
      responseId: helpResponse.id,
      responderId: input.responderId,
      responseType: input.responseType,
    });

    // Notify requester
    await this.notifyRequester(helpRequest, helpResponse);

    return { success: true, response: helpResponse };
  }

  /**
   * Mark a response as helpful/not helpful
   */
  async rateResponse(input: {
    responseId: string;
    raterId: string;
    isHelpful: boolean;
  }): Promise<{ success: boolean; error?: string }> {
    const responses = Array.from(this.helpResponses.values()).flat();
    const response = responses.find(r => r.id === input.responseId);
    
    if (!response) {
      return { success: false, error: 'Response not found' };
    }

    const helpRequest = this.helpRequests.get(response.helpRequestId);
    if (!helpRequest || helpRequest.requesterId !== input.raterId) {
      return { success: false, error: 'Only requester can rate responses' };
    }

    // Update response status
    response.status = input.isHelpful ? 'helpful' : 'not_helpful';
    response.viewedAt = Date.now();

    // Update request helpful count
    if (input.isHelpful) {
      helpRequest.helpfulResponseCount += 1;
      helpRequest.status = 'resolved';
      helpRequest.resolvedAt = Date.now();
    }

    // Update responder reputation
    if (input.isHelpful) {
      this.updateReputation(response.responderId, helpRequest.squadId, 'helpful');
    }

    // Emit event
    eventBus.publish('help:response_rated', {
      responseId: input.responseId,
      isHelpful: input.isHelpful,
      raterId: input.raterId,
    });

    return { success: true };
  }

  /**
   * Get help requests for a squad
   */
  getSquadHelpRequests(squadId: string, status?: HelpRequest['status']): HelpRequest[] {
    const requests = Array.from(this.helpRequests.values())
      .filter(req => req.squadId === squadId);
    
    if (status) {
      return requests.filter(req => req.status === status);
    }
    
    return requests.sort((a, b) => b.createdAt - a.createdAt);
  }

  /**
   * Get help requests for a user
   */
  getUserHelpRequests(userId: string): { requested: HelpRequest[]; responded: HelpResponse[] } {
    const requested = Array.from(this.helpRequests.values())
      .filter(req => req.requesterId === userId)
      .sort((a, b) => b.createdAt - a.createdAt);

    const responded = Array.from(this.helpResponses.values())
      .flat()
      .filter(resp => resp.responderId === userId)
      .sort((a, b) => b.createdAt - a.createdAt);

    return { requested, responded };
  }

  /**
   * Get help credits for a user in a squad
   */
  getHelpCredits(userId: string, squadId: string): HelpCredits {
    const creditsKey = `${userId}_${squadId}`;
    let credits = this.helpCredits.get(creditsKey);
    
    if (!credits) {
      credits = {
        userId,
        squadId,
        creditsRemaining: HELP_CONFIG.DAILY_HELP_CREDITS,
        lastRestoreAt: Date.now(),
        totalHelpGiven: 0,
        totalHelpReceived: 0,
      };
      this.helpCredits.set(creditsKey, credits);
    }

    // Restore credits if needed
    this.restoreCreditsIfNeeded(credits);
    this.helpCredits.set(creditsKey, credits);

    return { ...credits };
  }

  /**
   * Get help reputation for a user in a squad
   */
  getHelpReputation(userId: string, squadId: string): HelpReputation {
    const repKey = `${userId}_${squadId}`;
    let reputation = this.helpReputation.get(repKey);
    
    if (!reputation) {
      reputation = {
        userId,
        squadId,
        reputationScore: 0,
        helpGivenCount: 0,
        helpReceivedCount: 0,
        averageResponseTime: 0,
        helpfulnessRating: 0,
        lastActiveAt: Date.now(),
      };
      this.helpReputation.set(repKey, reputation);
    }

    return { ...reputation };
  }

  /**
   * Get help statistics for a squad
   */
  getSquadHelpStats(squadId: string): HelpStats {
    const requests = this.getSquadHelpRequests(squadId);
    const now = Date.now();
    const todayStart = new Date().setHours(0, 0, 0, 0);

    const pendingRequests = requests.filter(req => req.status === 'pending').length;
    const activeRequests = requests.filter(req => req.status === 'active').length;
    const resolvedToday = requests.filter(req => 
      req.status === 'resolved' && 
      req.resolvedAt && 
      req.resolvedAt >= todayStart
    ).length;

    // Calculate average response time
    const allResponses = Array.from(this.helpResponses.values()).flat();
    const squadResponses = allResponses.filter(resp => {
      const req = this.helpRequests.get(resp.helpRequestId);
      return req?.squadId === squadId;
    });

    const responseTimes = squadResponses.map(resp => {
      const req = this.helpRequests.get(resp.helpRequestId);
      return req ? (resp.createdAt - req.createdAt) / (1000 * 60) : 0; // minutes
    });

    const averageResponseTime = responseTimes.length > 0 
      ? responseTimes.reduce((sum, time) => sum + time, 0) / responseTimes.length 
      : 0;

    // Get top helpers
    const helperStats = new Map<string, { name: string; helpGiven: number; reputation: number }>();
    
    squadResponses.forEach(resp => {
      const existing = helperStats.get(resp.responderId) || {
        name: resp.responderId, // Would normally fetch user name
        helpGiven: 0,
        reputation: 0,
      };
      existing.helpGiven += 1;
      existing.reputation = this.getHelpReputation(resp.responderId, squadId).reputationScore;
      helperStats.set(resp.responderId, existing);
    });

    const topHelpers = Array.from(helperStats.entries())
      .map(([userId, stats]) => ({ userId, ...stats }))
      .sort((a, b) => b.helpGiven - a.helpGiven)
      .slice(0, 5);

    return {
      pendingRequests,
      activeRequests,
      resolvedToday,
      averageResponseTime,
      topHelpers,
    };
  }

  // ============================================================================
  // Private Helper Methods
  // ============================================================================

  /**
   * Restore credits if enough time has passed
   */
  private restoreCreditsIfNeeded(credits: HelpCredits): void {
    const now = Date.now();
    const hoursSinceRestore = (now - credits.lastRestoreAt) / (1000 * 60 * 60);

    if (hoursSinceRestore >= HELP_CONFIG.CREDIT_RESTORE_HOURS) {
      credits.creditsRemaining = HELP_CONFIG.DAILY_HELP_CREDITS;
      credits.lastRestoreAt = now;
    }
  }

  /**
   * Update user reputation
   */
  private updateReputation(userId: string, squadId: string, action: 'given' | 'received' | 'helpful'): void {
    const repKey = `${userId}_${squadId}`;
    let reputation = this.helpReputation.get(repKey);
    
    if (!reputation) {
      reputation = {
        userId,
        squadId,
        reputationScore: 0,
        helpGivenCount: 0,
        helpReceivedCount: 0,
        averageResponseTime: 0,
        helpfulnessRating: 0,
        lastActiveAt: Date.now(),
      };
      this.helpReputation.set(repKey, reputation);
    }

    reputation.lastActiveAt = Date.now();

    switch (action) {
      case 'given':
        reputation.helpGivenCount += 1;
        reputation.reputationScore += HELP_CONFIG.HELP_GIVEN_REP;
        break;
      case 'received':
        reputation.helpReceivedCount += 1;
        reputation.reputationScore += HELP_CONFIG.HELP_RECEIVED_REP;
        break;
      case 'helpful':
        reputation.reputationScore += HELP_CONFIG.HELP_GIVEN_REP * 2; // Bonus for helpful responses
        reputation.helpfulnessRating = Math.min(5, reputation.helpfulnessRating + 0.1);
        break;
    }

    // Apply decay (simplified - would normally be more sophisticated)
    const daysSinceActive = (Date.now() - reputation.lastActiveAt) / (1000 * 60 * 60 * 24);
    if (daysSinceActive > HELP_CONFIG.REP_DECAY_HOURS / 24) {
      reputation.reputationScore = Math.max(0, reputation.reputationScore * 0.9);
    }
  }

  /**
   * Notify squad members about new help request
   */
  private async notifySquadMembers(helpRequest: HelpRequest): Promise<void> {
    // This would typically fetch squad members and send notifications
    eventBus.publish('notification:send', {
      type: 'HELP_REQUESTED',
      squadId: helpRequest.squadId,
      excludeUserId: helpRequest.requesterId,
      title: 'Help Requested! 🆘',
      body: `${helpRequest.requesterId} needs help with ${helpRequest.helpType}`,
      data: {
        helpRequestId: helpRequest.id,
        helpType: helpRequest.helpType,
        urgency: helpRequest.urgency,
      },
      priority: helpRequest.urgency === 'high' ? 'high' : 'normal',
    });
  }

  /**
   * Notify requester about new response
   */
  private async notifyRequester(helpRequest: HelpRequest, response: HelpResponse): Promise<void> {
    eventBus.publish('notification:send', {
      userId: helpRequest.requesterId,
      type: 'HELP_RESPONSE',
      title: 'Help Response Received! 💬',
      body: `Someone responded to your help request`,
      data: {
        helpRequestId: helpRequest.id,
        responseId: response.id,
        responseType: response.responseType,
      },
    });
  }
}

// ============================================================================
// Factory & Exports
// ============================================================================

export function createHelpRequestService(): HelpRequestService {
  return new HelpRequestService();
}

// Singleton instance
let helpRequestService: HelpRequestService | null = null;

export function getHelpRequestService(): HelpRequestService {
  if (!helpRequestService) {
    helpRequestService = new HelpRequestService();
  }
  return helpRequestService;
}
