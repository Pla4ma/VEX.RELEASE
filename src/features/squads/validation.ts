...
export const SquadValidationRules = {
  createSquad: (input: CreateSquadInput) => {
    const extendedSchema = CreateSquadInputSchema.extend({
      name: z
        ...
        .regex(
          /^[a-zA-Z0-9\s-_]+$/,
          'Squad name can only contain letters, numbers, spaces, hyphens and underscores'
        )
        .trim(),
      ...
      maxMembers: z
        ...
        .max(20, 'Squad cannot exceed 20 members')
        .default(5),
      minLevelRequirement: z
        .number()
        .int()
        .min(1, 'Minimum level must be at least 1')
        .max(100, 'Minimum level cannot exceed 100')
        .nullable()
        .optional(),
    });
  ...
  updateSquad: (
    ...
    if (typeof updates.avatarUrl === 'string' && updates.avatarUrl.length > 0) {
      ...
      } catch (error) { captureSilentFailure(error, { feature: 'squads', operation: 'safe-fallback', type: 'data' });
  ...
  validateInvite: (
    input: InviteToSquadInput,
    context: {
      existingPendingInvite?: { createdAt: number } | null;
      inviterRole: SquadRole;
    ...
    if (
      ROLE_HIERARCHY.indexOf(context.offeredRole) > ROLE_HIERARCHY.indexOf(context.inviterRole)
    ) {
    ...
    if (context.existingPendingInvite) {
      const hoursSinceLastInvite =
        (Date.now() - context.existingPendingInvite.createdAt) / (1000 * 60 * 60);
  ...
  validateRoleTransfer: (
    ...
    if (targetIndex > currentIndex) {
      return {
        valid: false,
        error: `Cannot assign ${targetRole} role - higher than your ${currentRole} role`,
      };
    }
  ...
  validateSessionConfig: (config: {
    ...
    if (
      ...
      (config.maxParticipants < 2 || config.maxParticipants > 20)
    ) {
  ...
  validateContribution: (points: number, member: { role: string; isActive: boolean }): boolean => {
...
export const SquadBusinessRules = {
  shouldAutoArchive: (squad: Squad, members: Array<{ lastActiveAt: number }>): boolean => {
    const INACTIVE_THRESHOLD_DAYS = 30;
    const MINIMUM_MEMBERS_FOR_ACTIVE = 2;

    if (members.length < MINIMUM_MEMBERS_FOR_ACTIVE) {return true;}

    const now = Date.now();
    const activeMembers = members.filter(
      (member) =>
        now - member.lastActiveAt < INACTIVE_THRESHOLD_DAYS * 24 * 60 * 60 * 1000
    );

    return activeMembers.length < MINIMUM_MEMBERS_FOR_ACTIVE;
  },

  calculateOptimalSessionTime: (_memberTimezones: string[]): Date => {
    const now = new Date();
    const optimalHour = 14;
    const optimal = new Date(now);

    optimal.setUTCHours(optimalHour, 0, 0, 0);
  ...
  shouldAutoKick: (member: {
    joinedAt: number;
    lastContributionAt?: number | null;
    role: string;
  }): { shouldKick: boolean; reason?: string } => {
    const now = Date.now();
  ...
  isSquadNameValid: (name: string, existingNames: string[]): boolean => {
    const normalized = name.toLowerCase().trim();
    const normalizedExisting = existingNames.map((existing) => existing.toLowerCase().trim());
