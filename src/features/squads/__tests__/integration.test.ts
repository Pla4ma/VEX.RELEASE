/**
 * Squad Feature Integration Tests
 *
...
describe('Squad Integration Flows', () => {
  ...
  describe('Full Squad Lifecycle', () => {
    it('should handle complete squad creation to dissolution flow', async () => {
      ...
      vi.mocked(repository.fetchSquadMembers).mockResolvedValue([{ userId: founderId }, { userId: memberId1 }]);

      const joinedSquad = await service.respondToInvite(memberId1, {
        inviteId: invite.id,
        accept: true,
      });

      expect(joinedSquad).toBeTruthy();

      // Step 4: Promote member to admin
      ...
      await service.leaveSquad(founderId, {
        squadId,
        newFounderId: memberId1,
      });

      // Verify transfer happened
      expect(repository.transferFounderRole).toHaveBeenCalledWith(squadId, memberId1);
    });
  });
  ...
  describe('Concurrent Operations', () => {
    it('should handle concurrent invite acceptance race condition', async () => {
      ...
      vi.mocked(repository.fetchSquadById)
        .mockResolvedValueOnce({ ...squad, memberCount: 4 } as any)
        .mockResolvedValueOnce({ ...squad, memberCount: 5 } as any); // Second call sees full squad
      ...
      await expect(
        service.respondToInvite('user-b', { inviteId: 'invite-2', accept: true })
      ).rejects.toThrow();
    });
  });
  ...
  describe('Validation Edge Cases', () => {
    ...
    it('should reject profane squad names', () => {
      ...
      expect(profane.success).toBe(false);
    });
  });
  ...
  describe('Retry Logic', () => {
    ...
    it('should not retry non-retryable errors', async () => {
      ...
      await expect(
        withRetry(operation, 'test-operation', { maxAttempts: 3, baseDelayMs: 10 })
  ...
  describe('Synergy Calculations', () => {
    it('should calculate synergy progress correctly', async () => {
      ...
      vi.mocked(repository.fetchSquadById).mockResolvedValue({
        id: squadId,
        totalContribution: initialPoints,
        currentSynergyLevel: 1,
        pointsToNextSynergy: 100,
        focusMultiplier: 1.0,
        focusMultiplierBonus: 0.0,
      } as any);
      ...
      expect(repository.updateSquadStats).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          totalContribution: initialPoints + pointsToAdd,
        })
      );
    });
  });
  ...
  describe('Session Management', () => {
    it('should track session participation and rewards', async () => {
      ...
      vi.mocked(repository.updateSessionParticipant).mockResolvedValue({} as any);
      ...
      // Join session
      await service.joinSquadSession(userId, { sessionId, squadId });

      // Complete session
      await service.completeSquadSession(userId, {
        sessionId,
        focusTime: 1500,
        completed: true,
      });

      expect(repository.updateSessionParticipant).toHaveBeenCalled();
    });
  });
});
