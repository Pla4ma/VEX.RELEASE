...
import { FINAL_RELEASE_CLASSIFICATION, ClassificationStatusSchema, FeatureClassificationEntrySchema, ACTIVE, PROGRESSIVE, INTERNAL, ARCHIVED, LEGACY } from './final-release-classification';
import type { FeatureClassificationEntry, ClassificationStatus } from './final-release-classification';
...
const _archivedFolders: ReadonlySet<string> = new Set(
  [...ARCHIVED, ...LEGACY].filter((e) => e.status === 'archived_or_deactivated' || e.status === 'test_or_legacy').map((e) => e.folder)
);
...
export function isFolderActive(folder: string): boolean {
  return FINAL_RELEASE_CLASSIFICATION
    ...
    .some((e) => e.status === 'final_release_active' || e.status === 'final_release_progressive');
}

export function isFolderArchived(folder: string): boolean {
  const entries = FINAL_RELEASE_CLASSIFICATION.filter((e) => e.folder === folder);
  if (entries.length === 0) return false;
  return entries.every((e) => e.status === 'archived_or_deactivated' || e.status === 'test_or_legacy');
}
...
export function getEntryBySystemId(id: string): FeatureClassificationEntry | undefined {
  return FINAL_RELEASE_CLASSIFICATION.find((e) => e.systemId === id);
}

export function getEntriesByFolder(folder: string): FeatureClassificationEntry[] {
