...
describe("Final release classification", () => {
  ...
  it("keeps archived routes, prefetches, and notifications inert", () => {
    ...
    expect(calls).not.toContainEqual(
      expect.objectContaining({ queryKey: QueryKeys.USER.WALLET }),
    );
    expect(calls).not.toContainEqual(
  ...
  it("keeps progressive features inert until their unlock session count", () => {
    ...
    expect(getFeatureAvailability(featuresAt(5).challenges).canQuery).toBe(
      true,
    );
    expect(getFeatureAvailability(featuresAt(0).content_study).canQuery).toBe(
      false,
    );
    expect(getFeatureAvailability(featuresAt(12).content_study).canQuery).toBe(
      true,
    );
    expect(
      getFeatureAvailability(featuresAt(0).ai_coach_advanced).canQuery,
    ).toBe(false);
    expect(
      getFeatureAvailability(featuresAt(8).ai_coach_advanced).canQuery,
    ).toBe(true);
  });
});
