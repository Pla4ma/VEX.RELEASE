...
function collectPartFiles(dir: string): string[] {
  return readdirSync(dir).flatMap((entry) => {
    ...
    return /\.part-\d+\.ts$/.test(fullPath) ? [fullPath.replace(/\\/g, '/')] : [];
  });
}
...
describe('part file policy', () => {
  it('documents freeze and migration plan', () => {
    ...
    expect(policy).toContain('Do not add new files named');
  ...
  it('does not create part files outside known frozen features', () => {
    ...
    const unexpected = collectPartFiles('src').filter((file) => !allowed.some((prefix) => file.startsWith(prefix)));
