...
describe('keyed FeatureAvailability', () => {
  ...
  it('premium degraded is disabled only through keyed helper', () => {
    ...
    const premium = getFeatureAvailabilityFor(
      'premium_paywall',
      degradedFeature(),
    );
  ...
  it('content_study degraded blocks upload and advanced study surfaces', () => {
    ...
    expect(blocked).toEqual(
      expect.arrayContaining(['upload_cta', 'content_generation']),
    );
  });

  it('ai_coach_advanced degraded falls back to CoachPresence surface', () => {
    const coach = getFeatureAvailabilityFor(
      'ai_coach_advanced',
      degradedFeature(),
    );
    ...
    expect(getDegradedFallbackSurface('ai_coach_advanced')).toBe(
      'coach_presence',
    );
  });
});
