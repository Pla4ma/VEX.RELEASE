...
describe("Classification — archived features cannot notify", () => {
  ...
  it("SQUAD and RIVAL notification types blocked (archived in classification)", () => {
    ...
    expect(
      isNotificationTypeFilterable("SQUAD" as NotificationType, features),
    ).toBe(false);
    expect(
      isNotificationTypeFilterable("RIVAL" as NotificationType, features),
    ).toBe(false);
  });
  ...
  it("active notification types still available", () => {
    ...
    expect(
      isNotificationTypeFilterable("ACHIEVEMENT" as NotificationType, features),
    ).toBe(true);
    expect(
      isNotificationTypeFilterable("STREAK_RISK" as NotificationType, features),
    ).toBe(true);
  });
});
