...
function importSpecifiers(source: string): string[] {
  const matches = source.matchAll(/\b(?:import|export)\b[\s\S]*?\bfrom\s*['"]([^'"]+)['"]|require\(\s*['"]([^'"]+)['"]\s*\)/g);
...
describe('dead feature archive policy', () => {
  ...
  it('app root does not import spectacle or emotion retention', () => {
    const rootSources = [
      ...
      fs.readFileSync(path.join(srcRoot, 'app/providers/AppProviders.tsx'), 'utf8'),
    ].join('\n');
    expect(rootSources).not.toMatch(/spectacle|initializeEmotionRetention|emotion-retention/);
  });

  it('features index does not export archived systems', () => {
    const source = fs.readFileSync(path.join(srcRoot, 'features/index.ts'), 'utf8');
  ...
  it('home does not import daily weekly social or squads dead features', () => {
    const homeFiles = files.filter((file) => normalize(file).includes('/src/screens/home/'));
    expectNoImports(homeFiles, ['daily-mission', 'weekly-quests', 'features/social', 'features/squads']);
  });

  it('completion does not import chest battle pass shop or inventory', () => {
    const completionFiles = files.filter((file) => normalize(file).includes('/src/screens/session/'));
    expectNoImports(completionFiles, ['features/battle-pass', 'features/shop', 'features/inventory']);
    const source = completionFiles.map((file) => fs.readFileSync(file, 'utf8')).join('\n');
    expect(source).not.toMatch(/SessionChestCard|SessionPremiumChestCard|useSessionCompleteChest/);
  });

  it('route registry does not include dead routes', () => {
    const source = fs.readFileSync(path.join(srcRoot, 'navigation/feature-route-registry.ts'), 'utf8');
    const registry = source.slice(0, source.indexOf('const ARCHIVED_ROUTE_SET'));
    expect(registry).not.toMatch(/Guild|Shop|Inventory|Vault|PostSessionStory|BattlePass/);
  });
});
