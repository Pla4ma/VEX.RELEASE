/**
 * Shared fixtures for Phase 3 Lane Polish tests.
 */
...
export { getLaneMechanicPolicy, getLanePresentationPolicy, buildLaneSessionBrief, decideNudge };
export { resolveCompletionExperiencePolicy, resolveLaneCopy, LANE_USER_FACING_NAMES, decideHomeSurfaces };
export { SessionMode };
export type { Lane, LaneProfile, LaneMechanicPolicy, CompletionExperiencePolicy, CompletionExperiencePolicyInput };
...
export function sessionSummary(overrides: {
  sessionId?: string; userId?: string; sessionMode?: SessionModeString;
  plannedDuration?: number; effectiveDuration?: number; completionPercentage?: number;
  focusQuality?: number; finalScore?: number; streakMaintained?: boolean; createdAt?: number;
...
export function completionInput(overrides: {
  ...
  return {
    ...
    sessionMode: overrides.sessionMode ?? SessionMode.LIGHT_FOCUS,
    summary: sessionSummary({ sessionMode: overrides.sessionMode, ...(overrides.summary ?? {}) }),
  };
}
