...
import {
  getLaneMechanicPolicy, getLanePresentationPolicy,
  buildLaneSessionBrief, decideNudge,
  baseLaneProfile, auditLane,
} from './helpers';
...
describe('Phase 3A — Lane Pipeline Audit', () => {
  ...
  it('audit table: Clean notification budget is max 1/day', () => {
    const cleanNudge = decideNudge({
      lane: 'minimal_normal', completedSessions: 3,
      daysSinceOnboarding: 3, sentToday: 1,
    });
    expect(cleanNudge.allowed).toBe(false);
    expect(cleanNudge.budgetRemaining).toBe(0);

    const studyNudge = decideNudge({
      lane: 'student', completedSessions: 3,
      daysSinceOnboarding: 3, sentToday: 1,
    });
