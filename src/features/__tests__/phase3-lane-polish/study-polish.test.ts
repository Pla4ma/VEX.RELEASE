...
import { buildLaneSessionBrief, decideHomeSurfaces } from './helpers';
import { resolveCompletionExperiencePolicy, completionInput, SessionMode } from './helpers';
import { resolveLaneCopy, baseLaneProfile, baseStats, baseProfile, featureAvailability } from './helpers';
...
describe('Phase 3B — Study Mode Polish', () => {
  ...
  it('Study completion produces study_progress adaptive payoff', () => {
    const input = completionInput({
      lane: 'student', motivationStyle: 'study_focused',
  ...
  it('premium Study depth is hidden when study feature unavailable', () => {
    const input = completionInput({
      lane: 'student', motivationStyle: 'calm',
      primaryGoal: 'WORK', sessionMode: SessionMode.STUDY,
