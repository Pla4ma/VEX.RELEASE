...
import {
  ...
  buildLaneSessionBrief, decideNudge,
...
describe('Phase 3C — Clean Mode Polish', () => {
  ...
  it('Clean completion has minimal animation and hides reward noise', () => {
    const input = completionInput({
      lane: 'minimal_normal', motivationStyle: 'calm',
  ...
  it('Clean notification budget is max 1/day by policy', () => {
    expect(decideNudge({ lane: 'minimal_normal', completedSessions: 3, daysSinceOnboarding: 3, sentToday: 0 }).allowed).toBe(true);
    expect(decideNudge({ lane: 'minimal_normal', completedSessions: 3, daysSinceOnboarding: 3, sentToday: 0 }).budgetRemaining).toBe(1);
    expect(decideNudge({ lane: 'minimal_normal', completedSessions: 3, daysSinceOnboarding: 3, sentToday: 1 }).allowed).toBe(false);
    expect(decideNudge({ lane: 'minimal_normal', completedSessions: 3, daysSinceOnboarding: 3, sentToday: 1 }).budgetRemaining).toBe(0);
    expect(decideNudge({ lane: 'student', completedSessions: 3, daysSinceOnboarding: 3, sentToday: 1 }).budgetRemaining).toBe(1);
  });
});
