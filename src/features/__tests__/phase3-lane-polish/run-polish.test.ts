...
import {
  ...
  buildLaneSessionBrief, baseLaneProfile, baseStats, baseProfile,
  featureAvailability, SessionMode,
} from './helpers';
...
describe('Phase 3D — Run Mode Polish', () => {
  ...
  it('Run completion hides boss_consequence_card without boss consequences', () => {
    const input = completionInput({
      lane: 'game_like', motivationStyle: 'coach_led',
  ...
  it('PersonalBoss requires evidence: boss_damage fires with game_like + boss consequences', () => {
    const input = completionInput({
      lane: 'game_like', motivationStyle: 'game_like',
      sessionMode: SessionMode.SPRINT,
  ...
  it('Run completion hides all currency/economy surfaces', () => {
    ...
    const currencySurfaces = [
      'coins_gems_wallet', 'premium_chest', 'battle_pass_card',
      'shop_inventory_prompts', 'chest_reward_animation',
    ];
  ...
  it('non-Run modes do not see full Run systems on home surfaces', () => {
    for (const lane of ['student', 'deep_creative', 'minimal_normal'] as const) {
      ...
      if (lane === 'minimal_normal') expect(hasRunMechanics).toBe(false);
    }
  });
