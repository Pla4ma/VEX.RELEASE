  ...
  it('covers every required context and tone variant', () => {
    for (const context of COACH_PRESENCE_MESSAGE_CONTEXTS) {
      for (const style of COACH_PRESENCE_MESSAGE_STYLES) {
        ...
        expect(message).not.toMatch(/great job|keep going|you can do it|unlock your potential/i);
      }
    }
  });

  it('keeps boss and damage language to game-like or intense users', () => {
    const calm = getCoachPresenceMessage({ context: 'strong_streak', style: 'calm' });
    const gameLike = getCoachPresenceMessage({ context: 'strong_streak', style: 'game_like' });
