...
function makeContext(overrides: Partial<CoachPresenceContext> = {}): CoachPresenceContext {
  return {
...
describe('getCoachPresenceMessage', () => {
  ...
  it('short and human — under 96 chars', () => {
    const styles = ['CALM', 'FRIENDLY', 'COACH_LED', 'GAME_LIKE', 'INTENSE', 'STUDY_FOCUSED'] as const;
    for (const style of styles) {
      const result = getCoachPresenceMessage(makeContext({ motivationStyle: style }));
      expect(result.message.length).toBeLessThanOrEqual(96);
    }
  });
