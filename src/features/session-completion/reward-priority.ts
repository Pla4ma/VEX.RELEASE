...
function compareRewards(
  left: RewardPriorityReward,
  right: RewardPriorityReward,
): number {
...
export function buildRewardPrioritySummary(
  input: RewardPriorityInput,
...
export function buildSessionRewardPriority(
  ...
  if (
    streak?.streakSaved ||
    (streak && streak.currentDays > streak.previousDays)
  ) {
    rewards.push({
