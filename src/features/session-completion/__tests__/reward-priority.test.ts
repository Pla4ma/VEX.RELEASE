...
describe('buildRewardPrioritySummary', () => {
  ...
  it('puts a band change ahead of boss damage', () => {
    const summary = build({
      rewards: [
        {
          detail: 'Boss took 35 damage',
          kind: 'boss',
          label: 'Major boss damage',
          priority: 'major',
        },
        {
          detail: 'Silver to Gold',
          kind: 'focus_score',
          label: 'Focus Score band up',
          priority: 'major',
        },
      ],
    });
  ...
  it('puts streak recovery ahead of small currency', () => {
    const summary = build({
      rewards: [
        ...
        {
          detail: 'Insurance protected day 6',
          kind: 'streak',
          label: 'Streak recovered',
          priority: 'major',
        },
      ],
    });
  ...
  it('returns one headline and grouped secondary rewards', () => {
    const summary = build({
      rewards: [
        ...
        {
          detail: 'Chapter memory saved',
          kind: 'companion',
          label: 'Companion memory',
          priority: 'supporting',
        },
        {
          detail: 'Best Deep Work score',
          kind: 'personal_best',
          label: 'New personal best',
          priority: 'major',
        },
    ...
    expect(summary.headline.kind).toBe('personal_best');
    expect(summary.secondaryRewards.map((reward) => reward.kind)).toEqual([
      'companion',
      'currency',
    ]);
  });
