/**
 * Session Completion Service — Hero and Return Plan Tests
 */
...
describe('buildSessionCompletionHero', () => {
  it('returns Clean finish title for zero interruptions', () => {
    const hero = buildSessionCompletionHero({
      focusedDurationLabel: '20:00 focused',
      interruptions: 0,
  ...
  it('returns Work locked in title for interruptions', () => {
    const hero = buildSessionCompletionHero({
      focusedDurationLabel: '18:00 focused',
      interruptions: 3,
      streakIncreased: false,
    });
  ...
  it('shows streak protected eyebrow when streak increased', () => {
    const hero = buildSessionCompletionHero({
      focusedDurationLabel: '25:00 focused',
      interruptions: 1,
...
describe('buildSessionCompletionReturnPlan', () => {
  ...
  it('shows returnReasonTitle based on streak', () => {
    const withStreak = buildSessionCompletionReturnPlan({
      completionPercentage: 85,
      hasStudyFollowUp: false,
      streakDays: 3,
      streakIncreased: true,
    ...
    const noStreak = buildSessionCompletionReturnPlan({
      completionPercentage: 85,
      hasStudyFollowUp: false,
      streakDays: 0,
      streakIncreased: false,
