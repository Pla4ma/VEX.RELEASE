...
jest.mock('../companion-memory-integration', () => ({ recordCompletionCompanionMemories: jest.fn().mockResolvedValue([]) }));
jest.mock('../story-view-model-service', () => ({
  buildPostSessionStoryViewModel: jest.fn((input: Record<string, unknown>) => ({
    ...input,
    gradeCard: { grade: 'A' },
    rewardReveal: { rewardIds: (input as { ledger?: { rewardIds?: string[] } }).ledger?.rewardIds ?? [] },
    companionReaction: { reactionId: (input as { ledger?: { companionReactionId?: string } }).ledger?.companionReactionId ?? null },
    companionPromise: (input as { companionPromise?: unknown }).companionPromise ?? null,
    dailyMission: { status: (input as { ledger?: { dailyMissionResult?: { status?: string } } }).ledger?.dailyMissionResult?.status ?? 'unchanged' },
    headline: (input as { personalBest?: { isPersonalBest?: boolean } }).personalBest?.isPersonalBest ? { type: 'personal_best' } : { type: 'normal' },
    pendingSync: false,
  })),
}));
