...
export class SessionCompletionOfflineSyncService {
  ...
  async getSyncStatus(sessionId: string): Promise<SyncStatusResult> {
    const fallbackEntry = fallbackStorage
      .getEntries()
      .find((e) => e.payload.sessionId === sessionId);
    return {
      status: "pending_sync",
  ...
  async generateHealthReport(): Promise<OfflineSyncReport> {
    ...
    const issues =
      d.oldestEntryAge && d.oldestEntryAge > 86400000
    ...
    return {
      ...
      averageRetryCount: 0,
      lastSyncTime: d.lastSyncAt > 0 ? d.lastSyncAt : null,
    ...
    try {
      ...
      if (
        ...
        ((error.cause as { code?: string }).code === "23505" ||
          (error.cause as { code?: string }).code === "409")
      )
        return ledger;
