...
async function executeWithFallback<T>(
  ...
  try {
    ...
    const repoError = error instanceof RepositoryError
      ...
      : new StreaksRepositoryError(operation, error);
...
): Promise<{ successful: Streak[]; failed: Array<{ userId: string; error: StreaksRepositoryError }> }> {
  const results = { successful: [] as Streak[], failed: [] as Array<{ userId: string; error: StreaksRepositoryError }> };
...
export async function fetchExpiredRepairQuestsEnhanced(): Promise<RepositoryResult<Array<{
  id: string;
  userId: string;
