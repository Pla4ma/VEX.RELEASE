...
function getIntensityColor(durationMinutes: number, theme: any): string {
  // Color intensity based on session duration
  if (durationMinutes === 0) {return 'transparent';}
  if (durationMinutes < 15) {return `${theme.colors.warning[400]}40`;} // Light amber
  if (durationMinutes < 30) {return `${theme.colors.warning[500]}60`;} // Medium amber
  if (durationMinutes < 60) {return `${theme.colors.warning[500]}80`;} // Strong amber
  if (durationMinutes < 120) {return `${theme.colors.error[500]}70`;} // Light red (fire)
  return `${theme.colors.error[500]}90`; // Strong fire color
}

function getFlameGradient(durationMinutes: number): [string, string] {
  if (durationMinutes < 30) {return ['#fbbf24', '#f59e0b'];} // Yellow flame
  if (durationMinutes < 60) {return ['#f59e0b', '#ef4444'];} // Orange-red flame
...
function DayCell({ day, dayData, isToday, theme }: DayCellProps) {
  ...
  const [flameStart, flameEnd] = useMemo(
    () => getFlameGradient(durationMinutes),
    [durationMinutes]
  );
