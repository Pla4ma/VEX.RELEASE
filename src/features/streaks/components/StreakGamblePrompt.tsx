...
export const StreakGamblePrompt: React.FC<StreakGamblePromptProps> = ({
  ...
  const getRiskText = (hours: number): { text: string; color: string } => {
    if (hours < 1) {return { text: 'CRITICAL - About to break!', color: theme.colors.error.DEFAULT };}
    if (hours < 2) {return { text: 'HIGH RISK', color: theme.colors.error.DEFAULT };}
      ...
      <Animated.View entering={FadeInUp.springify()}>
        ...
        >
          <Box alignItems="center">
            ...
            <Box mt={5}>
              <Button variant="primary" size="md" onPress={onDismiss}
...
export function useShouldShowGamblePrompt(
  ...
  return hoursRemaining < CRITICAL_HOURS_THRESHOLD &&
