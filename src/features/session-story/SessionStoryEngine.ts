...
import {
  ...
  StoryAnalyticsEventSchema,
  type SessionStory,
  type GenerateStoryInput,
  type StoryAnalyticsEvent,
} from './schemas';
...
async function fetchBossContext(
  userId: string,
  sessionId: string
): Promise<StoryContext['bossContext'] | null> {
  try {
    ...
    const damageDealt = calculateBossDamage({
      ...
      focusQuality: 0,
    } as SessionSummary, encounter.bossId);
...
async function generateSessionStory(context: StoryContext): Promise<SessionStory | null> {
  ...
  const input: GenerateStoryInput = {
    ...
    streakContext: {
      ...
      isComeback: context.streakResult?.action === 'BROKEN' && context.streakResult?.newStreak === 1 &&
