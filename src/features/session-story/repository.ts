...
export async function fetchStoryBySessionId(
  sessionId: string,
  userId: string,
...
export async function markStoryShared(storyId: string): Promise<void> {
  ...
  const updatedStory: SessionStory = { ...story, sharedAt: Date.now() };

  const { error } = await supabase
    .from("session_stories")
    .update({ story_data: serializeStory(updatedStory) })
    .eq("id", storyId);
...
export async function updateStoryCompletionRate(
  storyId: string,
  completionRate: number,
): Promise<void> {
  const { error } = await supabase
...
export async function getStoryEngagementStats(
  ...
  const { data, error } = await supabase
    .from("session_stories")
    .select("viewed, completion_rate, story_data")
    .eq("user_id", userId)
  ...
  const viewedStories = stories.filter((s) => s.viewed).length;
  const avgCompletion =
    totalStories > 0
      ? stories.reduce((sum, s) => sum + (s.completion_rate ?? 0), 0) / totalStories
      : 0;
  ...
  const mostEngagingBeatType =
    Object.entries(beatTypeCounts).sort((a, b) => b[1] - a[1])[0]?.[0] ?? null;
