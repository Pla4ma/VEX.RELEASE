...
export const PostSessionStoryScreen: React.FC<PostSessionStoryScreenProps> = ({
  story,
  onComplete,
  onSkip,
  onShare,
  ...
  const triggerHaptic = useCallback((pattern: StoryBeat['hapticPattern']) => {
    ...
    switch (pattern) {
      ...
      case 'HEAVY': haptics.impact('heavy'); break;
      case 'SUCCESS': haptics.success('light'); break;
      case 'CELEBRATION': haptics.celebration(); break;
    }
  }, [isReducedMotion]);
  ...
  const currentBeat = phase === 'beats' && currentBeatIndex >= 0 && currentBeatIndex < story.totalBeats
    ? story.beats[currentBeatIndex] : null;
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.background.primary }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 24, paddingBottom: 16, paddingTop: 32 }}>
        ...
        <Pressable onPress={handleSkip} style={{ paddingHorizontal: 12, paddingVertical: 6 }}>
          <Text style={{ fontSize: 14, fontWeight: '500', color: theme.colors.text.muted }}>
            Skip
          </Text>
        </Pressable>
      </View>
      ...
      >
        ...
        {phase === 'intro' && (
          <View style={{ alignItems: 'center' }}>
            <Text style={{ fontSize: 12, fontWeight: '700', letterSpacing: 2, marginBottom: 16, color: theme.colors.text.secondary }}>
              YOUR SESSION STORY
            </Text>
            <Text style={{ fontSize: 32, fontWeight: '800', textAlign: 'center', marginBottom: 12, color: theme.colors.text.primary }}>
              {story.title}
            </Text>
            {story.subtitle && (
              <Text style={{ fontSize: 16, textAlign: 'center', color: theme.colors.text.secondary }}>
                {story.subtitle}
              </Text>
            )}
          </View>
        ...
        {phase === 'outro' && (
          <View style={{ alignItems: 'center' }}>
            <Text style={{ fontSize: 24, fontWeight: '700', marginBottom: 12, color: theme.colors.text.primary }}>
              Story Complete
            </Text>
            <Text style={{ fontSize: 16, textAlign: 'center', color: theme.colors.text.secondary }}>
        ...
        {phase === 'complete' && (
          <View style={{ width: '100%', alignItems: 'center' }}>
            <Text style={{ fontSize: 28, fontWeight: '800', marginBottom: 24, color: theme.colors.text.primary }}>
              Session Complete
            </Text>
            ...
            {story.nextSessionHooks.length > 0 && (
              <View style={{ width: '100%', marginBottom: 32 }}>
                <Text style={{ fontSize: 12, fontWeight: '700', letterSpacing: 1, marginBottom: 12, color: theme.colors.text.secondary }}>
                  NEXT SESSION
                </Text>
                {story.nextSessionHooks.slice(0, 2).map((hook, index) => (
                  <View
                    ...
                    style={{
                      ...
                      borderRadius: 12,
                      marginBottom: 8,
                      backgroundColor: hook.urgency === 'HIGH'
                        ? `${theme.colors.error.DEFAULT}15`
                        : `${theme.colors.primary[500]}10`,
                    }}
                  >
                    <Text style={{ fontSize: 14, fontWeight: '600', flex: 1, color: theme.colors.text.primary }}>
                      {hook.description}
                    </Text>
                    {hook.urgency === 'HIGH' && (
            ...
            <View style={{ width: '100%', gap: 12 }}>
              {onShare && (
                <Pressable
                  style={{ paddingVertical: 14, borderRadius: 12, alignItems: 'center', borderWidth: 1, borderColor: theme.colors.border.DEFAULT }}
                  onPress={handleShare}
                >
              ...
              <Pressable
                style={{ paddingVertical: 16, borderRadius: 12, alignItems: 'center', backgroundColor: emotionColor }}
                onPress={handleNext}
              >
...
const StoryBeatView: React.FC<StoryBeatViewProps> = ({ beat, emotionColor, isReducedMotion }) => {
      ...
      case 'STREAK_FLAME':
        return (
          <View style={{ width: 120, height: 120, borderRadius: 60, justifyContent: 'center', alignItems: 'center', marginBottom: 32, backgroundColor: `${emotionColor}20` }}>
            ...
            {beat.metadata?.value && (
              <Text style={{ position: 'absolute', bottom: -8, fontSize: 20, fontWeight: '800', color: emotionColor }}>
                {beat.metadata.value}
              </Text>
            )}
          </View>
  ...
  return (
    ...
    >
      ...
      <Text style={{ fontSize: 28, fontWeight: '700', textAlign: 'center', marginBottom: 12, lineHeight: 36, color: theme.colors.text.primary }}>
        {beat.headline}
      </Text>

      {beat.subtext && (
        <Text style={{ fontSize: 16, textAlign: 'center', lineHeight: 24, marginBottom: 20, color: theme.colors.text.secondary }}>
          {beat.subtext}
        </Text>
      )}
