...
interface CalculatedStory {
  ...
  nextSessionHooks: Array<{
    type: 'STREAK_AT_RISK' | 'BOSS_ALMOST_DEFEATED' | 'MILESTONE_APPROACHING' |
          'TIER_UNLOCK_SOON' | 'PERFECT_RUN_CONTINUING' | 'COMEBACK_MOMENTUM';
    description: string;
...
const BEAT_TEMPLATES: BeatCandidate[] = [
  ...
  // ==========================================================================
  {
    ...
    generator: (input) => {
      ...
      return {
        ...
        emotion: mood === 'DETERMINED' ? 'DETERMINATION' :
                 mood === 'FOCUSED' ? 'MASTERY' : 'RESILIENCE',
        visualCue: 'NONE',
  ...
  // ==========================================================================
  {
    ...
    generator: (input) => {
      ...
      return {
        ...
        metadata: {
          comparison: input.sessionSummary.duration >= 3600000
  ...
  // ==========================================================================
  {
    ...
    generator: (input) => ({
      ...
      subtext: input.bossContext && input.bossContext.damageDealt > 100
  ...
  // ==========================================================================
  {
    ...
    generator: (input) => {
      ...
      return {
        type: 'COMEBACK_TRIUMPH',
        headline: daysAbsent >= 7
          ? 'You returned. Stronger than before.'
          : 'Welcome back. Let\'s continue.',
        subtext: daysAbsent >= 7
  ...
  // ==========================================================================
  {
    ...
    generator: (input) => {
      ...
      return {
        ...
        subtext: daysToMilestone && daysToMilestone <= 3
  ...
  // ==========================================================================
  {
    ...
    generator: (input) => {
      ...
      return {
        ...
        metadata: {
          value: duration,
          comparison: quality >= 90 ? 'exceptional focus' :
                      quality >= 75 ? 'strong focus' : 'completed',
        },
      };
    },
  },
  ...
  // ==========================================================================
  {
    ...
    generator: (input) => ({
      type: 'PROGRESSION_CLIFFHANGER',
      headline: input.progressionContext.sessionsToNextTier === 1
  ...
  // ==========================================================================
  {
    ...
    generator: (input) => {
      ...
      const isFirstSession = input.streakContext.newStreak === 1 &&
                             !input.streakContext.isComeback;
      ...
      let key: string;
      if (isFirstSession) {key = 'FIRST';}
      else if (totalFocusTime >= 60) {key = 'LONG';}
      else if (totalFocusTime >= 25) {key = 'MEDIUM';}
      else if (input.sessionSummary.interruptions === 0) {key = 'PERFECT';}
      else {key = 'SHORT';}
...
function determineOverallEmotion(beats: StoryBeat[]): EmotionalArc {
  // Count emotions
  const emotionCounts = beats.reduce((acc, beat) => {
  ...
  const priority: EmotionalArc[] = [
    'TRIUMPH', 'WONDER', 'MASTERY', 'RESILIENCE',
    'DETERMINATION', 'ANTICIPATION', 'GRATITUDE', 'RELIEF',
  ];
...
export function calculateStory(input: GenerateStoryInput): CalculatedStory {
  ...
  const matchingBeats = BEAT_TEMPLATES
    .filter(template => template.condition(input))
    .map(template => ({
