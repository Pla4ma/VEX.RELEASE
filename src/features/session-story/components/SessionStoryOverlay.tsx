...
export const SessionStoryOverlay: React.FC<SessionStoryOverlayProps> = ({
  ...
  const handleHaptic = useCallback((pattern: StoryBeat['hapticPattern']) => {
            ...
            <View
              ...
              style={{
                ...
                backgroundColor: index <= currentBeatIndex
                  ? emotionColor
                  : (theme.colors.border?.DEFAULT ?? '#3333'),
              }}
            />
          ))}
        </View>
...
const StoryBeatCard: React.FC<StoryBeatCardProps> = ({ beat, emotionColor, theme }) => {
  ...
  return (
    ...
    >
      ...
      {/* Subtext */}
      {beat.subtext && (
        <Text style={{ fontSize: 16, textAlign: 'center', lineHeight: 24, marginBottom: 20, color: theme.colors.text?.secondary ?? '#aaa' }}>
          {beat.subtext}
        </Text>
      )}
