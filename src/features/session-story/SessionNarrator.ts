...
export class SessionNarrator {
  ...
  recordInterruption(sessionId: string, recoveryTime: number): void {
    ...
    const template = narrative.totalInterruptions === 1
      ? this.selectTemplate('INTERRUPTION', { count: 1 })
      : this.selectTemplate('INTERRUPTION', { count: 'multiple' });
  ...
  recordPureFocusStreak(sessionId: string, durationSeconds: number): void {
    ...
    const template = isLong
      ? this.selectTemplate('PURE_FOCUS_STREAK', { duration: 'long' })
      : this.selectTemplate('PURE_FOCUS_STREAK', { duration: 'short' });
  ...
  recordBossEvent(
    ...
    switch (eventType) {
      ...
      case 'VICTORY':
        ...
        template = isComeback
          ? this.selectTemplate('VICTORY', { healthRemaining: 'low' })
          : this.selectTemplate('VICTORY', { healthRemaining: 'high' });
      ...
      case 'DEFEAT':
        ...
        template = close
          ? this.selectTemplate('DEFEAT', { margin: 'close' })
