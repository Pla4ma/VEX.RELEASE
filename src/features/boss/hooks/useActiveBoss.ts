...
export function useActiveBoss(
  ...
  const calculateDamage = useCallback(
    (elapsedMinutes: number, purityScore: number): DamageCalculation => {
      ...
      const totalDamage = Math.floor(
        baseDamage * purityMultiplier * streakMultiplier
      );
  ...
  const estimatedKill = useMemo((): KillEstimate => {
    ...
    const percentDamage = Math.min(
      100,
      (currentDamage / encounter.maxHealth) * 100
    );
  ...
  const combatState = useMemo((): ActiveBossState['combatState'] => {
    if (!encounter) {return 'ENCOUNTER_START';}

    const percentHealth = encounter.percentHealthRemaining;

    if (percentHealth <= 0) {return 'VICTORY';}
    if (percentHealth <= 10) {return 'NEAR_DEATH';}
    if (percentHealth <= 25) {return 'BOSS_RAGE';}
