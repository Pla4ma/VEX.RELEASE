...
export class AdaptiveDifficultyEngine {
  ...
  updateMetrics(
    ...
    const avgPurity = last7Days.length > 0
      ? last7Days.reduce((sum, s) => sum + s.purity, 0) / last7Days.length
      : 75;
    ...
    const bossDefeatRate = bossFights.length > 0
      ? bossFights.filter(s => s.bossDefeated).length / bossFights.length
      : 0.5;
    const averageBossDamage = bossFights.length > 0
      ? bossFights.reduce((sum, s) => sum + s.bossDamage, 0) / bossFights.length
      : 0;
  ...
  setDifficulty(userId: string, rating: DifficultyRating): DifficultyAdjustment {
    ...
    return {
      ...
      newRating: rating,
      changes: {
        healthChange: ((newConfig.baseHealth * newConfig.healthMultiplier) /
          (oldConfig.baseHealth * oldConfig.healthMultiplier) - 1) * 100,
