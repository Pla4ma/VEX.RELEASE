 ...
 */
export function createSquadBossEncounter(
  bossId: string,
  squadId: string,
  baseHealth: number,
  baseDuration: number,
...
export function calculateMVP(contributions: SquadMemberContribution[]): SquadMemberContribution | null {
  ...
  const scored = contributions.map((c) => ({
    ...
    score:
      c.damageDealt * 1 +
      c.sessionsContributed * 10 +
      c.criticalHits * 50 +
      (c.bountyPlaced ? 100 : 0),
  }));
 ...
 * Get member ranking
 */
export function getMemberRanking(
  contributions: SquadMemberContribution[]
): Array<SquadMemberContribution & { rank: number; damagePercent: number }> {
 ...
 */
export function createVictoryCeremony(
  encounter: SquadBossEncounter,
  bossName: string
): SquadVictoryCeremony {
 ...
 */
export function getSquadProgressSummary(
  encounter: SquadBossEncounter
): {
  ...
  const topContributor = encounter.memberContributions.reduce(
    (top, c) => (c.damageDealt > (top?.damageDealt || 0) ? c : top),
    null as SquadMemberContribution | null
  );
...
export function canMemberJoin(encounter: SquadBossEncounter, userId: string): boolean {
  ...
  if (encounter.expiresAt < Date.now()) {return false;}

  // Check if already a member
  if (encounter.memberContributions.some((c) => c.userId === userId)) {return true;}
 ...
 */
export function addMemberToEncounter(
  encounter: SquadBossEncounter,
  userId: string,
  userName: string,
  avatarUrl: string | null
