...
async function executeWithFallback<T>(
  ...
  try {
    ...
    const repoError = error instanceof RepositoryError
      ? new BossRepositoryError(operation, error.originalError, error.code)
      : new BossRepositoryError(operation, error);
    ...
    if (offlineFn) {
      ...
      } catch (error) { captureSilentFailure(error, { feature: 'boss', operation: 'network-fallback', type: 'network' });
        // Cache miss
      }
    }
...
export async function fetchActiveEncounterEnhanced(
  userId: string,
  squadId?: string
): Promise<RepositoryResult<BossEncounter | null>> {
...
export async function createEncounterEnhanced(
  userId: string,
  bossId: string,
  maxHealth: number,
  squadId?: string
): Promise<RepositoryResult<BossEncounter>> {
  const newEncounter = {
...
export async function updateEncounterHealthEnhanced(
  encounterId: string,
...
export async function markEncounterDefeatedEnhanced(
  encounterId: string
): Promise<RepositoryResult<BossEncounter>> {
  return executeWithFallback(
...
export async function recordBossDefeatEnhanced(
  userId: string,
  bossId: string,
  encounterId: string,
  damageDealt: number
...
export async function getCooldownStatusEnhanced(
  userId: string,
  bossId: string
): Promise<RepositoryResult<{ onCooldown: boolean; remainingMs: number }>> {
  return executeWithFallback(
    'getCooldownStatus',
    async () => {
      const { data, error } = await supabase
