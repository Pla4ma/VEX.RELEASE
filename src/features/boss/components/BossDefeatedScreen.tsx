...
function BossShatter({
  ...
  bossTier: BossDefeatedScreenProps['bossTier'];
    ...
    transform: [
      ...
      },
      {
        rotate: withSequence(
          ...
          withSpring('0deg', { damping: 8, stiffness: 100 })
        ),
      },
    ],
...
function ContributorsList({
  ...
  contributors: NonNullable<BossDefeatedScreenProps['contributors']>;
}): JSX.Element {
  const { theme } = useTheme();
 ...
 * Defeat rewards card
 */
function DefeatRewards({
  rewards,
  bossTier,
}: {
  rewards: BossDefeatedScreenProps['rewards'];
  bossTier: BossDefeatedScreenProps['bossTier'];
}): JSX.Element {
  const { theme } = useTheme();
    ...
    <Animated.View entering={FadeInUp.duration(500).delay(1200)}>
      ...
      >
        ...
        <Box
          ...
          height={80}
          borderRadius="xl"
          bg={`${theme.colors.accent.purple}30`}
          justifyContent="center"
          alignItems="center"
        >
 ...
 */
export function BossDefeatedScreen({
  bossName,
  bossTier,
  totalDamage,
  sessionsContributed,
  ...
  onContinue,
  onShare,
}: BossDefeatedScreenProps): JSX.Element {
  const { theme } = useTheme();
