  ...
  const animatedStyle = useAnimatedStyle(() => ({
    opacity: withRepeat(
      withSequence(
        ...
        withTiming(0, { duration: 100 }),
        withDelay(800, withTiming(0, { duration: 100 }))
      ),
      -1,
      false
    ),
...
export function PrimeTimeBanner({
  ...
  onPress,
  compact = false,
}: PrimeTimeBannerProps): JSX.Element {
  ...
  accessibilityHint="Activates this control">
      <Animated.View style={glowStyle}>
        ...
        >
          ...
          accessibilityLabel="⚔️ Fight Now — Earn 2× XP button"
          accessibilityRole="button"
          accessibilityHint="Activates this control">
 ...
 */
export function PrePrimeTimeBanner({
  window,
  bossName,
  onPress,
}: Omit<PrimeTimeBannerProps, 'compact'>): JSX.Element {
  const { theme } = useTheme();
  const [displayTime, setDisplayTime] = useState(window.timeRemainingMinutes);
      ...
      >
        <Box flexDirection="row" alignItems="center" gap="md">
          <Box
            ...
            justifyContent="center"
            alignItems="center"
            style={{ backgroundColor: `${theme.colors.warning.DEFAULT}20` }}
          >
            <Text fontSize={24}>⏰</Text>
          </Box>
