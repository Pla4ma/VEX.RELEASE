 ...
 */
function RetreatingBoss({
  bossTier,
  escapeTaunt,
}: {
  bossTier: BossTimeoutScreenProps['bossTier'];
  escapeTaunt?: string;
}): JSX.Element {
    ...
    transform: [
      {
        translateY: withSequence(
          withDelay(500, withTiming(-20, { duration: 200 })),
 ...
 */
function ConsolationRewards({
  consolation,
}: {
  consolation: BossTimeoutScreenProps['consolation'];
}): JSX.Element {
  const { theme } = useTheme();
      ...
      >
        ...
        <Box flexDirection="row" gap="lg">
          <Box alignItems="center">
            ...
            <Text variant="caption" color="text.tertiary">XP</Text>
          </Box>
          <Box alignItems="center">
            <Text fontSize={24}>🪙</Text>
            <Text variant="h4" color="text.primary" fontWeight="700">
 ...
 */
export function BossTimeoutScreen({
  bossName,
  bossTier,
  remainingHealth,
  escapeTaunt,
  nextEncounterHealthBonus,
  consolation,
  retryCooldownDays,
  coachMessage,
  onStartSession,
  onGoHome,
}: BossTimeoutScreenProps): JSX.Element {
  const { theme } = useTheme();
    ...
    >
      <Box flex={1} px="xl" py="xl" gap="lg">
        ...
        <Animated.View entering={FadeInUp.duration(500).delay(1600)}>
          <Box gap="md">
            <Button variant="primary" size="lg" fullWidth onPress={onStartSession}
  accessibilityLabel="Start Fresh Session 🔥 button"
  accessibilityRole="button"
  accessibilityHint="Activates this control">
              Start Fresh Session 🔥
            </Button>
            <Button variant="ghost" size="md" fullWidth onPress={onGoHome}
