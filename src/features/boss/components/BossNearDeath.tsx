 ...
 */
export function BossNearDeath({
  healthPercent,
  hoursRemaining,
  bossName,
  compact = false,
}: BossNearDeathProps): JSX.Element | null {
  ...
  if (!isNearDeath) {return null;}

  const pulseStyle = useAnimatedStyle(() => ({
    borderColor: withRepeat(
      withSequence(
        withTiming(theme.colors.error.DEFAULT, { duration: 500 }),
        withTiming(theme.colors.error.light, { duration: 500 })
      ),
      -1,
      true
    ),
 ...
 */
export function BossEscapeWarning({
  bossName,
  hoursRemaining,
}: {
  bossName: string;
  ...
  const pulseStyle = useAnimatedStyle(() => ({
    backgroundColor: withRepeat(
      withSequence(
        withTiming(`${theme.colors.error.DEFAULT}40`, { duration: 600 }),
        withTiming(`${theme.colors.error.DEFAULT}20`, { duration: 600 })
      ),
      -1,
      true
    ),
