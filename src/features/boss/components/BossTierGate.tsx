...
export function BossTierGate({
  tier,
  userRank,
  children,
  onNavigateToMastery,
}: BossTierGateProps): JSX.Element {
...
export function LockedBossCard({
  tier,
  bossName,
  userRank,
  onNavigateToMastery,
}: LockedBossCardProps): JSX.Element {
  const { theme } = useTheme();
  const requiredRank = getRequiredRankForTier(tier);
  const rankDisplay = getMasteryRankDisplay(requiredRank);

  return (
    <Pressable onPress={onNavigateToMastery}
