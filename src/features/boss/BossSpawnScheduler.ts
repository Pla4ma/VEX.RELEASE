 ...
 */
export function calculatePrimeTimeWindow(
  bossTier: number,
  timezone: string,
  bossId: string
): PrimeTimeWindow {
 ...
 */
export function calculatePrimeTimeXP(
  baseXP: number,
  bossTier: number,
  timezone: string
): { totalXP: number; bonusXP: number; isPrimeTime: boolean } {
 ...
 */
export async function sendPrePrimeTimeNotification(
  userId: string,
  bossName: string,
  bossTier: number,
  startTime: number
 ...
 */
export async function sendPrimeTimeStartNotification(
  userId: string,
  bossName: string,
  bossTier: number
): Promise<void> {
...
export async function checkAndSendPrimeTimeNotifications(): Promise<void> {
  try {
    const { data: activeEncounters, error } = await getSupabaseClient()
    ...
    for (const encounter of activeEncounters) {
      ...
      const window = calculatePrimeTimeWindow(
        encounter.boss_tier,
        timezone,
        encounter.boss_id
      );
      ...
      if (window.isPreWindow && !encounter.pre_notification_sent) {
        await sendPrePrimeTimeNotification(
          encounter.user_id,
          encounter.boss_name,
          encounter.boss_tier,
          window.startTime
      ...
      if (window.isActive && !encounter.prime_time_start_notification_sent) {
        await sendPrimeTimeStartNotification(
          encounter.user_id,
          encounter.boss_name,
          encounter.boss_tier
        );
        ...
        await getSupabaseClient()
          .from('boss_encounters')
          .update({ prime_time_start_notification_sent: true })
          .eq('id', encounter.id);
      }
    }
  } catch (error) {
