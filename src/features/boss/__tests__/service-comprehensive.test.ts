...
describe('Boss Service - Comprehensive', () => {
  ...
  describe('applyDamage', () => {
    it('should apply damage and reduce health', async () => {
      ...
      vi.mocked(repository.updateEncounterHealth).mockResolvedValue({
        healthRemaining: 400,
        damageDealt: 600,
      } as any);
      ...
      expect(result.healthRemaining).toBe(400);
    ...
    it('should handle encounter timeout', async () => {
      ...
      vi.mocked(repository.markEncounterTimeout).mockResolvedValue({} as any);

      await expect(applyDamage({
        encounterId: 'encounter-1',
        damage: 100,
        sessionId: 'session-1',
  ...
  describe('getAvailableBosses', () => {
    it('should list all bosses with unlock status', async () => {
      ...
      expect(results[1].unlocked).toBe(true); // Level 7 >= 5
      expect(results[2].unlocked).toBe(false); // Level 7 < 10
    ...
    it('should show locked reason for locked bosses', async () => {
      const mockTemplates = [
        { id: 'boss-2', name: 'Boss 2', unlockLevel: 10, baseHealth: 2000, rewardType: 'COINS', rewardAmount: 100 },
      ];
  ...
  describe('Concurrency', () => {
    it('should handle concurrent damage applications', async () => {
      ...
      vi.mocked(repository.fetchActiveEncounter).mockImplementation(async () => ({
        id: 'encounter-1',
        bossId: 'boss-1',
        status: 'ACTIVE',
      ...
      vi.mocked(repository.updateEncounterHealth).mockImplementation(async (_, health, damage) => {
        currentHealth = health;
        return { healthRemaining: currentHealth, damageDealt: 1000 - currentHealth } as any;
      });

      // Apply damage from multiple sessions concurrently
      const results = await Promise.all([
        applyDamage({ encounterId: 'encounter-1', damage: 50, sessionId: 'session-1' }),
        applyDamage({ encounterId: 'encounter-1', damage: 50, sessionId: 'session-2' }),
        applyDamage({ encounterId: 'encounter-1', damage: 50, sessionId: 'session-3' }),
      ]);
