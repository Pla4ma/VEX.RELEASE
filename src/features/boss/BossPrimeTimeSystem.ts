 ...
 */
export function generatePrimeTimeWindows(
  encounterId: string,
  encounterStartTime: number,
  encounterDurationMs: number,
  pattern: PrimeTimePattern = 'EVENING',
  ...
  for (let i = 0; i < windowCount; i++) {
    ...
    const windowStart = calculatePrimeTimeStart(
      encounterStartTime + dayOffset,
      patternRange.start,
      userTimezone
    );
 ...
 */
function calculatePrimeTimeStart(
  baseTime: number,
  patternHour: number,
  timezone: string
): number {
 ...
 */
export function checkPrimeTimeStatus(
  windows: PrimeTimeWindow[],
  ...
  const nextWindow = futureWindows.length > 0
    ? futureWindows.reduce((closest, w) => (w.startTime < closest.startTime ? w : closest))
    : null;
 ...
 */
export function getPrimeTimeMultiplier(
  windows: PrimeTimeWindow[],
  currentTime: number = Date.now()
): number {
 ...
 */
export function shouldSendPrimeTimeNotification(
  window: PrimeTimeWindow,
  currentTime: number = Date.now()
): boolean {
  if (window.notificationSent) {return false;}
 ...
 */
export function getPrimeTimeNotificationMessage(
  bossName: string,
  ...
  return {
    ...
    body: `Prime Time active for ${hoursRemaining.toFixed(1)} more hours. Deal ${Math.round(
 ...
 */
export function recordPrimeTimeDamage(
  encounterId: string,
  damage: number,
  isPrimeTime: boolean
): void {
  ...
  if (isPrimeTime) {
    analytics.averageDamageDuringPrimeTime =
      (analytics.averageDamageDuringPrimeTime + damage) / 2;
  } else {
...
export function getRecommendedAttackMessage(status: PrimeTimeStatus): string {
  if (status.isPrimeTime) {
    return `🔥 PRIME TIME ACTIVE! Deal bonus damage for ${formatTimeRemaining(
