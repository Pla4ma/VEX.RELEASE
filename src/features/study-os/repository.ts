...
export async function listStoredStudyPlans(userId: string): Promise<StudyPlan[]> {
  const raw = storage.getString(keyFor(userId));
  if (!raw) return [];
  return StudyPlanSchema.array().parse(JSON.parse(raw));
}

export async function upsertStoredStudyPlan(plan: StudyPlan): Promise<StudyPlan> {
