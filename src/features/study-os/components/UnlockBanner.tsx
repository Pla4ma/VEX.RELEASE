...
export function UnlockBanner({ gate, currentSessions }: UnlockBannerProps): JSX.Element {
  const { theme } = useTheme();
  if (gate.isUnlocked) return <></>;

  const remaining = Math.max(0, UNLOCK_SESSIONS_TARGET - gate.completedSessions);
  ...
  return (
    <Animated.View entering={FadeInUp.duration(300)}>
      ...
      >
        <Text variant="label" color="semantic.primary" textTransform="uppercase">
        ...
        <Box mt={2} height={4} borderRadius="sm" style={{ backgroundColor: theme.colors.border.light, overflow: 'hidden' }}>
          <Box
            height={4}
            borderRadius="sm"
            style={{
