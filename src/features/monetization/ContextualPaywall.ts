...
export type PaywallTriggerCondition =
  ...
  | { type: 'BOSS_BOUNTY_PLACE_ATTEMPT' }
  | { type: 'STREAK_BREAK_NO_INSURANCE'; daysLost: number }
  | { type: 'ANALYTICS_VIEW_ATTEMPT' }
  | { type: 'PERSONALITY_SWITCH_ATTEMPT' }
  | { type: 'SQUAD_JOIN_ATTEMPT'; currentSquadCount: number }
  | { type: 'COSMETIC_PREVIEW_ATTEMPT'; cosmeticId: string }
  | { type: 'FEATURE_CLICK'; feature: string };
