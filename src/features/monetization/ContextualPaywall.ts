...
import { PAYWALL_MOMENTS, type PaywallMoment, type PaywallTriggerCondition } from './paywall-moments';
...
export function recordPaywallShow(
  userId: string, context: PaywallContextType, dismissed = false, converted = false,
): void {
...
export function canShowPaywall(
  userId: string, context: PaywallContextType, isInSession = false,
): { canShow: boolean; reason: string | null } {
  const moment = PAYWALL_MOMENTS[context];
  const history = getPaywallHistory(userId);
  const now = Date.now();

  if (isPremium(userId)) return { canShow: false, reason: 'User is already premium' };
  if (isInSession && moment.respectDND) return { canShow: false, reason: 'Respecting Do Not Disturb during session' };
...
export function getPaywallCooldownRemaining(
  userId: string, context: PaywallContextType,
): number {
  const moment = PAYWALL_MOMENTS[context];
  const history = getPaywallHistory(userId);
  const lastShow = history.filter((h) => h.context === context).pop();
  if (!lastShow) return 0;
  return Math.max(0, lastShow.shownAt + moment.cooldownHours * 60 * 60 * 1000 - Date.now());
}
...
export function evaluateTrigger(
  userId: string, condition: PaywallTriggerCondition, tier: SubscriptionTier = 'free',
  ...
  const { show, context } = shouldShowPaywall(tier, feature as 'deepCoachMemory');
  if (!show || !context) return { shouldShow: false, context: null };
  const result = canShowPaywall(userId, context);
  return { shouldShow: result.canShow, context: result.canShow ? context : null };
}

export function selectBestPaywall(
  userId: string, availableContexts: PaywallContextType[],
): { context: PaywallContextType; data: PaywallContextData } | null {
  const validContexts = availableContexts.filter((ctx) => canShowPaywall(userId, ctx).canShow);
  if (validContexts.length === 0) return null;
  validContexts.sort((a, b) => (PAYWALL_MOMENTS[b]?.priority ?? 0) - (PAYWALL_MOMENTS[a]?.priority ?? 0));
  const selected = validContexts[0]!;
  return { context: selected, data: getPaywallContext(selected) };
}

export { recordConversion, getConversionRate, getBestConvertingContext } from './conversion-tracking';

export function shouldPreventPaywall(
  userId: string, sessionsCompleted: number, isOnboarding: boolean, isFirstSession: boolean,
  ...
  if (sessionsCompleted < 1) return { prevent: true, reason: 'Not enough sessions completed' };
  const history = getPaywallHistory(userId);
  const last24h = history.filter(
    (h) => h.shownAt > Date.now() - 24 * 60 * 60 * 1000 && h.dismissed,
  );
  if (last24h.length >= 3) return { prevent: true, reason: 'Paywall fatigue detected' };
