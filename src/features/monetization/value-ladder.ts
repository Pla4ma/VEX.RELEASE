...
function buildTierConfig(id: ValueTier): TierConfig {
  ...
  return {
    ...
    period: 'month',
    monthlyEquivalent: def.yearlyPrice
      ? def.yearlyPrice / 12
      : (def.monthlyPrice ?? 0),
...
export function calculateUpgradeDiscount(
  ...
  if (daysActive >= 30)
    {return { eligible: true, discountPercent: 15, reason: 'Consistent rhythm' };}
  if (daysActive >= 14)
    {return { eligible: true, discountPercent: 10, reason: 'Building momentum' };}
...
export async function trackLadderInteraction(
  ...
  debug.info(
    'Value ladder interaction: user=%s action=%s tier=%s',
    userId,
    action,
    tier ?? 'none',
  );
