...
describe('free first session flow remains free', () => {
  it('free features include core session loop', () => {
    const strategy = resolvePremiumStrategy({ billingConfigured: true, completedSessions: 0 });
  ...
  it('noFakeBillingChecklist forbids paywalling basic focus loop', () => {
    const strategy = resolvePremiumStrategy({ billingConfigured: true, completedSessions: 0 });
    expect(strategy.noFakeBillingChecklist.join(' ').toLowerCase()).toMatch(/do not paywall/i);
  });
