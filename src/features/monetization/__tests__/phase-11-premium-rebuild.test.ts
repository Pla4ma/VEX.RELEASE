...
describe('Phase 11 premium rebuild gates', () => {
  it('keeps premium hidden on Day 0, even when billing is configured', () => {
    expect(resolveFirstWeekExperience(baseFirstWeekInput).premiumMoment).toBe(
      'none',
    );
    expect(
      resolveFirstWeekExperience({
        ...
        featureAvailability: {
          ...baseFirstWeekInput.featureAvailability,
          boss: true,
        },
      }).premiumMoment,
    ).toBe('none');
    ...
    expect(
      resolvePremiumStrategy({ billingConfigured: true, completedSessions: 0 }),
    ).toMatchObject({
  ...
  it('keeps basic session loop free through the first week', () => {
    ...
    expect(
      resolvePremiumStrategy({
        billingConfigured: true,
        completedSessions: 5,
      }).noFakeBillingChecklist.join(' '),
    ).toMatch(/do not paywall/i);
