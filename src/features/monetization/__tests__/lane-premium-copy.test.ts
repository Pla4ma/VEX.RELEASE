...
const OLD_ECONOMY_TERMS = [
  'coins', 'gems', 'shop', 'inventory', 'premium chest',
  'paid streak', 'battle pass', 'pay-to-win', 'fake AI memory',
];
...
describe('lane-premium-copy', () => {
  ...
  it('each lane headline is not empty', () => {
    for (const lane of Object.keys(LANE_PREMIUM_COPY) as Array<keyof typeof LANE_PREMIUM_COPY>) {
  ...
  it('premium copy has no old economy terms', () => {
    for (const lane of Object.keys(LANE_PREMIUM_COPY) as Array<keyof typeof LANE_PREMIUM_COPY>) {
      const copy = `${LANE_PREMIUM_COPY[lane].headline} ${LANE_PREMIUM_COPY[lane].body}`.toLowerCase();
  ...
  it('premium copy is lane-specific — no two lanes share same body', () => {
    const bodies = new Set(
      Object.values(LANE_PREMIUM_COPY).map((c) => c.body),
    );
  ...
  it('premium copy never mentions shop, coins, or gems', () => {
    for (const lane of Object.keys(LANE_PREMIUM_COPY) as Array<keyof typeof LANE_PREMIUM_COPY>) {
      const full = `${LANE_PREMIUM_COPY[lane].headline} ${LANE_PREMIUM_COPY[lane].body}`.toLowerCase();
      expect(full).not.toMatch(/\b(shop|coins|gems|inventory|chest|battle.pass|pay.to.win)\b/);
    }
  });
