...
export function BattlePassTrack({
  ...
  error,
  onRetry,
}: BattlePassTrackProps): JSX.Element {
        ...
        <View style={headerStyle}>
          <View style={progressInfoStyle}>
            ...
            <Text style={xpTextStyle}>
              {userProgress.tierXp} / 1000 XP
            </Text>
          </View>
          {!hasPremium && (
            <Pressable onPress={onPurchasePremium}
        ...
        <View style={xpBarContainerStyle}>
          <View style={xpBarTrackStyle}>
            <View
              style={[
                xpBarFillStyle,
                { width: `${(userProgress.tierXp / 1000) * 100}%` },
              ]}
            />
          </View>
        </View>
            ...
            return (
              ...
              >
                ...
                <View style={rewardContainerStyle}>
                  <View style={[
                    rewardBoxStyle,
                    isCompleted && !freeClaimed && rewardBoxClaimableStyle,
                    freeClaimed && rewardBoxClaimedStyle,
                  ]}>
                    ...
                    {isCompleted && !freeClaimed && (
                      <Pressable
                        onPress={() => onClaimTier?.(tier.tierNumber)}
                        style={claimButtonStyle}

                      accessibilityLabel="Claim button"
                      accessibilityRole="button"
                      accessibilityHint="Activates this control">
                        <Badge variant="success" size="sm">Claim</Badge>
                      </Pressable>
                    )}
                    {freeClaimed && (
                ...
                <View style={rewardContainerStyle}>
                  <View style={[
                    rewardBoxStyle,
                    premiumBoxStyle,
                    !hasPremium && premiumBoxLockedStyle,
                    hasPremium && isUnlocked && !premiumClaimed && rewardBoxClaimableStyle,
                    premiumClaimed && rewardBoxClaimedStyle,
                  ]}>
