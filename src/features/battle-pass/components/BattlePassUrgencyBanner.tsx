...
export function BattlePassUrgencyBanner({
  daysRemaining,
  currentTier,
  totalTiers,
  unclaimedTiers,
  ...
  onViewBattlePass,
  onUpgradeToPremium,
}: BattlePassUrgencyBannerProps): JSX.Element | null {
      ...
      >
        ...
        <View style={styles.content}>
          ...
          <View style={styles.progressContainer}>
            <View style={[styles.progressBar, { backgroundColor: 'rgba(255,255,255,0.3)' }]}>
              <View
                style={[
                  styles.progressFill,
                  { width: `${progressPercent}%`, backgroundColor: '#FFFFFF' },
                ]}
              />
              {isCapped && (
                <View
                  style={[
                    styles.capIndicator,
                    { left: `${(freeTierCap / totalTiers) * 100}%` },
                  ]}
                >
          ...
          <View style={styles.statsGrid}>
            <View style={styles.stat}>
              ...
              <Text style={styles.statLabel}>
                Unclaimed {unclaimedTiers === 1 ? 'Tier' : 'Tiers'}
              </Text>
            </View>
            ...
            <View style={styles.stat}>
              ...
              <Text style={styles.statLabel}>
                {remainingTiers === 1 ? 'Tier' : 'Tiers'} Remaining
              </Text>
            </View>
            {!hasPremium && (
              <>
                <View style={styles.statDivider} />
                <View style={styles.stat}>
                  <Text style={[styles.statValue, isCapped && styles.cappedValue]}>
                    {isCapped ? 'CAPPED' : tiersUntilCap}
                  </Text>
                  <Text style={styles.statLabel}>
                    {isCapped ? 'Upgrade to continue' : 'Tiers until cap'}
                  </Text>
                </View>
              </>
            )}
          ...
          {!hasPremium && isCapped && (
            <View style={styles.lockedMessage}>
              <Text style={styles.lockedText}>
                🔒 Free users cannot progress past Tier {freeTierCap}.
                Upgrade to unlock all {remainingTiers} remaining tiers!
              </Text>
            </View>
          )}
          ...
          <View style={styles.actions}>
            <Button
              onPress={handleViewBattlePass}
              variant="secondary"
              size="md"
              style={styles.viewButton}

            accessibilityLabel="View Battle Pass → button"
            ...
            {!hasPremium && onUpgradeToPremium && (
              ...
              accessibilityLabel="🔓 Unlock Premium button"
              accessibilityRole="button"
              accessibilityHint="Activates this control">
                🔓 Unlock Premium
              </Button>
