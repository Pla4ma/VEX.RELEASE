...
export const PATH_CONFIG: Record<JourneyPath, {
  name: string;
  description: string;
  color: string;
  icon: string;
  ...
  for (const path of JOURNEY_PATHS) {
    for (let tier = 1; tier <= 50; tier++) {
      ...
      const node: JourneyNode = {
        ...
        nextNodeIds: isIntersection
          ? [nodeId('PURITY', tier + 1), nodeId('SPEED', tier + 1), nodeId('SOCIAL', tier + 1), nodeId('BALANCED', tier + 1)].filter(id => id !== nodeId(path, tier + 1) || tier < 50)
          : tier < 50 ? [nodeId(path, tier + 1)] : [],
...
export function advanceJourneyXp(
...
export function getJourneyProgressPercent(
  progress: UserJourneyProgress,
  nodes: JourneyNode[]
): { pathPercent: number; overallPercent: number; estimatedDaysRemaining: number } {
  const currentNode = nodes.find(n => n.id === progress.currentNodeId);
  if (!currentNode) {
