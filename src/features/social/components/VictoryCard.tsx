...
let captureRef: ((ref: React.RefObject<View>, options: {
  format: 'png' | 'jpg';
  quality: number;
    ...
    >
      ...
      <Box alignItems="center" gap="lg">
        ...
        <Text fontSize={isStory ? 64 : 48} fontWeight="800" color="text.inverse">
          {data.duration || 25}
          <Text fontSize={isStory ? 32 : 24} color="text.tertiary">m</Text>
        </Text>
...
function StreakMilestoneCard({ data, format }: { data: VictoryCardData; format: 'square' | 'story' }): JSX.Element {
  ...
  const getMilestoneTitle = (d: number): string => {
    if (d >= 100) {return 'CENTURY LEGEND';}
    if (d >= 60) {return 'CONSISTENCY KING';}
    if (d >= 30) {return 'MONTHLY MASTER';}
    if (d >= 14) {return 'TWO WEEK CHAMP';}
    if (d >= 7) {return 'WEEK WARRIOR';}
  ...
  return (
    ...
    >
      ...
      <Box mb="lg">
        <Text fontSize={isStory ? 64 : 48}>
          {'🔥'.repeat(Math.min(Math.floor(days / 7) + 1, 6))}
        </Text>
      </Box>

      {/* Day Count */}
      <Text
        fontSize={isStory ? 120 : 80}
        fontWeight="900"
      ...
      <Text fontSize={isStory ? 16 : 14} color="text.secondary" textAlign="center" style={{ maxWidth: '80%' }}>
        {days >= 30
          ? 'A month of dedication. You are unstoppable!'
          : days >= 7
            ? 'A week of focus. Keep the fire burning!'
 ...
 */
export function VictoryCardWithShare({
  data,
  format = 'square',
}: Omit<VictoryCardProps, 'captureRef'>): JSX.Element {
