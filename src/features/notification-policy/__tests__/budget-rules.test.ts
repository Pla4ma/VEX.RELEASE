...
describe("notification policy — budget rules", () => {
  ...
  it("produces lane-specific copy", () => {
    ...
    expect(project.body).toBe(
      "Your project thread is waiting at the next move.",
    );

    const run = decideNudge({
      lane: "game_like",
      completedSessions: 5,
    ...
    expect(rescue.body).toBe("Do 5 minutes. Stop cleanly. That is enough.");

    const generic = decideNudge({
      lane: "student",
      completedSessions: 2,
      daysSinceOnboarding: 2,
    });
    expect(generic.body).toBe("Pick up with one focused study block. 15 minutes.");
  });
