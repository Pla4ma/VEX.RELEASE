...
import { computeWeeklyIntelligence, hasWeeklyIntelligenceData } from './service';
...
export function useWeeklyIntelligence(input: {
  ...
  const { data: plans, isPending: plansPending } = useStudyOsPlans(input.userId, input.enabled !== false);

  const query = useQuery({
    enabled: Boolean(input.userId) && input.enabled !== false && !plansPending,
    queryFn: () => computeWeeklyIntelligence({ plans, streakDays: input.streakDays }),
...
export function useHasWeeklyIntelligenceData(userId: string | null, enabled = true): boolean {
