...
export function useLaneViewModel(homeModel: HomeModelResult): LaneViewModel {
  ...
  return {
    todayFocusMinutes: sessions.reduce((sum: number, s: { duration?: number }) => sum + (s.duration ?? 0), 0) / 60,
