...
function hydrateChallenges(state: StoredMasteryState): StoredMasteryState {
  ...
  while (nextChallenges.length < 3) {
    const generated = generateMasteryChallenges(state.techniques, state.rank).filter(
...
export const MasteryService = {
  ...
  applySessionXp(userId: string, xpGains: TechniqueXpGains): {
    ...
    if (rankChanged) {
      ...
      spectacleService.triggerSpectacle(
    ...
    const updatedState = persistMasteryState(
      hydrateChallenges(updateChallengeProgress({ ...currentState, techniques, totalMasteryPoints, rank }, xpGains)),
    );
