...
function personalizeMessage(template: string, personalization: {
  userName?: string;
  streakDays?: number;
  sessionCount?: number;
}): string {
...
export async function processNearMissOpportunities(
  repository: {
    fetchUsersWithAtRiskStreaks: () => Promise<Array<{ userId: string; hoursRemaining: number; streakDays: number }>>;
    fetchUsersWithAbandonedSessions: () => Promise<Array<{ userId: string; sessionId: string; progress: number }>>;
  }
