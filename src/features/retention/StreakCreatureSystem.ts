...
export const PersonalityTraitSchema = z.enum([
  'EARLY_BIRD', 'NIGHT_OWL', 'STEADY', 'INTENSE', 'SOCIAL', 'EXPLORER',
]);
...
export class StreakCreatureService {
   ...
   */
  async updateCreatureFromSession(userId: string, sessionData: {
   ...
   */
  async performCareAction(userId: string, action: 'FEED' | 'PLAY' | 'TRAIN' | 'GROOM'): Promise<{
    success: boolean;
   ...
   */
  private calculateStreakIncrement(sessionData: any): number {
   ...
   */
  private calculateEvolutionProgress(creature: StreakCreature, requirements: any): number {
