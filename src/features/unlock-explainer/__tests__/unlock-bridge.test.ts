import { checkRouteSafety, canRegisterFeatureRouteWithSafety, canNavigateToRouteWithSafety } from '../route-safety-bridge';
...
describe('route-safety-bridge', () => {
  ...
  it('merges with FeatureAvailability (both must pass)', () => {
    // Both true → true
    expect(canRegisterFeatureRouteWithSafety(unlocked, false, false, true)).toBe(true);
    expect(canNavigateToRouteWithSafety(unlocked, false, false, true)).toBe(true);

    // Safety false → false even if FA true
    expect(canRegisterFeatureRouteWithSafety(unlocked, true, false, true)).toBe(false);
    // FA false → false even if safety true
    expect(canRegisterFeatureRouteWithSafety(unlocked, false, false, false)).toBe(false);
  });
});
...
describe('completion-bridge', () => {
  ...
  it('hidden by user → blocked with reason preserved', () => {
    const result = buildCompletionUnlock('study_os', 3, 'student', ['study_os']);
