...
export async function skipContract(
  sessionId: string,
  userId: string,
): Promise<void> {
...
export async function getCompletionRate(
  ...
  const rate = await repository.getContractCompletionRate(
    parsedUserId,
    windowDays,
  );
...
export async function getCompletionSignal(
  ...
  const parsedUserId = UserIdSchema.parse(userId);
  const contracts = await repository.getRecentContracts(
    parsedUserId,
    Math.max(3, windowDays * 3),
  );
  const reflectedCount = contracts.filter(
  ...
  return {
    ...
    completedContractCount: reflectedCount,
  };
}
