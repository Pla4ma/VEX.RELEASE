...
describe('focus-contract service', () => {
  ...
  it('creates a contract with a valid task and emits created', async () => {
    ...
    const result = await createContract(
      { sessionId, taskDescription: ' Draft the report intro ' },
      userId,
    );
  ...
  it('rejects empty or oversized contract tasks', async () => {
    await expect(
      createContract({ sessionId, taskDescription: '' }, userId),
    ).rejects.toThrow();
    await expect(
      createContract({ sessionId, taskDescription: 'x'.repeat(81) }, userId),
    ).rejects.toThrow();
    expect(repository.createContract).not.toHaveBeenCalled();
  });

  it.each(['done', 'partial', 'not_done'] as const)(
    'reflects %s and emits reflected',
    async (status) => {
      jest.mocked(repository.reflectOnContract).mockResolvedValue(undefined);

      await reflectOnContract(contractId, status, userId);

      expect(repository.reflectOnContract).toHaveBeenCalledWith(
        contractId,
        status,
      );
  ...
  it('returns repository completion rate when contract history exists', async () => {
    jest
      .mocked(repository.getRecentContracts)
      .mockResolvedValue([{ ...contract, completionStatus: 'done' }]);
