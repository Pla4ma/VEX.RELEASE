...
import type { CompanionPromise, CompanionPromiseInsert, CompanionPromiseUpdate, CreateCompanionPromiseInput } from './types';
...
function mapRow(row: unknown): CompanionPromise {
  ...
  return CompanionPromiseSchema.parse({
    ...
    status: DomainStatusByDatabaseStatus[parsed.status as keyof typeof DomainStatusByDatabaseStatus] ?? 'pending',
...
async function updatePromiseById(promiseId: string, patch: CompanionPromiseUpdate): Promise<CompanionPromise> {
...
export async function createPromise(input: CreateCompanionPromiseInput): Promise<CompanionPromise> {
  try {
    ...
    if (error instanceof CompanionPromiseRepositoryError || error instanceof z.ZodError) {
      throw error;
    }
    throw new CompanionPromiseRepositoryError('create', error);
  }
...
export async function getRecentPromises(userId: string, limit = 3): Promise<CompanionPromise[]> {
  const { data, error } = await getSupabaseClient()
    .from('companion_promises')
    .select('*')
    .eq('user_id', userId)
  ...
  return z.array(CompanionPromiseRepositoryRowSchema).parse(data ?? []).map(mapRow);
}

export async function getPendingPromise(userId: string): Promise<CompanionPromise | null> {
  const promises = await getRecentPromises(userId, 5);
  return promises.find((promise) => promise.status === 'pending') ?? null;
}

export async function replacePromise(promiseId: string): Promise<CompanionPromise> {
...
export async function markPromiseMissed(promiseId: string, missedAt: string): Promise<CompanionPromise> {
  return updatePromiseById(promiseId, { missed_at: missedAt, status: 'missed' });
}

export async function dismissRecoveryPromise(promiseId: string): Promise<CompanionPromise> {
  return updatePromiseById(promiseId, { status: 'skipped' });
}
