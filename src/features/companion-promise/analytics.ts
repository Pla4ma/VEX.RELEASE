...
export function trackPromiseCreated(promise: CompanionPromise): void {
  track(CompanionPromiseAnalyticsEvents.CREATED, 'Companion promise created', promise);
}

export function trackPromiseViewed(promise: CompanionPromise, surface: string): void {
  track(CompanionPromiseAnalyticsEvents.VIEWED, 'Companion promise viewed', promise, { surface });
}

export function trackPromiseFulfilled(promise: CompanionPromise): void {
  track(CompanionPromiseAnalyticsEvents.FULFILLED, 'Companion promise fulfilled', promise);
}

export function trackPromiseMissed(promise: CompanionPromise): void {
  track(CompanionPromiseAnalyticsEvents.MISSED, 'Companion promise missed', promise);
}

export function trackPromiseRecovered(promise: CompanionPromise): void {
  track(CompanionPromiseAnalyticsEvents.RECOVERED, 'Companion promise recovery started', promise);
}
