import {
  type CompleteReflectionInput, CompleteReflectionInputSchema,
  type CreateFocusSessionInput, CreateFocusSessionInputSchema,
  type StartRescueInput, StartRescueInputSchema,
  type StartSessionInput, StartSessionInputSchema,
  type UpdateLaneOverrideInput, UpdateLaneOverrideInputSchema,
...
import { createStarterSessionConfig, buildLaneSessionBrief } from '../session-start/service';
...
export { vexScheduleFocusWindow, vexCreateStudyBlock, vexUpdateProjectThread, vexReadMemorySummary } from './service-async';
...
export function vexCreateFocusSession(
  ...
  if (!parsed.success) return validationError('create_focus_session', parsed.error.message);
...
export function vexStartSession(
  ...
  if (!parsed.success) return validationError('start_session', parsed.error.message);
...
export function vexCompleteReflection(
  ...
  if (!parsed.success) return validationError('complete_reflection', parsed.error.message);

  const { summary: inputSummary, lane, reflectionAnswer, isComeback, userId } = parsed.data;
  const now = Date.now();
  ...
  try {
    const fullSummary: SessionSummary = {
      ...
      sessionMode: (inputSummary.sessionMode ?? 'FLOW') as SessionSummary['sessionMode'],
...
export function vexStartRescue(
  ...
  if (!parsed.success) return validationError('start_rescue', parsed.error.message);
...
export function vexUpdateLaneOverride(
  ...
  if (!parsed.success) return validationError('update_lane_override', parsed.error.message);
