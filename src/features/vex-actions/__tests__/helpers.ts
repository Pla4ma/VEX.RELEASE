...
jest.mock('../../focus-run/service', () => ({
  ...
  buildFocusRunDisplay: jest.fn().mockReturnValue({
    ...
    nextAction: 'test', modifiers: [], completedEncounters: 0,
    cleanStarts: 0, recoveryWins: 0, reflectionUpgrades: 0,
    finalGrade: null, weekSummary: 'test',
  }),
}));

jest.mock('../../study-os/service', () => ({
  createManualStudyPlan: jest.fn().mockResolvedValue({
    id: 'plan-1', userId: 'u1', title: 'Test Plan', blocks: [],
    reviewItems: [],
...
jest.mock('../../project-focus/service', () => ({
  completeProjectSession: jest.fn().mockResolvedValue({
    id: 'thread-1', userId: 'u1', projectTitle: 'Test',
    currentObjective: 'Test objective', nextMove: 'Test move',
    lastSessionSummary: 'ok', blocker: null, handoffNote: null,
    openQuestions: [], state: 'active', staleRisk: 'none',
    bestSessionMode: 'CREATIVE', lastTouched: 0, rescuedAt: null,
  }),
}));
...
const validCreateFocusSession = { userId: 'u1', durationMinutes: 25, category: 'focus' };
const validStartSession = { userId: 'u1', lane: 'minimal_normal' as const, durationSeconds: 1500 };
const validStartRescue = { userId: 'u1', lane: 'student' as const, reason: 'unclear' as const };
const validUpdateLaneOverride = { userId: 'u1', manualOverride: 'deep_creative' as const };
const validCompleteReflection = {
  userId: 'u1', lane: 'student' as const, reflectionAnswer: 'I stayed focused', isComeback: false,
  summary: { sessionId: 's1', effectiveDuration: 1500, completionPercentage: 100, interruptions: 0, sessionMode: 'STUDY', status: 'COMPLETED' },
};
...
export {
  ...
  checkFeatureGate, getActionFeatureKey,
