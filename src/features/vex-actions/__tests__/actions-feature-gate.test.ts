...
import {
  GATE_OPEN, GATE_CLOSED, validCreateFocusSession, validStartSession, expectData,
...
describe('VexAction Boundaries', () => {
  ...
  describe('start_session', () => {
    ...
    it('generates lane-adaptive output (not static)', () => {
      ...
      expect(expectData(student.data, 'sessionMode')).not.toBe(expectData(game.data, 'sessionMode'));
    });
  });
});
