    ...
    <Animated.View entering={FadeInDown.duration(400).delay(100)}>
      <Box flexDirection="row" alignItems="center" justifyContent="space-between" px="lg" py="md">
        <Box flexDirection="row" alignItems="center" gap="md" flex={1}>
          <ProfileAvatar avatarUrl={avatarUrl} displayName={displayName} onPressProfile={onPressProfile} />
          <Box gap="xs">
            <Text variant="caption" color="text.secondary" textTransform="uppercase" letterSpacing={0.5}>
              {greeting}
            </Text>
            <Box flexDirection="row" alignItems="center" gap="xs">
              <Text variant="h4" color="text.primary" numberOfLines={1} ellipsizeMode="tail">
                {displayName}
              </Text>
              <CompanionHeaderAvatar mood={companionMood} onPress={onPressCompanion} />
            </Box>
          </Box>
        </Box>
        <Box flexDirection="row" alignItems="center" gap="sm">
          ...
          {onPressNotifications && (
            ...
            >
              <Icon
                ...
                size={24}
                color={hasUnread ? theme.colors.error.DEFAULT : theme.colors.text.secondary}
              />
              {hasUnread && (
                <Box
                  style={{
          ...
          {streakDays > 0 ? (
            <StreakIndicator days={streakDays} hoursRemaining={streakHoursRemaining} />
          ) : null}
        </Box>
      </Box>
    </Animated.View>
