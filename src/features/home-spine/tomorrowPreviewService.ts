...
function getStorage() {
  try {
    ...
    return storage;
  } catch (error) { captureSilentFailure(error, { feature: 'home-spine', operation: 'network-fallback', type: 'network' });
    return null;
  }
}
...
export function computeTomorrowPreview(
  ...
  if (bossData && bossData.healthPercent < 25) {
    candidates.push({
      ...
      priority: 2,
      headline: bossData.canDefeatTomorrow
        ? `${bossData.bossName} Will Fall!`
        : 'Boss Almost Defeated!',
      subtext: bossData.canDefeatTomorrow
        ? 'One good session tomorrow defeats the boss!'
        : `${bossData.bossName} at ${bossData.healthPercent.toFixed(0)}% — squad needs your help!`,
      emoji: '👹',
  ...
  if (rivalData && rivalData.gapMinutes > 0) {
    ...
    candidates.push({
      ...
      subtext: willOvertake
        ? `One session tomorrow puts you ahead of ${rivalData.rivalName}!`
        : `${rivalData.rivalName} is ${rivalData.gapMinutes} min ahead. Catch up tomorrow!`,
      emoji: '⚔️',
  ...
  const fallback: TomorrowPreviewData = {
    ...
    subtext: streakWillContinue
      ? `Keep your ${currentStreakDays + 1}-day streak alive tomorrow!`
