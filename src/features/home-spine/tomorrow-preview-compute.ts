...
function buildPowerHourCandidate(input: ComputeTomorrowPreviewInput): TomorrowPreviewData | null {
...
function buildChallengeCandidate(input: ComputeTomorrowPreviewInput): TomorrowPreviewData | null {
  ...
  return {
    ...
    metadata: { count: challenge.incompleteChallenges, xpAvailable: challenge.xpAvailable },
    priority: 5,
...
function buildFallback(input: ComputeTomorrowPreviewInput): TomorrowPreviewData {
  return {
    emoji: input.streakWillContinue ? '🔥' : '✨',
    headline: input.streakWillContinue ? 'Streak Continues!' : 'New Day, New Focus',
    priority: 6,
...
export function computeTomorrowPreview(input: ComputeTomorrowPreviewInput): TomorrowPreviewData {
  ...
  const preview = candidates.sort((a, b) => a.priority - b.priority)[0] ?? buildFallback(input);
