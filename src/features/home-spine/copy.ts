...
import { HomeReturnReasonStateSchema, type HomeHighlight, type HomeReturnReasonState } from './schemas';
...
function getNextBlockMinutes(remainingMinutes: number): number {
  return Math.max(STREAK_SAVE_MINUTES, Math.min(DEFAULT_FOCUS_BLOCK_MINUTES, remainingMinutes));
}

function getProjectedDailyPercent(todayFocusMinutes: number, nextBlockMinutes: number): number {
  return Math.min(
    100,
    Math.round(((todayFocusMinutes + nextBlockMinutes) / DAILY_ANCHOR_MINUTES) * 100),
  );
}
...
export function buildProgressSignal(input: {
  ...
  if (input.todayFocusMinutes > 0) {
    ...
    const projectedPercent = getProjectedDailyPercent(input.todayFocusMinutes, nextBlockMinutes);
  ...
  return {
    ...
    ctaLabel: 'Open progress',
    eyebrow: 'Daily momentum',
    title: input.isAtRisk ? 'Your streak needs one clean rep today' : 'Today needs one anchor session',
  };
}
