...
export const LevelUpOverlay: React.FC<LevelUpOverlayProps> = ({
  ...
  const getTierColor = (level: number): [string, string] => {
    if (level >= 100) {return ['#FFD700', '#FF6B35'];}
    if (level >= 50) {return ['#9C27B0', '#E91E63'];}
    if (level >= 25) {return ['#2196F3', '#03A9F4'];}
    if (level >= 10) {return ['#4CAF50', '#8BC34A'];}
    ...
    >
      <View style={styles.container}>
        ...
        >
          ...
          {/* Continue Button */}
          <Pressable style={({ pressed }) => [styles.continueButton, pressed && { opacity: 0.8 }]} onPress={onContinue}
