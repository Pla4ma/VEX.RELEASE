...
export function ProgressionDashboard({
  ...
  if (progressionQuery.isPending) {
    return (
      ...
      >
        <Skeleton width="35%" height={16} borderRadius={theme.borderRadius.sm} />
        <Skeleton width="65%" height={28} borderRadius={theme.borderRadius.md} />
        <Skeleton width="100%" height={10} borderRadius={theme.borderRadius.sm} />
        <View style={{ flexDirection: 'row', gap: theme.spacing[3] }}>
          <Skeleton width="48%" height={72} borderRadius={theme.borderRadius.md} />
          <Skeleton width="48%" height={72} borderRadius={theme.borderRadius.md} />
        </View>
      </View>
    );
    ...
    >
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: theme.spacing[3] }}>
        <ProgressionStatCard
          ...
          detail={streak?.isAtRisk ? 'Needs a session today' : 'Protected by focus'}
          tone={streak?.isAtRisk ? 'warning' : 'default'}
        />
        <ProgressionStatCard
          label="XP multiplier"
          value={`x${multiplier.toFixed(2)}`}
          detail={multiplier > 1 ? 'Streak bonus active' : 'Build a 3-day streak'}
