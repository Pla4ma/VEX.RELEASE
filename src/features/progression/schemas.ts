...
export const XpMetadataSchema = z.object({
...
export const LevelUpRecordSchema = z.object({
  level: z.number().min(1),
  achievedAt: z.number(),
...
export const PrestigeInputSchema = z.object({
  userId: z.string().uuid(),
}).strict();

export const PrestigeRewardPreviewSchema = z.object({
  gems: z.number().min(0),
  badge: z.string().min(1),
  seasonalBonus: z.number().min(0),
}).strict();

export const CalculateLevelThresholdInputSchema = z.object({
  level: z.number().min(1),
  baseXp: z.number().positive().default(100),
  growthFactor: z.number().positive().default(1.25),
}).strict();

export const CheckUnlocksInputSchema = z.object({
  userId: z.string().uuid(),
  level: z.number().min(1),
