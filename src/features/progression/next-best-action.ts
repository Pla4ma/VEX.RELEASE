...
export function getNextBestAction(raw: NextBestActionInput): NextBestAction {
  ...
  if (input.completedSessions <= 0) {
    return {
      ...
      description: 'Your first finish wakes up streaks, coach guidance, and the reward loop.',
  ...
  if (input.completedSessions === 1) {
    return {
      ...
      description: 'A second session tells VEX this is becoming a pattern, not a one-off.',
  ...
  if (input.completedSessions === 2) {
    ...
    return {
      ...
      description: 'Three wins is the moment the app opens up without feeling noisy.',
  ...
  return {
    ...
    description: input.currentStreak >= 3
