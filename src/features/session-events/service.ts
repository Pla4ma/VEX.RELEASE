...
function buildPurityPulse(bucket: number, purityScore: number): MidSessionEvent {
  if (purityScore >= 90) {
    return {
  ...
  if (purityScore < 60) {
    return {
      ...
      message: 'The boss is feeding on the drift. One clean minute turns it around.',
      toastType: 'warning',
      haptic: 'warning',
    };
  }
...
function buildBossTaunt(input: EvaluateMidSessionEventInput, bucket: number): MidSessionEvent | null {
...
function buildTimedEvent(input: EvaluateMidSessionEventInput, bucket: number): MidSessionEvent {
  const health = input.bossHealthPercent;
  ...
  if (input.purityScore >= 80 && bucket % 2 === 0) {
    return {
      ...
      type: 'FOCUS_ZONE',
      title: 'Focus zone',
      message: 'You found the rhythm. Keep the app foregrounded and let it stack.',
      toastType: 'success',
      haptic: 'impactLight',
    };
  }
...
export function evaluateMidSessionEvent(input: EvaluateMidSessionEventInput): MidSessionEvent | null {
  ...
  const candidate = buildBossTaunt(validated, bucket) ?? buildTimedEvent(validated, bucket);
