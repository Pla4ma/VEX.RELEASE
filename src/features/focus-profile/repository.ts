...
export async function getStoredFocusProfile(userId: string): Promise<FocusProfile | null> {
  const raw = storage.getString(keyFor(userId));
  if (!raw) return null;
  return FocusProfileSchema.parse(JSON.parse(raw));
}

export async function upsertStoredFocusProfile(profile: FocusProfile): Promise<FocusProfile> {
