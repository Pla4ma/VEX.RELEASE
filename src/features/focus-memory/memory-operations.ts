...
export async function acceptMemory(
  memoryId: string,
  userId: string,
): Promise<FocusMemory> {
...
export async function listActiveMemories(
  ...
  const now = Date.now();
  return (await readMemories(userId)).filter((memory) =>
    isActive(memory, now),
  );
}

export async function listDeletedMemoryHashes(
  userId: string,
): Promise<string[]> {
  const all = await readMemories(userId);
  return all
...
export async function findMemoriesForRecommendation(
  rawInput: MemoryRecommendationInput,
): Promise<FocusMemory[]> {
  const input = MemoryRecommendationInputSchema.parse(rawInput);
  const now = input.now ?? Date.now();
  return (await readMemories(input.userId)).filter((memory) => {
    const typeAllowed = input.types
      ? input.types.includes(memory.type)
      : true;
    return (
      typeAllowed &&
      memory.accepted &&
      memory.confidence >= input.minConfidence &&
      isActive(memory, now)
    );
  });
}
