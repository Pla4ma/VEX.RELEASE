...
import { trackProjectThreadCreated, trackProjectSessionStarted, trackProjectThreadUpdated, trackProjectThreadRescued, trackProjectHandoffStored } from './analytics';
...
export async function createProjectThread(input: {
  ...
  const thread = await upsertStoredProjectThread(ProjectThreadSchema.parse({
...
export function rescueStaleProject(thread: ProjectThread, now = Date.now()): ProjectThread {
...
export function buildProjectSessionBrief(thread: ProjectThread, now = Date.now()): ProjectSessionBrief {
  ...
  return ProjectSessionBriefSchema.parse({
    ...
    successCondition: isRescued ? `Review: ${thread.lastSessionSummary ?? thread.currentObjective}` : thread.nextMove,
    title: isRescued ? `Recover ${thread.projectTitle}` : `Resume ${thread.projectTitle}`,
    warmup: isRescued ? `Lost the thread? Review: ${thread.lastSessionSummary ?? thread.currentObjective}` : thread.lastSessionSummary ? `Review: ${thread.lastSessionSummary}` : null,
  });
}

export function buildProjectResumeBrief(thread: ProjectThread, now = Date.now()): ProjectSessionBrief {
...
export type ProjectMemoryCandidate = {
  content: string;
  metadata: { projectTitle: string; threadId: string; state: ProjectThread['state'] };
  type: 'project_handoff';
};

export function buildProjectMemoryCandidate(thread: ProjectThread): ProjectMemoryCandidate {
...
export async function completeProjectSession(input: {
  ...
  const now = input.now ?? Date.now();
  const updated = await upsertStoredProjectThread(ProjectThreadSchema.parse({
    ...thread,
...
export function refreshProjectThreadState(thread: ProjectThread, now = Date.now()): ProjectThread {
  ...
  return ProjectThreadSchema.parse({
    ...thread,
    staleRisk: risk,
    state: thread.state === 'completed' || thread.state === 'blocked' || thread.state === 'rescued'
      ? thread.state
