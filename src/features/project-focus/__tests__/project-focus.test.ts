...
describe('project-focus service', () => {
  ...
  it('completion stores handoff note', async () => {
    const thread = await createProjectThread({
      currentObjective: 'Write', nextMove: 'Open doc', now: 10,
      projectTitle: 'Essay', userId: 'creative-1',
    });
  ...
  it('next session picks up correct next move', async () => {
    const thread = await createProjectThread({
      currentObjective: 'Make demo', nextMove: 'Record clip', now: 10,
      projectTitle: 'Demo', userId: 'creative-1',
    });
  ...
  it('lost-the-thread rescue creates recovery session', async () => {
    const thread = await createProjectThread({
      currentObjective: 'Compose', nextMove: 'Find motif', now: 10,
      projectTitle: 'Track', userId: 'creative-1',
    });
  ...
  it('ensureProjectThread creates lightweight thread when none exists', async () => {
    const thread = await ensureProjectThread({
      projectTitle: 'Quick Sketch', userId: 'creative-1', now: 10,
    });
  ...
  it('ensureProjectThread returns existing active thread', async () => {
    await createProjectThread({
      currentObjective: 'Build', nextMove: 'Design API', now: 10,
      projectTitle: 'App', userId: 'creative-1',
    });
    const thread = await ensureProjectThread({
      projectTitle: 'Other', userId: 'creative-1', now: 20,
    });
  ...
  it('buildProjectMemoryCandidate includes handoff and next move', async () => {
    const thread = await createProjectThread({
      currentObjective: 'Write docs', nextMove: 'Review', now: 10,
      projectTitle: 'Docs', userId: 'creative-1',
    });
    const updated = await completeProjectSession({
  ...
  it('non-deep_creative home surface is hidden', () => {
    expect(buildProjectHomeSurface({ lane: 'student', thread: null }).hidden).toBe(true);
    expect(buildProjectHomeSurface({ lane: 'game_like', thread: null }).hidden).toBe(true);
  });
  ...
  it('stale project triggers re-entry', async () => {
    ...
    expect(stale.state).toBe('stale');
    expect(buildProjectHomeSurface({ lane: 'deep_creative', thread: stale }).recoveryPrompt)
      .toContain('Re-enter');
  });
});
