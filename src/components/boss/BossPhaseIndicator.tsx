...
type BossPhaseState = { currentPhase: BossPhase | null; previousPhase: BossPhase | null };
...
export const BossPhaseIndicator: React.FC<BossPhaseIndicatorProps> = ({
  ...
  const phaseInfo = useMemo(() => {
    switch (phase) {
      case 'PHASE_1':
        return { name: 'Phase 1: Standard', color: launchColors.hex_38a169, description: 'Normal combat conditions' };
      case 'PHASE_2':
        return { name: 'Phase 2: Enrage', color: launchColors.hex_d69e2e, description: 'Increased pressure - pauses deal damage!' };
      case 'PHASE_3':
        return { name: 'Phase 3: Execute', color: launchColors.hex_e53e3e, description: 'FINAL PUSH! Maintain 90%+ purity or fail!' };
      default:
        return { name: 'Archived', color: launchColors.hex_718096, description: 'Boss combat archived' };
    }
  }, [phase]);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.progressSection}>
        ...
        <View style={styles.healthBarContainer}>
          <View style={styles.healthBarBackground}>
            <Animated.View
              style={[styles.healthBar, { width: `${bossHealthPercent}%`, backgroundColor: phaseInfo.color }]}
            />
          </View>
        ...
        {nextThreshold > 0 && (
          <Text style={styles.nextPhaseText}>{healthToNextPhase.toFixed(1)}% until next phase</Text>
        )}
      </View>
      ...
      {mechanicActive && (
        <View style={styles.mechanicWarning}>
          ...
          {mechanicTimeRemaining && (
            <Text style={styles.mechanicTimer}>{Math.ceil(mechanicTimeRemaining / 1000)}s remaining</Text>
          )}
        </View>
      )}
...
const styles = createSheet({
  ...
  enragedBadge: { backgroundColor: launchColors.rgb_0_0_0_0_3, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4 },
  enragedText: { color: 'white', fontSize: 12, fontWeight: 'bold' },
  progressSection: { padding: 16, borderBottomWidth: 1, borderBottomColor: launchColors.hex_e2e8f0 },
  thresholds: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 },
  thresholdContainer: { alignItems: 'center' },
  thresholdDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: launchColors.hex_e2e8f0 },
  thresholdDotActive: { backgroundColor: launchColors.hex_4a5568 },
  thresholdDotPhase: { width: 12, height: 12, borderRadius: 6, borderWidth: 2, borderColor: launchColors.hex_4a5568 },
  thresholdLabel: { fontSize: 10, color: launchColors.hex_a0aec0, marginTop: 4 },
  ...
  healthBarBackground: { flex: 1, height: 12, backgroundColor: launchColors.hex_e2e8f0, borderRadius: 6, overflow: 'hidden' },
  healthBar: { height: '100%', borderRadius: 6 },
  healthPercent: { fontSize: 12, fontWeight: '600', color: launchColors.hex_4a5568, minWidth: 45 },
  nextPhaseText: { fontSize: 12, color: launchColors.hex_718096, marginTop: 8, fontStyle: 'italic' },
  descriptionSection: { padding: 16, backgroundColor: launchColors.hex_f7fafc, borderBottomWidth: 1, borderBottomColor: launchColors.hex_e2e8f0 },
  description: { fontSize: 14, color: launchColors.hex_4a5568, fontStyle: 'italic' },
  mechanicWarning: { backgroundColor: launchColors.hex_fed7d7, padding: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  mechanicTitle: { fontSize: 12, fontWeight: 'bold', color: launchColors.hex_c53030 },
  mechanicTimer: { fontSize: 14, fontWeight: 'bold', color: launchColors.hex_c53030 },
  tipsSection: { padding: 16 },
  tipsHeader: { fontSize: 12, fontWeight: '600', color: launchColors.hex_718096, marginBottom: 8 },
  tip: { fontSize: 12, color: launchColors.hex_4a5568, marginBottom: 4, lineHeight: 18 },
});
