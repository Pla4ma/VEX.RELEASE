 ...
 interface JourneyMapProps{nodes:JourneyNode[];progress:UserJourneyProgress;onNodePress?:(node:JourneyNode)=>void;onPathSwitch?:(path:JourneyPath)=>void;}export const JourneyMap:React.FC<JourneyMapProps> = ({nodes,progress,onNodePress,onPathSwitch})=>{const{theme} = useTheme(); const paths = useMemo(()=>{const pathGroups:Record<JourneyPath,JourneyNode[]> = {PURITY:[],SPEED:[],SOCIAL:[],BALANCED:[]}; nodes.forEach(node=>{if(node.tier > 0){pathGroups[node.path].push(node);}}); Object.keys(pathGroups).forEach(key=>{pathGroups[key as JourneyPath].sort((a,b)=>a.tier - b.tier);}); return pathGroups;},[nodes]); const currentNode = useMemo(()=>{return nodes.find(n=>n.id === progress.currentNodeId);},[nodes,progress.currentNodeId]); return<ScrollView style={{flex:1,backgroundColor:theme.colors.background.secondary}}horizontal showsHorizontalScrollIndicator={false}>
      <View style={{flexDirection:'row',padding:theme.spacing[4],gap:theme.spacing[3]}}>
        ...
        <View style={{width:60,height:60,borderRadius:30,backgroundColor:theme.colors.text.secondary,justifyContent:'center'as const,alignItems:'center'as const,marginTop:40}}>
          <Text style={{color:'white',fontWeight:'bold'as const,fontSize:12}}>
            START
          </Text>
        </View>

        {}
        <View style={{position:'absolute'as const,left:46,top:100,width:200,height:4,flexDirection:'row'as const,gap:48}}>
        ...
        {}
        {(Object.keys(paths)as JourneyPath[]).map(path=><PathColumn key={path}path={path}nodes={paths[path]}currentNode={currentNode}progress={progress}onNodePress={onNodePress}onPathSwitch={onPathSwitch}/>)}
      </View>
    </ScrollView>;}; interface PathColumnProps{path:JourneyPath;nodes:JourneyNode[];currentNode?:JourneyNode;progress:UserJourneyProgress;onNodePress?:(node:JourneyNode)=>void;onPathSwitch?:(path:JourneyPath)=>void;}const PathColumn:React.FC<PathColumnProps> = ({path,nodes,currentNode,progress,onNodePress,onPathSwitch})=>{const config = PATH_CONFIG[path]; const isCurrentPath = progress.currentPath === path; const nodeSections = useMemo(()=>{const sections:JourneyNode[][] = []; for(let i = 0; i < 5; i++){sections.push(nodes.filter(n=>n.tier > i * 10 && n.tier <= (i + 1) * 10));}return sections;},[nodes]); return<View style={{width:80,alignItems:'center'as const}}>
      {}
      <View style={[{width:'100%',paddingVertical:8,paddingHorizontal:4,borderRadius:8,alignItems:'center'as const,marginBottom:8},{backgroundColor:config.color}]}>
      ...
      {}
      <View style={{gap:8}}>
        {nodeSections.map((sectionNodes:JourneyNode[],sectionIndex:number)=><View key={sectionIndex}style={{gap:4}}>
