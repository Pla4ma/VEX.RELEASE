...
export function generateAccessibleLabel(
  ...
  if (element.progress !== undefined) {parts.push(`, ${Math.round(element.progress)}% complete`);}
  if (element.action) {parts.push(`, double tap to ${element.action}`);}
    ...
    if (element.color && element.backgroundColor) {
      ...
      if (contrast < 4.5) {
        issues.push({
          ...
          suggestion: getAccessibleAlternatives(element.color, element.backgroundColor, 4.5)[0]
            ? `Try color: ${getAccessibleAlternatives(element.color, element.backgroundColor, 4.5)[0]}`
