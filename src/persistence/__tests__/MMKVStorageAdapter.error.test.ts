...
describe('MMKVStorageAdapter error paths', () => {
  ...
  it('handles concurrent write and read gracefully', async () => {
    ...
    await expect(adapter.getJSON<{ value: number }>('payload')).resolves.toEqual({ value: 1 });
  });
  ...
  it('recovers on next read after write failure', async () => {
    ...
    expect(() => adapter.setItemSync('payload', JSON.stringify({ value: 1 }))).toThrow('MMKV write failed');
    adapter.setItemSync('payload', JSON.stringify({ value: 2 }));

    expect(adapter.getJSONSync<{ value: number }>('payload')).toEqual({ value: 2 });
  });
});
