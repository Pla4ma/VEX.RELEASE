...
export function useFeatureFlags(): UseFeatureFlagsReturn {
  ...
  const get = useCallback(
    (key: string): FeatureFlagValue | null => {
      return service.get(key, false as FeatureFlagValue);
    },
    [service],
  );
  const setOverride = useCallback(
    async (key: string, value: FeatureFlagValue): Promise<void> => {
  ...
  return { ...state, isEnabled, get, setOverride, clearOverride, refresh };
}
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        if (mounted) {
          setState({ enabled: false, value: false, loading: false });
        }
      }
    }
    init();
