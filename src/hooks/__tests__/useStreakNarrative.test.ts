...
describe('useStreakNarrative', () => {
  describe('narrative', () => {
    it('returns a streak narrative with title and message', () => {
      ...
      expect(result.current.narrative.title).toBe('Keep Going!');
      expect(result.current.narrative.message).toBe('Your streak is building momentum.');
      expect(result.current.narrative.type).toBe('motivational');
    });
  });
  ...
  describe('riskWarning', () => {
    ...
    it('shows warning when hours > 20 and streak >= 3', () => {
      ...
      expect(result.current.riskWarning!.urgency).toBe('MEDIUM');
      expect(result.current.riskWarning!.headline).toBe('Streak at Risk');
      expect(result.current.riskWarning!.callToAction).toBe('Keep your streak going!');
    });
  });
  ...
  describe('breakRecovery', () => {
    ...
    it('returns recovery message when streak is 0 and maxStreak > 0', () => {
      ...
      expect(result.current.breakRecovery!.title).toBe('Welcome Back!');
      expect(result.current.breakRecovery!.type).toBe('recovery');
    });
