...
export class EventService {
  ...
  private setupEventListeners(): void {
    eventBus.subscribe(
      ...
      (payload: { userId: string; duration: number }) => {
        if (payload.userId === this.userId) {
          this.updateChallengeProgress("SESSION_COUNT", 1);
          this.updateChallengeProgress("SESSION_DURATION", payload.duration);
        }
      },
    );
    eventBus.subscribe(
      ...
      (payload: { userId: string; amount: number }) => {
        if (payload.userId === this.userId) {
          this.updateChallengeProgress("XP_EARNED", payload.amount);
        }
      },
    );
    eventBus.subscribe(
    ...
    eventBus.subscribe(
      ...
      (payload: { userId: string; newLevel: number }) => {
        if (payload.userId === this.userId) {
          this.updateChallengeProgress("LEVEL_REACHED", payload.newLevel);
        }
      },
    );
    eventBus.subscribe("achievement:unlock", (payload: { userId: string }) => {
      if (payload.userId === this.userId) {
        this.updateChallengeProgress("ACHIEVEMENT_UNLOCKED", 1);
      }
    });
  }
  registerEvent(event: Omit<Event, "status" | "currentParticipants">): Event {
    const fullEvent = EventSchema.parse({
      ...event,
      status: this.calculateEventStatus(event.startAt, event.endAt),
  ...
  joinEvent(eventId: string): void {
    if (!this.userId) {
      throw new Error("EventService: No user set");
    }
  ...
  joinChallenge(challengeId: string): void {
    ...
    if (challenge.status === "LOCKED" && challenge.prerequisites) {
      for (const prereqId of challenge.prerequisites) {
        ...
        if (!prereq || !prereq.completedBy.includes(this.userId)) {
          debug.debug("Challenge prerequisite not met: %s", prereqId);
          return;
        }
      }
    }
    if (!challenge.participants.includes(this.userId)) {
      challenge.participants.push(this.userId);
    }
  ...
  private updateChallengeProgress(type: ChallengeType, amount: number): void {
    if (!this.userId) {
      return;
    }
    for (const challenge of this.activeChallenges.values()) {
      ...
      challenge.progressHistory.push({
        timestamp: Date.now(),
        value: challenge.progress,
      });
      this.userProgress.set(`${this.userId}:${challenge.id}`, {
        challengeId: challenge.id,
        progress: challenge.progress,
      });
      if (
        challenge.progress >= challenge.requirement.target &&
        !challenge.completedBy.includes(this.userId)
      ) {
  ...
  private completeChallenge(challengeId: string): void {
    ...
    challenge.completedBy.push(this.userId);
    ...
    if (challenge.eventId) {
      this.checkEventCompletion(challenge.eventId);
    }
  ...
  claimChallengeReward(challengeId: string): void {
    ...
    if (!challenge.completedBy.includes(this.userId)) {
      throw new Error("Challenge not completed");
    }
    if (challenge.claimedBy.includes(this.userId)) {
      throw new Error("Reward already claimed");
    }
    challenge.claimedBy.push(this.userId);
  ...
  private checkEventCompletion(eventId: string): void {
    ...
    const allChallengesComplete = event.challenges.every((challengeId) => {
      const challenge = this.activeChallenges.get(challengeId);
      return challenge && challenge.completedBy.includes(this.userId!);
    });
    if (allChallengesComplete && this.userId) {
  ...
  private calculateEventStatus(startAt: number, endAt: number): EventStatus {
    const now = Date.now();
    if (now < startAt) {
      return "UPCOMING";
    }
  ...
  getActiveChallenges(): Challenge[] {
    return Array.from(this.activeChallenges.values()).filter(
      (c) => c.status === "ACTIVE" && c.participants.includes(this.userId!),
    );
  }
  ...
  getUserChallengeProgress(challengeId: string): number {
    ...
    const progress = this.userProgress.get(`${this.userId}:${challengeId}`);
  ...
  getCompletedChallenges(): Challenge[] {
    ...
    return Array.from(this.activeChallenges.values()).filter((c) =>
      c.completedBy.includes(this.userId!),
    );
  }
  private async loadState(): Promise<void> {
    ...
    try {
      ...
      if (data) {
        ...
        debug.info(
          "Loaded event progress for %d challenges",
          this.userProgress.size,
        );
      }
    } catch (error) {
  ...
  private async saveState(): Promise<void> {
    ...
    try {
      const key = `events:progress:${this.userId}`;
      const data = { userProgress: Object.fromEntries(this.userProgress) };
      const storage = getMMKVStorageAdapter();
