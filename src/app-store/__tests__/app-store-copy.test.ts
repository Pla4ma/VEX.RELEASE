...
describe('app-store-copy', () => {
  ...
  it('opening description mentions adaptive system', () => {
    expect(APP_STORE_METADATA.description).toContain('VEX changes based on how you work');
  ...
  it('no "second brain" claim unless memory UI proves it', () => {
    expect(APP_STORE_METADATA.description.toLowerCase()).not.toContain('second brain');
  });
});
