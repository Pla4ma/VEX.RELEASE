...
describe('PRIVACY_NUTRITION_LABEL', () => {
  ...
  it('should declare data linked to user', () => {
    ...
    expect(PRIVACY_NUTRITION_LABEL.dataLinkedToUser).toContain('Contact Info (Email)');
    expect(PRIVACY_NUTRITION_LABEL.dataLinkedToUser).toContain('Identifiers (User ID)');
    expect(PRIVACY_NUTRITION_LABEL.dataLinkedToUser).toContain('Usage Data (Sessions, Focus Score, streaks)');
    expect(PRIVACY_NUTRITION_LABEL.dataLinkedToUser).toContain('Purchases (Subscription status)');
  });

  it('should declare diagnostics not linked to user', () => {
    expect(PRIVACY_NUTRITION_LABEL.dataNotLinkedToUser).toContain('Diagnostics (Crash logs, performance)');
  });
