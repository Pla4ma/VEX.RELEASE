    ...
    switch (toast.type) {
      ...
      case "warning":
        return {
          ...
          icon: toast.icon ?? "alert-triangle",
        };
      case "info":
      default:
        return {
  ...
  return (
    <>
      {(Object.keys(toastsByPosition) as ToastPosition[]).map((position) => (
        ...
        >
          {toastsByPosition[position].map((toast, index) => (
            <ToastComponent
              key={toast.id}
              toast={toast}
              onDismiss={onDismiss}
...
export { ToastComponent as Toast, ToastContainerComponent as ToastContainer };
