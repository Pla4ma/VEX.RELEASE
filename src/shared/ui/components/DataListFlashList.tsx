/**
 * DataList Component (FlashList Version)
 *
 * High-performance list using FlashList for smooth 60fps scrolling.
 * Supports selection, infinite scroll, pull-to-refresh, and sections.
 *
...
const PullToRefreshIndicator: React.FC<{ progress: number; theme: any }> = ({
  progress,
  theme,
}) => {
...
const ListFooter: React.FC<{ loading: boolean; hasMore?: boolean; theme: any }> = ({
  loading,
  hasMore = true,
  theme,
}) => {
...
export function DataList<T extends Record<string, unknown>>({
  ...
  const renderItemWrapper: ListRenderItem<DataListItem<T>> = useCallback(
    ({ item, index }) => {
    ...
    [renderItem, effectiveSelectedIds, handleSelect, handleItemPress, getItemAccessibilityLabel, theme]
  );

  const resolvedKeyExtractor = useCallback(
    (item: DataListItem<T>, index: number) => {
      return keyExtractor ? keyExtractor(item.data, index) : item.id;
    },
    [keyExtractor]
  );
...
}> = ({ selectedCount, totalCount, onClear, onSelectAll, actions }) => {
  const { theme } = useTheme();

  return (
    <View style={[styles.toolbar, { backgroundColor: theme.colors.background.secondary }]}>
          ...
          {actions.map((action, index) => (
            <Pressable
              ...
              style={[
                ...
                action.destructive && { backgroundColor: theme.colors.error.DEFAULT + '20' },
              ]}
