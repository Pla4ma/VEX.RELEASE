...
export function assignUserToExperiment(
  ...
  // Check targeting
  if (!isUserEligible(experiment, userProfile)) {return null;}
...
function isUserEligible(
  ...
  profile: { sessions: number; isPremium: boolean; platform: string; segment: string }
): boolean {
  const { targeting } = experiment;

  // Check session count
  if (targeting.minSessions && profile.sessions < targeting.minSessions) {return false;}
  if (targeting.maxSessions && profile.sessions > targeting.maxSessions) {return false;}

  // Check premium status
  if (targeting.premiumStatus === 'free' && profile.isPremium) {return false;}
  if (targeting.premiumStatus === 'premium' && !profile.isPremium) {return false;}

  // Check platform
  if (targeting.platforms && !targeting.platforms.includes(profile.platform as never)) {return false;}

  // Check segment
  if (targeting.userSegments && !targeting.userSegments.includes(profile.segment)) {return false;}
...
function selectVariant(allocation: Record<string, number>): string | null {
  ...
  for (const [variantId, percentage] of Object.entries(allocation)) {
    ...
    if (random <= cumulative) {return variantId;}
  }
