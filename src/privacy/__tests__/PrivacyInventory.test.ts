...
describe('PrivacyInventory', () => {
  it('returns the launch privacy categories including notifications and coach memory', () => {
    expect(getDataCategories().map((item) => item.category)).toEqual(
      expectedCategories,
    );
  });
  ...
  it('declares no tracking fields for launch', () => {
    ...
    expect(
      PRIVACY_INVENTORY.every((item) => item.usedForTracking === false),
    ).toBe(true);
  });

  it('includes coach conversation history in User Content', () => {
    const userContent = PRIVACY_INVENTORY.find(
      (item) => item.category === 'User Content',
    );
  ...
  it('includes lane recommendation in Usage Data', () => {
    const usageData = PRIVACY_INVENTORY.find(
      (item) => item.category === 'Usage Data',
    );
  ...
  it('includes Notification Tokens category', () => {
    const notifTokens = PRIVACY_INVENTORY.find(
      (item) => item.category === 'Notification Tokens',
    );
