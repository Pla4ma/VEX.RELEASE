...
class FeatureFlagEngine {
  ...
  private evaluateFlag(flag: FeatureFlagConfig): FlagEvaluation {
    ...
    if (flag.rolloutPercentage !== undefined && this.userContext) {
      const userBucket = this.getUserBucket(this.userContext.userId, flag.key);
      if (userBucket <= flag.rolloutPercentage) {
        value = flag.defaultValue;
        source = 'local';
      } else {
        value = typeof flag.defaultValue === 'boolean' ? !flag.defaultValue : flag.defaultValue;
        source = 'default';
      }
    }
  ...
  private checkVersion(current: string, constraint: string): boolean {
    ...
    for (let i = 0; i < Math.max(currentParts.length, constraintParts.length); i++) {
      ...
      if (currentPart > constraintPart) { return true; }
      if (currentPart < constraintPart) { return false; }
    }
  ...
  /**
   * Get inverse value for disabled features
   */
  private getInverseValue(value: FeatureFlagValue): FeatureFlagValue {
