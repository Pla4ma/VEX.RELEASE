...
class EventEmitter<Events extends object> {
  private listeners: Map<keyof Events, Array<(data: unknown) => void>> = new Map();
    ...
    if (callbacks) {
      callbacks.forEach((cb) => {
        try {
          ...
          captureException(
            e instanceof Error ? e : new Error(`Event listener error for ${String(event)}`),
...
// Content Study specific event emitter
      ...
      try {
        ...
        if (result instanceof Promise) {
          result.catch((error: unknown) => {
            captureException(
              error instanceof Error ? error : new Error('Async event handler error'),
...
/**
 * Integrates content study events with the main app event bus
 * Call this during app initialization
 */
export function initializeContentStudyEventIntegration(
  appEventBus?: {
