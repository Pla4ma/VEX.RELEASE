    ...
    <View style={styles.container}>
      ...
      >
        ...
        {value.length > 0 && !disabled && (
          <Pressable onPress={clearInput} style={({ pressed }) => [styles.clearButton, pressed && { opacity: 0.8 }]}
        ...
        <View style={styles.messageContainer}>
          {validationState.errors.map((error, index) => (
            <View key={index} style={styles.messageRow}>
              ...
              <Text style={[styles.messageText, { color: theme.colors.error[500] }]}>
                {error}
              </Text>
            </View>
          ))}
        ...
        <View style={styles.messageContainer}>
          {validationState.warnings.map((warning, index) => (
            <View key={index} style={styles.messageRow}>
              ...
              <Text style={[styles.messageText, { color: theme.colors.warning[500] }]}>
                {warning}
              </Text>
            </View>
          ))}
        ...
        >
          {isExtracting ? (
            <View style={styles.extractingContainer}>
              ...
              <Text style={[styles.extractingText, { color: theme.colors.text.secondary }]}>
            ...
            <>
              ...
              <View style={styles.previewInfo}>
                ...
                {videoInfo.channelName && (
                  <Text style={[styles.channelName, { color: theme.colors.text.secondary }]}>
                    {videoInfo.channelName}
                  </Text>
                )}
                {videoInfo.duration && (
          ...
          {extractionError && (
            <View style={styles.extractionErrorContainer}>
              ...
              <Text style={[styles.extractionErrorText, { color: theme.colors.error[500] }]}>
                {extractionError}
              </Text>
              {onExtract && (
                <Button variant="ghost" size="sm" onPress={onExtract}
  ...
  accessibilityHint="Activates this control">
      ...
      {!videoInfo && !isExtracting && validationState.isValid && (
        <Text style={[styles.helpText, { color: theme.colors.text.muted }]}>
          We'll extract the video's transcript and create study materials from it.
        </Text>
      )}
    </View>
  );
