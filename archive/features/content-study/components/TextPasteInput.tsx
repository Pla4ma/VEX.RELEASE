...
export const TextPasteInput: React.FC<TextPasteInputProps> = ({
  value,
  onChange,
  onValidationChange,
  onAutoSave,
      ...
      {showCharacterCount && (
        <View style={styles.footer}>
          ...
          >
            ...
            {showMinLengthIndicator && characterCount < CONTENT_STUDY_CONSTANTS.MIN_PASTE_LENGTH
              ? ` / min ${CONTENT_STUDY_CONSTANTS.MIN_PASTE_LENGTH}`
              : ''}
      ...
      {errors.length > 0 && (
        <View style={styles.validationContainer}>
          {errors.map((error, index) => (
            <Text
              key={`error-${index}`}
              style={[styles.validationText, { color: theme.colors.error[500] }]}
            >
              {error.message}
            </Text>
          ))}
      ...
      {errors.length === 0 && warnings.length > 0 && (
        <View style={styles.validationContainer}>
          {warnings.slice(0, 2).map((warning, index) => (
            <Text
              key={`warning-${index}`}
              style={[styles.validationText, { color: theme.colors.warning[500] }]}
            >
              {warning.message}
            </Text>
          ))}
