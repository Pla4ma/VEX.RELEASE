...
export class ContentStudyErrorHandler {
  ...
  private errorCallbacks: Map<ContentStudyErrorCode, Array<(error: ContentStudyError) => void>> = new Map();
  ...
  private triggerCallbacks(error: ContentStudyError): void {
    const callbacks = this.errorCallbacks.get(error.code as ContentStudyErrorCode);
    if (callbacks) {
      callbacks.forEach((cb) => {
        try {
          ...
          captureException(
            e instanceof Error ? e : new Error('Content study error callback failed'),
...
export async function executeRecoveryAction(
  ...
  switch (action) {
    ...
    case 'reupload':
      // Navigate back to input with error state
...
export const DefaultRetryStrategy: RetryStrategy = {
  ...
  shouldRetry: (error, attempt) => {
    if (attempt >= CONTENT_STUDY_CONSTANTS.MAX_RETRY_ATTEMPTS) {return false;}
...
export const ExponentialBackoffStrategy: RetryStrategy = {
  ...
  shouldRetry: (error, attempt) => {
    ...
    return error.code === ContentStudyErrorCode.NETWORK_ERROR ||
