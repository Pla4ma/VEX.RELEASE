...
const PersonaSchema = z.enum(['CHEERLEADER', 'DRILL_SERGEANT', 'FRIEND', 'MENTOR', 'RIVAL', 'MINDFUL']);
const AIRequestTypeSchema = z.enum(['GENERATE_COACH_MESSAGE', 'GENERATE_SESSION_SUMMARY', 'GENERATE_COMEBACK_PROMPT', 'GENERATE_STREAK_RISK_NUDGE', 'GENERATE_WEEKLY_REFLECTION']);
...
const CoachContextSchema = z.object({
  category: z.enum(['STREAK_RISK', 'SESSION_SUGGESTION', 'MILESTONE_HYPE', 'COMEBACK_SUPPORT', 'POST_FAILURE', 'PROGRESS_REMINDER', 'DIFFICULTY_ADJUST', 'CHALLENGE_PROMPT', 'MOTIVATION_BOOST', 'BREAK_SUGGESTION', 'OVERLOAD_WARNING']),
  ...
  recentSessionOutcomes: z.array(z.object({
    score: z.number(),
...
serve(async (request) => {
  ...
  if (request.method !== 'POST') {
    return respond(buildError('GENERATE_COACH_MESSAGE', Date.now(), 'INVALID_REQUEST', 'Method not allowed', false), 405);
  }
  ...
  if (!parsed.success) {
    return respond(buildError(readRequestType(body), startedAt, 'INVALID_REQUEST', 'Invalid AI request payload', false), 400);
  }

  const apiKey = Deno.env.get('GEMINI_API_KEY');
  if (!apiKey) {
    return respond(buildError(parsed.data.requestType, startedAt, 'GEMINI_API_ERROR', 'Missing Gemini configuration', true), 200);
  }

  if (parsed.data.requestType === 'GENERATE_COACH_MESSAGE') {
    const rateLimited = await isRateLimited(parsed.data.userId);
    if (rateLimited) {
      return respond(buildError(parsed.data.requestType, startedAt, 'GEMINI_RATE_LIMIT', 'Hourly AI coach message limit reached', true), 200);
    }
  }
  ...
  try {
    ...
    return respond(buildSuccess(parsed.data, gemini, startedAt), 200);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Gemini request failed';
    return respond(buildError(parsed.data.requestType, startedAt, 'GEMINI_API_ERROR', message, true), 200);
  }
});
...
function buildPrompt(request: AIRequest): { system: string; user: string; maxOutputTokens: number } {
  if (request.requestType === 'GENERATE_COACH_MESSAGE') {
    ...
    const recentOutcomes = (request.context.recentSessionOutcomes ?? []).map((entry, index) => `#${index + 1}: score=${entry.score}, quality=${entry.focusQuality ?? entry.score}, duration=${entry.durationMinutes ?? 0}m`).join('; ') || 'none';
    return {
...
async function isRateLimited(userId: string): Promise<boolean> {
  ...
  const { count, error } = await supabase.from('coach_messages').select('id', { count: 'exact', head: true }).eq('user_id', userId).gte('created_at', oneHourAgo);
  if (error) {
    return false;
  }
  return (count ?? 0) >= 5;
...
function buildError(requestType: z.infer<typeof AIRequestTypeSchema>, startedAt: number, code: string, message: string, retryable: boolean) {
  return {
    success: false,
    requestType,
    content: '',
    structuredData: requestType === 'GENERATE_COACH_MESSAGE' ? { message: '', tone: 'calm', urgency: 'low' } : {},
    metadata: { model: 'gemini-1.5-flash', processingTimeMs: Date.now() - startedAt, cached: false },
...
function readRequestType(rawBody: unknown): z.infer<typeof AIRequestTypeSchema> {
  if (typeof rawBody !== 'object' || rawBody === null || !('requestType' in rawBody)) {
