-- EDGE-2: Add session ownership verification to complete_session RPC
-- Prevents user A from completing a session owned by user B
-- Migration: 20260606_session_ownership_check.sql
...
DECLARE
  ...
  v_session_owner_exists BOOLEAN;
  v_streak RECORD;
  v_xp_result JSONB;
  v_total_xp INTEGER;
  v_coins INTEGER := 0;
...
BEGIN
  ...
  -- === CLAMP CLIENT-PROVIDED VALUES ===
  p_final_score := LEAST(p_final_score, 10000);
  p_mode_bonus := LEAST(p_mode_bonus, 500);

  -- === SESSION OWNERSHIP VERIFICATION ===
  -- Verify the session belongs to the authenticated user before any processing
  SELECT EXISTS(
    SELECT 1 FROM public.sessions
    WHERE id = p_session_id AND user_id = p_user_id
  ) INTO v_session_owner_exists;

  IF NOT v_session_owner_exists THEN
