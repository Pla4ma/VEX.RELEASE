...
export function CompanionDetailScreen(): JSX.Element {
  ...
  if (loadState.status === 'loading') {
    return (
      <Box flex={1} bg="background.primary" p="lg" justifyContent="center">
        <Box height={28} width={180} borderRadius="sm" bg={theme.colors.background.tertiary} />
        <Box mt={4} height={120} borderRadius="lg" bg={theme.colors.background.secondary} />
      </Box>
    );
  }
  ...
  if (loadState.status === 'error') {
    return (
      <Box flex={1} bg="background.primary" p="lg" justifyContent="center">
        <Text variant="h4" color={theme.colors.error.DEFAULT}>Companion details did not load.</Text>
        <Text variant="body" color="text.secondary" mt={2}>
          VEX kept the session safe. Retry the companion profile.
        </Text>
        <Button variant="secondary" onPress={load} style={{ marginTop: theme.spacing[4] }}
  ...
  return (
    <Box flex={1} bg="background.primary" p="lg" justifyContent="center">
      <Text variant="label" color={theme.colors.primary[400]}>Living Companion</Text>
      <Text variant="h2" color="text.primary" mt={2}>{companion.currentMood}</Text>
      <Box mt={5} p={4} borderRadius="lg" bg={theme.colors.background.secondary}>
        <Text variant="h4" color="text.primary">{companion.phase} - Level {companion.level}</Text>
        <Text variant="body" color="text.secondary" mt={2}>
