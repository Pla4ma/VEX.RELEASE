/**
 * Streak Funeral Screen
 *
 * Shown when a user's streak has broken within the last 48 hours.
 * Acknowledges the loss and encourages starting fresh.
 * PHASE 8: Enhanced with dying flame animation and gem restore option.
 */
...
function calculateRestoreCost(streakDays: number): number {
  if (streakDays < 7) {return RESTORE_COSTS.UNDER_7_DAYS;}
  if (streakDays < 30) {return RESTORE_COSTS.DAYS_7_TO_29;}
  return RESTORE_COSTS.DAYS_30_PLUS;
}

type StreakFuneralRoute = RouteProp<ExtendedRootStackParams, 'StreakFuneral'>;
type StreakFuneralNavigation = NativeStackNavigationProp<
  ExtendedRootStackParams,
  'StreakFuneral'
>;

function DyingFlame(): JSX.Element {
  const flameScale = useSharedValue(1);
  const flameOpacity = useSharedValue(1);

  // Animate scale from 1.0 → 0 over 2 seconds
  React.useEffect(() => {
...
export const StreakFuneralScreen: React.FC = () => {
  ...
  const restoreCost = useMemo(() => calculateRestoreCost(previousStreak), [previousStreak]);

  // Get actual gem balance from economy hook
  ...
  const handleReminisce = useCallback(() => {
    Sentry.addBreadcrumb({
      category: 'streaks',
      message: 'User chose to view streak history',
      level: 'info',
    });
    // Could navigate to streak history here
    completeFuneral();
  }, [completeFuneral]);
        ...
        <Animated.View entering={FadeInUp.delay(1000)}>
          <Box gap="md">
            ...
            <Button
              ...
              accessibilityLabel={`Restore streak for ${restoreCost} gems`}
              accessibilityRole="button"
              accessibilityHint={`Protects your ${previousStreak}-day streak for ${restoreCost} gems`}
            >
              Restore for {restoreCost} 💎
            </Button>

            {/* Show insufficient gems message if needed */}
            {gemBalance < restoreCost && (
            ...
            {/* Secondary CTA: Start fresh with comeback */}
            <Button
              variant="secondary"
              size="md"
              fullWidth
              onPress={handleStartFresh}
              accessibilityLabel="Start fresh and begin comeback quest"
              accessibilityRole="button"
              accessibilityHint="Begins a new streak with bonus XP for your first sessions"
            >
              Start fresh — begin comeback
            </Button>

            {/* Tertiary CTA: View history */}
            <Button
              variant="ghost"
              size="sm"
              fullWidth
              onPress={handleReminisce}
              accessibilityLabel="View Streak History button"
              accessibilityRole="button"
