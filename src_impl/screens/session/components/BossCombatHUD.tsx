/**
 * BossCombatHUD Component
 *
 * HUD for active boss combat during session.
 * Shows boss health, phase, attack warnings, and ability button.
 *
 * @phase 2
 */
...
const ATTACK_NAMES: Record<string, string> = {
  ...
  SOCIAL_MEDIA_TRAP: '🕸️ Social Media Trap',
  MULTITASKING_TEMPEST: '🌪️ Multitasking Tempest',
};
...
export function BossCombatHUD({
  ...
  const phaseColors = PHASE_COLORS[currentPhase] ?? PHASE_COLORS.CALM;

  // Select best available ability
  const availableAbility = useMemo(() => {
    return COMBAT_ABILITIES.find(
      (a) =>
        userStreakDays >= a.requiresStreak &&
        a.focusEnergyCost <= 50 && // Can afford
        a.cooldownSeconds <= 15 // Quick cooldown
    ) ?? COMBAT_ABILITIES[0]; // Fallback to Focus Strike
  }, [userStreakDays]);
  ...
  useEffect(() => {
    ...
    if (cooldownEnd > now) {
      ...
      cooldownProgress.value = withTiming(1, { duration: remaining });
    } else {
  ...
  const handleActivateAbility = useCallback(() => {
    ...
    // Mock encounter for local execution (real would come from props/context)
    const mockEncounter = {
      id: encounterId,
      userId,
      bossId: 'active-boss',
    ...
    const result = executeCombatAbility(mockEncounter, availableAbility.id, userStreakDays, now);
    ...
    if (result.success) {
      ...
      // Track analytics
      trackCombatAbilityActivated(userId, encounterId, availableAbility.id, result.damageDealt, result.comboBonus > 0);
    } else {
      haptics.error();
    }
  }, [encounterId, userId, bossHealth, bossMaxHealth, currentPhase, currentAttackPattern, availableAbility, userStreakDays, cooldownEnd, haptics]);
    ...
    <View style={containerStyle}>
      ...
      <Animated.View style={[abilityRowStyle, cooldownStyle]}>
        <Button
          variant="primary"
          size="md"
          onPress={handleActivateAbility}
          disabled={isPaused || isOnCooldown}
          style={abilityButtonStyle}
          accessibilityLabel={`Activate ${availableAbility.name}`}
