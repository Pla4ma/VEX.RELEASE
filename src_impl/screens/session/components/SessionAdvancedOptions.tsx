...
export function SessionAdvancedOptions({
  ...
  accessibilityHint="Activates this control">
        <Box flexDirection="row" alignItems="center" py="md">
          ...
          <Box ml="sm">
            <Icon name={showAdvanced ? 'chevron-up' : 'chevron-down'} size="sm" color={theme.colors.text.secondary} />
          </Box>
        </Box>
      </Pressable>
        ...
        <Animated.View entering={FadeInDown.duration(250)}>
          <Box p="lg" bg="background.secondary" borderRadius="lg" mb="lg">
            ...
            <Box flexDirection="row" justifyContent="space-between" mb="md">
              ...
              <Text variant="body" fontWeight="600">{`${Math.round(selectedDurationSeconds / 60)} min`}</Text>
            </Box>
            <Box flexDirection="row" justifyContent="space-between" mb="md">
              <Text variant="body">Intervals</Text>
              <Text variant="body" fontWeight="600">{selectedPreset.intervals}</Text>
            </Box>
            <Box flexDirection="row" justifyContent="space-between" mb="md">
              <Text variant="body">Break Duration</Text>
              <Text variant="body" fontWeight="600">{`${Math.round(selectedPreset.breakDuration / 60)} min`}</Text>
            </Box>
            <Box flexDirection="row" justifyContent="space-between">
              <Text variant="body">Strict Mode</Text>
              <Text variant="body" fontWeight="600">{selectedPreset.strictMode ? 'On' : 'Off'}</Text>
            </Box>
          </Box>
        </Animated.View>
      ) : null}
