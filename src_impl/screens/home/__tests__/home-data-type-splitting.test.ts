...
describe('HomeData type splitting — no unsafe casts in NewUser/Activating', () => {
  ...
  it('NewUserHomeData type has intervention: null', () => {
    ...
    const nextInterfaceIndex = source.indexOf('interface A', newUserDataIndex + 1);
    const section = source.slice(newUserDataIndex, nextInterfaceIndex > newUserDataIndex ? nextInterfaceIndex : undefined);
  ...
  it('ActivatingHomeData type has intervention: null', () => {
    ...
    const nextInterfaceIndex = source.indexOf('interface E', activatingIndex + 1);
    const section = source.slice(activatingIndex, nextInterfaceIndex > activatingIndex ? nextInterfaceIndex : undefined);
    expect(section).toContain('intervention: null');
    expect(section).toContain('interventionLoading: false');
  });
