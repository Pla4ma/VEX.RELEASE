...
export function buildCompletionHeadline(params: {
  ...
  if (params.wasStudySession) {
    return { headline: 'Study progress locked in.', subheadline: 'Your effort moved your understanding forward.', tone: 'calm' };
  }
  if (params.dailyGoalComplete) {
    return { headline: 'Daily goal complete.', subheadline: "You showed up and followed through. That's the core of progress.", tone: 'celebratory' };
  }
  if (params.challengeCompleted) {
    return { headline: 'Challenge complete.', subheadline: 'You finished what you set out to do today.', tone: 'celebratory' };
  }
  if (params.bossDamaged) {
    return { headline: 'Progress made.', subheadline: 'You pushed your current challenge forward.', tone: 'calm' };
  }
  if (params.streakProtected) {
    return { headline: 'Streak protected.', subheadline: 'You kept your momentum alive today.', tone: 'calm' };
  }
  return { headline: 'You showed up.', subheadline: 'One focused session is enough to build momentum.', tone: 'calm' };
}
