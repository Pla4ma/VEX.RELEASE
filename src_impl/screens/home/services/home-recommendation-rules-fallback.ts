...
export const FALLBACK_RECOMMENDATION_RULES: RecommendationRule[] = [
  {
    ...
    priority: 60,
    condition: (ctx) => ctx.totalSessions <= 3 && (!ctx.streak || ctx.streak.currentDays === 0),
    generate: () => ({
  ...
  },
  {
    ...
    priority: 55,
    condition: (ctx) => {
      if (!ctx.streak || ctx.streak.currentDays !== 0 || ctx.totalSessions < 5) {
        return false;
      }
      return ctx.lastSessionTimestamp ? getDaysSince(ctx.lastSessionTimestamp) < 7 : false;
    },
    generate: () => ({
