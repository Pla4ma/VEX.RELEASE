 ...
 * Search Screen
 *
 * Premium search screen with filtering, recent searches, and categories.
 */
...
// Mock search results
const MOCK_RESULTS = [
  { id: '1', type: 'session', title: 'Morning Meditation', subtitle: '10 min • Beginner', icon: 'play' },
  { id: '2', type: 'challenge', title: '7-Day Mindfulness', subtitle: 'Challenge • 1.2k participants', icon: 'trophy' },
  { id: '3', type: 'user', title: 'Sarah Johnson', subtitle: 'Level 24 • 45 day streak', icon: 'user' },
  { id: '4', type: 'content', title: 'Focus Techniques Guide', subtitle: 'Article • 5 min read', icon: 'file' },
];
...
export const SearchScreen: React.FC = () => {
  ...
  const renderSearchBar = () => (
    <Box px={16} pb={12} pt={insets.top + 16} style={{ backgroundColor: theme.colors.background.primary }}>
      <Box flexDirection="row" alignItems="center" height={48} borderRadius={12} px={12} style={{ backgroundColor: theme.colors.background.secondary }}>
        ...
        <TextInput
          style={{ flex: 1, fontSize: 16, height: '100%', color: theme.colors.text.primary }}
        ...
        renderItem={({ item }: { item: SearchCategory }) => (
          <Pressable
            style={{
              ...
              paddingVertical: 8,
              borderRadius: 20,
              backgroundColor: activeCategory === item.id ? theme.colors.primary[500] : theme.colors.background.secondary,
            }}
    ...
    return (
      <FlashList
        ...
        renderItem={({ item }: { item: SearchResult }) => (
          <Card style={{ flexDirection: 'row', alignItems: 'center' } as ViewStyle} size="md" onPress={() => {}}>
            <Box
              width={44}
              height={44}
              borderRadius={12}
        ...
        <Box flex={1} p={16}>
          ...
          <Box mt={8}>
            ...
            <Box flexDirection="row" flexWrap="wrap" gap={8}>
              {['mindfulness', 'productivity', 'sleep', 'focus', 'meditation', 'yoga'].map((tag) => (
                <Pressable
                  key={tag}
                  style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 16, backgroundColor: theme.colors.background.secondary }}
                  onPress={() => {
                    setQuery(tag);
                    handleSearch();
                  }}
