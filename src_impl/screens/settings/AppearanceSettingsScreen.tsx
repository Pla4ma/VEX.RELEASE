 ...
 *
 * Phase 14.3 - Complete appearance customization.
 *
 * Features:
 * - Theme selector (Dark, Light, System)
 * - Accent color picker (8 colors)
 * - Font size selector (Small, Medium, Large)
 * - Timer display format selector
...
type TimerFormat = 'countdown' | 'countup';
type AccentColor = 'indigo' | 'purple' | 'blue' | 'green' | 'orange' | 'pink' | 'red' | 'teal';
...
const TIMER_FORMAT_OPTIONS: TimerFormatOption[] = [
        ...
        <Box px={16} mb={24}>
          ...
          >
            <Box
              ...
              style={{
                backgroundColor:
                  ACCENT_COLORS.find((c) => c.id === accentColor)?.hex || theme.colors.primary[500],
              }}
            >
          ...
          <Card size="sm" style={{ padding: 16 }}>
            <Box flexDirection="row" flexWrap="wrap" justifyContent="space-between">
              {ACCENT_COLORS.map((color) => (
                <Pressable
                  key={color.id}
                  onPress={() => handleAccentColorChange(color.id)}
                  style={{
                    width: 64,
                    height: 64,
                    borderRadius: 16,
        ...
        <Box px={16} mb={24}>
          ...
          <Card size="sm" style={{ overflow: 'hidden' }}>
            {TIMER_FORMAT_OPTIONS.map((option, index) => (
              <React.Fragment key={option.id}>
                <Pressable
                  onPress={() => handleTimerFormatChange(option.id)}
                  style={{
                    flexDirection: 'row',
                    alignItems: 'center',
                    paddingVertical: 16,
