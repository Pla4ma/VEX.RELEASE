 ...
 *
 * Phase 14.4 - Complete AI Coach customization.
 *
 * Features:
 * - Coach persona selector (Cheerleader, Mentor, Drill Sergeant)
 * - Message frequency selector (Frequent, Normal, Minimal)
 * - Reset coach memory option
 * - Language selector
 */
...
type CoachPersona = 'cheerleader' | 'mentor' | 'drill-sergeant';
type MessageFrequency = 'frequent' | 'normal' | 'minimal';
type CoachLanguage = 'en';
...
export const CoachSettingsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const selectedPersonaData = PERSONA_OPTIONS.find((p) => p.id === selectedPersona);
          ...
          >
            <Box p={20} alignItems="center">
              <Box
                ...
                mb={16}
                style={{
                  backgroundColor: selectedPersonaData?.color || theme.colors.primary[500],
                }}
              >
              ...
              {/* Example Messages */}
              <Box width="100%">
                {selectedPersonaData?.exampleMessages.map((message, index) => (
                  <Box
                    key={index}
                    p={12}
                    borderRadius={12}
                    ...
                    style={{
                      ...
                      borderLeftWidth: 3,
                      borderLeftColor: selectedPersonaData?.color,
                    }}
                  >
          ...
          <Card size="sm" style={{ overflow: 'hidden' }}>
            {PERSONA_OPTIONS.map((option, index) => (
              <React.Fragment key={option.id}>
                <Pressable
                  onPress={() => handlePersonaChange(option.id)}
                  style={{
                    flexDirection: 'row',
                    alignItems: 'center',
                    paddingVertical: 16,
                ...
                accessibilityHint="Activates this control">
                  ...
                  <Box flex={1} ml={12}>
                    <Text
                      variant="body"
                      style={{
                        fontWeight: selectedPersona === option.id ? '600' : '500',
                ...
                {index < PERSONA_OPTIONS.length - 1 && (
                  <Box height={1} ml={76} style={{ backgroundColor: theme.colors.border.light }} />
                )}
              </React.Fragment>
            ))}
          </Card>
