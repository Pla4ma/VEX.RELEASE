/**
 * OnboardingChooseElement
 *
 * PHASE 13.3 - Element selection step in onboarding
 * 6 elements with visual preview and subtle gameplay effects
 *
 * @phase 13.3
 */
...
// Element definitions with lore and gameplay effects
...
function ElementCard({ element, isSelected, onSelect, delay }: ElementCardProps): JSX.Element {
  const { theme } = useTheme();
  ...
  return (
    <Animated.View entering={FadeInUp.duration(400).delay(delay)} style={{ width: '48%' }}>
      <Pressable
        onPress={onSelect}
          ...
          style={[
            {
              ...
              borderWidth: 2,
              borderColor: isSelected ? themeColors.primary : `${themeColors.primary}30`,
              minHeight: 160,
            },
            animatedStyle,
          ]}
...
// Simple visual representation for each element
function ElementVisual({ element, color }: { element: CompanionElement; color: string }): JSX.Element {
