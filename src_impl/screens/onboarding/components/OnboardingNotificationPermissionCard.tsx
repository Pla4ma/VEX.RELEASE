...
type PermissionCardState = 'loading' | 'idle' | 'submitting' | 'complete' | 'hidden';
...
export function OnboardingNotificationPermissionCard({
  ...
  useEffect(() => {
    ...
    const loadPromptState = async () => {
      const hasRequested = await hasRequestedOnboardingNotificationPrompt(userId);
      if (!isMounted) {
        return;
      }
  ...
  const handleMaybeLater = async () => {
    ...
    try {
      ...
      setStatusMessage('No pressure. You can turn reminders on later in settings.');
    } catch (error) {
      debug.warn('Failed to update notification prompt state after deferral', error);
      setStatusMessage('We could not save that preference just now. You can try again later.');
    }
  ...
  const handleEnableReminders = async () => {
    ...
    try {
      ...
      // If granted, get push token and store in Supabase
      if (granted) {
        try {
          ...
          // Schedule onboarding retention notifications
          await scheduleOnboardingNotifications(userId);
          debug.info('Onboarding notifications scheduled for user: %s', userId);
        } catch (tokenError) {
          // Token storage/scheduling failure is non-fatal
          debug.warn('Failed to store push token or schedule notifications', tokenError);
        }
      }
      ...
      debug.warn('Notification permission request failed during onboarding', error);
      setStatusMessage('We could not enable reminders just now. You can try again later.');
    } finally {
      setState('complete');
    }
  };
    ...
    >
      <View style={{ gap: theme.spacing[2] }}>
        ...
        <Text style={[styles.body, { color: theme.colors.text.secondary }]}>
          Get a reminder when your streak is at risk. We&apos;ll only send 1 per day.
        </Text>
      </View>
