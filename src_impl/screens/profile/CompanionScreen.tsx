...
import { type CompanionMood, type CompanionState, ELEMENT_THEMES } from '../../features/companion/types';
import { LivingCompanion } from '../../features/companion/components/LivingCompanion';
import { loadCompanionState, loadRecentSessionMoods } from '../../features/companion/session-storage';
...
type LoadState =
  ...
  | { status: 'loading' }
  | { companion: CompanionState; moodHistory: SessionMoodEntry[]; status: 'success' };
...
export function CompanionScreen(): JSX.Element {
  ...
  if (loadState.status === 'empty') {
    return (
      <Box flex={1} bg="background.primary" justifyContent="center" alignItems="center" p="xl">
        <Text variant="h4" color="text.primary" textAlign="center">Your companion needs an active profile.</Text>
      </Box>
    );
  }
  ...
  const fadeUp = (delay: number) => (reducedMotion ? undefined : FadeInUp.duration(400).delay(delay));
    ...
    >
      <Animated.View entering={fadeIn}>
        <Box height={HERO_HEIGHT} bg="background.secondary" justifyContent="center" alignItems="center">
          <LivingCompanion
            ...
            totalSeconds={1800}
          />
          <Box position="absolute" px="md" py="sm" borderRadius="full" bg="background.tertiary" style={{ right: theme.spacing[4], top: theme.spacing[4] }}>
            <Text variant="caption" color="text.primary" fontWeight="600">{PHASE_NAMES[companion.phase]} Phase</Text>
          </Box>
        </Box>
      </Animated.View>
      <Animated.View entering={fadeUp(200)}>
        <Box px="lg" py="xl" gap="lg">
          ...
          <Box flexDirection="row" gap="md">
            <StatCard label="Focus Minutes" value={Math.floor(companion.totalFocusMinutes).toLocaleString()} />
            <StatCard label="Sessions Together" value={companion.sessionCount.toLocaleString()} />
            <StatCard label="Perfect Sessions" value={companion.perfectSessions.toLocaleString()} />
          </Box>
        </Box>
      </Animated.View>
        ...
        <Box px="lg" pb="xl" gap="md">
          ...
          <Card size="md">
            <Box gap="md">
              ...
              <Text variant="body" color="text.secondary">Level {companion.level}/100</Text>
      ...
      <Animated.View entering={fadeUp(400)}>
        <Box px="lg" pb="xl" gap="md">
          ...
          <Card size="md" style={{ borderColor: themeColors.primary, borderWidth: 1 }}>
            <Text variant="h4" color="text.primary" fontWeight="600">{companion.element}</Text>
            <Text variant="caption" color="text.tertiary">Affinity: {companion.elementAffinity}%</Text>
            <Text variant="bodySmall" color="text.secondary">{ELEMENT_LORE[companion.element]}</Text>
          </Card>
        </Box>
      </Animated.View>
      <Animated.View entering={fadeUp(500)}>
        <Box px="lg" pb="xl" gap="md">
          ...
          <Card size="md">
            <Box gap="md">
              <Text variant="bodySmall" color="text.secondary">Last {moodHistory.length} session{moodHistory.length === 1 ? '' : 's'}</Text>
              <Box flexDirection="row" gap="sm" alignItems="center">
                {moodHistory.length > 0 ? (
                  moodHistory.map((entry) => <MoodDot key={entry.sessionId} mood={entry.mood} />)
                ) : (
                  <Text variant="caption" color="text.tertiary">No recent sessions recorded</Text>
                )}
              </Box>
            </Box>
          </Card>
