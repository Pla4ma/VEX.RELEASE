 ...
 * InventoryScreen
 *
 * Main inventory screen with:
 * - Tabs: All, Consumable, Equipment, Cosmetic, Crafting
 * - 4-column grid layout with rarity borders
 * - Item count header
 * - Loading states
 * - Item detail modal
 * - Equipment loadout panel (optional)
 */

import React, { useState, useCallback, useMemo } from 'react';
import { Pressable, ScrollView, View } from 'react-native';
import Animated, { FadeInUp } from 'react-native-reanimated';
import { Box, Text, Button, Skeleton } from '@/components/primitives';
...
import type { InventoryItem, InventoryFilter, ItemType } from '@/features/inventory/schemas';
...
}> = ({ totalItems, uniqueItems, equippedCount }) => {
  const { theme } = useTheme();

  return (
    <Box p={4} bg={theme.colors.background.secondary}>
...
const TabBar: React.FC<{
  ...
  itemCounts: Record<TabType, number>;
}> = ({ activeTab, onTabChange, itemCounts }) => {
  const { theme } = useTheme();

  return (
    <Box py={2} bg={theme.colors.background.secondary}>
      <ScrollView
        horizontal
  ...
  accessibilityHint="Activates this control">
                <Box
                  ...
                  py={2}
                  borderRadius={20}
                  bg={isActive ? theme.colors.primary[500] : theme.colors.background.tertiary}
                  style={{
...
const EquipmentLoadoutPanel: React.FC<{
  equippedItems: InventoryItem[];
  onUnequip: (itemId: string) => void;
  isVisible: boolean;
  onToggle: () => void;
}> = ({ equippedItems, onUnequip, isVisible, onToggle }) => {
  const { theme } = useTheme();

  if (!isVisible) {
    return (
      <Pressable onPress={onToggle}
  accessibilityLabel="⚔️ Equipment Loadout ▶ button"
  accessibilityRole="button"
  accessibilityHint="Activates this control">
        <Box
          p={3}
  ...
  return (
    <Box bg={theme.colors.background.secondary}>
      <Pressable onPress={onToggle}
  accessibilityLabel="⚔️ Equipment Loadout ( /5) ▼ button"
  accessibilityRole="button"
  accessibilityHint="Activates this control">
        <Box
          p={3}
...
// ============================================================================
// Item Detail Modal (placeholder - would use actual modal)
// ============================================================================
...
const ItemDetailModal: React.FC<{
  ...
  onEquip?: () => void;
  onUnequip?: () => void;
}> = ({ item, visible, onClose, onUse, onEquip, onUnequip }) => {
  // In real implementation, this would use a BottomSheetModal
  ...
  accessibilityRole="button"
  accessibilityHint="Activates this control">Close</Button>
      </Box>
    </View>
  );
};
...
export const InventoryScreen: React.FC = () => {
  ...
  const filter = useMemo<InventoryFilter>(() => {
  ...
  const { data: inventory, isLoading, isError, refetch } = useFilteredInventory(userId, filter);
  const { data: stats } = useInventoryStats(userId);
  ...
  const itemCounts = useMemo(() => {
    ...
    if (inventory) {
      inventory.forEach((item) => {
        // This would need actual item type data
        // Placeholder logic
      });
    }
  ...
  // Get equipped items (placeholder)
  const equippedItems = useMemo(() => {
    return inventory?.filter((item) => item.status === 'EQUIPPED') || [];
  }, [inventory]);
  ...
  const handleUnequip = useCallback((itemId: string) => {
    ...
    void queryClient.invalidateQueries({ queryKey: inventoryKeys.state(userId) });
  }, [queryClient, userId]);
  ...
  if (isError) {
    return (
      <Box flex={1} bg={theme.colors.background.primary} alignItems="center" justifyContent="center" p={8}>
        <Text style={{ fontSize: 48 }}>⚠️</Text>
        <Text variant="h3" color={theme.colors.text.secondary} mt={4} textAlign="center">
          Failed to load inventory
        </Text>
        <Button variant="primary" onPress={() => refetch()} mt={4}
  ...
  return (
    <Box flex={1} bg={theme.colors.background.primary}>
      ...
      <EquipmentLoadoutPanel
        ...
        isVisible={showLoadout}
        onToggle={() => setShowLoadout(!showLoadout)}
      />
