...
const TECHNIQUES = [
  { key: 'durationMastery' as const, label: 'Duration Focus', color: '#6366F1', description: 'Long sessions without interruption' },
  { key: 'purityMastery' as const, label: 'Purity', color: '#14B8A6', description: 'Sustained high focus scores' },
  { key: 'consistencyMastery' as const, label: 'Consistency', color: '#F97316', description: 'Daily streaks maintained' },
  { key: 'comebackMastery' as const, label: 'Comeback', color: '#EC4899', description: 'Recovering from broken streaks' },
  { key: 'bossMastery' as const, label: 'Boss', color: '#EAB308', description: 'Boss defeat efficiency' },
];
...
function calculatePointsToNextRank(currentPoints: number, currentRank: MasteryRank): number {
...
function useMasteryState(userId: string | null) {
  ...
  return { state, isLoading, error, refetch: loadMastery, claimChallenge, refreshChallenges };
}
    ...
    <Card size="md" style={{ backgroundColor: theme.colors.background.secondary }}>
      <View style={{ gap: theme.spacing[3] }}>
        ...
        {challenge.status === 'COMPLETED' && (
          <Button size="sm" variant="primary" onPress={onClaim} accessibilityLabel={`Claim reward for ${challenge.title}`} accessibilityRole="button" accessibilityHint={`Claims ${challenge.masteryPoints} mastery points`}>
...
function RankUnlocks({ currentRank }: { currentRank: MasteryRank }): JSX.Element {
  const { theme } = useTheme();
  const ranks: MasteryRank[] = ['APPRENTICE', 'ADEPT', 'EXPERT', 'MASTER', 'GRANDMASTER'];
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text
                ...
                style={{ color: isUnlocked ? rankDisplay.color : theme.colors.text.secondary }}
              >
                {rankDisplay.title}
                {isCurrent && ' (Current)'}
                {!isUnlocked && ' (Locked)'}
...
function HistoryChart({ state }: { state: MasteryState }): JSX.Element {
  ...
  const mockHistory = useMemo(() => {
    ...
    for (let i = 0; i <= days; i++) {
      ...
      points.push(Math.round(state.totalMasteryPoints * progress * (0.8 + Math.random() * 0.4)));
    }
    return points;
            ...
            return (
              <View
                ...
                style={{
                  flex: 1,
                  height: `${height}%`,
                  backgroundColor: isCurrent ? theme.colors.primary[500] : `${theme.colors.primary[500]}50`,
                  borderRadius: 2,
                }}
              />
            );
...
export function MasteryScreen({ navigation }: Props): JSX.Element {
  const { theme } = useTheme();
  const insets = useSafeAreaInsets();
  const { user } = useAuthStore();
  const userId = user?.id ?? null;

  const { state, isLoading, error, refetch, claimChallenge, refreshChallenges } = useMasteryState(userId);

  const rankDisplay = state ? getMasteryRankDisplay(state.rank) : getMasteryRankDisplay('APPRENTICE');
  const pointsToNext = state ? calculatePointsToNextRank(state.totalMasteryPoints, state.rank) : 0;
  const nextRankName = useMemo(() => {
    if (!state) {return '';}
    const ranks: MasteryRank[] = ['APPRENTICE', 'ADEPT', 'EXPERT', 'MASTER', 'GRANDMASTER'];
    const currentIndex = ranks.indexOf(state.rank);
    if (currentIndex >= ranks.length - 1) {return 'Max Rank';}
    return getMasteryRankDisplay(ranks[currentIndex + 1]).title;
  }, [state]);

  const rankProgress = useMemo(() => {
    if (!state || pointsToNext === 0) {return 1;}
      ...
      >
        ...
        <Animated.View entering={FadeInUp.duration(400).delay(300)} style={{ gap: theme.spacing[3] }}>
          <Box flexDirection="row" justifyContent="space-between" alignItems="center">
            ...
            <Button size="sm" variant="ghost" onPress={refreshChallenges} accessibilityLabel="Refresh challenges" accessibilityRole="button"
  accessibilityHint="Activates this control">
