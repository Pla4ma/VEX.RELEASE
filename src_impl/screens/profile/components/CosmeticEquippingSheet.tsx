/**
 * Cosmetic Equipping Sheet Component
 *
 * Phase 17.3 - Bottom sheet for equipping cosmetics from profile.
 *
 * Features:
 * - Tap cosmetic element to open sheet
 * - Shows all owned cosmetics of that type
 * - Live preview on profile
 * - Equip button applies immediately
 */
...
const CosmeticCard: React.FC<{
  ...
  return (
    <Animated.View style={[{ flex: 1, minWidth: '30%', maxWidth: '31%' }, animatedStyle]}>
...
export const CosmeticEquippingSheet: React.FC<CosmeticEquippingSheetProps> = ({
  ...
  const modalStyle = useAnimatedStyle(() => ({
    transform: [
      {
        translateY: (1 - slideUp.value) * 400,
      },
    ],
  }));
  ...
  // Sort items by rarity and ownership
  ...
  accessibilityHint="Activates this control">
            ...
            >
              <Pressable
                onPress={handleEquip}
                disabled={!selectedId || selectedId === currentEquippedId || isEquipping}
                style={{
                  backgroundColor:
