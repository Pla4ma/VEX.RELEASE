import React,{useState,useCallback,useMemo}from'react';import{Pressable,ViewStyle,PanResponder,Animated as RNAnimated}from'react-native';import Animated,{useSharedValue,useAnimatedStyle,withSpring,withTiming}from'react-native-reanimated';import{useTheme}from'../../../theme';import{Box,Text,Card}from'../../../components/primitives';import{Icon}from'../../../icons';export interface FeaturedAchievement{id:string;name:string;description:string;icon:string;emoji:string;unlockedAt:number;rarity:'common'|'rare'|'epic'|'legendary';}interface AchievementShowcaseProps{achievements:FeaturedAchievement[];onReorder?:(newOrder:FeaturedAchievement[])=>void;onSelectSlot?:(slotIndex:number)=>void;style?:ViewStyle;isEditable?:boolean;}const RARITY_COLORS={common:'#94A3B8',rare:'#3B82F6',epic:'#A855F7',legendary:'#F59E0B'};const LockedSlot:React.FC<{slotIndex:number;onPress?:()=>void;isEditable?:boolean;}>=({slotIndex,onPress,isEditable})=>{const{theme}=useTheme();return<Pressable onPress={onPress}disabled={!isEditable}accessibilityLabel="? button"accessibilityRole="button"accessibilityHint="Activates this control">
      ...
      <Box flexDirection="row"justifyContent="space-between"alignItems="center"mb={16}>
        ...
        {unlockedCount>=2&&<Pressable onPress={handleEditToggle}accessibilityLabel="Interactive control"accessibilityRole="button"accessibilityHint="Activates this control">
            <Box px={12}py={8}borderRadius={8}style={{backgroundColor:isEditMode?theme.colors.success[50]||'#ECFDF5':theme.colors.background.secondary,borderWidth:1,borderColor:isEditMode?theme.colors.success.DEFAULT:theme.colors.border.light}}>
