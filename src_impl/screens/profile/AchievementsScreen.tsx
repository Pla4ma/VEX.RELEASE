 ...
 * AchievementsScreen
 *
 * Main achievements collection screen with:
 * - Tab layout: All, Session, Streak, Boss, Social, Progression
 * - Filter: All, Locked, Unlocked
 * - Sort: Rarity, Recent, Category
 * - Trophy count header
...
import { useAchievements, useAchievementStats, achievementKeys } from '@/features/achievements/hooks';
import { ALL_ACHIEVEMENTS, getRarityColor, getAchievementDisplayInfo } from '@/features/achievements/definitions';
import type { Achievement, AchievementCategory, AchievementRarity } from '@/features/achievements/types';
...
type SortType = 'RARITY' | 'RECENT' | 'CATEGORY';
type CategoryItem = { id: AchievementCategory | 'ALL'; label: string; icon: string };
    ...
    >
      ...
      <Box flexDirection="row" gap={2}>
        {(['ALL', 'UNLOCKED', 'LOCKED'] as FilterType[]).map((f) => (
          <Pressable key={f} onPress={() => onFilterChange(f)}
  accessibilityLabel="Interactive control"
  accessibilityRole="button"
  accessibilityHint="Activates this control">
            <Box
...
const AchievementSkeletonCard: React.FC = () => {
  const { theme } = useTheme();
      ...
      {selectedAchievement && (
        <Box p={4} bg={theme.colors.background.secondary} mb={4}>
          <Box flexDirection="row" justifyContent="space-between" alignItems="center">
            ...
            <Pressable onPress={() => setSelectedAchievement(null)} style={({ pressed }) => [pressed && { opacity: 0.8 }]}
