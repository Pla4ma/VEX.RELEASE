 ...
 */
function ChestItem({ chest, onPress, index }: { chest: MysteryChest; onPress: () => void; index: number }): JSX.Element {
  ...
  const pulseStyle = useAnimatedStyle(() => ({
    transform: [
      {
        scale: withRepeat(withSequence(withTiming(1, { duration: 1500 }), withTiming(1.05, { duration: 1500 }), withTiming(1, { duration: 1500 })), -1, true),
      },
    ],
  }));
  ...
  const glowStyle = useAnimatedStyle(() => ({
    ...
    shadowOpacity: appearance.glow ? withRepeat(withSequence(withTiming(0.5, { duration: 1000 }), withTiming(0.8, { duration: 1000 })), -1, true) : 0.3,
    shadowRadius: appearance.glow ? 15 : 5,
  }));

  return (
    <Animated.View entering={FadeIn.duration(400).delay(index * 100)} style={[{ marginRight: 16 }, pulseStyle]}>
      <Pressable onPress={onPress} accessibilityLabel="Interactive control" accessibilityRole="button" accessibilityHint="Activates this control">
        ...
        >
          ...
          <Box position="absolute" bottom={-8} px="sm" py="xs" borderRadius="full" bg={appearance.color}>
...
function EmptyChests(): JSX.Element {
  ...
  return (
    <Box p="lg" borderRadius="xl" bg="background.tertiary" borderWidth={1} borderColor="border.light" alignItems="center">
 ...
 */
export function MysteryChestList({ chests, onOpenChest, isLoading = false }: MysteryChestListProps): JSX.Element | null {
  const { theme } = useTheme();
      ...
      <Box flexDirection="row" justifyContent="space-between" alignItems="center" mb="md">
        <Box flexDirection="row" alignItems="center" gap="sm">
          ...
          {unopenedChests.length > 0 && (
            <Box px="sm" py="xs" borderRadius="full" bg="accent.orange">
      ...
      {unopenedChests.length > 0 ? (
        <Box flexDirection="row" flexWrap="wrap">
          {unopenedChests.map((chest, index) => (
            <ChestItem key={chest.id} chest={chest} onPress={() => onOpenChest(chest.id)} index={index} />
          ))}
        </Box>
      ) : (
        <EmptyChests />
