import React,{useCallback,useMemo}from'react'; import{View,Text,Modal,Pressable,ScrollView,TouchableWithoutFeedback,Dimensions}from'react-native'; import type{InventoryItem,ItemType,ItemRarity,EquipmentSlot}from'../schemas'; import{createSheet}from'@/shared/ui/create-sheet'; interface ItemDetailModalProps{visible:boolean;item:InventoryItem|null;onClose:()=>void;onUse?:()=>void;onEquip?:()=>void;onUnequip?:()=>void;onDestroy?:()=>void;onCraft?:()=>void;onGift?:()=>void;isProcessing?:boolean}interface ItemDefinition{id:string;name:string;description:string;type:ItemType;rarity:ItemRarity;iconUrl:string|null;baseValue:number;effects?:Array<{type:string;value:number;duration:number}>;passiveEffects?:Array<{type:string;value:number}>;equipmentSlot?:EquipmentSlot;maxUses?:number;craftingRecipe?:{ingredients:Array<{itemDefinitionId:string;quantity:number}>}}const RARITY_CONFIG:Record<ItemRarity,{color:string;backgroundColor:string;label:string}> = {COMMON:{color:'theme.colors.primary[500]',backgroundColor:'theme.colors.primary[500]',label:'Common'},UNCOMMON:{color:'theme.colors.primary[500]',backgroundColor:'theme.colors.primary[500]',label:'Uncommon'},RARE:{color:'theme.colors.primary[500]',backgroundColor:'theme.colors.primary[500]',label:'Rare'},EPIC:{color:'theme.colors.primary[500]',backgroundColor:'theme.colors.primary[500]',label:'Epic'},LEGENDARY:{color:'theme.colors.primary[500]',backgroundColor:'theme.colors.primary[500]',label:'Legendary'}}; const TYPE_LABELS:Record<ItemType,string> = {CONSUMABLE:'Consumable',EQUIPMENT:'Equipment',COSMETIC:'Cosmetic',CRAFTING:'Crafting Material',COLLECTIBLE:'Collectible'}; const SLOT_LABELS:Record<EquipmentSlot,string> = {HEAD:'Head',BODY:'Body',HANDS:'Hands',FEET:'Feet',ACCESSORY_1:'Accessory 1',ACCESSORY_2:'Accessory 2',FOCUS_TOOL:'Focus Tool',PET:'Pet'}; export function ItemDetailModal({visible,item,onClose,onUse,onEquip,onUnequip,onDestroy,onCraft,onGift,isProcessing = false}:ItemDetailModalProps){const definition:ItemDefinition|null = useMemo(()=>{if(!item){return null;}return{id:item.itemDefinitionId,name:'Sample Item',description:'This is a sample item description that would come from the item definition.',type:'CONSUMABLE',rarity:'RARE',iconUrl:null,baseValue:100,effects:[{type:'HEAL',value:50,duration:0}],equipmentSlot:undefined};},[item]); const rarity = definition ? RARITY_CONFIG[definition.rarity] : null; const isEquipped = item?.status === 'EQUIPPED'; const isConsumable = definition?.type === 'CONSUMABLE'; const isEquipment = definition?.type === 'EQUIPMENT'; const isCrafting = definition?.type === 'CRAFTING'; const handleAction = useCallback((action:()=>void)=>{if(!isProcessing){action();}},[isProcessing]); if(!item || !definition){return null;}return<Modal visible={visible}transparent animationType="fade"onRequestClose={onClose}>
  ...
  accessibilityHint="Activates this control">
    ...
    </Modal>;}const{width,height} = Dimensions.get('window'); const MODAL_WIDTH = Math.min(width * 0.9,400); const styles = createSheet({overlay:{flex:1,backgroundColor:'rgba(0, 0, 0, 0.5)',justifyContent:'center',alignItems:'center'},container:{width:MODAL_WIDTH,maxHeight:height * 0.8,backgroundColor:'theme.colors.background.primary',borderRadius:16,overflow:'hidden'},header:{padding:20,paddingTop:16,alignItems:'center',position:'relative'},closeButton:{position:'absolute',top:12,right:12,width:32,height:32,justifyContent:'center',alignItems:'center',backgroundColor:'rgba(0, 0, 0, 0.05)',borderRadius:16},closeButtonText:{fontSize:24,color:'theme.colors.primary[500]',lineHeight:28},iconContainer:{marginTop:8},iconPlaceholder:{width:80,height:80,borderRadius:12,borderWidth:3,justifyContent:'center',alignItems:'center',backgroundColor:'theme.colors.background.primary'},iconText:{fontSize:40},rarityBadge:{marginTop:12,paddingHorizontal:12,paddingVertical:4,backgroundColor:'theme.colors.background.primary',borderRadius:12},rarityText:{fontSize:12,fontWeight:'700',textTransform:'uppercase'},content:{padding:20,paddingTop:0},itemName:{fontSize:22,fontWeight:'700',color:'theme.colors.primary[500]',textAlign:'center',marginTop:16,marginBottom:8},metaRow:{flexDirection:'row',justifyContent:'center',alignItems:'center',gap:12,marginBottom:16},typeBadge:{paddingHorizontal:10,paddingVertical:4,borderRadius:12},typeText:{fontSize:12,fontWeight:'600'},quantityText:{fontSize:14,color:'theme.colors.primary[500]'},quantityValue:{fontWeight:'700',color:'theme.colors.primary[500]'},equippedBadge:{backgroundColor:'theme.colors.primary[500]',paddingHorizontal:12,paddingVertical:6,borderRadius:8,alignSelf:'center',marginBottom:16},equippedText:{color:'theme.colors.primary[500]',fontSize:12,fontWeight:'600'},section:{marginTop:20,paddingTop:16,borderTopWidth:1,borderTopColor:'theme.colors.primary[500]'},sectionTitle:{fontSize:12,fontWeight:'600',color:'theme.colors.primary[500]',textTransform:'uppercase',marginBottom:8},description:{fontSize:14,color:'theme.colors.primary[500]',lineHeight:20},effectRow:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',paddingVertical:6},effectType:{fontSize:14,color:'theme.colors.primary[500]'},effectValue:{fontSize:14,fontWeight:'600',color:'theme.colors.primary[500]'},slotText:{fontSize:14,color:'theme.colors.primary[500]'},usesText:{fontSize:14,color:'theme.colors.primary[500]'},enhancementText:{fontSize:18,fontWeight:'700',color:'theme.colors.primary[500]'},valueText:{fontSize:14,color:'theme.colors.primary[500]'},acquiredText:{fontSize:12,color:'theme.colors.primary[500]'},spacer:{height:20},actions:{padding:16,paddingTop:12,backgroundColor:'theme.colors.primary[500]',borderTopWidth:1,borderTopColor:'theme.colors.primary[500]',flexDirection:'row',flexWrap:'wrap',gap:8},actionButton:{flex:1,minWidth:80,paddingVertical:12,paddingHorizontal:16,borderRadius:8,alignItems:'center'},actionButtonText:{fontSize:12,fontWeight:'700',color:'theme.colors.background.primary'},useButton:{backgroundColor:'theme.colors.primary[500]'},equipButton:{backgroundColor:'theme.colors.primary[500]'},unequipButton:{backgroundColor:'theme.colors.primary[500]'},craftButton:{backgroundColor:'theme.colors.primary[500]'},giftButton:{backgroundColor:'theme.colors.primary[500]'},destroyButton:{backgroundColor:'theme.colors.primary[500]',borderWidth:1,borderColor:'theme.colors.primary[500]'},destroyText:{color:'theme.colors.primary[500]'}});
