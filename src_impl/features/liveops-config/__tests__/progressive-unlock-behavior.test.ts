...
function exposureFor(sessions: number, feature: FeatureKey): boolean {
  ...
  const key = feature as keyof typeof flags;
  return flags[key] as unknown as boolean;
}
...
describe('Progressive unlock — behavior contracts', () => {
  ...
  describe('hidden/disabled features (session 0)', () => {
    HIDDEN_FEATURES.forEach((feature) => {
      ...
      it(`${feature}: home runtime query is disabled`, () => {
        ...
        const queryKey = `canQuery${feature.charAt(0).toUpperCase() + feature.slice(1).replace(/_b_tab|_detail|_view|_basic|_advanced|_only|_study|_mode|_beta/gi, '')}` as keyof typeof rt;
        expect(rt[queryKey as string]).toBeDefined();
      });
    });
  });
  ...
  describe('unlocked progressive features', () => {
    ...
    it('quiz_review_mode unlocks at 10 sessions', () => {
      const avail = availabilityFor(10, 'quiz_review_mode');
      expect(avail.state).toBe('unlocked');
    });
    ...
    it('seasonal_features unlocks at 15 sessions', () => {
  ...
  describe('home feature runtime query gating', () => {
    ...
    it('at 7 sessions, boss query is enabled', () => {
      const rt = runtimeFor(7);
      expect(rt.canQueryBoss).toBe(true);
    });
    ...
    it('at 12 sessions, content study query is enabled', () => {
      ...
      expect(rt.canQueryContentStudy).toBe(true);
    });
  });
  ...
  describe('CTA safety contracts', () => {
    ...
    it('core features are always visible (canRenderEntryPoint true at 0 sessions)', () => {
      const coreFeatures: FeatureKey[] = ['focus_session', 'home_tab', 'focus_tab', 'profile_tab', 'progress_view', 'ai_coach_basic'];
  ...
  describe('openFeature safe navigation resolution', () => {
    it('resolveFeatureRoute returns false for hidden features', async () => {
      const { resolveFeatureRoute } = await import(
        '../../../navigation/openFeature'
      );
    ...
    it('resolveFeatureRoute returns true for core features', async () => {
      const { resolveFeatureRoute } = await import(
        '../../../navigation/openFeature'
      );
      const { features } = buildFeatureAccess({ totalCompletedSessions: 0 });
      const result = resolveFeatureRoute('focus_session', features.focus_session);
...
describe('Notification routing feature gating', () => {
  it('routeNotificationAction gates boss notification when boss_tab is locked', async () => {
    const { routeNotificationAction } = await import(
      '../../../navigation/notification-routing-core'
    );
    const { buildFeatureAccess } = await import('../feature-access');
  ...
  it('routeNotificationAction allows boss notification when boss_tab is unlocked', async () => {
    const { routeNotificationAction } = await import(
      '../../../navigation/notification-routing-core'
    );
    const { buildFeatureAccess } = await import('../feature-access');
    ...
    let navigatedTo = '';
    const mockNav = {
      navigate: (screen: string): void => { navigatedTo = screen; },
    };
