import { calculatePriceTrustScore, getActiveTrustSignals, getPriceExplanation, getRefundEligibility, getRemainingDays, isPurchaseValid, isSuspiciousPurchase, logPurchaseAttempt, PurchaseReceiptSchema, restorePurchases, TRUST_SIGNALS, verifyPurchaseHash, verifyPurchaseReceipt } from "../purchase-trust";
...
describe("Purchase Trust Utilities", () => {
  ...
  describe("logPurchaseAttempt", () => {
    ...
    it("logs failure with error", async () => {
      await expect(logPurchaseAttempt("user-1", "pro", false, "Payment declined")).resolves.not.toThrow();
    });
  });
