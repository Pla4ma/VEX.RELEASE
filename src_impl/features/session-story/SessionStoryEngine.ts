 ...
 *
 * Orchestrates the generation and persistence of post-session stories.
 * Connects session completion events to narrative generation.
 *
 * Features:
...
interface StoryContext {
  ...
  bossContext?: {
    encounter: BossEncounterSummary | null;
    damageDealt: number;
    defeated: boolean;
  };
  streakResult?: StreakEngineResult;
  xpEarned: number;
  currentLevel: number;
...
export function initializeSessionStoryEngine(): () => void {
  ...
  const unsubscribeCompleted = eventBus.subscribe('session:completed', async (event) => {
    try {
      ...
      // Build full context from various sources
      const context = await buildStoryContext(sessionId, userId, summary);
  ...
  const unsubscribeViewed = eventBus.subscribe('session:story:viewed', (event) => {
    ...
    void storyRepository.markStoryViewed(storyId, completionRate);
...
async function buildStoryContext(sessionId: string, userId: string, summary: SessionSummary): Promise<StoryContext> {
  // Fetch boss context if available
  ...
  const sessionsToNextTier = calculateSessionsToNextTier(progression.progressPercent);
  ...
  return {
    ...
    streakResult: streakState
      ...
      : undefined,
    xpEarned: 0, // Would need to track from session
    currentLevel: progression.currentLevel,
    xpToNextLevel: progression.xpToNextLevel,
    tierProgress: progression.progressPercent,
    sessionsToNextTier,
  };
}
...
function calculateSessionsToNextTier(currentProgress: number): number {
  // Assuming roughly 3% progress per session average
  const progressPerSession = 3;
  const remainingProgress = 100 - currentProgress;
  return Math.max(1, Math.ceil(remainingProgress / progressPerSession));
}

async function generateSessionStory(context: StoryContext): Promise<SessionStory | null> {
  // Build input for story calculator
...
export async function getStoryForSession(sessionId: string, userId: string): Promise<SessionStory | null> {
  return storyRepository.fetchStoryBySessionId(sessionId, userId);
}

export async function getStoriesForUser(userId: string, limit: number = 10): Promise<SessionStory[]> {
  return storyRepository.fetchStoriesForUser(userId, limit);
}

export async function markStoryBeatViewed(storyId: string, beatType: string, userId: string): Promise<void> {
  trackStoryEvent({
    storyId,
    userId,
...
export async function shareStory(storyId: string, userId: string): Promise<void> {
