...
class SquadPersistenceManager {
  ...
  async cacheSquad(squad: Squad): Promise<void> {
    ...
    // In real implementation, use MMKV or similar
    // await MMKV.setItem(key, JSON.stringify(data));
  }

  async getCachedSquad(squadId: string): Promise<{ squad: Squad; cachedAt: number } | null> {
    const key = STORAGE_KEYS.squadCache(squadId);
    // const data = await MMKV.getItem(key);
    // if (!data) return null;
  ...
  async processQueue(): Promise<void> {
    if (this.syncInProgress || this.pendingOps.length === 0) {
      return;
    }
  ...
  private async persistPendingOps(): Promise<void> {
    // await MMKV.setItem(STORAGE_KEYS.pendingOperations, JSON.stringify(this.pendingOps));
  }

  // Conflict resolution
  async resolveConflict(localState: Squad, remoteState: Squad, lastSyncedAt: number): Promise<Squad> {
  ...
  async persistSessionState(
    ...
    // await MMKV.setItem(key, JSON.stringify({
    //   ...state,
...
export function createOptimisticSquadMember(squadId: string, userId: string, role: string, displayName: string): SquadMember {
  return {
    squadId,
    userId,
...
export function createOptimisticSquadInvite(squadId: string, invitedBy: string, invitedUserId: string, roleOffered: string): SquadInvite {
  return {
    id: `temp-${Date.now()}`,
    squadId,
    invitedBy,
...
function resolveLocalConflict(remoteState: Squad, conflict: { field: string; local: unknown; remote: unknown }): Squad {
  switch (conflict.field) {
    case 'name':
      return typeof conflict.local === 'string' ? { ...remoteState, name: conflict.local } : remoteState;
    case 'description':
      return typeof conflict.local === 'string' || conflict.local === null ? { ...remoteState, description: conflict.local } : remoteState;
    case 'maxMembers':
