...
async function addSynergyPoints(squadId: string, userId: string, points: number, type: SynergyActivityType): Promise<void> {
...
export async function getSquadMembers(squadId: string): Promise<SquadMemberDetail[]> {
...
export async function getUserSquads(userId: string): Promise<SquadSummary[]> {
  return repository.fetchSquadsForUser(userId);
}
export async function getSquadWeeklyStats(squadId: string) {
  return repository.fetchSquadWeeklyStats(squadId);
}
export async function getPublicSquads(options?: { limit?: number; offset?: number; search?: string }): Promise<SquadSummary[]> {
  return repository.fetchPublicSquads(options);
}
export async function getSquadInvitesForUser(userId: string): Promise<SquadInviteDetail[]> {
  return repository.fetchSquadInvitesForUser(userId);
}
 ...
 */
export function updateSquadStreak(currentState: SquadStreakState, memberId: string, sessionDuration: number): SquadStreakState {
  const now = Date.now();
  ...
  let memberStatus = isNewDay ? { [memberId]: true } : { ...currentState.memberCompletionStatus, [memberId]: true };
  ...
  if (isNewDay) {
    ...
    const yesterdayComplete = lastDate && new Date(lastDate).setHours(0, 0, 0, 0) === today - 24 * 60 * 60 * 1000 && Object.values(currentState.memberCompletionStatus).every(Boolean);
 ...
 */
export function checkSquadStreakRisk(squadStreak: number, members: SquadMemberWithStreakInfo[], currentHour: number = new Date().getHours()): AtRiskMember[] {
  // Only check if streak is worth protecting (>= 3 days)
  if (squadStreak < MIN_SQUAD_STREAK_FOR_RISK) {
    return [];
  }

  // Only check after 8 PM
  if (currentHour < RISK_CHECK_HOUR) {
    return [];
  }
 ...
 */
export function generateNudgeMessage(nudgerName: string, squadName: string, squadStreak: number): string {
...
export async function processSquadRiskNotifications(): Promise<{
  ...
  for (const squad of squads) {
    ...
    for (const member of squad.members) {
      if (atRiskMembers.some((atRisk) => atRisk.userId === member.userId)) {
        continue;
      }
