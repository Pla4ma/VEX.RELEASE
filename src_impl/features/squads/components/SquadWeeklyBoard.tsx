import React from'react'; import{ScrollView}from'react-native'; import Animated,{FadeInUp}from'react-native-reanimated'; import{Box}from'../../../components/primitives/Box'; import{Text}from'../../../components/primitives/Text'; import{useTheme}from'../../../theme'; export interface SquadLeaderboardEntry{userId:string;name:string;avatarUrl?:string;focusMinutes:number;sessionsCompleted:number;rank:number;deltaFromLastWeek:number;isMe:boolean;}export interface SquadWeeklyBoardProps{entries:SquadLeaderboardEntry[];squadName:string;weekNumber:number;}function RankBadge({rank}:{rank:number;}):JSX.Element{const{theme} = useTheme(); const getRankStyle = ()=>{if(rank === 1){return{bg:'#F59E0B',text:'#FFFFFF'};}if(rank === 2){return{bg:'#6B7280',text:'#FFFFFF'};}if(rank === 3){return{bg:'#B45309',text:'#FFFFFF'};}return{bg:theme.colors.background.tertiary,text:theme.colors.text.secondary};}; const style = getRankStyle(); return<Box width={36}height={36}borderRadius="full"justifyContent="center"alignItems="center"style={{backgroundColor:style.bg}}>
