/**
 * SquadWeeklyBoard Component
 *
 * Weekly leaderboard ranking squad members by focus minutes.
 * Shows your position highlighted.
 * Delta from last week (↑2 / ↓1 / =).
 * "Squad MVP" badge for #1 position.
 *
 * @phase 4B.4
 */
...
export interface SquadLeaderboardEntry {
  ...
  rank: number;
  deltaFromLastWeek: number; // positive = up, negative = down, 0 = same
  isMe: boolean;
}

export interface SquadWeeklyBoardProps {
 ...
 */
function LeaderboardRow({ entry, index }: { entry: SquadLeaderboardEntry; index: number }): JSX.Element {
  const { theme } = useTheme();
    ...
    <Animated.View entering={FadeInUp.duration(400).delay(index * 50)}>
      <Box flexDirection="row" alignItems="center" p="md" borderRadius="xl" bg={entry.isMe ? `${theme.colors.primary[500]}15` : "background.secondary"} borderWidth={2} borderColor={entry.isMe ? "primary.500" : isTop3 ? "accent.orange" : "transparent"} mb="sm">
        ...
        <Box width={44} height={44} borderRadius="full" bg="background.tertiary" justifyContent="center" alignItems="center" mx="md">
          {entry.avatarUrl ? (
            <Box width={40} height={40} borderRadius="full" bg="primary.500" />
          ) : (
            <Text fontSize={18} color="text.tertiary" fontWeight="600">
...
/**
 * Squad weekly leaderboard
 */
export function SquadWeeklyBoard({ entries, squadName, weekNumber }: SquadWeeklyBoardProps): JSX.Element {
  const { theme } = useTheme();
  ...
  return (
    <Box flex={1} bg="background.primary">
      <ScrollView contentContainerStyle={{ padding: 20 }} showsVerticalScrollIndicator={false}>
        ...
        {/* Your Position Card */}
        {myEntry && (
          <Box p="lg" borderRadius="xl" bg={`${theme.colors.primary[500]}15`} borderWidth={2} borderColor="primary.500" mb="lg" alignItems="center">
...
export default SquadWeeklyBoard;
