...
function SquadStreak({ days }: { days: number }): JSX.Element {
  const flameStyle = useAnimatedStyle(() => ({
    transform: [
      {
        scale: withRepeat(withSequence(withTiming(1.1, { duration: 800 }), withTiming(1, { duration: 800 })), -1, true),
      },
    ],
  }));
 ...
 */
function SquadMemberCard({ member, onNudge, onView }: { member: SquadMember; onNudge?: (userId: string) => void; onView?: (userId: string) => void }): JSX.Element {
  const { theme } = useTheme();
  ...
  return (
    <Pressable onPress={() => onView?.(member.userId)} accessibilityLabel="Interactive control" accessibilityRole="button" accessibilityHint="Activates this control">
      <Box flexDirection="row" alignItems="center" justifyContent="space-between" p="md" borderRadius="xl" bg={member.isMe ? `${theme.colors.primary[500]}15` : "background.secondary"} borderWidth={1} borderColor={member.isMe ? "primary.500" : "border.light"} mb="sm">
        <Box flexDirection="row" alignItems="center" gap="md">
          ...
          <Box>
            <Box flexDirection="row" alignItems="center" gap="sm">
              ...
              {member.isMe && (
                <Box px="xs" py="xs" borderRadius="sm" bg="primary.500">
        ...
        {/* Nudge button */}
        {canNudge && (
          <Pressable onPress={() => onNudge?.(member.userId)} accessibilityLabel="Nudge ⚡ button" accessibilityRole="button" accessibilityHint="Activates this control">
    ...
    <Box flex={1} bg="background.primary">
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        ...
        <Box mb="lg">
          ...
          {members.map((member) => (
            <SquadMemberCard key={member.userId} member={member} onNudge={onNudgeMember} onView={onViewMember} />
          ))}
        </Box>
      </ScrollView>
