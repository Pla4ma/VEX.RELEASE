import React,{useState,useCallback}from'react';import{View,Text,ScrollView,Pressable,Image,ActivityIndicator}from'react-native';import{FlashList}from'@shopify/flash-list';import{useThemeObject}from'../../../theme';import{Card,Button,Avatar,Badge}from'../../../components';import{useSquad,useSquadMembers,useSquadPermissions,useStartSquadSession}from'../hooks';import{type SquadMemberDetail}from'../schemas';import{createSheet}from'@/shared/ui/create-sheet';interface SquadHubProps{squadId:string;userId:string;onInvitePress?:()=>void;onSettingsPress?:()=>void;onMemberPress?:(member:SquadMemberDetail)=>void;}export const SquadHub:React.FC<SquadHubProps>=({squadId,userId,onInvitePress,onSettingsPress,onMemberPress})=>{const theme=useThemeObject();const{data:squad,isLoading:squadLoading,error:squadError}=useSquad(squadId);const{data:members,isLoading:membersLoading}=useSquadMembers(squadId);const{hasPermission}=useSquadPermissions(squadId,userId);const startSession=useStartSquadSession(userId,squadId);const[isStartingSession,setIsStartingSession]=useState(false);const handleStartSession=useCallback(async()=>{setIsStartingSession(true);try{await startSession.mutateAsync({name:'Squad Focus',description:null,duration:1500,category:'focus',maxParticipants:10});}finally{setIsStartingSession(false);}},[startSession]);if(squadLoading||membersLoading){return<View style={[styles.container,{backgroundColor:theme.colors.background.primary}]}>
    ...
    </ScrollView>;};const styles=createSheet({container:{flex:1},content:{padding:16,gap:16},headerCard:{padding:16},headerRow:{flexDirection:'row',alignItems:'center',gap:12},avatar:{width:64,height:64,borderRadius:32},avatarPlaceholder:{width:64,height:64,borderRadius:32,justifyContent:'center',alignItems:'center'},avatarText:{fontSize:24,fontWeight:'bold',color:'#fff'},headerInfo:{flex:1,gap:4},squadName:{fontSize:20,fontWeight:'bold'},badgeRow:{flexDirection:'row',gap:8},description:{marginTop:12,fontSize:14,lineHeight:20},card:{padding:16},sectionHeader:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:12},sectionTitle:{fontSize:16,fontWeight:'600'},memberCount:{fontSize:14},progressBar:{marginVertical:8},progressText:{fontSize:12,textAlign:'center'},bonusText:{fontSize:12,textAlign:'center',marginTop:4},actionRow:{flexDirection:'row',flexWrap:'wrap',gap:8},actionButton:{flex:1,minWidth:120},onlineMembersList:{paddingVertical:8,gap:16},onlineMemberItem:{alignItems:'center',position:'relative'},onlineIndicator:{position:'absolute',bottom:16,right:-2,width:12,height:12,borderRadius:6,backgroundColor:'#4CAF50',borderWidth:2,borderColor:'#fff'},memberRow:{flexDirection:'row',alignItems:'center',gap:12,paddingVertical:8},memberInfo:{flex:1},memberName:{fontSize:14,fontWeight:'500'},memberRole:{fontSize:12},memberDivider:{marginVertical:4},statsGrid:{flexDirection:'row',justifyContent:'space-around',marginTop:12},statItem:{alignItems:'center'},statValue:{fontSize:24,fontWeight:'bold'},statLabel:{fontSize:12,marginTop:4},errorText:{textAlign:'center',fontSize:16,marginTop:24}});
