...
export const SquadValidationRules = {
  ...
  updateSquad: (current: Squad, updates: UpdateSquadInput, memberCount: number): { valid: boolean; errors: string[]; warnings: string[] } => {
    const errors: string[] = [];
  ...
  validateRoleTransfer: (currentRole: SquadRole, targetRole: SquadRole, isFounderTransfer: boolean): { valid: boolean; error?: string } => {
...
export const SquadBusinessRules = {
  shouldAutoArchive: (squad: Squad, members: Array<{ lastActiveAt: number }>): boolean => {
    ...
    if (members.length < MINIMUM_MEMBERS_FOR_ACTIVE) {
      return true;
    }

    const now = Date.now();
    const activeMembers = members.filter((member) => now - member.lastActiveAt < INACTIVE_THRESHOLD_DAYS * 24 * 60 * 60 * 1000);
  ...
  shouldAutoKick: (member: { joinedAt: number; lastContributionAt?: number | null; role: string }): { shouldKick: boolean; reason?: string } => {
    const now = Date.now();
