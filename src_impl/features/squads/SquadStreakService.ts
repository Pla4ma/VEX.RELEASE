...
export async function checkSquadActivityToday(squadId: string): Promise<{ hasActivity: boolean; memberCount: number; totalMinutes: number }> {
  ...
  const { data: members, error: memberError } = await supabase.from("squad_members").select("user_id").eq("squad_id", squadId).eq("is_active", true);
  ...
  const totalMinutes = qualifyingSessions.reduce((sum: number, s: { effective_duration: number }) => sum + s.effective_duration / 60, 0);
...
export async function getSquadStreak(squadId: string): Promise<SquadStreak> {
  ...
  const { data: streakData, error } = await supabase.from("squad_streaks").select("*").eq("squad_id", squadId).single();
 ...
 */
export async function updateSquadStreak(squadId: string, streak: Partial<SquadStreak>): Promise<void> {
  const supabase = getSupabaseClient();
 ...
 */
export function getSquadStreakMotivation(squadStreak: number, individualStreak: number): string | null {
  if (squadStreak <= individualStreak) {
    return null;
  }
...
export async function processSquadStreakOnSessionComplete(squadId: string, userId: string, sessionDuration: number): Promise<SquadStreak | null> {
  ...
  // Check for milestone
  if (hasSquadStreakMilestone(newStreak.currentStreak) && !hasSquadStreakMilestone(currentStreak.currentStreak)) {
 ...
 */
export async function getSquadMemberCompletionStatus(squadId: string): Promise<SquadMemberSessionStatus[]> {
  const supabase = getSupabaseClient();
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const todayStart = today.getTime();
