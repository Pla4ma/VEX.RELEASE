/**
 * Squad Operations Retry Logic
 *
 * Handles transient failures, network issues, and race conditions.
 */
...
const DEFAULT_RETRY_CONFIG: RetryConfig = {
  ...
  maxDelayMs: 10000,
  backoffMultiplier: 2,
  retryableErrors: ['NETWORK_ERROR', 'TIMEOUT', 'RATE_LIMITED', 'CONCURRENT_MODIFICATION'],
};
...
export class RetryableOperation<T> {
  ...
  async execute(): Promise<T> {
    while (this.attempts < this.config.maxAttempts) {
      try {
        ...
        if (!this.isRetryable(error) || this.attempts >= this.config.maxAttempts) {
    ...
    throw this.lastError || new Error(`Max retry attempts exceeded for ${this.context}`);
  }
  ...
  private calculateDelay(): number {
    const exponentialDelay = this.config.baseDelayMs * Math.pow(this.config.backoffMultiplier, this.attempts - 1);
  ...
  private enhanceError(error: unknown): Error {
    ...
    enhanced.message = `[${this.context}] After ${this.attempts} attempts: ${enhanced.message}`;
    return enhanced;
  }
}
...
// Offline queue for operations during network issues
