...
export async function getUserBattlePassSummary(userId: string, seasonId: string): Promise<UserBattlePassSummary | null> {
  ...
  const unclaimedPremium = userBp.isPremium ? userBp.currentTier - userBp.claimedPremiumTiers.length : 0;
...
export async function claimAllAvailable(
  ...
  if (!summary) {
    return { freeClaimed: 0, premiumClaimed: 0, rewards: [] };
  }
...
async function claimRetroactivePremium(userId: string, seasonId: string, currentTier: number): Promise<RetroactiveClaimResult> {
...
export async function getBattlePassDisplay(
  userId: string,
  seasonId: string,
): Promise<{
  tiers: TierDisplay[];
  ...
  const [userBp, tiers] = await Promise.all([repository.fetchUserBattlePass(userId, seasonId), repository.fetchBattlePassTiers(seasonId)]);
