...
export const BattlePassSchema = z
  .object({
    ...
    tierCount: z.number().int().min(BATTLE_PASS_LIMITS.MIN_TIER_COUNT).max(BATTLE_PASS_LIMITS.MAX_TIER_COUNT),
    xpPerTier: z.number().int().min(BATTLE_PASS_LIMITS.MIN_XP_PER_TIER).max(BATTLE_PASS_LIMITS.MAX_XP_PER_TIER),
    premiumPriceGems: z.number().int().min(BATTLE_PASS_LIMITS.MIN_PREMIUM_PRICE).max(BATTLE_PASS_LIMITS.MAX_PREMIUM_PRICE),
...
export const TierRewardSchema = z
  .object({
    id: z.string().uuid(),
...
export const UserBattlePassSchema = z
  .object({
    id: z.string().uuid(),
...
export const UserBattlePassSummarySchema = z
  .object({
    seasonId: z.string().uuid(),
    currentTier: z.number().int().nonnegative(),
...
export const ProgressionCalculationSchema = z
  .object({
    currentTier: z.number().int().nonnegative(),
    tierXp: z.number().int().nonnegative(),
    xpToNextTier: z.number().int().nonnegative(),
...
export const TierAdvanceResultSchema = z
  .object({
...
export const PremiumPurchaseResultSchema = z
  .object({
    success: z.boolean(),
...
export const RetroactiveClaimResultSchema = z
  .object({
...
export const CreateBattlePassInputSchema = z
  .object({
    seasonId: z.string().uuid(),
    tierCount: z.number().int().min(BATTLE_PASS_LIMITS.MIN_TIER_COUNT).max(BATTLE_PASS_LIMITS.MAX_TIER_COUNT).default(50),
    xpPerTier: z.number().int().min(BATTLE_PASS_LIMITS.MIN_XP_PER_TIER).max(BATTLE_PASS_LIMITS.MAX_XP_PER_TIER).default(1000),
    premiumPriceGems: z.number().int().min(BATTLE_PASS_LIMITS.MIN_PREMIUM_PRICE).max(BATTLE_PASS_LIMITS.MAX_PREMIUM_PRICE).default(499),
    theme: z.string().max(50).optional(),
  })
  .strict();

export const CreateBattlePassTierInputSchema = z
  .object({
    seasonId: z.string().uuid(),
    tierNumber: z.number().int().positive(),
    xpRequired: z.number().int().positive(),
...
export const AddBattlePassXpInputSchema = z
  .object({
    userId: z.string().uuid(),
    seasonId: z.string().uuid(),
...
export const ClaimTierInputSchema = z
  .object({
    userId: z.string().uuid(),
    seasonId: z.string().uuid(),
    tierNumber: z.number().int().positive(),
...
export const PurchasePremiumInputSchema = z
  .object({
    userId: z.string().uuid(),
    seasonId: z.string().uuid(),
...
export const TierDisplaySchema = z
  .object({
    tierNumber: z.number().int().positive(),
    xpRequired: z.number().int().positive(),
...
export const BattlePassDisplaySchema = z
  .object({
    seasonId: z.string().uuid(),
...
export const BattlePassAnalyticsSchema = z
  .object({
