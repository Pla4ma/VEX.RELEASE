...
export function getBattlePassProgress(userBattlePass: UserBattlePass): {
  currentTier: number;
  maxTier: number;
  ...
  const tierProgressPercent = xpToNextTier > 0 ? Math.floor((xpInCurrentTier / xpToNextTier) * 100) : 100;
...
export async function processSeasonEnd(repository: { fetchAllActivePasses: () => Promise<UserBattlePass[]>; archiveSeason: (seasonId: string, passes: UserBattlePass[]) => Promise<void>; startNewSeason: (season: BattlePassSeason) => Promise<void>; resetUserPasses: () => Promise<void> }): Promise<{ archived: number; unclaimedRewardsDistributed: number }> {
  const now = Date.now();
