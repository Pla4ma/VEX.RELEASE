...
export const challengeKeys = {
  ...
  byType: (seasonId: string, type: string) => [...challengeKeys.all, 'type', seasonId, type] as const,
  ...
  completable: (userId: string) => [...challengeKeys.all, 'completable', userId] as const,
};
...
/**
 * Get challenges by type for a season
 */
export function useChallengesByType(seasonId: string, type: 'DAILY' | 'WEEKLY' | 'EVENT') {
  return useQuery({
...
/**
 * Get challenge summaries for user
 */
...
export function useChallengeProgress(userId: string) {
  ...
  const completed = challenges.filter((item) => item.userChallenge.status === 'COMPLETED' || item.userChallenge.status === 'CLAIMED').length;
  return {
    ...query,
    completed,
    total,
...
export function useRerollChallenge() {
  ...
  return useMutation({
    mutationFn: (input: { userId: string; challengeId: string; usePaidReroll: boolean; idempotencyKey?: string }) =>
...
/**
 * Hook to listen for challenge events and refresh data
 */
export function useChallengeEvents(userId: string) {
  const queryClient = useQueryClient();
