 ...
 *
 * Validates challenge generation, completion, and balancing.
 *
 * @phase 11 - Deepening: Challenge validation
 */
...
export function validateChallengeCompletion(
  ...
  if (completion.timeSpent < challenge.minDuration) {
    result.errors.push(`Completed too fast: ${completion.timeSpent}s (min: ${challenge.minDuration}s)`);
    result.suspicious = true;
  }

  if (completion.timeSpent > challenge.maxDuration) {
    result.errors.push(`Exceeded time limit: ${completion.timeSpent}s (max: ${challenge.maxDuration}s)`);
    result.valid = false;
  }
  ...
  // Check 3: Speed vs difficulty
  const speedRatio = challenge.expectedDuration / Math.max(completion.timeSpent, 1);
  if (speedRatio > 5) {
    result.errors.push(`Suspiciously fast completion (${speedRatio.toFixed(1)}x expected speed)`);
    result.suspicious = true;
  }

  // Check 4: Comparison to user history
  if (userHistory.avgTimeForDifficulty > 0) {
    const userRatio = completion.timeSpent / userHistory.avgTimeForDifficulty;
    if (userRatio < 0.2) {
      result.errors.push(`Much faster than user's average (${userRatio.toFixed(2)}x)`);
      result.suspicious = true;
    }
  }

  // Check 5: Duplicate completion
  const isDuplicate = userHistory.previousCompletions.some(
    c => c.challengeId === completion.challengeId &&
...
): { balanced: boolean; recommendations: string[]; difficultyAdjustment: number } {
