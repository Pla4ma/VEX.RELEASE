...
export function XPBundleCard({ gemBalance, hasPurchasedThisWeek, hoursUntilReset, onPurchase, onViewDetails }: XPBundleCardProps): JSX.Element | null {
  ...
  React.useEffect(() => {
    if (!hasPurchasedThisWeek && hoursUntilReset < 24) {
      pulseScale.value = withRepeat(withSequence(withTiming(1.03, { duration: 1000 }), withTiming(1, { duration: 1000 })), -1, true);
    }
  ...
  const timeDisplay = useMemo(() => {
    ...
    if (hoursUntilReset < 1) {
      return `${Math.floor(hoursUntilReset * 60)}m left`;
    }
    if (hoursUntilReset < 24) {
      return `${Math.floor(hoursUntilReset)}h left`;
    }
  ...
  const handlePurchase = async () => {
    if (!canAfford || hasPurchasedThisWeek) {
      return;
    }
  ...
  return (
    <Animated.View entering={FadeInRight.duration(400)} style={[styles.container, pulseStyle]}>
      <Pressable onPress={handlePress} style={[styles.card, { backgroundColor: colors.background.secondary }, hasPurchasedThisWeek && styles.purchasedCard]} accessibilityLabel="Interactive control" accessibilityRole="button" accessibilityHint="Activates this control">
            ...
            <View style={styles.activeSessions}>
              ...
              <View style={styles.progressDots}>
                {Array.from({ length: BUNDLE_CONFIG.sessionCount }).map((_, i) => (
                  <View key={i} style={[styles.dot, { backgroundColor: colors.success.DEFAULT }]} />
                ))}
              </View>
            </View>
          )}
        ...
        <View style={styles.footer}>
          ...
          {hasPurchasedThisWeek ? (
            ...
            <Button onPress={handlePurchase} disabled={!canAfford} variant={canAfford ? "primary" : "secondary"} size="md" style={!canAfford ? styles.disabledButton : styles.buyButton} accessibilityLabel="Action button" accessibilityRole="button" accessibilityHint="Activates this control">
