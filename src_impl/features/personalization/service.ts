...
function resolveHiddenSystems(
  ...
  if (profile.motivationStyle === 'calm' || profile.motivationStyle === 'study_focused') {
  ...
  if (stats.bossChallengeEngagement === 'none' && stats.totalCompletedSessions > 5) {
...
export function resolveVexExperience(
  ...
  const bossMode = !boss.isVisible ? 'hidden' : boss.intensity === 'subtle' ? 'momentum' : 'personal_boss';
  ...
  return VexExperienceSchema.parse({
    ...
    primaryHomeCTA: {
      ...
      label: isStudyRelevant ? `Continue ${profile.studyLayerName}` : 'Start session',
    },
    secondaryHomeCTA: stats.totalCompletedSessions > 0
      ? { intent: 'OPEN_PROGRESS', label: 'Review progress' }
      : null,
    homeSections: home.sections,
    homeSpotlight: stats.totalCompletedSessions === 0 ? 'none'
      : isStudyRelevant ? 'study_layer'
        : boss.isVisible ? 'boss_momentum' : 'progress_proof',
    coachMessageStyle: resolveCoachMode(profile),
    companionMode: companionIntensity === 'active'
      ? 'active_partner'
      : companionIntensity === 'subtle' ? 'quiet_presence' : 'visual_coach',
    companionVisualIntensity: companionIntensity,
    bossMode,
    bossCopyStyle: boss.intensity === 'intense'
      ? 'controlled_intensity'
      : boss.intensity === 'game-like' ? 'boss_health' : 'subtle_momentum',
    ...
    studyLayerProminence: isStudyRelevant ? 'spotlight' : availability.study ? 'supporting' : 'hidden',
    sessionSuggestion: behavior.sessionSuggestion,
    completionSequence: completion.sequence,
    progressEmphasis: stats.totalCompletedSessions >= 7
      ? 'intelligence'
      : stats.totalCompletedSessions >= 2 ? 'rhythm' : 'basic',
    ...
    allowedRoutes: [
      ...
      ...(availability.boss && boss.isVisible && boss.intensity !== 'subtle' ? ['Boss'] : []),
    ],
    allowedNotificationTypes: [
      'gentle_return',
      'progress_milestone',
      ...(availability.study ? ['study_continuation'] : []),
      ...(availability.boss && boss.isVisible && boss.intensity !== 'subtle' ? ['boss_momentum'] : []),
    ],
    nextBestAction: {
      intent: isStudyRelevant ? 'CONTINUE_STUDY_PATH' : 'START_SESSION',
    ...
    release: {
      ...
      teasedOnly: systems.teasedSystems.concat(['deep_coach_memory', 'weekly_intelligence', 'visual_identity_depth']),
    },
    routeGates: {
      boss: {
