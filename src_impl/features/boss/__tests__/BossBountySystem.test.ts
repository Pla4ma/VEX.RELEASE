...
// Mock the BossBountySystem's internal storage
...
describe('BossBountySystem', () => {
  ...
  describe('placeBounty', () => {
    ...
    it('allows stacking up to 4 bounties on the same encounter', async () => {
      ...
      expect(status.maxStackReached).toBe(true);
    ...
    it('prevents user from placing duplicate bounty on same encounter', async () => {
      ...
      const result1 = await placeBounty({
        userId: mockUserId,
        encounterId: mockEncounterId,
      });
      expect(result1.success).toBe(true);

      // Second bounty from same user fails
      const result2 = await placeBounty({
        userId: mockUserId,
        encounterId: mockEncounterId,
      });
  ...
  describe('getBountyStatus', () => {
    it('returns correct status when no bounties exist', () => {
      ...
      expect(status.hasActiveBounty).toBe(false);
      expect(status.bountyCount).toBe(0);
      expect(status.totalLootMultiplier).toBe(0);
      expect(status.canPlaceBounty).toBe(true);
    ...
    it('returns correct status with active bounties', async () => {
      ...
      expect(status.bountyCount).toBe(1);
      expect(status.totalLootMultiplier).toBe(2);
      expect(status.canPlaceBounty).toBe(true); // Different user can place
      expect(status.hoursRemaining).toBeGreaterThanOrEqual(23);
    });
  });
  ...
  describe('consumeBountiesOnDamage', () => {
    it('returns 2× loot multiplier when one bounty is active', async () => {
      ...
      const result = consumeBountiesOnDamage(mockEncounterId, 25);
    ...
    it('returns 8× loot multiplier when 4 bounties are stacked', async () => {
      ...
      const result = consumeBountiesOnDamage(mockEncounterId, 50);

      expect(result.lootMultiplier).toBe(8);
    ...
    it('marks bounties as consumed after damage and cannot be reused', async () => {
      ...
      // First damage consumes the bounty
      const result1 = consumeBountiesOnDamage(mockEncounterId, 25);
      expect(result1.consumedCount).toBe(1);

      // Second damage finds no active bounties
      const result2 = consumeBountiesOnDamage(mockEncounterId, 25);
      expect(result2.consumedCount).toBe(0);
      expect(result2.lootMultiplier).toBe(1);
    });
  });
  ...
  describe('getActiveBounties', () => {
    ...
    it('returns only active (non-consumed, non-expired) bounties', async () => {
      ...
      // Before consumption
      let bounties = getActiveBounties(mockEncounterId);
      expect(bounties.length).toBe(1);

      // After consumption
      consumeBountiesOnDamage(mockEncounterId, 25);
      bounties = getActiveBounties(mockEncounterId);
