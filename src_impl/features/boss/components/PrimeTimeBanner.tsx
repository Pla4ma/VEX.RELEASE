/**
 * PrimeTimeBanner Component
 *
 * Glowing banner displayed on Home screen during boss Prime Time.
 * Shows countdown, XP multiplier, and urgent call-to-action.
 *
 * @phase 11.1
 */
...
interface PrimeTimeBannerProps {
  ...
  /** Boss name */
  bossName: string;
  /** On press - navigate to boss fight */
  onPress: () => void;
...
/**
 * Glowing pulse animation for the banner
 */
function useGlowAnimation(): ReturnType<typeof useAnimatedStyle> {
  return useAnimatedStyle(() => ({
    shadowOpacity: withRepeat(withSequence(withTiming(0.3, { duration: 1000 }), withTiming(0.6, { duration: 1000 })), -1, true),
    transform: [
      {
        scale: withRepeat(withSequence(withTiming(1, { duration: 1000 }), withTiming(1.02, { duration: 1000 })), -1, true),
      },
    ],
  }));
}
 ...
 */
function LightningAnimation(): JSX.Element {
  const animatedStyle = useAnimatedStyle(() => ({
    opacity: withRepeat(withSequence(withTiming(0, { duration: 100 }), withTiming(1, { duration: 100 }), withTiming(0, { duration: 100 }), withDelay(800, withTiming(0, { duration: 100 }))), -1, false),
    transform: [
      {
        translateX: withRepeat(withSequence(withTiming(-2, { duration: 50 }), withTiming(2, { duration: 50 }), withTiming(0, { duration: 50 })), -1, false),
      },
    ],
  }));
...
/**
 * Prime Time Banner - Full size
 */
export function PrimeTimeBanner({ window, bossName, onPress, compact = false }: PrimeTimeBannerProps): JSX.Element {
  ...
  useEffect(() => {
    setDisplayTime(window.timeRemainingMinutes);
    const interval = setInterval(() => {
      setDisplayTime((prev) => Math.max(0, prev - 1));
    }, 60000);
    return () => clearInterval(interval);
  }, [window.timeRemainingMinutes]);
