/**
 * BossTimeoutScreen Component
 *
 * Boss "escaped" screen — must sting but not crush.
 * Shows boss retreating, consolation rewards, comeback encouragement.
 *
 * @phase 3A.3
 */
...
function RetreatingBoss({ bossTier, escapeTaunt }: { bossTier: BossTimeoutScreenProps['bossTier']; escapeTaunt?: string }): JSX.Element {
  ...
  const retreatStyle = useAnimatedStyle(() => ({
    transform: [
      {
        translateX: withTiming(200, { duration: 2000 }),
      },
      {
        scale: withSequence(withTiming(1, { duration: 500 }), withTiming(0.5, { duration: 1500 })),
      },
    ],
    opacity: withSequence(withTiming(1, { duration: 500 }), withTiming(0, { duration: 1500 })),
  }));

  const fadeStyle = useAnimatedStyle(() => ({
    opacity: withDelay(1000, withTiming(1, { duration: 500 })),
  }));

  const laughStyle = useAnimatedStyle(() => ({
    opacity: withSequence(withDelay(500, withTiming(1, { duration: 300 })), withDelay(1000, withTiming(0, { duration: 500 }))),
    transform: [
      {
        translateY: withSequence(withDelay(500, withTiming(-20, { duration: 200 })), withTiming(0, { duration: 200 })),
      },
    ],
  }));

  return (
    <Box height={240} justifyContent="center" alignItems="center" overflow="hidden">
      {/* Boss retreating */}
      <Animated.View
        style={[
          {
...
/**
 * Main boss timeout screen
 */
export function BossTimeoutScreen({ bossName, bossTier, remainingHealth, escapeTaunt, nextEncounterHealthBonus, consolation, retryCooldownDays, coachMessage, onStartSession, onGoHome }: BossTimeoutScreenProps): JSX.Element {
  const { theme } = useTheme();
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.colors.background.primary }} contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
      <Box flex={1} px="xl" py="xl" gap="lg">
        ...
        {/* PHASE 12.2: Harder respawn warning */}
        {nextEncounterHealthBonus && nextEncounterHealthBonus > 0 && (
...
export default BossTimeoutScreen;

export * from "./BossTimeoutScreen.types";
