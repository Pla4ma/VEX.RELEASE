 ...
 */
function FloatingCheer({ cheer, onComplete }: { cheer: CheerMessage; onComplete: () => void }): JSX.Element {
  const animatedStyle = useAnimatedStyle(() => ({
    transform: [
      {
        translateY: withSequence(withTiming(-20, { duration: 100 }), withTiming(-100, { duration: 1500 })),
      },
      {
        translateX: withTiming(Math.random() * 40 - 20, { duration: 1500 }),
      },
    ],
    opacity: withSequence(withTiming(1, { duration: 100 }), withTiming(0, { duration: 1500 })),
  }));
    ...
    <Box gap="sm">
      ...
      <Box flexDirection="row" flexWrap="wrap" justifyContent="center" gap="sm">
        {CHEER_OPTIONS.map((cheer, index) => (
          <Pressable key={index} onPress={() => onCheer(cheer)} accessibilityLabel="Interactive control" accessibilityRole="button" accessibilityHint="Activates this control">
            <Box
              px="md"
              py="sm"
              borderRadius="full"
 ...
 */
export function BossSpectatorMode({ boss, contributors, spectatorUserId, spectatorName, onCheer, onJoinFight, onClose }: BossSpectatorModeProps): JSX.Element {
  const { theme } = useTheme();
  ...
  return (
    <Box flex={1} bg="background.primary">
        ...
        <Box>
          ...
          <Box gap="sm">
            {sortedContributors.slice(0, 5).map((contributor, index) => (
              <ContributorRow key={contributor.userId} contributor={contributor} rank={index + 1} />
            ))}
          </Box>
        </Box>
      ...
      {activeCheers.map((cheer) => (
        <FloatingCheer key={cheer.id} cheer={cheer} onComplete={() => removeCheer(cheer.id)} />
      ))}
