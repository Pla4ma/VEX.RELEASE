describe('Content Study — Generation Edge Cases', () => {
  describe('generateStudyPlan — Gemini timeout', () => {
    ...
    it('falls back to default study plan on timeout', () => {
      const fallbackTasks = [
        { id: 'task-1', content: 'Read through the content carefully', estimatedMinutes: 15, priority: 'HIGH' as const },
        { id: 'task-2', content: 'Identify and note key concepts', estimatedMinutes: 10, priority: 'HIGH' as const },
        { id: 'task-3', content: 'Review and summarize main points', estimatedMinutes: 15, priority: 'MEDIUM' as const },
      ];
  ...
  describe('generateStudyPlan — Gemini invalid JSON', () => {
    ...
    it('handles JSON with wrong types', () => {
      const wrongTypes = JSON.stringify({ tasks: 'not an array', quizItems: 123 });
    ...
    it('handles markdown-wrapped JSON', () => {
      ...
      const cleaned = markdownWrapped.replace(/```json\s*/g, '').replace(/```\s*/g, '').trim();
  ...
  describe('generateStudyPlan — quota exceeded', () => {
    ...
    it('includes remaining count in error response', () => {
      const response = { success: false, error: 'Daily limit reached', remaining: 0 };
