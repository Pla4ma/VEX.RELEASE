...
export class StudySessionManager {
  ...
  async saveSession(session: PersistedStudySession): Promise<void> {
    ...
    const existingIndex = sessions.findIndex((s) => s.generationId === session.generationId && !s.endTime);
  ...
  async completeSession(generationId: string, updates: Partial<PersistedStudySession>): Promise<boolean> {
    const sessions = await this.getAllSessions();
    const index = sessions.findIndex((s) => s.generationId === generationId && !s.endTime);
