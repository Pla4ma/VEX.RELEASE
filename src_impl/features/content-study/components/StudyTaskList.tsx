...
export const StudyTaskList: React.FC<StudyTaskListProps> = ({ tasks, completedIds, activeId, onTaskComplete, onTaskSelect, showDependencies = true, readOnly = false, estimatedTotalTime, completedTime }) => {
  ...
  const sortedTasks = useMemo(() => {
    // Sort by priority (HIGH -> MEDIUM -> LOW) then by estimated time
  ...
  const completionPercentage = tasks.length > 0 ? (completedIds.size / tasks.length) * 100 : 0;
          ...
          return (
            <Pressable
              ...
              style={({ pressed }) => [
                styles.taskCard,
                {
                  backgroundColor: isCompleted ? theme.colors.success[50] : isActive ? theme.colors.primary[50] : theme.colors.background.secondary,
                  borderColor: isActive ? theme.colors.primary[500] : theme.colors.border.DEFAULT,
                  opacity: isLocked ? 0.5 : pressed ? 0.8 : 1,
                },
              ]}
              onPress={() => !isLocked && onTaskSelect(task.id)}
            ...
            >
              ...
              <Pressable
                ...
                onPress={() => !readOnly && onTaskComplete(task.id)}
                disabled={isLocked || readOnly}
                accessibilityLabel="Interactive control"
                accessibilityRole="button"
                accessibilityHint="Activates this control"
