...
export const YouTubeInput: React.FC<YouTubeInputProps> = ({ value, onChange, onValidationChange, onExtract, disabled = false, isExtracting = false, extractionError, videoInfo }) => {
  ...
  const handlePaste = useCallback(async () => {
    // On mobile, this would use Clipboard API
    ...
    <View style={styles.container}>
      ...
      <View
        style={[
          ...
          {
            borderColor: isFocused ? theme.colors.primary[500] : extractionError || validationState.errors.length > 0 ? theme.colors.error[500] : validationState.warnings.length > 0 ? theme.colors.warning[500] : theme.colors.border.DEFAULT,
          },
        ]}
      >
        ...
        {value.length > 0 && !disabled && (
          <Pressable onPress={clearInput} style={({ pressed }) => [styles.clearButton, pressed && { opacity: 0.8 }]} accessibilityLabel="Interactive control" accessibilityRole="button" accessibilityHint="Activates this control">
        ...
        >
          ...
          {extractionError && (
            <View style={styles.extractionErrorContainer}>
              ...
              {onExtract && (
                <Button variant="ghost" size="sm" onPress={onExtract} accessibilityLabel="Retry button" accessibilityRole="button" accessibilityHint="Activates this control">
                  Retry
                </Button>
              )}
            </View>
