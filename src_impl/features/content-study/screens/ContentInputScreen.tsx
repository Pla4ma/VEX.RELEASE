  ...
  return (
    <AppScreen keyboardAvoiding contentStyle={{ gap: theme.spacing[5] }}>
      <View style={{ gap: theme.spacing[2] }}>
        <Text color="primary.300" variant="label">AI study lab</Text>
        <Text color="text.primary" variant="h1">{UI_TEXT.INPUT_TITLE}</Text>
        <Text color="text.secondary" variant="body">{UI_TEXT.INPUT_SUBTITLE}</Text>
      </View>
      ...
      <Card size="lg" variant="elevated" style={{ minHeight: theme.spacing[8] * 4 }}>
        {renderActiveInput()}
      </Card>

      {error && !showExtractionProgress ? (
        <Card size="md" state="error" variant="glass">
          <Text color="error.DEFAULT" variant="label">Study processing paused</Text>
          <Text color="text.secondary" mt="xs" variant="bodySmall">{error}</Text>
          <Button mt="md" onPress={clearError} size="sm" variant="ghost">Dismiss</Button>
        </Card>
      ) : null}
