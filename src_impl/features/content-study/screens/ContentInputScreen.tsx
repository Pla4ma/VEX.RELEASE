...
export function ContentInputScreen(): JSX.Element {
  ...
  const { state, uploadProgress, isSubmitting, error, setTab, setPastedText, setYoutubeUrl, setSelectedFile, clearError, submit } = useContentInput();
  ...
  }, [route.params?.preferredTab, route.params?.prefillData?.text, route.params?.prefillData?.url, setPastedText, setTab, setYoutubeUrl, state.activeTab, state.pastedText, state.youtubeUrl]);
  ...
  const renderActiveInput = () => {
    ...
    return <TextPasteInput value={state.pastedText} onChange={setPastedText} disabled={isSubmitting} autoFocus />;
  };
    ...
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.keyboardView}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <InputTypeSelector activeTab={state.activeTab} onTabChange={setTab} disabled={isSubmitting} hasDrafts={hasDrafts} />
          ...
          {error && !showExtractionProgress ? (
            <View style={styles.errorContainer}>
              ...
              <Pressable onPress={clearError} accessibilityLabel="Dismiss button" accessibilityRole="button" accessibilityHint="Activates this control">
                <Text style={styles.dismissText}>Dismiss</Text>
              </Pressable>
            </View>
          ) : null}

          <Pressable style={({ pressed }) => [styles.submitButton, isSubmitting && styles.submitButtonDisabled, pressed && { opacity: 0.8 }]} onPress={() => void handleSubmit()} disabled={isSubmitting} accessibilityLabel="Interactive control" accessibilityRole="button" accessibilityHint="Activates this control">
            {isSubmitting ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.submitButtonText}>{UI_TEXT.SUBMIT_BUTTON}</Text>}
          </Pressable>
        </ScrollView>
