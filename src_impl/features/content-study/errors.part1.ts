...
export class ContentStudyErrorHandler {
  ...
  registerCallback(codes: ContentStudyErrorCode | ContentStudyErrorCode[], callback: (error: ContentStudyError) => void): () => void {
    ...
    if (callbacks) {
      callbacks.forEach((cb) => {
        try {
          ...
          captureException(e instanceof Error ? e : new Error('Content study error callback failed'), { area: 'content-study.error-callback' });
        }
      });
    }
  }
