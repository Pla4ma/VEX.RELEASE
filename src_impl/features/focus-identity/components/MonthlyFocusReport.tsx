/**
 * Monthly Focus Report Component
 *
 * Generates a "Monthly Focus Report" like a credit report but for focus.
 * Shows score changes, drivers, streak highlights, percentile, and identity statement.
...
// ============================================================================
function MonthlyReportSkeleton(): JSX.Element {
...
export function MonthlyFocusReport({ userId, onClose, visible }: MonthlyFocusReportProps) {
  ...
  React.useEffect(() => {
    if (report && !hasPublishedView) {
      publishMonthlyReportViewed(userId, report.month, report.grade, report.change);
  ...
  // Get score color early to avoid conditional hook call
  ...
  const percentile = useMemo(() => {
    ...
    // Approximate percentile calculation based on score ranges
    if (report.endingScore >= 800) {
      return 99;
    }
    if (report.endingScore >= 740) {
      return 95;
    }
    if (report.endingScore >= 670) {
      return 85;
    }
    if (report.endingScore >= 580) {
      return 70;
    }
    if (report.endingScore >= 500) {
      return 50;
    }
    if (report.endingScore >= 420) {
      return 30;
    }
    return 15;
  }, [report]);
...
#VEX #FocusProductivity`;
  ...
  // Handle close with dismiss event
  const handleClose = () => {
    if (report) {
