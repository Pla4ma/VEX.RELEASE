...
export function FocusScoreHomeWidget({ model, onPress, onRetry }: FocusScoreHomeWidgetProps): JSX.Element {
  ...
  return (
    <View style={{ gap: theme.spacing[2] }}>
      ...
      >
        ...
        <Text variant="bodySmall" color={theme.colors.text.secondary}>Delta since last session: {delta >= 0 ? `+${delta}` : delta}</Text>
        <Text variant="bodySmall" color={theme.colors.text.secondary} numberOfLines={2}>{reason}</Text>
      </Pressable>
    </View>
  );
}
