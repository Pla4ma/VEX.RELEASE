...
export function initializeFocusScoreIntegration(): () => void {
  ...
  const unsubscribeComeback = eventBus.subscribe('retention:comebackCompleted', async (rawEvent) => {
    const event = rawEvent as { userId: string; questId: string; questType: string; completedAt: number | null };
    const userId = event.userId;
    if (!userId) {return;}
    ...
    try {
      ...
      const result = calculateFocusScoreUpdate({
        ...
        occurredAt: new Date(event.completedAt ?? Date.now()).toISOString(),
        signals: {
          consistency: 45,
          streakStability: 40,
          sessionQuality: 60,
      ...
      Sentry.captureException(error, {
        tags: { feature: 'focus-identity', operation: 'comeback-completed-integration' },
      });
    }
  });
