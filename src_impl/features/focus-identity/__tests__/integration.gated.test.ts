...
jest.mock('../../focus-contract/service', () => ({
  getCompletionSignal: jest.fn().mockResolvedValue({ rate: 0.8, completedContractCount: 5 }),
}));
jest.mock('../score-algorithm', () => ({
  calculateFocusScoreUpdate: jest.fn().mockReturnValue({
    newScore: 620, previousScore: 600, delta: 20, band: 'Strong',
    userFacingReason: 'Test', topPositiveFactor: 'sessionQuality',
    topNegativeFactor: 'intentionalDifficulty',
    factors: {}, historyPoint: { timestamp: new Date().toISOString(), score: 620, delta: 20, reason: 'test' },
  }),
}));
