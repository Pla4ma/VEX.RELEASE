...
describe('FocusScoreDashboard', () => {
  it('renders loading state', () => {
    (useFocusScore.useFocusScore as jest.Mock).mockReturnValue({ status: 'pending' });
  ...
  it('renders error state', () => {
    ...
    (useFocusScore.useFocusScore as jest.Mock).mockReturnValue({ status: 'error', error: { message: 'Test Error' }, refetch: mockRefetch });
  ...
  it('renders empty state', () => {
    (useFocusScore.useFocusScore as jest.Mock).mockReturnValue({ status: 'success', score: null });
    const { getByText } = render(<FocusScoreDashboard />);
    expect(getByText('Start your first session to see your Focus Score.')).toBeTruthy();
  });
  ...
  it('renders success state with all data', () => {
    const score = {
      ...
      factors: {
        consistency: { score: 80, explanation: 'Consistency is a clear strength right now.' },
        streakStability: { score: 70, explanation: 'Streak stability is helping your score stay stable.' },
        sessionQuality: { score: 90, explanation: 'Session quality is a clear strength right now.' },
        intentionalDifficulty: { score: 60, explanation: 'Intentional difficulty is helping your score stay stable.' },
        recency: { score: 85, explanation: 'Recency is a clear strength right now.' },
      },
      topPositiveFactor: 'sessionQuality',
      topNegativeFactor: 'intentionalDifficulty',
    };
    const history = [
      { timestamp: new Date('2026-04-01').toISOString(), score: 700, delta: 0, reason: '' },
      { timestamp: new Date('2026-04-02').toISOString(), score: 710, delta: 10, reason: '' },
    ];
    const mockNavigate = jest.fn();
    (useFocusScore.useFocusScore as jest.Mock).mockReturnValue({ status: 'success', score, history, isRefetching: false });
    ...
    expect(mockNavigate).toHaveBeenCalledWith('Analytics', { month: new Date().toISOString().slice(0, 7) });
  });
  ...
  it('renders refetching indicator when refetching', () => {
    ...
    (useFocusScore.useFocusScore as jest.Mock).mockReturnValue({ status: 'success', score, isRefetching: true });
    (useNetInfo.useNetInfo as jest.Mock).mockReturnValue({ isOffline: false });
    const { getByText } = render(<FocusScoreDashboard />);
  ...
  it('renders 30-day trend with history data', () => {
    ...
    const history = [
      { timestamp: new Date('2026-04-01').toISOString(), score: 700, delta: 0, reason: '' },
      { timestamp: new Date('2026-04-02').toISOString(), score: 710, delta: 10, reason: '' },
      { timestamp: new Date('2026-04-03').toISOString(), score: 705, delta: -5, reason: '' },
    ];
    (useFocusScore.useFocusScore as jest.Mock).mockReturnValue({ status: 'success', score, history, isRefetching: false });
    (useNetInfo.useNetInfo as jest.Mock).mockReturnValue({ isOffline: false });
    const { getByText } = render(<FocusScoreDashboard />);
