/**
 * Focus Identity Engine Tests
 *
 * Comprehensive test coverage for the FICO-style Focus Score system.
 */

import { FocusIdentityEngine, FocusIdentityService, FOCUS_SCORE_CONFIG } from "../FocusIdentityEngine";
...
describe("FocusIdentityEngine", () => {
  ...
  describe("Consistency Factor Calculation", () => {
    it("should give high score for perfect consistency", () => {
      const result = (engine as any).calculateConsistencyFactor(
        16, // 16 sessions (4 per week target)
        4, // target per week
        0, // no missed days
      );
  ...
  describe("Streak Stability Factor", () => {
    it("should reward long current streak", () => {
      const result = (engine as any).calculateStreakStabilityFactor(
        45, // current streak
        60, // longest
...
describe("FocusIdentityService", () => {
  ...
  describe("Score Updates", () => {
    ...
    it("should update score on session complete", async () => {
      ...
      expect(result.newScore).toBeGreaterThan(result.change); // Initial + change
    ...
    it("should detect band change when crossing threshold", async () => {
      // Create profile at edge of band
      const profile = await service.getProfile();
      if (profile) {
        profile.currentScore = 579; // Edge of Good band
        // Simulate crossing into Strong
        const result = await service.updateScore("PERFECT_SESSION", {
          streakLength: 10,
        });
        // With recovery bonus could trigger band change
    ...
    it("should complete recovery after enough positive events", async () => {
      ...
      // Multiple positive events to complete recovery
      for (let i = 0; i < 15; i++) {
