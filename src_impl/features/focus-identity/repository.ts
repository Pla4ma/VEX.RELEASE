...
async function withRetry<T>(operation: () => Promise<T>, operationName: string, maxRetries = 3, delayMs = 1000): Promise<T> {
 ...
 */
export async function getMonthlyReportData(userId: string, yearMonth: string): Promise<MonthlyReportData | null> {
  return withRetry(async () => {
    const supabase = getSupabaseClient();
 ...
 */
export async function saveMonthlyReportData(userId: string, report: MonthlyReportData): Promise<void> {
  return withRetry(async () => {
    const supabase = getSupabaseClient();
