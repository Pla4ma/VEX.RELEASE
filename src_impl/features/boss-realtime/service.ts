...
export class RealTimeBossService {
  ...
  tick(
    ...
    const pureDuration = this.pureFocusStartTime ? (now - this.pureFocusStartTime) / 1000 : 0;
    const healthPercent = this.encounter.currentHealth / this.encounter.maxHealth;

    const attackType = determineAttackType(purityScore, this.currentCombo, pureDuration, healthPercent);
  ...
  calculateFinalRewards(): {
    ...
    if (!this.encounter) {
      return { xp: 0, coins: 0, gems: 0, bonusReward: false };
    }
...
export function createBossEncounter(bossId: string, bossName: string, bossAvatar: string, userId: string, sessionId: string, sessionDurationMinutes: number, userLevel: number): RealTimeBossEncounter {
