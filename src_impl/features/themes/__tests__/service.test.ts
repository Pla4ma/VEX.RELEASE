/**
 * Themes Service Tests
 *
 * Comprehensive test coverage for theme management, unlocks, and purchases
 */
...
describe('Themes Service', () => {
  ...
  describe('getUserThemes', () => {
    ...
    it('should return stored themes for existing user', async () => {
      const storedThemes = {
        unlocked: ['default_light', 'default_dark', 'ocean_blue', 'forest_green'],
        equipped: 'ocean_blue',
        purchasedAt: {
          ocean_blue: Date.now(),
          forest_green: Date.now(),
        },
      };
  ...
  describe('isThemeUnlocked', () => {
    ...
    it('should return false for locked themes', async () => {
      ...
      const isUnlocked = await isThemeUnlocked(TEST_USER_ID, 'locked_premium_theme');
  ...
  describe('getThemePrice', () => {
    ...
    it('should throw error for non-existent theme', async () => {
      await expect(getThemePrice('non_existent_theme')).rejects.toThrow('Theme not found');
    });
  });
  ...
  describe('purchaseTheme', () => {
    ...
    it('should throw error for already unlocked theme', async () => {
      ...
      await expect(purchaseTheme(TEST_USER_ID, 'premium_ocean')).rejects.toThrow('Theme already unlocked');
    });

    it('should throw error for non-existent theme', async () => {
      await expect(purchaseTheme(TEST_USER_ID, 'non_existent')).rejects.toThrow('Theme not found');
    });

    it('should throw error on insufficient funds', async () => {
      const mockSpendCurrency = jest.fn().mockRejectedValue(new Error('Insufficient funds'));
      (getEconomyService as jest.Mock).mockReturnValue({
        spendCurrency: mockSpendCurrency,
      });

      await expect(purchaseTheme(TEST_USER_ID, 'premium_ocean')).rejects.toThrow('Insufficient funds');
    });
  });
  ...
  describe('unlockTheme', () => {
    ...
    it('should throw error for already unlocked theme', async () => {
      ...
      await expect(
        unlockTheme(TEST_USER_ID, 'achievement_theme', { type: 'achievement', achievementId: 'test' })
  ...
  describe('equipTheme', () => {
    ...
    it('should throw error for locked theme', async () => {
      ...
      await expect(equipTheme(TEST_USER_ID, 'locked_theme')).rejects.toThrow('Theme not unlocked');
    });

    it('should throw error for non-existent theme', async () => {
      await expect(equipTheme(TEST_USER_ID, 'non_existent')).rejects.toThrow('Theme not found');
    });
  });
});
