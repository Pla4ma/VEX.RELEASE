 ...
 *
 * 7-day rotating reward table appearing once per day on first app open.
 * Shows all 7 days, current day highlighted, past days checked off.
 * Claim button with coin/gem animation.
 * Auto-dismiss after claiming or 5 seconds.
 *
 * @phase 5C.1
 */
 ...
 */
function FlyingReward({ reward, onComplete }: { reward: DailyReward; onComplete: () => void }): JSX.Element {
  const flyStyle = useAnimatedStyle(() => ({
    transform: [
      {
        translateY: withSequence(withTiming(-50, { duration: 300 }), withTiming(-150, { duration: 500 })),
      },
      {
        scale: withSequence(withSpring(1.5, { damping: 5 }), withTiming(0.8, { duration: 400 })),
      },
    ],
    opacity: withSequence(withTiming(1, { duration: 100 }), withTiming(0, { duration: 600 })),
  }));
...
/**
 * Main daily login modal
 */
export function DailyLoginModal({ visible, currentDay, streakDays, isClaimed, onClaim, onDismiss }: DailyLoginModalProps): JSX.Element {
  const { theme } = useTheme();
  ...
  // Auto-dismiss after claiming or 5 seconds if already claimed
  useEffect(() => {
    if (visible && claimed) {
    ...
    return undefined;
  }, [visible, claimed, onDismiss]);

  const handleClaim = useCallback(() => {
    setShowFlyingReward(true);
    onClaim();
    setTimeout(() => setClaimed(true), 800);
  }, [onClaim]);
        ...
        >
          <Box bg="background.secondary" borderRadius="2xl" borderWidth={1} borderColor="border.light" overflow="hidden">
            ...
            <Box p="xl" bg="primary.500" alignItems="center">
              ...
              {streakDays >= 7 && (
                <Box mt="sm" px="md" py="xs" borderRadius="full" bg="accent.orange">
            ...
            <Box p="xl" style={{ borderTopWidth: 1, borderTopColor: theme.colors.border.light }}>
              {claimed ? (
                ...
                <Button variant="primary" size="lg" fullWidth onPress={handleClaim} accessibilityLabel="🎁 Claim Reward button" accessibilityRole="button" accessibilityHint="Activates this control">
                  🎁 Claim Reward
                </Button>
              )}
            </Box>
