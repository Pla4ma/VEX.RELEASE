/**
 * DailyLoginCard Component
 *
 * Compact card for Home screen showing today's daily login reward.
 * Displays reward preview and claim button (if available).
 *
 * @phase 1
 */
...
import type { DailyLoginReward } from '../hooks';
...
export function DailyLoginCard({
  ...
  onClaim,
  onPress,
}: DailyLoginCardProps): JSX.Element {
  ...
  return (
    <Animated.View style={[containerStyle, pulseStyle]}>
        ...
        <View style={contentStyle}>
          ...
          {canClaim ? (
            <Button
              ...
              accessibilityLabel="Claim daily login reward"
              accessibilityRole="button"
            >
              {isClaiming ? '...' : 'Claim'}
            </Button>
          ) : (
            <View style={claimedBadgeStyle}>
              <Icon
                name="check"
                size={16}
