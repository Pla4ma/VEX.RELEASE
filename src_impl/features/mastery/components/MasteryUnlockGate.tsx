...
export function getPointsToUnlock(feature: UnlockableFeature, currentPoints: number): number {
  const requiredRank = FEATURE_REQUIREMENTS[feature];
...
export function MasteryUnlockGate({ userRank, userPoints, feature, children, onNavigateToMastery }: MasteryUnlockGateProps): JSX.Element {
  ...
  return (
    ...
    >
      <View style={{ gap: theme.spacing[3], alignItems: "center" }}>
        ...
        {onNavigateToMastery && (
          <Button size="sm" variant="outline" onPress={onNavigateToMastery} accessibilityLabel="View mastery progression" accessibilityRole="button" accessibilityHint="Opens the mastery screen to track your progress">
...
export function LockedFeaturePreview({ feature, userRank, userPoints, onNavigateToMastery }: LockedFeaturePreviewProps): JSX.Element {
  const { theme } = useTheme();
  const featureInfo = FEATURE_INFO[feature];
  const requiredRank = FEATURE_REQUIREMENTS[feature];
  const rankDisplay = getMasteryRankDisplay(requiredRank);
  ...
  return (
    <Pressable onPress={onNavigateToMastery} accessibilityLabel={`${featureInfo.name} is locked. Requires ${rankDisplay.title} rank.`} accessibilityRole="button" accessibilityHint="Tap to view mastery progression">
      <Card
        size="sm"
        style={{
          backgroundColor: theme.colors.background.secondary,
