          ...
          {DAYS.map((day) => (
            <View key={day} style={styles.row}>
              ...
              {HOURS.map((hour) => {
                ...
                return (
                  <Pressable key={`${day}-${hour}`} style={({ pressed }) => [styles.cell, { backgroundColor: getCellColor(value) }, !isVisible && styles.hiddenCell, value > 0 && styles.activeCell, pressed && { opacity: 0.7 }]} onPress={() => onCellPress?.(day, hour, value)} accessibilityLabel="Interactive control" accessibilityRole="button" accessibilityHint="Activates this control">
...
function calculatePeakDay(data: HeatmapData[]): string {
...
function calculateTotal(data: HeatmapData[]): number {
