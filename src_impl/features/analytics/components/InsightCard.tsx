...
export function InsightCard({ insight, onPress, onAction, onDismiss }: InsightCardProps) {
  const severity = SEVERITY_CONFIG[insight.severity];

  return (
    <Pressable style={({ pressed }) => [styles.container, { backgroundColor: severity.bgColor, borderColor: severity.color }, insight.isRead && styles.readContainer, pressed && { opacity: 0.8 }]} onPress={onPress} accessibilityRole="button" accessibilityLabel={`Insight: ${insight.title}`} accessibilityHint="Activates this control">
      ...
      {insight.actionType && !insight.isActioned && (
        <View style={styles.actions}>
          ...
          {onDismiss && (
            <Pressable style={({ pressed }) => [styles.dismissButton, pressed && { opacity: 0.8 }]} onPress={onDismiss} accessibilityRole="button" accessibilityLabel="Dismiss button" accessibilityHint="Activates this control">
