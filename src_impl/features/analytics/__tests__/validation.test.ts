 ...
 * Comprehensive tests for validation layer with edge cases
 */

import { validateTimeRange, validateMetrics, validateExportConfig, validateInsight, validateFilter, batchValidate, formatValidationErrors, AnalyticsValidationError } from '../validation';
...
describe('AnalyticsValidation', () => {
  ...
  describe('validateMetrics', () => {
    ...
    it('should warn about too many metrics', () => {
      const result = validateMetrics(['sessions_completed', 'xp_earned', 'streak_days', 'boss_damage_dealt', 'items_crafted', 'coins_spent', 'challenges_completed', 'sessions_abandoned', 'total_focus_time', 'average_session_duration', 'longest_streak']);
    ...
    it('should warn about duplicates', () => {
      const result = validateMetrics(['sessions_completed', 'sessions_completed']);
  ...
  describe('validateExportConfig', () => {
    it('should validate correct config', () => {
      ...
      expect(result.valid).toBe(true);
      expect((result.sanitized as { estimatedSize: number })?.estimatedSize).toBeGreaterThan(0);
    });
  ...
  describe('validateInsight', () => {
    it('should validate correct insight', () => {
      ...
      expect(result.valid).toBe(true);
      expect((result.sanitized as { title: string })?.title).toBe('Test Insight');
    });
  ...
  describe('batchValidate', () => {
    it('should validate all items', async () => {
      const items = [
        { title: 'Valid 1', description: 'Desc', severity: 'info', metric: 'xp' },
        { title: 'Valid 2', description: 'Desc', severity: 'warning', metric: 'sessions' },
      ];
    ...
    it('should track progress', async () => {
      const items = [{ title: 'Test', description: 'Desc', severity: 'info', metric: 'xp' }];
  ...
  describe('AnalyticsValidationError', () => {
    it('should create error with all properties', () => {
      const error = new AnalyticsValidationError('Test error', 'title', 'EMPTY_TITLE', 'Provide a title', '');
