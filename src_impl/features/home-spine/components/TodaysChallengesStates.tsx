...
export function ChallengesEmptyState(): JSX.Element {
  ...
  return (
    <Box alignItems="center" gap="sm" py="md">
      ...
      <Text variant="bodySmall" color="text.secondary" textAlign="center">Challenges reset at midnight</Text>
      <Text variant="caption" color="text.tertiary">{hoursUntilReset}h {minutesUntilReset}m remaining</Text>
    </Box>
  );
}

export function ChallengesErrorState({ onRetry }: { onRetry?: () => void }): JSX.Element {
  return (
    <Box alignItems="center" gap="md" py="md">
      <Text fontSize={32}>⚠️</Text>
      <Text variant="bodySmall" color="error.DEFAULT" textAlign="center">Couldn't load challenges</Text>
      {onRetry ? (
