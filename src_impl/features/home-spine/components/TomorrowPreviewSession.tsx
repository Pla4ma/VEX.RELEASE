...
export function TomorrowPreviewSession({ preview, onPress }: TomorrowPreviewSessionProps): JSX.Element {
  const { theme } = useTheme();
  const accentColor = useTypeColor(preview.type);
  const CardWrapper = onPress ? Pressable : View;
  return (
    <Animated.View entering={FadeInUp.duration(500).delay(800)}>
      <CardWrapper onPress={onPress}>
        <Box m="lg" p="lg" borderRadius="xl" bg={theme.colors.background.secondary} borderWidth={2} borderColor={accentColor}>
          ...
          <Box flexDirection="row" alignItems="flex-start" gap="md">
            ...
            <View style={{ flex: 1 }}>
              <Text variant="h4" color="text.primary" fontWeight="700" style={{ marginBottom: theme.spacing[1] }}>{preview.headline}</Text>
              <Text variant="body" color="text.secondary">{preview.subtext}</Text>
              {preview.actionPrompt ? (
                <Box mt="sm" px="sm" py="xs" borderRadius="md" bg={`${accentColor}15`}>
                  <Text variant="caption" color={accentColor} fontWeight="600">{preview.actionPrompt}</Text>
                </Box>
              ) : null}
            </View>
          </Box>
