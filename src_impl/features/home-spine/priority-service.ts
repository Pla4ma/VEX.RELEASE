...
import { HomePrioritySchema, HomeContextSnapshotSchema, type HomePriority, type HomeContextSnapshot, type HomePrimaryPriority, type HomeSecondaryAction, type HomeStakes, type HomeProgress } from "./priority-schemas";
...
function checkCoachIntervention(snapshot: HomeContextSnapshot): HomePrimaryPriority | null {
  ...
  return {
    ...
    urgency,
    reason: snapshot.coach.interventionType === "STREAK_AT_RISK" ? "Your streak is at risk - the AI Coach has a suggestion" : "The AI Coach has personalized guidance for you",
    cta: {
      text: "View Coach Tip",
      action: "VIEW_COACH",
    },
...
function buildSecondaryActions(sortedPriorities: HomePrimaryPriority[], snapshot: HomeContextSnapshot): HomeSecondaryAction[] {
...
async function buildHomeContextSnapshot(userId: string): Promise<HomeContextSnapshot> {
  ...
  const [{ onboardingRepository }, { fetchStreak }, { fetchActiveEncounter }] = await Promise.all([import("../onboarding/repository"), import("../streaks/repository"), import("../boss/repository")]);

  const [progressState, streak, bossEncounter] = await Promise.all([onboardingRepository.getProgress(userId), fetchStreak(userId).catch(() => null), fetchActiveEncounter(userId).catch(() => null)]);
  ...
  const snapshot = {
    ...
    boss: {
      ...
      isFinalStrike: bossEncounter ? bossEncounter.healthRemaining / bossEncounter.maxHealth < 0.2 : false,
    },
...
export async function selectHomePriority(userId: string): Promise<HomePriority> {
  ...
  const candidates = [checkFirstSession(snapshot), checkStreakCritical(snapshot), checkBossFinalStrike(snapshot), checkComeback(snapshot), checkCoachIntervention(snapshot), checkStudyPlanDue(snapshot), checkDailyGoal(snapshot)].filter((p): p is HomePrimaryPriority => p !== null);
