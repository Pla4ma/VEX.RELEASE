...
export function buildStreakMilestoneCandidate(
  ...
  const nextMilestone = milestoneDays.find((day) => day > input.currentStreakDays);
  if (!input.streakWillContinue || nextMilestone !== input.currentStreakDays + 1) {
    return null;
  }
...
export function buildBossCandidate(input: ComputeTomorrowPreviewInput): TomorrowPreviewData | null {
  const boss = input.bossData;
  if (!boss || boss.healthPercent >= 25) {
    return null;
  }
  return {
    actionPrompt: boss.canDefeatTomorrow ? 'Prepare to strike!' : 'Join the squad!',
    emoji: 'boss',
    headline: boss.canDefeatTomorrow ? `${boss.bossName} Will Fall!` : 'Boss Almost Defeated!',
...
export function buildRivalCandidate(input: ComputeTomorrowPreviewInput): TomorrowPreviewData | null {
  const rival = input.rivalData;
  if (!rival || rival.gapMinutes <= 0) {
    return null;
  }
  const willOvertake = rival.myMinutes + Math.min(rival.gapMinutes, 30) > rival.theirMinutes;
  return {
