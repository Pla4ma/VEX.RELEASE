/**
 * Home surface decision integration tests.
 */
import { decideHomeSurfaces, getPrimarySurface, getSpotlightSurface } from '../../../features/home-experience/home-surface-decision';
...
describe('Home surface decision — integration', () => {
  describe('Day 0 calm user', () => {
    ...
    it('coach_presence is tiny_tease', () => { expect(map.coach_presence).toBe('tiny_tease'); });
    it('unlock_strip is tiny_tease', () => { expect(map.unlock_strip).toBe('tiny_tease'); });
    ...
    it('premium is hidden', () => { expect(map.premium_tease).toBe('hidden'); });
    it('study is hidden unless selected', () => { expect(map.study_layer).toBe('hidden'); });
  });

  describe('Study-focused user after first session', () => {
    const map = decideHomeSurfaces({
      featureAvailability: { boss: false, challenges: true, premium: false, study: true },
      personalizationProfile: {
        motivationStyle: 'study_focused', primaryGoal: 'study',
        gamificationIntensity: 'medium', studyLayerName: 'Study OS', userStage: 'engaged',
      },
      behaviorStats: {
        totalCompletedSessions: 3, studyUsageRatio: 0.4,
        bossChallengeEngagement: 'none', coachInteractions: 1,
        comebackSessions: 0, ignoredFeatures: [], premiumFeatureAttempts: [],
        completionStreak: 2,
      },
      hasActiveStudyPlan: true, hasActiveRecommendation: false,
  ...
  describe('Game-like user boss teaser', () => {
    const map = decideHomeSurfaces({
      featureAvailability: { boss: true, challenges: true, premium: false, study: false },
      personalizationProfile: {
        motivationStyle: 'game_like', primaryGoal: 'focus',
        gamificationIntensity: 'strong', studyLayerName: 'Study OS', userStage: 'engaged',
      },
      behaviorStats: {
    ...
    it('boss_compact is secondary for engaged game-like user', () => {
      expect(['secondary', 'spotlight', 'tiny_tease']).toContain(map.boss_compact);
  ...
  describe('Calm user blocks boss full CTA', () => {
    const map = decideHomeSurfaces({
      ...
      personalizationProfile: {
        motivationStyle: 'calm', primaryGoal: 'focus',
        gamificationIntensity: 'minimal', studyLayerName: 'Study OS', userStage: 'power',
      },
      behaviorStats: {
        totalCompletedSessions: 15, studyUsageRatio: 0.1,
        bossChallengeEngagement: 'none', coachInteractions: 2,
        comebackSessions: 0, ignoredFeatures: [], premiumFeatureAttempts: [],
        completionStreak: 5,
      },
      hasActiveStudyPlan: false, hasActiveRecommendation: false,
