  ...
  switch (category) {
    ...
    case 'MILESTONE_HYPE':
      ...
      if (memory.totalSessionsCompleted > 50) {
        suggestions.push(`${memory.totalSessionsCompleted} total sessions! You're building something amazing.`, `Over ${Math.round(memory.totalFocusMinutes / 60)} hours of focused work. That's dedication!`);
      }
      break;
    ...
    case 'COMEBACK_SUPPORT':
      ...
      if (memory.longestStreak > 7) {
        suggestions.push(`That ${memory.longestStreak}-day streak? You built that. You can build again.`, "You've done long streaks before. The ability is still there.");
      }
      break;
    ...
    case 'PROGRESS_REMINDER':
      ...
      if (memory.mostUsedSessionDuration > 0) {
        suggestions.push(`${memory.mostUsedSessionDuration} minutes is your sweet spot. Stick with what works!`, `You've preferred ${memory.mostUsedSessionDuration}-minute sessions. Trust that pattern.`);
      }
      break;
  }
