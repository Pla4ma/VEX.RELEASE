 ...
 * Message Generator
 *
 * Production-grade message generation with template selection,
 ...
 * Production-grade with template selection, validation, and fallback handling
 */
export async function generateMessage(params: { userId: string; category: MessageCategory; context: Record<string, unknown>; preferredDelivery: DeliveryMethod; config?: Partial<GenerationConfig> }): Promise<CoachMessage | null> {
  ...
  try {
    ...
    const selectedTemplate = selectBestTemplate(templates, params.context, config.maxTemplateAttempts);

    // Generate content
    let content: string;
    if (personalityTemplates.length > 0) {
      // Use personality-specific template
      content = generateFromPersonalityTemplate(personalityTemplates, params.context, config);
    ...
    // Log error and return null (caller should handle fallback)
...
async function fetchTemplatesWithCache(personaId: string, category: MessageCategory): Promise<CoachMessageTemplate[]> {
  const cacheKey = `${personaId}_${category}`;
  const now = Date.now();

  // Check cache
  if (templateCache.templates.has(category) && now - templateCache.lastUpdated < CACHE_TTL_MS) {
...
function selectBestTemplate(templates: CoachMessageTemplate[], context: Record<string, unknown>, maxAttempts: number): CoachMessageTemplate | null {
  ...
  // Find first template with matching conditions
  ...
  // Return highest priority if no conditions match
  return sorted[0];
}

function checkTemplateConditions(template: CoachMessageTemplate, context: Record<string, unknown>): boolean {
...
function generateFromTemplate(template: CoachMessageTemplate, context: Record<string, unknown>, config: GenerationConfig): string {
  // Select variation or use main content
  const allVariations = [template.content, ...(template.variations || [])];
  const selectedContent = allVariations[Math.floor(Math.random() * allVariations.length)];
...
function generateFromDefaults(category: MessageCategory, context: Record<string, unknown>): string {
...
/**
 * Generate content from personality-specific templates (Phase 9.1)
 */
...
function validateContent(content: string, config: GenerationConfig): ValidationResult {
  const errors: string[] = [];

  // Length validation
  if (content.length < config.minContentLength) {
    errors.push(`Content too short: ${content.length} < ${config.minContentLength}`);
  }
  if (content.length > config.maxContentLength) {
    errors.push(`Content too long: ${content.length} > ${config.maxContentLength}`);
  }

  // Forbidden patterns
  const forbiddenPatterns = [
    /\b(hate|stupid|idiot|kill)\b/i, // Harmful language
...
function calculatePriority(category: MessageCategory, context: Record<string, unknown>): number {
...
async function isCategoryMuted(category: MessageCategory, coachState: CoachState | null, userId: string): Promise<boolean> {
  if (!coachState) {
    return false;
  }
