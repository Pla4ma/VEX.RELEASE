...
export async function generateContextSnapshot(userId: string): Promise<ContextSnapshot> {
  const now = Date.now();
  const hour = new Date(now).getHours();
  const dayOfWeek = new Date(now).getDay();

  // In production: query all relevant data sources
  // const sessionData = await getActiveSession(userId);
...
export function determineInterventionPriority(
  ...
  if (snapshot.bossContext.activeBoss && snapshot.bossContext.timeRemaining && snapshot.bossContext.timeRemaining < 24 * 60 * 60 * 1000) {
    return 'high';
  }
...
export function shouldCoachIntervene(
  ...
  const hoursSinceLast = (snapshot.capturedAt - lastInterventionAt) / (60 * 60 * 1000);
  ...
  // Always intervene for critical situations
  if (snapshot.streakContext.streakAtRisk) {
    return true;
  }

  // Intervene for boss danger
  if (snapshot.bossContext.activeBoss && snapshot.bossContext.timeRemaining && snapshot.bossContext.timeRemaining < 12 * 60 * 60 * 1000) {
    return true;
  }

  // Intervene for optimal time
  if (snapshot.temporalContext.hourOfDay === 9 && !snapshot.sessionContext.activeSession) {
    return true;
  }
