import React,{useEffect}from'react'; import{View,type ViewStyle}from'react-native'; import Animated,{useAnimatedStyle,useSharedValue,withRepeat,withSequence,withTiming}from'react-native-reanimated'; import{createSheet}from'@/shared/ui/create-sheet'; interface SkeletonProps{width?:number|string;height?:number;borderRadius?:number;style?:ViewStyle;shimmer?:boolean;}export function Skeleton({width = '100%',height = 16,borderRadius = 4,style,shimmer = true}:SkeletonProps){const shimmerAnim = useSharedValue(0); useEffect(()=>{shimmerAnim.value = shimmer ? withRepeat(withSequence(withTiming(1,{duration:1500}),withTiming(0,{duration:1500})),-1,false) : 0;},[shimmer,shimmerAnim]); const shimmerStyle = useAnimatedStyle(()=>({transform:[{translateX:-200 + shimmerAnim.value * 400}]})); return<View style={[styles.container,{width:width as ViewStyle['width'],height,borderRadius},style]}>
    ...
    </View>;}const styles = createSheet({container:{backgroundColor:'#E1E4E8',overflow:'hidden'},shimmer:{width:'100%',height:'100%',backgroundColor:'rgba(255, 255, 255, 0.3)'},textContainer:{gap:8},textLine:{marginBottom:8},cardContainer:{padding:16,backgroundColor:'#fff',borderRadius:12,gap:12},cardHeader:{flexDirection:'row',alignItems:'center',gap:12},headerText:{flex:1,gap:6},headerLine:{marginBottom:4},button:{marginTop:8},personaContainer:{flexDirection:'row',gap:12},personaCard:{alignItems:'center',gap:8,padding:16,backgroundColor:'#fff',borderRadius:12,flex:1},personaName:{marginTop:8},listContainer:{gap:12},listItem:{flexDirection:'row',alignItems:'center',gap:12,padding:12,backgroundColor:'#fff',borderRadius:8},listContent:{flex:1,gap:6},listLine:{marginBottom:4},streakContainer:{gap:16},streakContent:{padding:16,gap:12},streakBadge:{alignSelf:'center'},comebackContainer:{padding:16,backgroundColor:'#fff',borderRadius:12,gap:16},progressBar:{marginBottom:8},comebackContent:{gap:12}});
