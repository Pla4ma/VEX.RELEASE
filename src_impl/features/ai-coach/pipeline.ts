...
export async function generateMessage(userId: string, trigger: TriggerType | "COMEBACK" | "SESSION_COMPLETE", context: Record<string, unknown>): Promise<CoachMessage | null>;
export async function generateMessage(inputOrUserId: GenerateMessageArgs, trigger?: TriggerType | "COMEBACK" | "SESSION_COMPLETE", context: Record<string, unknown> = {}): Promise<CoachMessage | null> {
  ...
  const recentSummaries = (await sessionRepository.getAllSummaries()).slice(-3).reverse();

  const { data, error } = await getSupabaseClient().functions.invoke("ai-coach", {
    body: {
      ...
      context: {
        ...
        recentSessionOutcomes: recentSummaries.map((summary) => ({
          ...
          durationMinutes: Math.max(1, Math.round(summary.actualDuration / 60)),
        })),
      },
    },
  });
  ...
  if (error) {
    return repository.createCoachMessage(createFallbackMessage(input, state.personaId));
  }
...
export async function evaluateInterventions(userId: string, trigger: TriggerType | "COMEBACK" | "SESSION_COMPLETE", context: Record<string, unknown>): Promise<InterventionExecution[]>;
export async function evaluateInterventions(inputOrUserId: EvaluateArgs, trigger?: TriggerType | "COMEBACK" | "SESSION_COMPLETE", context: Record<string, unknown> = {}): Promise<InterventionExecution[]> {
  const input = normalizeEvaluateArgs(inputOrUserId, trigger, context);
  const streak = readNumber(input.context.streak) ?? readNumber(input.context.currentStreak) ?? 0;
  const daysSinceLastSession = readNumber(input.context.daysSinceLastSession) ?? 0;
  ...
  if (input.trigger === "STREAK_AT_RISK" && streak > 3) {
    const message = await generateMessage(input.userId, input.trigger, { ...input.context, urgency: "critical" });
    if (message) {
  ...
  if (input.trigger === "COMEBACK_WINDOW_OPEN" && daysSinceLastSession > 3) {
    ...
    executions.push(createExecution(input.userId, input.trigger, message?.id ?? null));
  }

  return executions;
}

export async function activateComeback(input: string | { userId: string; previousStreak?: number; daysInactive?: number }) {
  const normalized =
    typeof input === "string"
      ? { userId: input, previousStreak: 0, daysInactive: 4 }
      : ActivateComebackInputSchema.partial({ previousStreak: true, daysInactive: true }).parse({
          previousStreak: 0,
          daysInactive: 4,
          ...input,
        });
...
export async function detectStreakRisk(_userId: string, hoursSinceLastSession: number, currentStreak: number): Promise<boolean> {
