/**
 * useCoachRecommendation Hook
 *
 * Phase 2.1 - Coach as Recommendation Engine
 * Integrates CoachRecommendationService with React components
 * Provides coach-powered recommendations for the Home screen
 */
...
export function useCoachRecommendation(): UseCoachRecommendationReturn {
  ...
  const [behaviorProfile, setBehaviorProfile] = useState<RecommendationContext['behaviorProfile']>(null);
  ...
  const generateRecommendation = useCallback(() => {
    ...
    if (activeStudyPlan && activeStudyPlan.totalTasks > 0) {
      studyPlanProgress = activeStudyPlan.completedTasks / activeStudyPlan.totalTasks;
      // Estimate days behind based on expected daily progress
      const expectedProgress = Math.max(1, activeStudyPlan.totalTasks / 7); // Assume 7 days to complete
      const daysElapsed = Math.max(1, Math.floor(daysSinceLastSession) || 1);
      const expectedTasks = expectedProgress * daysElapsed;
      studyPlanDaysBehind = Math.max(0, Math.floor(expectedTasks - activeStudyPlan.completedTasks));
    }
    ...
    if (streak && streak.currentDays > 0 && !stats?.completedSessions) {
      ...
      hoursUntilStreakBreak = Math.max(0, (endOfDay.getTime() - now.getTime()) / (1000 * 60 * 60));
    }
    ...
    if (lastRecRef.current && !service.shouldRefresh(lastRefreshRef.current, lastRecRef.current)) {
      return;
    }
...
/**
 * Hook that provides coach recommendations in HomeRecommendation format
 * For direct integration with existing Home screen components
 */
export function useCoachHomeRecommendation() {
  const { recommendation, isLoading, refresh, getPersonaName } = useCoachRecommendation();
