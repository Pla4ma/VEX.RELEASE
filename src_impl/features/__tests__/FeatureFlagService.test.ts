 ...
 *
 * Comprehensive tests for feature flag service with:
...
import { FeatureFlagService, FeatureFlag, FeatureFlagValue } from '../FeatureFlagService';
...
describe('FeatureFlagService', () => {
  ...
  describe('Initialization', () => {
    it('should initialize with default flags', async () => {
      ...
      expect(service.isEnabled('beta_features')).toBe(false);
    });
    ...
    it('should load overrides when enabled', async () => {
      mockStorageManager.getItem.mockImplementation((key: string) => {
        if (key === 'test-flags') {return Promise.resolve({});}
    ...
    it('should set user ID for rollout', async () => {
      ...
      // Should be able to check rollout without error
      expect(service.isEnabled('new_design')).toBe(false); // 0% rollout
    });
  });
  ...
  describe('Flag Management', () => {
    ...
    it('should get all flags', () => {
      ...
      expect(all).toHaveProperty('beta_features');
    });

    it('should get enabled flags', () => {
      const enabled = service.getEnabled();
      expect(enabled).toEqual([]); // All default to false
    });
  ...
  describe('Override Management', () => {
    ...
    it('should clear all overrides', async () => {
      await service.setOverride('new_design', true);
      await service.setOverride('beta_features', true);

      await service.clearAllOverrides();

      expect(service.getOverride('new_design')).toBeNull();
      expect(service.getOverride('beta_features')).toBeNull();
    });
  });
  ...
  describe('Rollout Logic', () => {
    ...
    it('should respect rollout percentage', async () => {
      ...
      // Without user ID, should use default
  ...
  describe('Edge Cases', () => {
    ...
    it('should prevent double initialization', async () => {
      ...
      // Should only call storage init once
