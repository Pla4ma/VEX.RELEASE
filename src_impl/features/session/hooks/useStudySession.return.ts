/**
 * useStudySession - Return builder
 *
...
import { studySessionKeys, getExpectedDuration } from './useStudySession.helpers';

interface BuildReturnArgs {
...
export function buildReturn(args: BuildReturnArgs) {
  ...
  const requestCoachAdvice = useCallback(async () => {
    if (!currentSessionQuery.data) return null;
    try {
  ...
  const isBreak =
    currentSession?.phase === 'SHORT_BREAK' || currentSession?.phase === 'LONG_BREAK' || false;
