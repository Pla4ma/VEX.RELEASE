/**
 * Companion Growth Tests
 *
 * Tests for companion growth reactions to session completion and other events.
 */

import { describe, it, expect, beforeEach, vi } from 'vitest';
import { CompanionService } from '../service';
import { CompanionPhase, CompanionMood } from '../types';
...
// Mock analytics
vi.mock('../analytics', () => ({
  trackCompanionGrowth: vi.fn(),
  trackCompanionEvolution: vi.fn(),
  trackCompanionMilestone: vi.fn(),
}));
...
describe('Companion Growth', () => {
  ...
  describe('Session Completion Growth', () => {
    ...
    it('should emit growth events on session completion', () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('companion:state_changed', expect.objectContaining({
        userId,
    ...
    it('should evolve when threshold is reached', () => {
      // Set up companion close to evolution
      service = new CompanionService({
        id: 'companion-1',
        userId,
        phase: 'YOUNG',
        level: 99,
        totalFocusMinutes: 295, // Just 5 minutes from evolution
        element: 'FLAME',
        elementAffinity: 50,
        currentMood: 'CONTENT',
        sessionProgress: 0,
    ...
    it('should emit milestone events for significant achievements', () => {
      ...
      service = new CompanionService({
        ...
        colorHue: 0,
        particleDensity: 50,
        sessionCount: 9, // Next session will be 10th
        perfectSessions: 1,
        longestFocusStreak: 10,
        nextEvolutionAt: 300,
        updatedAt: Date.now(),
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('companion:milestone_reached', expect.objectContaining({
        userId,
  ...
  describe('Streak Maintenance Reaction', () => {
    it('should react positively to streak maintenance', () => {
      ...
      expect(state?.energyLevel).toBe(Math.min(100, (previousEnergy || 0) + 20));
      expect(state?.level).toBeGreaterThan(10);
  ...
  describe('Comeback Completion Reaction', () => {
    it('should react strongly to comeback completion', () => {
      ...
      expect(state?.energyLevel).toBe(Math.min(100, (previousEnergy || 0) + 30));
      expect(state?.level).toBeGreaterThan(10);
  ...
  describe('Focus Score Change Reaction', () => {
    it('should react positively to score increase', () => {
      ...
      expect(state?.energyLevel).toBe(Math.min(100, (previousEnergy || 0) + 15));
      expect(state?.level).toBeGreaterThan(10);
  ...
  describe('Daily Mission Completion Reaction', () => {
    it('should react positively to daily mission completion', () => {
      ...
      const state = service.getState();
      expect(state?.currentMood).toBe('CONTENT');
      expect(state?.energyLevel).toBe(Math.min(100, (previousEnergy || 0) + 10));
      expect(state?.level).toBeGreaterThan(10);
  ...
  describe('Offline and Error Handling', () => {
    it('should handle reactions without state gracefully', () => {
      ...
      expect(() => emptyService.reactToFocusScoreChanged(userId, 600, 650)).not.toThrow();
      expect(() => emptyService.reactToDailyMissionCompleted(userId)).not.toThrow();
    });

    it('should handle session completion without state gracefully', () => {
      const emptyService = new CompanionService();
      const result = emptyService.completeSession(25, 85, userId, sessionId);
