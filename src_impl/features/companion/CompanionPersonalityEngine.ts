...
class CompanionPersonalityEngine {
  ...
  initialize(): void {
    ...
    this.unsubscribeFunctions = [eventBus.subscribe('boss:defeated', this.wrapHandler(this.handleBossDefeated.bind(this))), eventBus.subscribe('session:completed', this.wrapHandler(this.handleSessionCompleted.bind(this))), eventBus.subscribe('streak:milestone', this.wrapHandler(this.handleStreakMilestone.bind(this))), eventBus.subscribe('streak:broken', this.wrapHandler(this.handleStreakBroken.bind(this))), eventBus.subscribe('coach:comeback_detected', this.wrapHandler(this.handleUserReturned.bind(this))), eventBus.subscribe('progression:level_up', this.wrapHandler(this.handleLevelUp.bind(this)))];
  }
  ...
  private handleBossDefeated(event: unknown): void {
    ...
    const customDialogue = payload.bossName ? [`${payload.bossName} is down!`, 'Your focus was unstoppable!'] : undefined;
  ...
  private handleStreakMilestone(event: unknown): void {
    ...
    if (payload.days === 7 || payload.days === 14 || payload.days === 30) {
      const customDialogue = payload.days === 7 ? ['One week! We are just getting started.'] : payload.days === 14 ? ['14 days. That is not luck. That is discipline.'] : ['30-day streak!', 'You have built something real.'];
  ...
  private handleStreakBroken(event: unknown): void {
    ...
    const customDialogue = payload.previousStreak >= 7 ? [`${payload.previousStreak}-day streak ends.`, 'But your skills do not. Reset with me.'] : undefined;
  ...
  private handleUserReturned(event: unknown): void {
    ...
    if (payload.daysAbsent >= 3) {
      const customDialogue = payload.daysAbsent >= 7 ? ['Been a while.', 'But you are here now. That is enough.'] : ['You came back!', 'That is what matters.'];
