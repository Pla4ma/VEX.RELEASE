...
describe('reward-ledger service', () => {
  ...
  describe('createReward', () => {
    ...
    it('preserves idempotency - duplicate event does not create duplicate reward', async () => {
      const alreadyDelivered = { ...mockRecord, status: 'delivered' as const, deliveredAt: '2026-01-01T00:01:00.000Z' };
    ...
    it('captures Sentry on repository error', async () => {
      ...
      await expect(createReward(validInput)).rejects.toThrow('RewardLedgerService createReward failed');
  ...
  describe('deliverReward', () => {
    ...
    it('fetches ledger, credits economy, updates status to delivered', async () => {
      ...
      const delivered = { ...mockRecord, status: 'delivered' as const, deliveredAt: '2026-01-01T00:05:00.000Z' };
    ...
    it('captures Sentry on error', async () => {
      ...
      await expect(deliverReward(ledgerId)).rejects.toThrow('RewardLedgerService deliverReward failed');
      expect(captureException).toHaveBeenCalled();
    });
  });
  ...
  describe('failReward', () => {
    it('marks reward as failed with reason', async () => {
      const failed = { ...mockRecord, status: 'failed' as const, failedReason: 'inventory_full' };
  ...
  describe('expireReward', () => {
    ...
    it('captures Sentry on error', async () => {
      ...
      await expect(expireReward('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa')).rejects.toThrow(
        'RewardLedgerService expireReward failed'
      );
      expect(captureException).toHaveBeenCalled();
    });
  });
