...
import { DifficultySelector, type SessionDifficulty } from '../components/DifficultySelector';
...
describe('DifficultySelector', () => {
  ...
  it('marks selected card with accessibility state', () => {
    ...
    const focusedCard = screen.getByRole('button', { name: /Focused difficulty/ });
    const deepWorkCard = screen.getByRole('button', { name: /Deep Work difficulty/ });
  ...
  it('has correct accessibility labels', () => {
    ...
    expect(screen.getByLabelText('Casual difficulty: Unlimited pauses, 50% XP. Good for maintenance')).toBeTruthy();
    expect(screen.getByLabelText('Focused difficulty: 2 max pauses, 100% XP. Standard mode')).toBeTruthy();
    expect(screen.getByLabelText('Deep Work difficulty: 0 pauses, 150% XP. Maximum impact')).toBeTruthy();
  });
});
