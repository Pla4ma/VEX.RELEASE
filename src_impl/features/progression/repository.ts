 ...
 * All Supabase queries for progression data
 */
...
export async function fetchXpForPeriod(
  ...
  if (!data) {return 0;}
  return data.reduce((sum: number, entry: { amount: number | null }) => sum + (entry.amount || 0), 0);
}
...
export async function fetchLevelUpHistory(userId: string): Promise<Array<{ level: number; achievedAt: number; xpAtLevel: number }>> {
  const { data, error } = await supabase
    .from('level_up_history')
  ...
  return (data || []).map((row: { level: number; achieved_at: number; xp_at_level: number }) => ({
    level: row.level,
