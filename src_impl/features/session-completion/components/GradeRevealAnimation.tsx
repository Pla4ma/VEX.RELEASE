...
import { BurstParticle, hexToRgba, PARTICLE_COUNT } from './grade-reveal-helpers';
...
function getModeMessage(grade: string, mode?: SessionMode, chainCount?: number, mood?: string | null): string | null {
  if (!mode) {return null;}
  if (mode === SessionMode.DEEP_WORK && grade === 'S') {return 'Flawless deep work. Elite.';}
  if (mode === SessionMode.DEEP_WORK && grade === 'A') {return 'Strong deep work session.';}
  if (mode === SessionMode.SPRINT && (chainCount ?? 0) >= 3) {return `Sprint chain × ${chainCount}. Momentum is yours.`;}
  if (mode === SessionMode.CREATIVE && mood) {return `Creative session done. ${mood} energy.`;}
  if (mode === SessionMode.LIGHT_FOCUS) {return 'Streak protected. Every session counts.';}
  return null;
}
...
export function GradeRevealAnimation({
  ...
  const modeMessage = getModeMessage(gradeLetter, sessionMode, chainCount, creativeMoodLabel);
  ...
  useEffect(() => {
    ...
    letterScale.value = withDelay(
      ...
      withSpring(1.3, { damping: 8, stiffness: 150 }, () => {
        letterScale.value = withTiming(1, { duration: 260, easing: Easing.out(Easing.cubic) });
      }),
    );
    if (gradeLetter === 'S') {
    ...
    letterTranslateY.value = withDelay(
      1500,
      withTiming(-height * 0.18, { duration: 440, easing: Easing.inOut(Easing.cubic) }),
    );
    overlayOpacity.value = withDelay(
      1750,
  ...
  const letterStyle = useAnimatedStyle(() => ({
    transform: [{ translateY: letterTranslateY.value }, { scale: letterScale.value }],
  }));
