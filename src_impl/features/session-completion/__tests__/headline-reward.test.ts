...
describe('selectHeadlineReward', () => {
  it('selects streak_saved when a critical streak was saved', () => {
    expect(select({ streak: { currentDays: 6, previousDays: 5, streakSaved: true } }).type).toBe('streak_saved');
  });

  it.each([7, 14, 30, 100])('selects streak_milestone for day %i', (currentDays) => {
    expect(select({ streak: { currentDays, previousDays: currentDays - 1, streakSaved: false } }).type).toBe('streak_milestone');
  });

  it('selects companion_evolved when the companion crossed an evolution threshold', () => {
    expect(select({ companion: { evolved: true } }).type).toBe('companion_evolved');
  });

  it('selects boss_defeated only when the boss feature is enabled', () => {
    expect(select({ boss: { currentHealth: 0, isEnabled: true } }).type).toBe('boss_defeated');
    expect(select({ boss: { currentHealth: 0, isEnabled: false } }).type).toBe('xp_earned');
  });

  it('selects level_up when the new level is higher than the previous level', () => {
    expect(select({ summary: { ...baseConsequences.summary, previousLevel: 4, newLevel: 5 } }).type).toBe('level_up');
  });

  it('selects challenge_complete only when challenges are enabled', () => {
    expect(select({ challenge: { completedThisSession: true, isEnabled: true } }).type).toBe('challenge_complete');
    expect(select({ challenge: { completedThisSession: true, isEnabled: false } }).type).toBe('xp_earned');
  });
