/**
 * Onboarding Store (Persistent State)
 *
...
/** Draft data shape used by getDraft / saveDraft */
type DraftData = {
  goal?: FocusGoal;
...
interface OnboardingActions {
  ...
  getDraft: (userId: string) => DraftData | undefined;
  saveDraft: (userId: string, draft: DraftData) => void;
}
...
export const useOnboardingStore = create<OnboardingStore>()(
  persist(
    (set, get) => ({
      ...
      skipOnboarding: () => {
        set({ isOnboarded: true, completedAt: Date.now() });
      },
      ...
      saveDraft: (_userId: string, draft: DraftData) => {
        if (draft.goal) { get().setGoal(draft.goal); }
        if (draft.focusDuration) { get().setFocusDuration(draft.focusDuration); }
        if (draft.displayName) { get().setDisplayName(draft.displayName); }
      },
    }),
    {
