...
const PersistedOnboardingStateSchema = z.object({
  ...
  focusDuration: z.number().refine((val): val is FocusDuration => [15, 25, 45, 60].includes(val)).nullable(),
...
export function persistOnboardingState(state: OnboardingState): void {
  try {
    const persistedState: PersistedOnboardingState = { ...state, lastUpdatedAt: Date.now(), version: 1 };
    ...
    if (!state.isOnboarded) storage.set(KEYS.INCOMPLETE_BACKUP, JSON.stringify(validated));
    debug.info('Onboarding state persisted', { step: state.currentStep, isOnboarded: state.isOnboarded });
  } catch (error) {
...
export function loadPersistedOnboarding(): OnboardingState | null {
  try {
    ...
    if (!data) { debug.info('No persisted onboarding state found'); return null; }
    ...
    if (isStale && !validated.isOnboarded) {
      ...
      eventBus.publish('analytics:track', { event: 'onboarding_stale_reset', properties: { age, step: validated.currentStep } });
      return null;
    }
    const state: OnboardingState = {
      isOnboarded: validated.isOnboarded, currentStep: validated.currentStep,
      goal: validated.goal, focusDuration: validated.focusDuration,
      displayName: validated.displayName, startedAt: validated.startedAt, completedAt: validated.completedAt,
    };
...
function recoverFromBackup(): OnboardingState | null {
  try {
    ...
    eventBus.publish('analytics:track', { event: 'onboarding_recovered_from_backup', properties: { step: state.currentStep } });
    return state;
  } catch (error) {
