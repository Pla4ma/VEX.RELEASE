...
import React,{useState,useCallback}from'react'; import{View,Text,ScrollView,Pressable,ActivityIndicator,RefreshControl,Alert}from'react-native'; import{useSettingsUIState,useSyncSettings,useResetSettings}from'../hooks'; import{type SettingCategory}from'../types'; import{eventBus}from'../../../events'; import*as Sentry from'@sentry/react-native'; import{SettingItem}from'./SettingItem'; import{ToggleSetting}from'./ToggleSetting'; import{SliderSetting}from'./SliderSetting'; import{createSheet}from'@/shared/ui/create-sheet'; type SettingsState='idle'|'loading'|'error'|'saving'|'syncing';interface SettingsScreenProps{userId:string;initialCategory?:SettingCategory;onBackPress?:()=>void;onCategoryChange?:(category:SettingCategory)=>void}const CATEGORIES:Array<{key:SettingCategory;label:string;icon:string}> = [{key:'general',label:'General',icon:'\u2699\uFE0F'},{key:'notifications',label:'Notifications',icon:'\uD83D\uDD14'},{key:'coach',label:'Coach',icon:'\uD83C\uDFAF'},{key:'appearance',label:'Appearance',icon:'\uD83C\uDFA8'},{key:'privacy',label:'Privacy',icon:'\uD83D\uDD12'},{key:'data',label:'Data',icon:'\uD83D\uDCBE'}]; export function SettingsScreen({userId,initialCategory = 'general',onBackPress,onCategoryChange}:SettingsScreenProps){const[activeCategory,setActiveCategory] = useState<SettingCategory>(initialCategory); const[settingsState,setSettingsState] = useState<SettingsState>('idle'); const{isLoading,isError,error,preferences,notifications,coach,appearance,privacy,refetch} = useSettingsUIState(userId); const syncMutation = useSyncSettings(userId); const resetMutation = useResetSettings(userId); const handleRefresh = useCallback(async()=>{setSettingsState('loading'); try{await refetch(); setSettingsState('idle');}catch (error) { captureSilentFailure(error, { feature: 'settings', operation: 'ui-fallback', type: 'ui' }); setSettingsState('error');}},[refetch]); const handleSync = useCallback(async()=>{setSettingsState('syncing'); try{await syncMutation.mutateAsync({}); setSettingsState('idle'); Sentry.addBreadcrumb({category:'settings',message:'Settings synced successfully',level:'info'});}catch(err){setSettingsState('error'); Sentry.captureException(err,{tags:{operation:'settingsSync'}});}},[syncMutation]); const handleReset = useCallback(()=>{Alert.alert('Reset Settings',`Are you sure you want to reset ${activeCategory} settings to defaults?`,[{text:'Cancel',style:'cancel'},{text:'Reset',style:'destructive',onPress:async()=>{setSettingsState('saving'); try{await resetMutation.mutateAsync({category:activeCategory}); setSettingsState('idle'); eventBus.publish('settings:reset',{category:activeCategory});}catch(err){setSettingsState('error');}}}]);},[activeCategory,resetMutation]); const handleCategoryChange = useCallback((category:SettingCategory)=>{setActiveCategory(category); onCategoryChange?.(category);},[onCategoryChange]); const renderContent = ()=>{if(isLoading || settingsState === 'loading'){return<View style={styles.centerContainer}>
  ...
  accessibilityHint="Activates this control">
        ...
        <View style={styles.categoryHeader}>
          ...
          {settingsState === 'syncing' && <ActivityIndicator size="small"color="theme.colors.primary[500]"/>}
        </View>
  ...
  accessibilityHint="Activates this control">
            ...
            <SettingItem label="Accent Color"value={appearance?.accentColor ?? 'theme.colors.primary[500]'}onPress={()=>{}}/>
  ...
  accessibilityHint="Activates this control">
      ...
      {}
      {renderContent()}
    </View>;}const styles = createSheet({container:{flex:1,backgroundColor:'theme.colors.primary[500]'},header:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',padding:16,backgroundColor:'theme.colors.background.primary',borderBottomWidth:1,borderBottomColor:'theme.colors.primary[500]'},backButton:{padding:8},backButtonText:{fontSize:16,color:'theme.colors.primary[500]'},headerTitle:{fontSize:18,fontWeight:'600',color:'theme.colors.primary[500]'},headerSpacer:{width:60},tabsContainer:{backgroundColor:'theme.colors.background.primary',borderBottomWidth:1,borderBottomColor:'theme.colors.primary[500]',maxHeight:80},tabsContent:{paddingHorizontal:12,paddingVertical:12,gap:8},tab:{alignItems:'center',paddingHorizontal:16,paddingVertical:8,borderRadius:20,backgroundColor:'theme.colors.primary[500]',marginHorizontal:4},activeTab:{backgroundColor:'theme.colors.primary[500]'},tabIcon:{fontSize:20,marginBottom:4},tabLabel:{fontSize:12,color:'theme.colors.primary[500]'},activeTabLabel:{color:'theme.colors.background.primary',fontWeight:'500'},contentScroll:{flex:1},centerContainer:{flex:1,justifyContent:'center',alignItems:'center',padding:32},loadingText:{marginTop:16,fontSize:16,color:'theme.colors.primary[500]'},errorIcon:{fontSize:48,marginBottom:16},errorTitle:{fontSize:18,fontWeight:'600',color:'theme.colors.primary[500]',marginBottom:8},errorMessage:{fontSize:14,color:'theme.colors.primary[500]',textAlign:'center',marginBottom:16},retryButton:{backgroundColor:'theme.colors.primary[500]',paddingHorizontal:24,paddingVertical:12,borderRadius:8},retryButtonText:{color:'theme.colors.background.primary',fontWeight:'600',fontSize:14},categoryHeader:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',padding:16,backgroundColor:'theme.colors.background.primary'},categoryTitle:{fontSize:20,fontWeight:'600',color:'theme.colors.primary[500]'},settingsGroup:{backgroundColor:'theme.colors.background.primary',marginTop:8,paddingHorizontal:16},settingItem:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',paddingVertical:16,borderBottomWidth:1,borderBottomColor:'theme.colors.primary[500]'},settingLabel:{fontSize:16,color:'theme.colors.primary[500]'},settingValueContainer:{flexDirection:'row',alignItems:'center'},settingValue:{fontSize:14,color:'theme.colors.primary[500]',marginRight:8},settingArrow:{fontSize:18,color:'theme.colors.primary[500]'},toggle:{width:50,height:28,borderRadius:14,backgroundColor:'theme.colors.primary[500]',padding:2},toggleActive:{backgroundColor:'theme.colors.primary[500]'},toggleKnob:{width:24,height:24,borderRadius:12,backgroundColor:'theme.colors.background.primary',shadowColor:'theme.colors.text.primary',shadowOffset:{width:0,height:1},shadowOpacity:0.2,shadowRadius:1},toggleKnobActive:{transform:[{translateX:22}]},sliderContainer:{flexDirection:'row',alignItems:'center',gap:12},sliderButton:{width:32,height:32,borderRadius:16,backgroundColor:'theme.colors.primary[500]',justifyContent:'center',alignItems:'center'},sliderButtonText:{fontSize:18,color:'theme.colors.primary[500]',fontWeight:'600'},sliderValue:{fontSize:16,color:'theme.colors.primary[500]',minWidth:40,textAlign:'center'},actionRow:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',paddingVertical:16,borderBottomWidth:1,borderBottomColor:'theme.colors.primary[500]'},actionRowText:{fontSize:16,color:'theme.colors.primary[500]'},actionRowSubtext:{fontSize:12,color:'theme.colors.primary[500]',marginTop:2},actionRowArrow:{fontSize:18,color:'theme.colors.primary[500]'},actionsContainer:{padding:16,gap:12,marginTop:16},actionButton:{backgroundColor:'theme.colors.primary[500]',paddingVertical:14,borderRadius:8,alignItems:'center'},actionButtonText:{color:'theme.colors.background.primary',fontSize:16,fontWeight:'600'},dangerButton:{backgroundColor:'theme.colors.primary[500]'},dangerButtonText:{color:'theme.colors.primary[500]'},dangerZone:{marginTop:24,padding:16,backgroundColor:'theme.colors.primary[500]',borderRadius:12,borderWidth:1,borderColor:'theme.colors.primary[500]'},dangerTitle:{fontSize:14,fontWeight:'600',color:'theme.colors.primary[500]',marginBottom:12},dangerAction:{backgroundColor:'theme.colors.primary[500]',paddingVertical:12,paddingHorizontal:16,borderRadius:8,alignItems:'center'},dangerActionText:{color:'theme.colors.background.primary',fontSize:14,fontWeight:'600'}});
