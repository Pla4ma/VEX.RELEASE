...
describe('companion promise service', () => {
  beforeEach(() => {
    ...
    repository.markPromiseMissed.mockResolvedValue({ ...basePromise, status: 'missed', missedAt: '2026-05-22T10:00:00.000Z' });
    repository.replacePromise.mockResolvedValue({ ...basePromise, status: 'replaced' });
    repository.fulfillPromise.mockResolvedValue({ ...basePromise, status: 'fulfilled', fulfilledAt: '2026-05-21T12:00:00.000Z' });
  });
  ...
  it('replaces an older pending promise, fulfills a matching target-day promise, and marks overdue as missed', async () => {
    ...
    repository.getPendingPromise.mockResolvedValue({ ...basePromise, targetDate: '2026-05-20' });
    await getHomePromiseState(basePromise.userId, true, 'America/New_York', Date.parse('2026-05-21T05:00:00.000Z'));
    expect(repository.markPromiseMissed).toHaveBeenCalledWith(basePromise.id, expect.any(String));
  });
  ...
  it('handles timezone boundaries, offline home state, and recovery actions', async () => {
    ...
    expect(repository.createPromise).toHaveBeenCalledWith(expect.objectContaining({ targetDate: '2026-05-22' }));

    repository.getPendingPromise.mockResolvedValueOnce(basePromise);
    const offline = await getHomePromiseState(basePromise.userId, false, 'America/New_York', Date.parse('2026-05-20T12:00:00.000Z'));
