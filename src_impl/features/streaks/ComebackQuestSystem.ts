...
export async function checkComebackEligibility(userId: string): Promise<{
  ...
  try {
    ...
    const { data: lastSession, error: sessionError } = await getSupabaseClient().from("sessions").select("completed_at").eq("user_id", userId).eq("status", "COMPLETED").order("completed_at", { ascending: false }).limit(1).single();
    ...
    const { data: streakData, error: streakError } = await getSupabaseClient().from("user_streaks").select("streak_before_break").eq("user_id", userId).single();
...
export async function createComebackQuest(userId: string): Promise<ComebackQuest | null> {
  try {
    ...
    const { data: existingQuest, error: checkError } = await getSupabaseClient().from("comeback_quests").select("*").eq("user_id", userId).eq("all_quests_completed", false).order("created_at", { ascending: false }).limit(1).maybeSingle();
 ...
 */
export function checkQuestCompletion(quest: ComebackQuest, sessionDuration: number, grade: string): { questCompleted: ComebackQuestStage | null; progress: ComebackQuestProgress } {
  ...
  // Check Quest 2
    ...
    if (sessionDuration >= COMEBACK_QUEST_CONFIG.quest2.duration && gradeUpper >= COMEBACK_QUEST_CONFIG.quest2.grade) {
      questCompleted = "QUEST_2";
    }
  }
  // Check Quest 3
  else if (!quest.quest3Completed) {
    if (sessionDuration >= COMEBACK_QUEST_CONFIG.quest3.duration && gradeUpper >= COMEBACK_QUEST_CONFIG.quest3.grade) {
 ...
 */
export function calculateQuestProgress(quest: ComebackQuest, currentSessionDuration = 0, currentGrade = "C"): ComebackQuestProgress {
  ...
  if (quest.quest3Completed) {
    ...
    currentStage = "QUEST_3";
  } else if (quest.quest1Completed) {
    currentStage = "QUEST_2";
  }
 ...
 */
export async function updateQuestProgress(questId: string, sessionDuration: number, grade: string): Promise<{ questCompleted: ComebackQuestStage | null; allComplete: boolean }> {
  try {
    // Get current quest state
    const { data: quest, error: fetchError } = await getSupabaseClient().from("comeback_quests").select("*").eq("id", questId).single();
    ...
    // Update database
    const { error: updateError } = await getSupabaseClient().from("comeback_quests").update(updates).eq("id", questId);
...
export async function getActiveComebackQuest(userId: string): Promise<ComebackQuest | null> {
  try {
    const { data, error } = await getSupabaseClient().from("comeback_quests").select("*").eq("user_id", userId).eq("all_quests_completed", false).order("created_at", { ascending: false }).limit(1).maybeSingle();
