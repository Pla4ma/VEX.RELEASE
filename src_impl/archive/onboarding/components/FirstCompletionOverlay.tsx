...
function XpOrb({ xp }: { xp: number }): JSX.Element {
  ...
  const bounceStyle = useAnimatedStyle(() => ({
    transform: [
      {
        scale: withRepeat(withSequence(withSpring(1.2, { damping: 3, stiffness: 200 }), withSpring(1, { damping: 3, stiffness: 200 })), -1, true),
      },
    ],
  }));

  const glowStyle = useAnimatedStyle(() => ({
    opacity: withRepeat(withSequence(withSpring(0.8, { damping: 2 }), withSpring(0.4, { damping: 2 })), -1, true),
  }));
 ...
 */
export function FirstCompletionOverlay({ xpEarned, onComplete }: FirstCompletionOverlayProps): JSX.Element {
  const { theme } = useTheme();
...
export * from "./FirstCompletionOverlay.types";
export * from "./FirstCompletionOverlay.types";
