...
export class CalendarSyncService {
  ...
  async getCombinedFreeBusy(timeMin: Date, timeMax: Date): Promise<FreeBusyInfo> {
  ...
  async findFocusTimeGaps(daysAhead: number = 3, minDuration: number = 25): Promise<CalendarGap[]> {
    const now = new Date();
  ...
  > {
    ...
    for (const { events } of results) {
      allEvents.push(
        ...events.map((e) => ({ title: e.title, startTime: e.startTime, description: e.description })),
      );
    }
