...
export const validateXSS = (input: string, config: SecurityConfig): SecurityValidationResult => {
  ...
  // Check for HTML entities that might be encoded attacks
...
export const validateSQLInjection = (input: string): SecurityValidationResult => {
  ...
  const commentCount = commentPatterns.reduce((count, pattern) => {
    ...
    return count + (input.match(new RegExp(patternString.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
  }, 0);
  ...
  // Check for quote usage patterns
  ...
  const suspiciousCount = suspiciousChars.reduce((count, char) => {
    return count + (input.match(new RegExp(`\\${char.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'g')) || []).length;
  }, 0);
