/**
 * Navigation Guard Tests - PHASE 8 Verification
 * 
 * Tests to verify navigation properly gates disabled features.
 */
...
import { isFeatureEnabled, isFeatureDisabled, getDisabledFeatures } from '../../../constants/features';

// Mock the feature flags
const mockIsFeatureEnabled = jest.fn();
const mockIsFeatureDisabled = jest.fn();
const mockGetDisabledFeatures = jest.fn(() => ['social-feed', 'duels', 'rankings', 'squad-wars', 'rivals', 'trading', 'emergency-gem-sinks', 'complex-crafting', 'ar-experimental']);
...
describe('Navigation Guard - PHASE 8 Verification', () => {
    ...
    it('should show appropriate message for disabled features', () => {
      ...
      const TestComponent = () => (
        <NavigationGuard feature="duels">
          <div testID="protected-content">Duels Content</div>
        </NavigationGuard>
      );
      ...
      expect(screen.getByText('This feature is currently disabled')).toBeTruthy();
      expect(screen.getByText('Duels will be available in a future update')).toBeTruthy();
    });
  });
  ...
  describe('Optional Feature Handling', () => {
    ...
    it('should show different message for optional features', () => {
      ...
      expect(screen.getByText('This feature is not yet available')).toBeTruthy();
  ...
  describe('Disabled Features Verification', () => {
    ...
    it('should verify banned features are properly blocked', () => {
      ...
      bannedFeatures.forEach(feature => {
        ...
        const TestComponent = () => (
          <NavigationGuard feature={feature}>
            <div testID="protected-content">{feature} Content</div>
          </NavigationGuard>
        );
  ...
  describe('Core Features Access', () => {
    it('should allow access to all core features', () => {
      const coreFeatures = ['sessions', 'focus-timer', 'streaks', 'rewards', 'progression'];
  ...
  describe('Error Handling', () => {
    ...
    it('should handle feature flag errors gracefully', () => {
      ...
      // Should default to showing unavailable message
      expect(screen.queryByTestId('protected-content')).toBeFalsy();
      expect(screen.getByText('Feature not available')).toBeTruthy();
    });
  });
  ...
  describe('PHASE 8 Compliance', () => {
    it('should ensure navigation respects PHASE 8 feature configuration', () => {
      // Test that navigation properly implements the feature matrix
      const testFeatures = {
      ...
      Object.entries(testFeatures).forEach(([category, features]) => {
        features.forEach(feature => {
          ...
          if (category === 'enabled') {
            ...
            expect(screen.queryByTestId('protected-content')).toBeFalsy();
            expect(screen.getByText(/Feature (not available|currently disabled|not yet available)/)).toBeTruthy();
          }
        });
      });
    });
