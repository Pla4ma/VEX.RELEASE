 ...
 * 
 * Tests for comprehensive performance gate system that enforces
...
// Mock global fetch for network monitoring
...
// Mock requestAnimationFrame for animation monitoring
...
describe('PerformanceGate', () => {
  ...
  describe('Configuration Management', () => {
    ...
    it('should allow custom target configuration', () => {
      ...
      expect(targets.maxMemoryMb).toBe(PRODUCTION_TARGETS.maxMemoryMb); // Should keep other defaults
    });
  ...
  describe('Performance Gate Evaluation', () => {
    ...
    it('should fail with low FPS', async () => {
      ...
      expect(result.issues.some(issue => issue.category === 'fps')).toBe(true);
    });
    ...
    it('should fail with high memory usage', async () => {
      ...
      expect(result.issues.some(issue => issue.category === 'memory')).toBe(true);
    });

    it('should detect animation performance issues', async () => {
      // Simulate animation metrics
      gate.setBundleSize(500); // Large bundle size

      const result = await gate.evaluatePerformanceGate();

      expect(result.passed).toBe(false);
      expect(result.metrics.bundle.passed).toBe(false);
      expect(result.issues.some(issue => issue.category === 'bundle')).toBe(true);
    });
    ...
    it('should detect network performance issues', async () => {
      ...
      expect(result.issues.some(issue => issue.category === 'network')).toBe(true);
    });
  ...
  describe('Performance Metrics Collection', () => {
    it('should collect FPS metrics correctly', () => {
      ...
      (gate as any).initializeMetricsCollection();
      ...
      if (frameCallback) {
        ...
        frameCallback(performance.now() + 16); // 60fps
        frameCallback(performance.now() + 33); // 60fps
      }
  ...
  describe('Performance Thresholds', () => {
    ...
    it('should use development targets in development', () => {
      ...
      expect(targets.minFps).toBe(30);
      expect(targets.maxMemoryMb).toBe(200); // More lenient for development
    });
  });
