...
export type TextVariant =
  | 'hero' | 'display' | 'h1' | 'h2' | 'h3' | 'h4' | 'h5'
  | 'heading' | 'heading2' | 'heading3' | 'heading4'
  | 'bodyLarge' | 'body' | 'bodySmall' | 'caption' | 'label' | 'button';
...
function getVariantStyles(variant: TextVariant, theme: ReturnType<typeof useTheme>['theme']): TextStyle {
  ...
  switch (variant) {
    case 'hero':
    case 'display':
      return { fontSize: display.fontSize, lineHeight: display.lineHeight, fontWeight: fontWeights.heavy };
    case 'h1':
...
export function Text({
  ...
  return (
    <RNText style={[textStyle, style]} numberOfLines={numberOfLines} ellipsizeMode={ellipsizeMode} {...props}>
      {children}
    </RNText>
  );
}
