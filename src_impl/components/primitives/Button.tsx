...
export function Button({
  ...
  const sizes = {
    sm: { py: theme.spacing[2], px: theme.spacing[3], minHeight: 44, fontSize: 14 },
    small: { py: theme.spacing[2], px: theme.spacing[3], minHeight: 44, fontSize: 14 },
    md: { py: theme.spacing[3], px: theme.spacing[4], minHeight: 48, fontSize: 16 },
    lg: { py: theme.spacing[4], px: theme.spacing[6], minHeight: 56, fontSize: 17 },
  }[size];
  ...
  const variantStyle = (pressed: boolean): ViewStyle => {
    ...
    if (variant === 'primary') {
      return { ...base, backgroundColor: pressed ? semantic.primaryPressed : semantic.primary };
    }
    if (variant === 'danger') {
    ...
    if (variant === 'ghost') {
      return { ...base, backgroundColor: pressed ? semantic.surfaceGlass : 'transparent' };
    }
    return {
      ...base,
      backgroundColor: pressed ? theme.colors.surface.pressed : theme.colors.surface.button,
  ...
  const onPressIn = useCallback((event: GestureResponderEvent) => {
    scale.value = isReducedMotion ? 1 : withSpring(0.98, { damping: 16, stiffness: 420 });
    if (haptic !== 'none') {
      void buttonTap();
    }
  ...
  const onPressOut = useCallback((event: GestureResponderEvent) => {
    scale.value = isReducedMotion ? 1 : withSpring(1, { damping: 14, stiffness: 260 });
    props.onPressOut?.(event);
  }, [isReducedMotion, props, scale]);

  const onPress = useCallback((event: GestureResponderEvent) => {
    if (haptic !== 'none') {
      void triggerHaptic(haptic === 'success' ? 'success' : haptic === 'warning' ? 'warning' : 'impactLight');
    }
    props.onPress?.(event);
  }, [haptic, props]);
    ...
    <Animated.View style={animatedStyle}>
      ...
      >
        ...
        {!isLoading && typeof children === 'string' ? (
          <Text color={textColor} fontSize={sizes.fontSize} fontWeight="700" ml={leftIcon ? 'sm' : undefined} mr={rightIcon ? 'sm' : undefined} variant="button">
            {children}
          </Text>
