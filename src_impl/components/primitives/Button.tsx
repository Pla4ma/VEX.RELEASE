...
export function Button({ variant = "primary", size = "md", fullWidth = false, isLoading = false, isDisabled = false, leftIcon, rightIcon, haptic = "medium", children, style, mt, mb, ml, mr, ...props }: ButtonProps): JSX.Element {
  const { theme } = useTheme();
  const { isReducedMotion } = useReducedMotion();
  const { colors, spacing } = theme;
  const scale = useSharedValue(1);
  const defaultAccessibilityLabel = typeof children === "string" ? `${children} button` : "Action button";
  ...
  return (
    <Animated.View style={animatedStyle}>
      ...
      >
        {isLoading ? (
          <ActivityIndicator color={variant === "primary" || variant === "danger" ? colors.text.inverse : colors.primary[500]} />
        ) : (
          <>
            {leftIcon}
            {typeof children === "string" ? (
              <Text variant="body" fontSize={sizeConfig.fontSize} fontWeight="600" color={getTextColor()} ml={leftIcon ? "sm" : undefined} mr={rightIcon ? "sm" : undefined}>
                {children}
              </Text>
            ) : (
              children
