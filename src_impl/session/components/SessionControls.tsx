import React,{useState}from'react';import{View,Text,Pressable,Modal}from'react-native';import{createSheet}from'@/shared/ui/create-sheet';interface SessionControlsProps{isActive:boolean;isPaused:boolean;onStart:()=>void;onPause:()=>void;onResume:()=>void;onAbandon:()=>void;disabled?:boolean;}export const SessionControls:React.FC<SessionControlsProps>=({isActive,isPaused,onStart,onPause,onResume,onAbandon,disabled=false})=>{const[showConfirmAbandon,setShowConfirmAbandon]=useState(false);const handleAbandon=()=>{setShowConfirmAbandon(false);onAbandon();};if(!isActive){return<View style={styles.container}>
    ...
    </View>;};const styles=createSheet({container:{padding:16},row:{flexDirection:'row',justifyContent:'space-between',gap:12},button:{flex:1,paddingVertical:16,borderRadius:12,alignItems:'center',justifyContent:'center'},startButton:{backgroundColor:'#4CAF50'},resumeButton:{backgroundColor:'#4CAF50'},pauseButton:{backgroundColor:'#FFA500'},abandonButton:{backgroundColor:'#e94560'},disabled:{opacity:0.5},buttonText:{color:'#fff',fontSize:16,fontWeight:'600'},modalOverlay:{flex:1,backgroundColor:'rgba(0, 0, 0, 0.7)',justifyContent:'center',alignItems:'center',padding:24},modalContent:{backgroundColor:'#1a1a2e',borderRadius:16,padding:24,width:'100%',maxWidth:400},modalTitle:{fontSize:20,fontWeight:'700',color:'#e94560',marginBottom:12,textAlign:'center'},modalText:{fontSize:16,color:'#9E9E9E',textAlign:'center',marginBottom:24,lineHeight:22},modalButtons:{flexDirection:'column',gap:12},modalButton:{paddingVertical:14,borderRadius:8,alignItems:'center'},cancelButton:{backgroundColor:'transparent',borderWidth:1,borderColor:'#4CAF50'},cancelButtonText:{color:'#4CAF50',fontSize:16,fontWeight:'600'},confirmAbandonButton:{backgroundColor:'#e94560'}});export default SessionControls;
