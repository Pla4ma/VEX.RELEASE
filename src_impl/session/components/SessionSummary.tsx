 ...
 * Session Summary
 *
 * Post-session results display showing score, rewards, streak status.
 * Includes reflection prompts and social sharing options.
 */
...
export const SessionSummary: React.FC<SessionSummaryProps> = ({
  ...
  const getGrade = (score: number): { letter: string; color: string } => {
    if (score >= 900) {return { letter: 'S', color: '#FFD700' };}
    if (score >= 800) {return { letter: 'A', color: '#4CAF50' };}
    if (score >= 700) {return { letter: 'B', color: '#8BC34A' };}
    if (score >= 600) {return { letter: 'C', color: '#FFC107' };}
    if (score >= 500) {return { letter: 'D', color: '#FF9800' };}
  ...
  return (
    <ScrollView style={styles.container}>
      <Animated.View style={[styles.content, scaleStyle]}>
        ...
        {/* Rewards */}
        {(summary.xpEarned > 0 || summary.coinsEarned > 0 || summary.gemsEarned > 0) && (
        ...
        <View style={styles.actions}>
          <Pressable style={({ pressed }) => [styles.shareButton, pressed && { opacity: 0.8 }]} onPress={onShare}
          ...
          <Pressable style={({ pressed }) => [styles.newSessionButton, pressed && { opacity: 0.8 }]} onPress={onStartNew}
          ...
          <Pressable style={({ pressed }) => [styles.closeButton, pressed && { opacity: 0.8 }]} onPress={onClose}
