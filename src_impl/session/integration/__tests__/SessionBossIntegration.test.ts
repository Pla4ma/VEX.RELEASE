import { calculateBossDamage, initializeSessionBossIntegration } from '../SessionBossIntegration';
...
describe('SessionBossIntegration', () => {
  const mockedEventBus = jest.mocked(eventBus);
  const mockedApplyDamage = jest.mocked(applyDamage);
  const mockedConsumeBountiesOnDamage = jest.mocked(consumeBountiesOnDamage);
  const mockedRecordBountyLootBoost = jest.mocked(recordBountyLootBoost);
  const mockedGetActiveEncounter = jest.mocked(getActiveEncounter);
