 ...
 *
 * Phase 8A.1 — Session service unit tests
 * Tests all critical session lifecycle operations.
 */
...
describe('SessionService', () => {
  ...
  describe('createSession', () => {
    ...
    it('should throw error when no user is set', async () => {
      ...
      await expect(serviceWithoutUser.createCustomSession(validConfig))
    ...
    it('should throw error when active session exists', async () => {
      ...
      await expect(service.createCustomSession(validConfig))
        .rejects.toThrow('Cannot create new session: one is already active');
    });

    it('should throw error for invalid duration (zero)', async () => {
      const invalidConfig = { ...validConfig, duration: 0 };

      await expect(service.createCustomSession(invalidConfig))
        .rejects.toThrow();
    });
  ...
  describe('pauseSession', () => {
    ...
    it('should throw error when pausing already paused session', async () => {
      ...
      await expect(service.pauseSession())
        .rejects.toThrow('Session already paused');
    });
    ...
    it('should throw error when pausing in strict mode', async () => {
      ...
      await expect(service.pauseSession())
        .rejects.toThrow('Cannot pause in strict mode');
    });
  ...
  describe('completeSession', () => {
    ...
    it('should calculate XP with multiple multipliers', async () => {
      const mockSummary = {
        ...
        status: 'completed',
        duration: 25 * 60 * 1000,
        xpEarned: 375, // 250 base * 1.5 streak
        baseXP: 250,
    ...
    it('should throw error when completing non-active session', async () => {
      mockOrchestrator.getSessionStatus.mockReturnValue({ status: 'idle' });
      mockOrchestrator.completeSession.mockRejectedValue(new Error('No active session to complete'));
  ...
  describe('abandonSession', () => {
    ...
    it('should not award streak credit when abandoned', async () => {
      ...
      // Verify the orchestrator was called with abandon
    ...
    it('should throw error when abandoning completed session', async () => {
      ...
      mockOrchestrator.abandonSession.mockRejectedValue(new Error('Cannot abandon completed session'));

      await expect(service.abandonSession())
  ...
  describe('offline queue fallback', () => {
    it('should queue session when Supabase call fails', async () => {
      ...
      // Service should queue for retry
      await expect(service.createCustomSession(validConfig))
        .rejects.toThrow('Network error');
    });

    it('should retry queued sessions when connection restored', async () => {
      // This would test the offline queue mechanism
      // Implementation depends on offline queue service
