 ...
 *
 * Comprehensive test coverage for session persistence utilities.
 *
 * @phase 1 - Deepening: Test coverage
 */
...
describe('SessionPersistence', () => {
  const mockSession: PersistedSessionState = {
  ...
  describe('persist', () => {
    ...
    it('should throw for invalid session state', () => {
      ...
      expect(() => SessionPersistence.persist(invalidSession as PersistedSessionState)).toThrow();
    });
  ...
  describe('load', () => {
    ...
    it('should recover from backup on corruption', () => {
      ...
      // First call throws (corruption), backup succeeds
      mockStorage.getString
        .mockReturnValueOnce(null) // Active session missing
        .mockReturnValueOnce(JSON.stringify(mockSession)); // Backup exists
  ...
  describe('recovery tracking', () => {
    it('should record recovery attempt', () => {
      ...
      expect(() => SessionPersistence.recordRecoveryAttempt(attempt)).not.toThrow();
    });
...
describe('canResumeSession', () => {
  const baseState: PersistedSessionState = {
    sessionId: 'test-123',
    userId: 'user-123',
    status: 'ACTIVE',
    phase: 'FOCUS',
...
describe('SessionPersistenceError', () => {
  ...
  it('should include details', () => {
    ...
    const error = new SessionPersistenceError('Test error', { cause, sessionId: '123' });
