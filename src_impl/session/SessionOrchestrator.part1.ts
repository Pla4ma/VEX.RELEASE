...
export class SessionOrchestrator {
  ...
  private flowPulseEngine: FlowPulseEngine;
  private companionPresenceEngine: CompanionPresenceEngine;
  private restChallengeEngine: RestPhaseChallengeEngine;
  private bossAbilityEngine: BossAbilityEngine;
  private previousFlowState: FlowState = 'SHALLOW';
  private nextPhaseQualityMultiplier: number = 1.0;
  constructor(config: OrchestratorConfig = {}) {
    ...
    this.flowPulseEngine = new FlowPulseEngine();
    this.companionPresenceEngine = new CompanionPresenceEngine();
    this.restChallengeEngine = new RestPhaseChallengeEngine();
    this.bossAbilityEngine = new BossAbilityEngine();
  ...
  async createSession(config: SessionConfig): Promise<SessionState> {
    ...
    this.session = {
      ...
      isOnline: true,
      modeBonus: 0,
      activeConsumables: config.activeConsumables || [],
    };
    ...
    this.flowPulseEngine.initialize(sessionId, this.userId);
    this.companionPresenceEngine.initialize(sessionId, this.userId);
    this.restChallengeEngine.initialize(sessionId, this.userId);
    this.bossAbilityEngine.initialize(sessionId, this.userId, false);
    this.previousFlowState = 'SHALLOW';
    this.nextPhaseQualityMultiplier = 1.0;
  ...
  private handleTimerTick(
    ...
    // ── Micro-interaction engine ticks ──
    const purityScore = this.antiCheatEngine.getCurrentPurityScore();
    const flowEvent = this.flowPulseEngine.tick(elapsed / 1000, purityScore, false);
    if (flowEvent && flowEvent.flowState !== this.previousFlowState) {
      this.previousFlowState = flowEvent.flowState;
    }
    this.companionPresenceEngine.tick(
      percentage, purityScore, this.session.phase,
      false, undefined, flowEvent?.flowState,
    );
    this.bossAbilityEngine.tick(purityScore);
  ...
  async startBreak(): Promise<void> {
    ...
    this.session.updatedAt = Date.now();
    this.companionPresenceEngine.reset();
    this.restChallengeEngine.triggerChallenge(this.session.phase);
  ...
  async gracefulExit(reflection?: string, mood?: any): Promise<SessionSummary> {
    ...
    const result = this.completionEngine.gracefulExit(
      this.session,
      this.focusMetrics,
      0,
      reflection,
  ...
  destroy(): void {
    ...
    this.flowPulseEngine.destroy();
    this.companionPresenceEngine.destroy();
    this.restChallengeEngine.destroy();
    this.bossAbilityEngine.destroy();
    this.isActive = false;
    this.session = null;
    debug.info('SessionOrchestrator destroyed');
  }

  // ── Micro-Interaction Public API ────────────────────────────────────────

  tapCompanion(): CompanionTappedEvent | null {
    return this.companionPresenceEngine.tapCompanion();
  }

  activateBossAbility(abilityType: BossAbilityType): boolean {
    const result = this.bossAbilityEngine.activateAbility(abilityType);
  ...
  completeRestChallenge(): RestChallengeCompletedEvent | null {
    const result = this.restChallengeEngine.completeChallenge();
    if (result) {
      this.nextPhaseQualityMultiplier = result.nextPhaseQualityMultiplier;
    }
    return result;
  }

  skipRestChallenge(): void {
    this.restChallengeEngine.skipChallenge();
  }

  getFlowState(): FlowState {
    return this.flowPulseEngine.getFlowState();
  }

  getFlowPulseQuality(): number {
    return this.previousFlowState === 'DEEP_FLOW' ? 1
      : this.previousFlowState === 'FLOW' ? 0.7
      : this.previousFlowState === 'SHALLOW' ? 0.4
      : 0.1;
  }

  getBossAbilityStates(): BossAbilityState[] {
    return this.bossAbilityEngine.getAbilityStates();
  }

  isBossAttacking(): boolean {
    return this.bossAbilityEngine.isBossAttacking();
  }

  getPurityAccelerationBonus(): number {
    return this.flowPulseEngine.getPurityAccelerationBonus();
  }

  getNextPhaseQualityMultiplier(): number {
    return this.nextPhaseQualityMultiplier;
  }

  setBossActive(active: boolean): void {
    this.bossAbilityEngine.setBossActive(active);
  }
