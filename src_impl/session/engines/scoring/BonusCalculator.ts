...
export function calculateQualityBonus(input: QualityBonusInput): number {
  ...
  const adjustedScore = Math.max(0, focusMetrics.overallScore - (interruptions * 5 + pauses * 2));
  if (adjustedScore >= BONUS_CONSTANTS.EXCELLENT_QUALITY_THRESHOLD) return BONUS_CONSTANTS.EXCELLENT_QUALITY_BONUS;
  if (adjustedScore >= BONUS_CONSTANTS.GOOD_QUALITY_THRESHOLD) return BONUS_CONSTANTS.GOOD_QUALITY_BONUS;
  if (adjustedScore >= BONUS_CONSTANTS.AVERAGE_QUALITY_THRESHOLD) return BONUS_CONSTANTS.AVERAGE_QUALITY_BONUS;
  return 0;
}
...
export function calculateIntervalBonus(input: IntervalBonusInput): number {
  ...
  let bonus = 0;
  if (completedIntervals > 1) bonus += completedIntervals * BONUS_CONSTANTS.MULTI_INTERVAL_BONUS;
...
export function calculateSpecialBonuses(input: SpecialBonusInput): SpecialBonusResult {
  ...
  const result: SpecialBonusResult = {
    perfectSession: false, marathon: false, earlyBird: false, nightOwl: false, totalBonus: 0, badges: [],
  };
...
export function calculateTotalBonus(input: TotalBonusInput): {
  ...
  return {
    total,
    breakdown: {
      time: input.timeBonus, streak: input.streakBonus,
      quality: input.qualityBonus, interval: input.intervalBonus, special: input.specialBonus,
    },
  };
}

export const BonusCalculator = {
  calculateTimeBonus, calculateStreakBonus, getStreakMultiplier,
  calculateQualityBonus, calculateIntervalBonus, calculateSpecialBonuses, calculateTotalBonus,
