 ...
 * TimerEngine Tests
 *
 * Comprehensive test suite for the TimerEngine.
 */
...
describe('TimerEngine', () => {
  ...
  const defaultConfig: TimerConfig = {
    duration: 60, // 60 seconds for faster tests
    breakDuration: 10,
    ...
    tickInterval: 100, // 100ms for faster tests
  ...
  describe('Warning Callbacks', () => {
    ...
    it('should call onWarning multiple times for multiple thresholds', () => {
      engine.start();
      // Advance through all warning thresholds
      jest.advanceTimersByTime(defaultConfig.duration * 1000);
      expect(warningCallback).toHaveBeenCalledTimes(defaultConfig.warningSeconds!.length);
    });
  });
