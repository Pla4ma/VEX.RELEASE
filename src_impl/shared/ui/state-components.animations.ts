...
export function usePulseStyle(enabled = true): ViewStyle {
...
export function useFadeStyle(visible: boolean, duration = 300): ViewStyle {
...
export function useScaleInStyle(initialScale: number, spring = true): ViewStyle {
  const scale = useSharedValue(initialScale);
  useEffect(() => {
    scale.value = spring ? withSpring(1, { damping: 10, stiffness: 120 }) : withTiming(1, { duration: 200 });
  }, [scale, spring]);
  return useAnimatedStyle(() => ({ transform: [{ scale: scale.value }] }));
}

export function useShakeStyle(): ViewStyle {
  const translateX = useSharedValue(0);
  useEffect(() => {
    translateX.value = withSequence(withTiming(10, { duration: 100 }), withTiming(-10, { duration: 100 }), withTiming(5, { duration: 100 }), withTiming(0, { duration: 100 }));
  }, [translateX]);
