...
class HapticEngine {
  ...
  private toHapticKind(context: HapticContext, intensity: HapticIntensity): HapticFeedbackKind {
  ...
  async doubleTap(intensity: HapticIntensity = 'light'): Promise<void> {
    if (!this.shouldTrigger()) return;
    await triggerHapticPattern([toImpactKind(intensity), toImpactKind(intensity)], 80);
  }
...
export const haptics = {
  ...
  trigger: (context: HapticContext, intensity?: HapticIntensity) => getHapticEngine().trigger(context, intensity),
