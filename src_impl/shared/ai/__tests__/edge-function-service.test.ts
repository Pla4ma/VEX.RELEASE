...
describe('edge-function-service', () => {
  it('validates route and request type alignment', async () => {
    ...
    expect(result.body).toEqual({
      error: 'Route expects GENERATE_COACH_MESSAGE, received GENERATE_STREAK_RISK_NUDGE',
    });
  });
  ...
  it('generates a streak nudge through the Gemini path end to end', async () => {
    const fetchImpl = jest.fn().mockResolvedValue({
      ...
      json: async () => ({
        candidates: [
          {
            content: {
              parts: [{ text: '🔥 Your 7-day streak is close-call territory. Lock in 15 minutes now.' }],
              role: 'model',
            },
            finishReason: 'STOP',
            index: 0,
    ...
    expect(result.body).toMatchObject({
      success: true,
      requestType: 'GENERATE_STREAK_RISK_NUDGE',
      content: '🔥 Your 7-day streak is close-call territory. Lock in 15 minutes now.',
      structuredData: {
        streakCount: 7,
