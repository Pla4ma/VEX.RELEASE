...
export const DETERMINISTIC_MESSAGES: Record<string, Record<string, string>> = {
  ...
  comeback_prompt: {
    ...
    default: 'Comeback in progress. {sessionsCompleted} of 3 sessions completed.',
  },
  streak_nudge: {
    low: 'Your {streak}-day streak is secure. Still, a quick session never hurts.',
    medium: '{hoursRemaining}h left to save your {streak}-day streak. A {duration}-min session is all it takes.',
    high: 'Only {hoursRemaining}h remaining! Your {streak}-day streak needs you.',
    critical: '{minutesRemaining}min left! Start a session NOW to save your streak.',
