...
class RevenueCatService {
  ...
  async getCustomerInfo(): Promise<CustomerInfoResult> {
    if (!this.isReady()) {
      return { success: false, entitlements: [], error: this.createError('CONFIGURATION_ERROR', 'RevenueCat not initialized') };
    }
    try {
      const customerInfo = await Purchases.getCustomerInfo();
      return { success: true, customerInfo, entitlements: this.mapEntitlements(customerInfo) };
    } catch (error) {
      const err = this.normalizeError(error);
      debug.error('[RevenueCat] Get customer info failed', err);
      Sentry.captureException(err.underlyingError || new Error(err.message), { tags: { component: 'RevenueCatService', operation: 'getCustomerInfo' } });
  ...
  async getOfferings(): Promise<OfferingsResult> {
    if (!this.isReady()) {
      return { success: false, currentOffering: null, error: this.createError('CONFIGURATION_ERROR', 'RevenueCat not initialized') };
    }
    try {
      ...
      Sentry.captureException(err.underlyingError || new Error(err.message), { tags: { component: 'RevenueCatService', operation: 'getOfferings' } });
      return { success: false, error: err };
    }
  }

  async purchasePackage(pkg: PurchasesPackage): Promise<PurchaseResult> {
    if (!this.isReady()) {return { success: false, error: this.createError('CONFIGURATION_ERROR', 'RevenueCat not initialized') };}
    try {
      const { customerInfo } = await Purchases.purchasePackage(pkg);
      return { success: true, customerInfo };
    } catch (error) {
      if (this.isUserCancelled(error)) {
        return { success: false, error: this.createError('PURCHASE_CANCELLED', 'User cancelled the purchase'), errorCode: 'PURCHASE_CANCELLED' };
      }
      const err = this.normalizeError(error);
  ...
  async restorePurchases(): Promise<PurchaseResult> {
    ...
    try {
      ...
      Sentry.captureException(err.underlyingError || new Error(err.message), { tags: { component: 'RevenueCatService', operation: 'restorePurchases' } });
      return { success: false, error: err, errorCode: err.code };
    }
  }
  ...
  async identifyUser(appUserId: string): Promise<boolean> {
    ...
    try {
      ...
      Sentry.captureException(err.underlyingError || new Error(err.message), { tags: { component: 'RevenueCatService', operation: 'identifyUser' }, extra: { appUserId } });
      return false;
    }
  }
  ...
  async logoutUser(): Promise<boolean> {
    ...
    try {
      ...
      Sentry.captureException(err.underlyingError || new Error(err.message), { tags: { component: 'RevenueCatService', operation: 'logoutUser' } });
      return false;
    }
  }
  ...
  private mapEntitlements(customerInfo: CustomerInfo): EntitlementInfo[] {
    return Object.entries(customerInfo.entitlements.active).map(([identifier, entitlement]) => ({
      identifier,
  ...
  private normalizeError(error: unknown): RevenueCatError {
    ...
    if (purchasesError.code !== undefined && Object.values(PURCHASES_ERROR_CODE).includes(purchasesError.code)) {
      const err = new Error(purchasesError.message || String(error)) as RevenueCatError;
      err.name = 'RevenueCatError';
      err.code = this.mapErrorCode(purchasesError.code);
      err.underlyingError = error instanceof Error ? error : new Error(String(error));
      return err;
    }
    const err = new Error(error instanceof Error ? error.message : String(error)) as RevenueCatError;
    err.name = 'RevenueCatError';
    err.code = 'UNKNOWN';
    err.underlyingError = error instanceof Error ? error : new Error(String(error));
    return err;
  ...
  private createError(code: RevenueCatError['code'], message: string): RevenueCatError {
