...
export function getEntitlementAccessState(
  ...
  const premiumEntitlementIds = active
    .filter((entitlement) => isPremiumEntitlementIdentifier(entitlement.identifier))
  ...
  return {
    ...
    hasBillingIssue: active.some((entitlement) => Boolean(entitlement.billingIssueDetectedAt)),
  };
}

export function hasPremiumEntitlement(entitlements: readonly EntitlementInfo[]): boolean {
