...
export function useRevenueCat(): UseRevenueCatState {
  const [status, setStatus] = useState<UseRevenueCatState['status']>('uninitialized');
  const [isInitialized, setIsInitialized] = useState(false);
  const [isReady, setIsReady] = useState(false);
  const [offerings, setOfferings] = useState<PurchasesOfferingDisplayInfo | null>(null);
  const [availablePackages, setAvailablePackages] = useState<PurchasesPackageDisplayInfo[]>([]);
  const [customerInfo, setCustomerInfo] = useState<CustomerInfo | null>(null);
  const [activeEntitlements, setActiveEntitlements] = useState<EntitlementInfo[]>([]);
  const [isPremium, setIsPremium] = useState(false);
  const [isLoadingOfferings, setIsLoadingOfferings] = useState(false);
  const [isLoadingCustomer, setIsLoadingCustomer] = useState(false);
  const [isPurchasing, setIsPurchasing] = useState(false);
  const [isRestoring, setIsRestoring] = useState(false);
  const [offeringsError, setOfferingsError] = useState<RevenueCatError | null>(null);
  const [customerError, setCustomerError] = useState<RevenueCatError | null>(null);
  const [purchaseError, setPurchaseError] = useState<RevenueCatError | null>(null);
    ...
    try {
      ...
      capture(
        ...
        createOfferingProperties(
          ...
          displayOffering.packages.map((item) => ({ packageType: item.packageType }))
        )
      );
    } catch (error) {
  ...
  const purchasePackage = useCallback(
    async (packageInfo: PurchasesPackageDisplayInfo): Promise<PurchaseResult> => {
      if (!revenueCatService.isReady()) {
        const error = buildError('CONFIGURATION_ERROR', 'RevenueCat is not ready');
      ...
      if (!rawPackage) {
        const error = buildError('PRODUCT_NOT_AVAILABLE', 'Package not found in active offering');
        setPurchaseError(error);
        return { success: false, error, errorCode: error.code };
      }
