---
name: vex-app-ai-coding-context
description: "Use when working in the VEX mobile app codebase. Enforces architecture, stack constraints, file layout, typing, validation, UI states, and integration rules to prevent off-stack or low-quality changes."
---

# VEX App — AI Coding Context
...
If you're about to write a Supabase query in a hook, stop and put it in `repository.ts`.
If you're about to put business logic in a component, stop and put it in `service.ts` and expose it via a hook.
...
- fallback/degraded state when recovery isn't possible
...
- `useNetInfo()` is already wired; use it to drive degraded UI.
...
Additional rules:
- All values from `src/theme/tokens/` (no hardcoded values).
...
- Every new screen registers params in `RootStackParamList` before screen creation.
- No untyped `navigation.navigate('X')` calls.
- Auth guards live in `navigation/guards.ts` (not screens).
- Deep link handlers live in `navigation/deeplinks.ts`.

## Security

- No secrets in source; use expo-constants + EAS Secrets.
- Auth tokens in expo-secure-store via the existing SecureStorage wrapper, not MMKV.
- No PII in Sentry events, breadcrumbs, or logs.
- Supabase client queries always rely on RLS.
- Sanitize user-generated content before rendering or persisting.

## Sentry

- Use `Sentry.captureException(error, { tags: { feature: '...' } })` for unexpected errors.
- Use `Sentry.addBreadcrumb({ message: '...', category: '...' })` at key flow steps.
- No PII in any Sentry payload.
- Sentry calls only in `analytics.ts`, service layer, or mutation `onError` handlers (never components).
...
- Zod schemas: valid/invalid/edge tests.
...
- offline hooks (`hooks-offline.ts`) for queued mutations
- realtime hooks (`hooks-realtime.ts`) for Supabase subscriptions
...
New design tokens must be added to token files before use, never inline.
...
- business logic inside JSX/component render functions
- dead code after refactors
- files over 200 lines that have not been split
