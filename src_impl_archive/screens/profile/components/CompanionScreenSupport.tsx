...
export const ELEMENT_LORE: Record<CompanionElement, string> = {
  FLAME: 'Born from determination. Flame companions thrive on intense focus sessions.',
  LUMINA: 'Pure and exacting. Lumina companions illuminate the path to mastery.',
  ...
  return (
    <Card size="sm" style={{ flex: 1 }}>
      <Box alignItems="center" gap="xs">
        <Text variant="h4" color="text.primary" fontWeight="700" textAlign="center">
          {value}
        </Text>
...
export function PhaseProgressBar({ currentPhase }: { currentPhase: CompanionPhase }): JSX.Element {
    ...
    <Box flexDirection="row" justifyContent="space-between" alignItems="center">
      {PHASE_ORDER.map((phase, index) => (
        <Box key={phase} alignItems="center" flex={1}>
          ...
          />
          <Text variant="caption" color="text.tertiary" mt="xs" textAlign="center">
            {PHASE_NAMES[phase]}
          </Text>
        </Box>
      ))}
...
export function ProgressToNext({ companion }: { companion: CompanionState }): JSX.Element {
  ...
  return (
    <Text variant="bodySmall" color="text.secondary">
      {Math.floor(getEvolutionProgress(companion) * 100)}% to {PHASE_NAMES[getNextPhase(companion.phase)]}
    </Text>
  );
}
...
export function MoodDot({ mood }: { mood: CompanionMood }): JSX.Element {
  ...
  return (
    <Animated.View accessibilityLabel={`Recent mood ${mood}`} style={animatedStyle}>
      <Box width={24} height={24} borderRadius="full" bg="primary.500" />
    </Animated.View>
  );
}

export function CompanionScreenSkeleton({ heroHeight }: { heroHeight: number }): JSX.Element {
  return (
    <Box flex={1} bg="background.primary">
      <Box height={heroHeight} bg="background.secondary" justifyContent="center" alignItems="center">
        <Box width={200} height={200} borderRadius="full" bg="background.tertiary" />
      </Box>
      <Box p="lg" gap="lg">
        <Box height={32} width={200} borderRadius="md" bg="background.tertiary" />
...
function getNextPhase(currentPhase: CompanionPhase): CompanionPhase {
  const currentIndex = PHASE_ORDER.indexOf(currentPhase);
  return PHASE_ORDER[Math.min(currentIndex + 1, PHASE_ORDER.length - 1)] ?? currentPhase;
}
