  ...
  return (
    <Box flex={1} style={{ backgroundColor }}>
      <Box px={20} pb={12} pt={insetsTop + 16}>
        <Box mb={16}><Skeleton width={150} height={28} variant="text" /></Box>
      ...
      <Box px={16} pt={12}><SkeletonList count={6} /></Box>
    </Box>
  );
}

export function NotificationErrorState({
  insetsTop, backgroundColor, message, onRetry,
}: {
  insetsTop: number; backgroundColor: string; message: string; onRetry: () => void;
}): JSX.Element {
  return (
    <Box flex={1} style={{ backgroundColor }}>
      <Box px={20} pb={12} pt={insetsTop + 16}><Text variant="h1">Notifications</Text></Box>
      <ErrorState title="Failed to load notifications" description={message}
...
export function NotificationEmptyState({
  ...
  backgroundColor: string; headerElement: JSX.Element;
}): JSX.Element {
  return (
    <Box flex={1} style={{ backgroundColor }}>
      {headerElement}
...
export function NotificationFilteredEmptyState({
  backgroundColor, headerElement, activeFilter,
}: {
  backgroundColor: string; headerElement: JSX.Element; activeFilter: string;
}): JSX.Element {
  return (
    <Box flex={1} style={{ backgroundColor }}>
      {headerElement}
      <Box flex={1} alignItems="center" justifyContent="center" px={24}>
        <Text fontSize={48} color="text.tertiary">{'\u{2315}'}</Text>
        <Text variant="h4" style={{ marginTop: 16, textAlign: 'center' }}>
          No {activeFilter === 'all' ? '' : ` ${FILTER_LABELS[activeFilter] ?? activeFilter}`} notifications
        </Text>
