...
import React,{useCallback,useMemo}from'react'; import{ScrollView}from'react-native'; import Animated,{FadeIn,FadeInUp,useAnimatedStyle,useSharedValue,withTiming,withSequence}from'react-native-reanimated'; import{useNavigation,useRoute,type RouteProp}from'@react-navigation/native'; import type{NativeStackNavigationProp}from'@react-navigation/native-stack'; import*as Sentry from'@sentry/react-native'; import{Box,Text}from'../../components/primitives'; import{Button}from'../../components/primitives/Button'; import{useAuthStore}from'../../store'; import{getStreakService}from'../../streaks/StreakService'; import{useTheme}from'../../theme'; import{useToast}from'../../shared/ui/components/Toast'; import{useBalance}from'../../features/economy/hooks'; import{useFeatureAccess}from'../../features/liveops-config'; import{getFeatureAvailability,isFeatureAvailableForNavigation}from'../../features/liveops-config/feature-availability';import{getPublicV1Status}from'../../features/liveops-config/public-v1-feature-map'; import type{ExtendedRootStackParams}from'../../navigation/types'; const RESTORE_COSTS = {UNDER_7_DAYS:100,DAYS_7_TO_29:200,DAYS_30_PLUS:500}as const; function calculateRestoreCost(streakDays:number):number{if(streakDays < 7){return RESTORE_COSTS.UNDER_7_DAYS;}if(streakDays < 30){return RESTORE_COSTS.DAYS_7_TO_29;}return RESTORE_COSTS.DAYS_30_PLUS;}type StreakFuneralRoute=RouteProp<ExtendedRootStackParams,'StreakFuneral'>;type StreakFuneralNavigation=NativeStackNavigationProp<ExtendedRootStackParams,'StreakFuneral'>;function DyingFlame():JSX.Element{const flameScale = useSharedValue(1); const flameOpacity = useSharedValue(1); React.useEffect(()=>{flameScale.value = withSequence(withTiming(0.9,{duration:500}),withTiming(0.7,{duration:500}),withTiming(0.4,{duration:500}),withTiming(0,{duration:500})); flameOpacity.value = withSequence(withTiming(0.8,{duration:500}),withTiming(0.6,{duration:500}),withTiming(0.3,{duration:500}),withTiming(0,{duration:500}));},[flameOpacity,flameScale]); const animatedStyle = useAnimatedStyle(()=>({transform:[{scale:flameScale.value}],opacity:flameOpacity.value})); return<Animated.View style={animatedStyle}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}contentContainerStyle={{flexGrow:1,justifyContent:'center'}}>
        ...
        <Animated.View entering={FadeInUp.delay(500)}>
          <Box alignItems="center"mb="xl">
            <Text variant="body"color="text.secondary"textAlign="center">
              {previousStreak} days of focus. Every day built your rhythm, and that rhythm starts again today.
            </Text>
          </Box>
        </Animated.View>
        ...
        <Animated.View entering={FadeInUp.delay(1000)}>
          <Box gap="md">
            {}
            {showGems && <Button variant="primary"size="lg"fullWidth onPress={handleStartFresh}disabled={gemBalance < restoreCost}accessibilityLabel={`Restore rhythm for ${restoreCost} gems`}accessibilityRole="button"accessibilityHint={`Protects your ${previousStreak}-day rhythm for ${restoreCost} gems`}>
              Restore rhythm for {restoreCost} 💎
            </Button>}

            {}
            {showGems && gemBalance < restoreCost && <Text variant="caption"color="text.tertiary"textAlign="center">
                Need {restoreCost} gems. You have {gemBalance}.
                {'\n'}
                {showShop ? <Text variant="caption"color="primary.500"onPress={()=>{const canGo = isFeatureAvailableForNavigation(getFeatureAvailability(disclosure.features.shop)); if(canGo)navigation.navigate('Shop');}}>
            ...
            {}
            <Button variant="secondary"size="md"fullWidth onPress={handleStartFresh}accessibilityLabel="Start fresh with a new rhythm"accessibilityRole="button"accessibilityHint="Begins a new rhythm with bonus XP for your first sessions">
              Start fresh — new rhythm
            </Button>

            {}
            <Button variant="ghost"size="sm"fullWidth onPress={handleReminisce}accessibilityLabel="View focus history button"accessibilityRole="button"accessibilityHint="Activates this control">
              View Focus History
            </Button>
          </Box>
        </Animated.View>
    ...
    </Box>;}; export default withScreenErrorBoundary(StreakFuneralScreen, 'StreakFuneral');
