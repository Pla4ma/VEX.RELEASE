...
export class PurchaseRepository {
  ...
  async savePurchase(userId: string, purchase: Purchase): Promise<void> {
    try {
      ...
      const existingIndex = purchases.findIndex(p => p.transactionId === purchase.transactionId);
      ...
      debug.info('Purchase saved', { userId, transactionId: purchase.transactionId });
    } catch (error) {
      debug.error('Failed to save purchase', error as Error);
      throw new PurchaseRepositoryError('Failed to save purchase', { cause: error });
    }
  }
  ...
  async saveSubscription(userId: string, subscription: Subscription): Promise<void> {
    try {
      ...
      debug.info('Subscription saved', { userId, subscriptionId: subscription.id });
    } catch (error) {
      debug.error('Failed to save subscription', error as Error);
      throw new PurchaseRepositoryError('Failed to save subscription', { cause: error });
    }
  }
