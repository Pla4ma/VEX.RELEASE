...
export async function sendAIRequest(request: AIRequest): Promise<AIResponse> {
  const fallbackCategory = REQUEST_TYPE_TO_CATEGORY[request.requestType] ?? 'coach_message';
  const { response } = await invokeAIWithFallback(
    request.requestType, request.userId, request,
    fallbackCategory, 'default', request.context as Record<string, unknown>,
  );
  return response;
}
...
export async function generateCoachMessage(
  ...
  const { response } = await invokeAIWithFallback(
    validated.requestType, validated.userId, body,
    'coach_message', category,
...
export async function generateComebackPrompt(
  ...
  const comebackDay = String((validated.context as Record<string, unknown>).comebackDay ?? '1');
  const { response } = await invokeAIWithFallback(
    validated.requestType, validated.userId, validated,
    'comeback_prompt', `day${comebackDay}`,
    validated.context as Record<string, unknown>,
  );
...
export async function generateStreakRiskNudge(
  ...
  const riskLevel = String((validated.context as Record<string, unknown>).riskLevel ?? 'medium');
  const { response } = await invokeAIWithFallback(
    validated.requestType, validated.userId, validated,
    'streak_nudge', riskLevel,
    validated.context as Record<string, unknown>,
  );
