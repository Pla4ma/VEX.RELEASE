...
export async function getStreakRestoreQuest(userId: string): Promise<RestoreQuest | null> {
  ...
  try {
    ...
    return quest;
  } catch (error) { captureSilentFailure(error, { feature: 'streaks', operation: 'network-fallback', type: 'network' });
    await storage.removeItem(getQuestKey(userId));
    return null;
  }
}

export async function hasUsedStreakRestoreThisMonth(userId: string): Promise<boolean> {
  const value = await storage.getItem(getUsageKey(userId, getMonthKey(Date.now())));
  return value === 'true';
}
...
export async function startStreakRestoreQuest(userId: string, streakBefore: number): Promise<RestoreQuest> {
...
export async function recordStreakRestoreSession(
  userId: string,
  sessionId: string,
): Promise<{ quest: RestoreQuest | null; shouldRestore: boolean; streakBefore: number | null }> {
