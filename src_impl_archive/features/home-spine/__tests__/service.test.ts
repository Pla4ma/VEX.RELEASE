...
describe('home-spine service', () => {
  ...
  it('surfaces the completion highlight inside the return-reason rail', () => {
    const model = buildHomeSpineModel({
      currentStreak: 6,
      homeHighlight: {
        message: 'Home is primed with one clear move that keeps this streak easy to protect.',
  ...
  it('uses a fast-start primary action for first-run users', () => {
    ...
    expect(model.progressSignal.title).toBe('The core loop unlocks after session one');
  });
  ...
  it('shows completion as protected momentum instead of urgent work', () => {
    ...
    expect(model.progressSignal.title).toBe('Today already has a real focus win');
