...
export const companionPromiseKeys = {
  ...
  home: (userId: string) => [...companionPromiseKeys.all, 'home', userId] as const,
};
...
function hasPromise(state: CompanionPromiseHomeState): state is Extract<CompanionPromiseHomeState, { promise: CompanionPromise }> {
...
export function useCompanionPromise(): {
  ...
  useEffect(() => {
    if (query.data && hasPromise(query.data) && query.data.promise.id !== viewedRef.current) {
  ...
  const invalidate = async (): Promise<void> => {
    if (!userId) { return; }
    await queryClient.invalidateQueries({ queryKey: companionPromiseKeys.home(userId) });
    await queryClient.invalidateQueries({ queryKey: ['companion-memories', userId] });
  ...
  const keepPromiseMutation = useMutation({
    ...
    onError: (error) => {
      Sentry.captureException(error, { tags: { feature: 'companion-promise', operation: 'keep' } });
      show({ type: 'error', title: 'Recovery did not save', message: 'Start small anyway. We will sync when the thread is steady again.' });
    },
  });
  const dismissRecoveryMutation = useMutation({
    ...
    onSuccess: () => { void invalidate(); },
    onError: (error) => {
      Sentry.captureException(error, { tags: { feature: 'companion-promise', operation: 'dismiss' } });
      show({ type: 'error', title: 'Could not dismiss recovery', message: 'Try again when your connection settles.' });
    },
  });
  ...
  return {
    ...
    refetch: rerunQuery,
    keepPromise: async (promise) => { await keepPromiseMutation.mutateAsync(promise); },
    dismissRecovery: async (promiseId) => { await dismissRecoveryMutation.mutateAsync(promiseId); },
  };
}
