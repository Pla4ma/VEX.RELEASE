...
export function MasteryCard({ userId, state, onStateChange }: Props): JSX.Element {
  ...
  const handleClaim = (challengeId: string) => {
    if (!userId || !MasteryService.claimChallenge(userId, challengeId)) {return;}
  ...
  return (
    <View style={{ borderWidth: 1, borderColor: theme.colors.border.light, backgroundColor: theme.colors.background.secondary, padding: theme.spacing[4], gap: theme.spacing[4], ...getPremiumCardStyle('medium') }}>
      <View style={{ gap: theme.spacing[2] }}>
        <MasteryRankBadge rank={cardState.rank} totalPoints={cardState.totalMasteryPoints} size="md" />
      ...
      <View style={{ gap: theme.spacing[3] }}>
        ...
        {cardState.activeChallenges.slice(0, 3).map((challenge) => {
          ...
          return (
            <View key={challenge.id} style={{ borderWidth: 1, borderColor: theme.colors.border.light, borderRadius: 16, padding: theme.spacing[3], gap: theme.spacing[2], backgroundColor: theme.colors.background.primary }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: theme.spacing[2] }}>
                ...
                <View style={{ borderRadius: 999, paddingHorizontal: theme.spacing[2], paddingVertical: theme.spacing[1], backgroundColor: withAlpha(badgeColor, 0.14) }}>
              ...
              <View style={{ gap: theme.spacing[1] }}>
                <View style={{ height: 6, borderRadius: 3, overflow: 'hidden', backgroundColor: theme.colors.background.tertiary }}>
                  <View style={{ width: `${progress * 100}%`, height: 6, backgroundColor: badgeColor }} />
                </View>
              ...
              {challenge.status === 'COMPLETED' ? (
                <Button size="sm" variant="outline" onPress={() => handleClaim(challenge.id)}
