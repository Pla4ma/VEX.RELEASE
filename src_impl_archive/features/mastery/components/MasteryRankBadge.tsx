...
export function MasteryRankBadge({ rank, totalPoints, size = 'md' }: Props): JSX.Element {
  ...
  const sizeMap = {
    sm: { icon: 14, title: 'caption' as const, points: 'caption' as const, px: theme.spacing[2], py: theme.spacing[1], radius: 999 },
    md: { icon: 18, title: 'body' as const, points: 'bodySmall' as const, px: theme.spacing[3], py: theme.spacing[3], radius: 18 },
    lg: { icon: 28, title: 'h3' as const, points: 'body' as const, px: theme.spacing[5], py: theme.spacing[4], radius: 24 },
  };
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: size === 'sm' ? 0 : 1 }}>
        <Text variant={current.title} color={theme.colors.text.primary} fontWeight="700">
          {config.label}
        </Text>
        {size !== 'sm' ? (
