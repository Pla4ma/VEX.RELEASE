...
import { launchColors } from '@theme/tokens/launch-colors';
 export interface LiveFocusingData{totalCount:number;friendsCount:number;squadCount:number;sampleAvatars?:Array<{url?:string;initials:string;}>;trend?:'up'|'down'|'stable';trendPercent?:number;}interface LiveFocusingWidgetProps{data:LiveFocusingData;onPress?:()=>void;compact?:boolean;isLoading?:boolean;}function PulsingLiveDot():JSX.Element{const animatedStyle = useAnimatedStyle(()=>({transform:[{scale:withRepeat(withSequence(withTiming(1,{duration:1000}),withTiming(1.5,{duration:1000})),-1,true)}],opacity:withRepeat(withSequence(withTiming(1,{duration:1000}),withTiming(0.5,{duration:1000})),-1,true)})); return<Animated.View style={[{width:8,height:8,borderRadius:4,backgroundColor:launchColors.hex_22c55e},animatedStyle]}/>;}function AvatarStack({avatars,maxDisplay = 4}:{avatars?:Array<{url?:string;initials:string;}>;maxDisplay?:number;}):JSX.Element{const{theme} = useTheme(); if(!avatars || avatars.length === 0){return<Box flexDirection="row"alignItems="center">
      ...
      </Pressable>;}return<Animated.View entering={FadeIn.duration(400)}>
      <Pressable onPress={onPress}accessibilityLabel="Interactive control"accessibilityRole="button"accessibilityHint="Activates this control">
        <Box p="xl"borderRadius="xl"bg="background.secondary"borderWidth={1}borderColor="border.light"gap="lg"style={{shadowColor:launchColors.hex_000,shadowOffset:{width:0,height:2},shadowOpacity:0.05,shadowRadius:4,elevation:2}}>
          {}
      ...
      <Box alignItems="center"gap="sm">
        <Box flexDirection="row"gap="sm">
          {[1,2,3,4].map(i=><Box key={i}width={40}height={40}borderRadius="full"bg="background.tertiary"/>)}
        </Box>
      ...
      <Box flexDirection="row"justifyContent="space-between"p="md"borderRadius="lg"bg="background.tertiary">
        {[1,2,3].map(i=><Box key={i}alignItems="center"gap="xs">
    ...
    </Box>;}export default LiveFocusingWidget;
