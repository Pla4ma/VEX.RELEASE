...
describe('Progression Validation', () => {
  describe('validateXPTransaction', () => {
    it('should validate a normal XP transaction', () => {
      ...
      const result = validateXPTransaction(
        transaction,
        {
          recentTransactions: [],
          currentLevel: 5,
  ...
  describe('validateXPBatch', () => {
    it('should validate a batch of transactions', () => {
      const transactions: XPTransaction[] = [
        { id: 't1', userId: 'user-1', amount: 100, source: 'SESSION_COMPLETE', sourceId: 's1', timestamp: Date.now(), applied: false },
        { id: 't2', userId: 'user-1', amount: 50, source: 'DAILY_LOGIN', timestamp: Date.now() - 1000, applied: false },
      ];
    ...
    it('should filter out invalid transactions', () => {
      const now = Date.now();
      const duplicate: XPTransaction = { id: 't1', userId: 'user-1', amount: 100, source: 'SESSION_COMPLETE', sourceId: 's1', timestamp: now, applied: false };
