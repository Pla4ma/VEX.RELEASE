...
export async function fetchPrestigeState(userId: string): Promise<PrestigeState | null> {
...
export async function createPrestigeState(userId: string): Promise<PrestigeState> {
  const { data, error } = await supabase
    .from(TABLE)
...
export async function incrementNightmareCompletions(userId: string): Promise<void> {
  const { error } = await supabase
    .from(TABLE)
    .update({
...
function dbToState(row: Record<string, unknown>): PrestigeState {
  return {
    ...
    firstPrestigeAt: row.first_prestige_at ? new Date(row.first_prestige_at as string).getTime() : null,
    lastPrestigeAt: row.last_prestige_at ? new Date(row.last_prestige_at as string).getTime() : null,
...
function stateToDb(state: PrestigeState): Record<string, unknown> {
  return {
    ...
    first_prestige_at: state.firstPrestigeAt ? new Date(state.firstPrestigeAt).toISOString() : null,
    last_prestige_at: state.lastPrestigeAt ? new Date(state.lastPrestigeAt).toISOString() : null,
