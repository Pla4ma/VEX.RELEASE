...
describe('personal bests service', () => {
  ...
  it('creates a first personal best when no current record exists', async () => {
    ...
    const result = await checkAndUpdatePersonalBest(userId, SessionMode.SPRINT, 900, 82, 'B');
  ...
  it('returns a margin when the session beats the current record', async () => {
    ...
    const result = await checkAndUpdatePersonalBest(userId, SessionMode.SPRINT, 900, 91, 'A');
  ...
  it('does not upsert when the current record is better', async () => {
    ...
    const result = await checkAndUpdatePersonalBest(userId, SessionMode.SPRINT, 900, 80, 'B');
  ...
  it('fetches a setup preview by mode and bucket', async () => {
    ...
    await expect(getBestPreview(userId, SessionMode.SPRINT, 900)).resolves.toEqual(best);
  });
});
