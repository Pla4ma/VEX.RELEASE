...
const factors = {
  consistency: { weightPercent: 35, score: 80, delta: 4, explanation: 'Consistent completions.' },
  streakStability: { weightPercent: 25, score: 70, delta: 2, explanation: 'Steady streak.' },
  sessionQuality: { weightPercent: 20, score: 85, delta: 5, explanation: 'High-quality sessions.' },
  intentionalDifficulty: { weightPercent: 10, score: 60, delta: 1, explanation: 'Moderate challenge.' },
  recency: { weightPercent: 10, score: 75, delta: 3, explanation: 'Recent activity.' },
} as const;
...
describe('focus identity repository (P2-02)', () => {
  ...
  it('fetchCurrentFocusScore returns parsed score record on success', async () => {
    ...
    await expect(fetchCurrentFocusScore(userId)).resolves.toMatchObject({ userId, currentScore: 620, band: 'Strong' });
  });

  it('fetchCurrentFocusScore returns null when the user has no score', async () => {
    useQueries(makeQuery({ data: null, error: { code: 'PGRST116', message: 'Not found' } }));
  ...
  it('fetchCurrentFocusScore throws typed repository error on Supabase error', async () => {
    useQueries(makeQuery({ data: null, error: { code: '500', message: 'db unavailable' } }));
    await expect(fetchCurrentFocusScore(userId)).rejects.toThrow('fetchCurrentFocusScore');
  });

  it('fetchCurrentFocusScore rejects invalid response shape', async () => {
    useQueries(makeQuery({ data: { ...currentRow, id: 'invalid' }, error: null }));
  ...
  it('upsertCurrentFocusScore returns parsed score record on success', async () => {
    useQueries(makeQuery({ data: { ...currentRow, current_score: 631, previous_score: 620 }, error: null }));
    await expect(
  ...
  it('upsertCurrentFocusScore resolves conflict by returning existing score', async () => {
    useQueries(
      ...
      makeQuery({ data: { ...currentRow, current_score: 630, previous_score: 620 }, error: null }),
    );
    await expect(
      upsertCurrentFocusScore(userId, {
        currentScore: 631,
  ...
  it('appendFocusScoreHistory returns parsed history point on success', async () => {
    useQueries(makeQuery({ data: { user_id: userId, occurred_at: '2026-05-06T02:00:00.000Z', score: 630, delta: 10, reason: 'Session completed' }, error: null }));
    await expect(
      appendFocusScoreHistory({ userId, timestamp: '2026-05-06T02:00:00.000Z', score: 630, delta: 10, reason: 'Session completed' }),
  ...
  it('fetchFocusScoreHistory returns empty and parsed history arrays', async () => {
    useQueries(makeQuery({ data: [{ user_id: userId, occurred_at: '2026-05-06T02:00:00.000Z', score: 630, delta: 10, reason: 'Session completed' }], error: null }));
    await expect(fetchFocusScoreHistory(userId, 30)).resolves.toEqual([{ timestamp: '2026-05-06T02:00:00.000Z', score: 630, delta: 10, reason: 'Session completed' }]);
  ...
  it('fetchMonthlyFocusReportInput returns month-scoped aggregate data', async () => {
    useQueries(
      makeQuery({ data: [{ user_id: userId, occurred_at: '2026-05-06T02:00:00.000Z', score: 630, delta: 10, reason: 'Session completed' }], error: null }),
      makeLtQuery({ data: [{ duration: 1800, effective_duration: 1500, quality_score: 88, status: 'completed', completed_at: '2026-05-05T00:00:00.000Z' }], error: null }),
    );
    await expect(fetchMonthlyFocusReportInput(userId, '2026-05')).resolves.toMatchObject({
      userId,
      month: '2026-05',
      sessionsCompleted: 1,
  ...
  it('fetchMonthlyFocusReportInput throws on invalid month and Supabase error', async () => {
    await expect(fetchMonthlyFocusReportInput(userId, '2026-5')).rejects.toThrow();

    useQueries(makeQuery({ data: null, error: { message: 'history failed' } }));
    await expect(fetchMonthlyFocusReportInput(userId, '2026-05')).rejects.toThrow('history');
  });
});
