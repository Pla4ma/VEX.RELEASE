...
export class OnboardingRepository {
  ...
  async saveOnboardingState(userId: string, state: OnboardingState): Promise<void> {
    try {
      ...
      throw new OnboardingRepositoryError('Failed to save state', { cause: error });
    }
  }
  ...
  async saveProgress(userId: string, progress: OnboardingProgress): Promise<void> {
    try {
      ...
      debug.info('Onboarding progress saved', { userId, status: progress.status });
    } catch (error) {
      debug.error('Failed to save onboarding progress', toError(error));
      throw new OnboardingRepositoryError('Failed to save progress', { cause: error });
    }
  }
}
