...
describe('OnboardingRepository persistence', () => {
  ...
  it('validates and restores onboarding state from primary storage', async () => {
    ...
    await expect(repository.getOnboardingState('user-1')).resolves.toEqual(state);
  });
  ...
  it('repairs primary onboarding state from backup when primary is corrupted', async () => {
    ...
    await expect(repository.getOnboardingState('user-2')).resolves.toEqual(state);
    expect(mockStore.get('onboarding:state:user-2')).toBe(JSON.stringify(state));
  });

  it('returns null instead of malformed persisted state', async () => {
    mockStore.set('onboarding:state:user-3', JSON.stringify({ currentStep: 99 }));

    await expect(new OnboardingRepository().getOnboardingState('user-3')).resolves.toBeNull();
  });
});
