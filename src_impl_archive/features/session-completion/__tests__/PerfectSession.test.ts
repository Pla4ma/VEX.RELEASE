...
import { trackSGradeSession, getSGradeStreak, resetSGradeStreak, isSGradeMilestone } from '../../mastery/SGradeStreakTracker';
...
describe('Perfect Session detection', () => {
  ...
  it('S-grade streak increments on consecutive perfect sessions', async () => {
    ...
    const result1 = await trackSGradeSession('user1', 'S', 'session1', mockEventBus);
    expect(result1.count).toBe(1);
    expect(result1.milestone).toBeUndefined();

    // Second S grade - streak becomes 2, no milestone
    const result2 = await trackSGradeSession('user1', 'S', 'session2', mockEventBus);
    expect(result2.count).toBe(2);
    expect(result2.milestone).toBeUndefined();

    // Third S grade - streak becomes 3, milestone reached
    const result3 = await trackSGradeSession('user1', 'S', 'session3', mockEventBus);
  ...
  it('S-grade streak resets to 0 on A grade', async () => {
    ...
    const result = await trackSGradeSession('user1', 'A', 'session3', mockEventBus);
  ...
  it('handles edge case of first-ever perfect session', async () => {
    ...
    const result = await trackSGradeSession('user1', 'S', 'session1', mockEventBus);
  ...
  it('reaches 10-streak milestone correctly', async () => {
    ...
    for (let i = 1; i <= 10; i++) {
      const result = await trackSGradeSession('user1', 'S', `session${i}`, mockEventBus);
  ...
  it('handles offline perfect session gracefully', async () => {
    ...
    const result = await trackSGradeSession('user1', 'S', 'offline-session', mockEventBus);
