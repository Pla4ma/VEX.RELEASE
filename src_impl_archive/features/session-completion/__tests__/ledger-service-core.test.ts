...
describe('buildCompletionLedger core contract', () => {
  it('ledger schema rejects missing required fields', () => {
    expect(() => CompletionLedgerSchema.parse({
  ...
  it('rejects invalid ledger inputs before persistence', () => {
    expect(() => buildCompletionLedger({
      sessionId: 'not-a-uuid',
      summary: createSessionSummary(),
      userId: USER_ID,
    } as BuildCompletionLedgerInput)).toThrow();
    ...
    expect(() => buildCompletionLedger({
      sessionId: SESSION_ID,
      summary: createSessionSummary({ sessionMode: 'INVALID_MODE' as SessionMode }),
      userId: USER_ID,
    })).toThrow();
  });
});
