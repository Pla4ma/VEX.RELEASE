...
const StoryBeatSchema = z.object({
  ...
  id: z.string().min(1),
  kind: z.enum(['result', 'grade', 'meaning', 'companion', 'personal_best', 'tomorrow']),
  metric: z.object({ label: z.string().min(1), value: z.string().min(1) }).nullable(),
  title: z.string().min(1),
}).strict();

const StoryRouteParamsSchema = z.object({
...
export const PostSessionStoryViewModelSchema = z.object({
  beats: z.array(StoryBeatSchema).min(5),
  companionReaction: z.object({ reactionId: z.string().nullable() }).strict(),
  companionMemory: z.object({ memoryId: z.string().uuid(), title: z.string(), type: z.string() }).nullable(),
  companionPromise: z.object({
    status: z.enum(['pending', 'fulfilled', 'missed', 'replaced']),
    targetDate: z.string(),
    targetDurationMinutes: z.number().int().min(5),
    targetMode: z.string().min(1),
  }).nullable(),
  dailyMission: z.object({ missionId: z.string().nullable(), progressDelta: z.number(), status: z.string() }).strict(),
  degradedWarnings: z.array(z.string()),
  focusScoreDeltaCard: z.object({ delta: z.number().int(), label: z.string() }).strict(),
  gradeCard: z.object({ grade: z.string(), label: z.string(), score: z.number().min(0).max(100) }).strict(),
