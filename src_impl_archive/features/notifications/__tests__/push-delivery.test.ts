...
describe('Push Delivery Service', () => {
  ...
  describe('PushNotificationPayloadSchema', () => {
    it('validates valid payload', () => {
      ...
      expect(PushNotificationPayloadSchema.parse(validPayload)).toEqual(validPayload);
    });
  ...
  describe('getPushToken', () => {
    it('returns token on success', async () => {
      (Notifications.getExpoPushTokenAsync as jest.Mock).mockResolvedValue({ data: 'test-token-123' });
  ...
  describe('schedulePushNotification', () => {
    it('schedules notification for future', async () => {
      (Notifications.scheduleNotificationAsync as jest.Mock).mockResolvedValue('scheduled-id-123');
