...
import { BehaviorStatsSchema, FeatureAvailabilitySnapshotSchema, StudyLayerNameSchema } from './schemas';
import type {
  BehaviorStats,
 ...
 */
export function useResolvedVexExperienceRuntime(input: VexExperienceRuntimeInput): VexExperience {
  const duration = useOnboardingStore((state) => state.focusDuration);
  const goal = useOnboardingStore((state) => state.goal);
  const explicitStyle = useOnboardingStore((state) => state.explicitMotivationStyle);
  const style = normalizeMotivationStyle(explicitStyle);
  ...
  return useMemo(() => {
    ...
    const stats = BehaviorStatsSchema.parse({
      ...input.behaviorStats,
      ...(input.overrideBossEngagement ? { bossChallengeEngagement: input.overrideBossEngagement } : {}),
    });
    const availability = FeatureAvailabilitySnapshotSchema.parse({
      ...input.featureAvailability,
      ...(input.overridePremiumAvailable !== undefined ? { premium: input.overridePremiumAvailable } : {}),
    });
    return resolveVexExperience(profile, stats, availability);
  }, [duration, goal, style, input.behaviorStats, input.featureAvailability, input.overrideStudyLayerLabel, input.overridePremiumAvailable, input.overrideBossEngagement]);
}
