...
import { computeVexRuntimeExperience, type VexRuntimeInput } from '../vex-runtime-experience';
...
describe('useVexRuntimeExperience', () => {
  it('Day 0 calm user sees only first-session path', () => {
    ...
    expect(result.completionSequence.steps).toContain('coach_companion_reflection');
  ...
  it('Day 3 user sees companion continuity', () => {
    ...
    const result = computeVexRuntimeExperience(input);
    expect(result.completionSequence.emphasis).toBe('confirmation_coach_progress_next_action');
  ...
  it('notifications respect first-week stage', () => {
    ...
    expect(day3Result.notificationPolicy.allowedTypes.length).toBeGreaterThan(0);
  });
  ...
  it('completion sequence changes by first-week stage', () => {
    ...
    expect(day5Result.completionSequence.steps.length).toBeGreaterThan(day0Result.completionSequence.steps.length);
  });
