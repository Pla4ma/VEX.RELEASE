...
export const focusContractKeys = {
  ...
  session: (sessionId: string) => [...focusContractKeys.all, 'session', sessionId] as const,
...
export function useCreateContract(): ReturnType<typeof useMutation<FocusContract, Error, FocusContractInput>> {
  ...
  return useMutation({
    ...
    onSuccess: (contract) => {
      ...
      void queryClient.invalidateQueries({ queryKey: focusContractKeys.session(contract.sessionId) });
    },
    onError: (error) => {
      Sentry.captureException(error, { tags: { feature: 'focus-contract', operation: 'create' } });
      show({ type: 'error', title: 'Contract saved locally failed', message: 'Your session can still start.' });
    },
  });
}

export function useReflectOnContract(): ReturnType<
  typeof useMutation<void, Error, { contract: FocusContract; status: ReflectionStatus }>
> {
  const queryClient = useQueryClient();
  const userId = useAuthStore((state) => state.user?.id ?? null);
  const { show } = useToast();
  ...
  return useMutation({
    ...
    onMutate: async ({ contract, status }) => {
      ...
      const previous = queryClient.getQueryData<FocusContract | null>(key) ?? null;
    ...
    onSuccess: (_data, variables) => {
      void queryClient.invalidateQueries({ queryKey: focusContractKeys.session(variables.contract.sessionId) });
      if (userId) {
        void queryClient.invalidateQueries({ queryKey: ['focus-score', userId] });
      }
    },
    onError: (error, _variables, context) => {
      ...
      Sentry.captureException(error, { tags: { feature: 'focus-contract', operation: 'reflect' } });
      show({ type: 'error', title: 'Reflection did not save', message: 'Retry when connection returns.' });
    },
  });
}

export function useContractCompletionRate(userId: string | null, windowDays: number): {
  rate: number;
  isPending: boolean;
  isError: boolean;
  error: Error | null;
