...
describe('focus-contract repository', () => {
  ...
  it('throws RepositoryError on Supabase create error', async () => {
    const single = jest.fn().mockResolvedValue({ data: null, error: { message: 'denied' } });
    mockFrom.mockReturnValue({ insert: jest.fn().mockReturnValue({ select: jest.fn().mockReturnValue({ single }) }) });

    await expect(createContract({ sessionId, userId, taskDescription: 'Draft' })).rejects.toThrow(
      FocusContractRepositoryError,
    );
  });
  ...
  it('finds a contract for a session', async () => {
    ...
    mockFrom.mockReturnValue({ select: jest.fn().mockReturnValue({ eq: eqSession }) });
  ...
  it('throws when Supabase returns an invalid shape', async () => {
    const maybeSingle = jest.fn().mockResolvedValue({ data: { ...dbRow, task_description: '' }, error: null });
    const eqUser = jest.fn().mockReturnValue({ maybeSingle });
    const eqSession = jest.fn().mockReturnValue({ eq: eqUser });
    mockFrom.mockReturnValue({ select: jest.fn().mockReturnValue({ eq: eqSession }) });
      ...
      .mockReturnValueOnce({
        select: jest.fn().mockReturnValue({
          eq: jest.fn().mockReturnValue({
            order: jest.fn().mockReturnValue({ limit: jest.fn().mockResolvedValue({ data: [dbRow], error: null }) }),
          }),
        }),
      });

    await expect(reflectOnContract(contractId, 'done')).resolves.toBeUndefined();
