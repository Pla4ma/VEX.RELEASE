...
import type { CoachPresenceMessageContext, CoachPresenceMessageStyle } from './message-library';
...
function contextToLibraryContext(
  ...
  if (sessionState === 'risk' && riskLevel === 'critical') return 'strong_streak';
...
export function getCoachPresenceMessageForInterruption(
  ...
  const libContext = contextToLibraryContext(context.sessionState, context.riskLevel);
  try {
...
export function getCoachSessionCompletionMessage(params: {
  ...
  if (params.isFirstSession) {
    try {
      return getMessageFromLibrary({ context: 'first_session_completion', style });
    } catch {
...
export function getCoachStreakMessage(params: {
  ...
  if (params.context === 'broken') {
    return style === 'calm' ? `${params.streak}-day streak ended. No pressure. Start fresh today.`
      : style === 'friendly' ? `${params.streak}-day streak ended, that is okay. Let us start a new one today.`
      : style === 'coach_led' ? `Streak ended at ${params.streak} days. Reset the rhythm with one block.`
      : style === 'game_like' ? `Streak broken at ${params.streak}. The run is not over. Restart now.`
      : style === 'intense' ? `Streak lost at ${params.streak}. No drift. Restart immediately.`
      : `${params.streak}-day streak ended. Do one review block before adding new material.`;
  }

  return style === 'calm' ? `${params.streak}-day streak is nudging you. One block keeps it safe.`
    : style === 'friendly' ? `${params.streak}-day streak, so close. One block protects it.`
    : style === 'coach_led' ? `${params.streak}-day streak. One block now keeps the rhythm.`
    : style === 'game_like' ? `${params.streak}-day streak at risk. Bank one block to save it.`
    : style === 'intense' ? `${params.streak}-day streak. One block before it cools.`
...
export function getCoachComebackMessage(params: {
  ...
  return style === 'calm' ? `Welcome back after ${params.daysSinceLastSession} days. One clean block, no pressure.`
    : style === 'friendly' ? `You are back after ${params.daysSinceLastSession} days. One small step is enough.`
    : style === 'coach_led' ? `Back after ${params.daysSinceLastSession} days. Reset with one clean block.`
    : style === 'game_like' ? `Welcome back. ${params.daysSinceLastSession} days off. One run restarts the engine.`
    : style === 'intense' ? `Back after ${params.daysSinceLastSession}. No speech. Prove it with one block.`
