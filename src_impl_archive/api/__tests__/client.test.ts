import { ApiClient, getApiClient, resetApiClient, type ApiConfig } from '../client';

const directHttpMessage = 'Direct HTTP calls are not allowed. Use Supabase client or a repository layer instead.';
...
async function expectDirectHttpBlocked(request: Promise<unknown>): Promise<void> {
...
describe('ApiClient regression firewall', () => {
  ...
  it('deduplicates identical blocked requests when requested', async () => {
    ...
    expect(results).toEqual([
      expect.objectContaining({ reason: expect.objectContaining({ code: 'NETWORK_ERROR' }), status: 'rejected' }),
      expect.objectContaining({ reason: expect.objectContaining({ code: 'NETWORK_ERROR' }), status: 'rejected' }),
    ]);
  });
