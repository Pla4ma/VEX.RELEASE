...
export function calculateInterruptionPenalty(
...
): { total: number; breakdown: Record<InterruptionSeverity, number> } {
  ...
  const severityMultipliers: Record<InterruptionSeverity, number> = {
    MINOR: 0.5, MODERATE: 1, MAJOR: 2, CRITICAL: 4,
  };
  ...
  return { total: Math.min(total, PENALTY_CONSTANTS.INTERRUPTION_PENALTY_MAX), breakdown };
}
...
export function calculateAntiCheatPenalty(
  ...
  const severityMultipliers: Record<string, number> = {
    LOW: 0.5, MEDIUM: 1, HIGH: 2, CRITICAL: 4,
  };
  let total = 0;
  ...
  for (const violation of input.violations) {
    ...
    if (currentRank !== undefined && maxRank !== undefined && currentRank > maxRank) {
      maxSeverity = violation.severity;
    }
  }
  const actionMap: Record<string, 'WARNING' | 'PENALTY' | 'DISQUALIFY' | 'BAN'> = {
    LOW: 'WARNING', MEDIUM: 'PENALTY', HIGH: 'DISQUALIFY', CRITICAL: 'BAN',
  };
...
export function calculateTotalPenalty(input: TotalPenaltyInput): {
  ...
  return {
    total, capped, percentage,
    breakdown: { pause: pausePenalty, interruption: interruptionPenalty, quality: qualityPenalty, antiCheat: antiCheatPenalty },
  };
}
