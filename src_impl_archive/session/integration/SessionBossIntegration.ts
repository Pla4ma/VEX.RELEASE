...
function getSessionDatePart(timestamp: number | undefined): { currentDay: number; currentHour: number } {
...
export function calculateBossDamage(
  ...
  const sprintMultiplier = mode === SessionMode.SPRINT
    ? getSprintChainMultiplier(summary.modeBonus > 0 ? Math.round(summary.modeBonus / 5) + 1 : 1)
    : 1;
  const baseDamage = Math.max(1, Math.round(durationMinutes * purityMultiplier * baseModeMultiplier * sprintMultiplier));
  ...
  return applyBossDamageRules(Math.round(baseDamage * dailyMultiplier), {
    ...
    sessionQuality: Math.max(0, Math.min(100, summary.focusPurityScore ?? summary.focusQuality ?? 0)),
...
export function initializeSessionBossIntegration(): () => void {
  ...
  const unsubscribe = eventBus.subscribe('session:completed', async (rawEvent) => {
    ...
    if (!parsedEvent.success) {
      debug.warn('Skipping boss damage because the session completion payload was invalid');
      return;
    }
    ...
    try {
      ...
      if (damage <= 0) {
        debug.debug('Skipping boss damage for session %s because the active boss blocked damage', sessionId);
        return;
      }
      ...
      debug.error('Failed to apply boss damage from session completion', error instanceof Error ? error : new Error(String(error)));
    }
  });
